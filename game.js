...
const Game = (() => {
  ...
  function _tutBlock(allowIds) {
    ...
    if (!allowIds) {
      // No allowed elements — full block with dim overlay
      const blocker = document.createElement('div');
      blocker.id = 'tut-blocker';
      blocker.style.cssText = 'position:fixed;inset:0;z-index:899;cursor:not-allowed;';
      blocker.addEventListener('click', e => {
        e.stopPropagation(); e.preventDefault();
        _tutShakePopup();
      }, true);
      blocker.addEventListener('touchstart', e => { e.stopPropagation(); e.preventDefault(); _tutShakePopup(); }, { capture: true, passive: false });
      document.body.appendChild(blocker);
      return;
    }
    // Selective block — allow specific elements
    const ids = Array.isArray(allowIds) ? allowIds : [allowIds];
    const blocker = document.createElement('div');
    blocker.id = 'tut-blocker';
    ...
    blocker.addEventListener('click', e => {
      for (const id of ids) {
        const el = document.getElementById(id);
        if (el && (el === e.target || el.contains(e.target))) return; // allow
      }
      e.stopPropagation(); e.preventDefault();
      _tutShakePopup();
    }, true);
    ...
    // Pointer events: none on the blocker itself so mouse events pass through naturally,
    // but we re-enable it only to intercept non-allowed clicks via JS above
