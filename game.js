...
const Game = (() => {
  ...
  const CHANGELOG = {
    '2.2.0': {
      date: '2026',
      title: 'Casate di Appartenenza & Diplomazia',
      notes: [
        '🏰 Le casate di appartenenza del personaggio (startAllies) non forniscono più truppe automaticamente in guerra',
        '🔘 Nuovo tasto grigio disabilitato "casata di appartenenza — Non disponibile" per le casate proprie',
        '⚔ Questa logica vale solo finché la casata rimane alleata e il patto non è rotto',
        '🤝 Le casate alleate esterne (non di appartenenza) mantengono la logica normale: chiedi rinforzi, possono rifiutare o chiedere compenso',
        '🛡 Protezione aggiuntiva in requestAllyArmy: le casate di appartenenza non possono essere chiamate neanche da altri percorsi',
      ],
    },
    '2.1.0': {
      date: '2026',
  ...
  function _openKingChallengeDiplomacy() {
    ...
    const allyRows = allies.length > 0
      ? allies.map(([id, hh]) => {
          ...
          // isHome = casata di appartenenza del personaggio (startAllies, ancora ally, patto non rotto)
          const isHome       = (char?.startAllies || []).includes(id) && !hh.pactBroken;
          const refusalState = (state.allyLoanRefusals || {})[id];

          // Casata di appartenenza → tasto grigio disabilitato (non fornisce truppe)
          if (isHome) {
  ...
  function requestAllyArmy(hId) {
    ...
    // Le casate di appartenenza del personaggio (startAllies, patto non rotto) non forniscono truppe
    const char = state.character;
    const isHomeHouse = (char?.startAllies || []).includes(hId) && !h.pactBroken;
