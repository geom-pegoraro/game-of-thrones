...
const Game = (() => {
  ...
  const CHARACTERS = [
    ...
    },
    {
      ...
      icon: '🐺', emoji: '⚔️',
      difficulty: 'medium',
      startResources: { gold: 38, faith: 48, people: 65, army: 48, power: 42 },  // Robb — Re del Nord, buon esercito ma non dominante
    ...
    },
    {
      ...
      icon: '⚔️', emoji: '🦁',
      difficulty: 'medium',
      startResources: { gold: 55, faith: 28, people: 38, army: 42, power: 42 },  // Jaime — guerriero ma in missione, non ha tutto l'esercito Lannister
    ...
    },
    {
      ...
      icon: '🌹', emoji: '👸',
      difficulty: 'easy',
      startResources: { gold: 62, faith: 50, people: 68, army: 25, power: 50 },  // Margaery — diplomatica, dipende dai Tyrell non dall'esercito diretto
    ...
    },
    {
      ...
      icon: '🐙', emoji: '⚓',
      difficulty: 'hard',
      startResources: { gold: 30, faith: 18, people: 28, army: 32, power: 22 },  // Theon — piccola flotta, esercito ridotto
    ...
    },
    {
      ...
      icon: '🪙', emoji: '🕵️',
      difficulty: 'medium',
      startResources: { gold: 68, faith: 22, people: 38, army: 12, power: 65 },  // Littlefinger — nessun esercito, solo oro e intrighi
    ...
    },
    {
      ...
      icon: '🔥', emoji: '🌹',
      difficulty: 'hard',
      startResources: { gold: 25, faith: 62, people: 30, army: 18, power: 40 },  // Melisandre — sacerdotessa, pochissimi soldati diretti
    ...
    },
    {
      ...
      icon: '☀️', emoji: '🐍',
      difficulty: 'medium',
      startResources: { gold: 50, faith: 38, people: 55, army: 35, power: 45 },  // Oberyn — guerriero personale, non ha l'esercito di Dorne con sé
    ...
    },
    {
      ...
      icon: '🐺', emoji: '⚖️',
      difficulty: 'hard',
      startResources: { gold: 38, faith: 60, people: 68, army: 32, power: 38 },  // Ned — Lord di Grande Inverno, ma ad Approdo del Re è vulnerabile
    ...
    },
    {
      ...
      icon: '🐟', emoji: '👩',
      difficulty: 'medium',
      startResources: { gold: 42, faith: 58, people: 60, army: 28, power: 38 },  // Catelyn — diplomatica, non è una comandante militare
    ...
    },
    {
      ...
      icon: '🗡️', emoji: '😏',
      difficulty: 'easy',
      startResources: { gold: 50, faith: 20, people: 38, army: 38, power: 32 },  // Bronn — mercenario, esercito medio ma deve guadagnarselo
  ...
  function makeChoice(side) {
    ...
    const el = document.getElementById('main-card');
    // Ensure no transform is blocking the animation start
    el.style.transition = '';
    el.style.transform = '';
    // Force reflow so the transition fires from the correct position
    void el.offsetWidth;
  ...
  function renderDiplomacy() {
    ...
    Object.entries(state.houses).forEach(([hId, h]) => {
      ...
      let allyArmyBtn = '';
      if (h.status === 'ally' && !isSuppressed && state.pendingWarTarget) {
        if (hasResponded) {
  ...
  function showWarDiplomacyChoice(houseId, h) {
    ...
    // Calcola tributo suggerito (~30% dell'esercito nemico)
    const tributeAmt = Math.max(15, Math.floor(h.army * 0.30 + Math.random() * 10));
    const tributeRes = rand(['gold', 'army', 'people']);
    ...
    overlay.innerHTML = `
      <div style="background:#12121a;border:2px solid rgba(239,68,68,0.5);border-radius:6px;width:92%;max-width:450px;padding:1.75rem;font-family:'Cinzel',serif;">
        ...
        <div style="background:rgba(201,168,76,0.06);border:1px solid rgba(201,168,76,0.2);border-radius:4px;padding:0.85rem;margin-bottom:1rem;font-family:'EB Garamond',serif;font-size:0.92rem;color:#e8dcc8;line-height:1.6">
          Prima di dichiarare guerra ufficialmente, potete inviare un ultimatum a Casa ${h.icon} ${h.name}.<br><br>
          <strong style="color:#c9a84c">⚖ Chiedi tributo:</strong> richiedi <strong>${tributeAmt} ${resLabels[tributeRes]}</strong> per evitare lo scontro. La casata potrebbe accettare o rifiutare.<br><br>
          <strong style="color:#f87171">⚔ Guerra senza tregua:</strong> nessuna via d'uscita. La battaglia inizierà tra 2 turni.
        </div>
        <div style="display:flex;flex-direction:column;gap:0.6rem">
          <button onclick="Game.warDiploTribute('${houseId}','${tributeRes}',${tributeAmt})" style="padding:0.75rem;background:linear-gradient(135deg,rgba(120,100,20,0.8),rgba(201,168,76,0.9));border:none;border-radius:2px;font-family:'Cinzel',serif;font-size:0.75rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#0a0a0f">
            ⚖ Invia Ultimatum — Chiedi ${tributeAmt} ${resLabels[tributeRes]}
          </button>
          <button onclick="Game.warDiploDirectWar('${houseId}')" style="padding:0.75rem;background:linear-gradient(135deg,#7f1d1d,#dc2626);border:none;border-radius:2px;font-family:'Cinzel',serif;font-size:0.75rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#fff">
            ⚔ Guerra Senza Tregua — Nessuna Via d'Uscita
          </button>
          <button onclick="document.getElementById('war-diplo-overlay').remove()" style="padding:0.65rem;background:transparent;border:1px solid rgba(201,168,76,0.35);border-radius:2px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;color:#c9a84c">
            Annulla — Non ancora
          </button>
        </div>
      </div>`;
  ...
  function warDiploTribute(houseId, tributeRes, tributeAmt) {
    ...
    if (accepts) {
      const resLabels = { gold: 'oro 💰', army: 'soldati ⚔', people: 'popolo 👥' };
      state.resources[tributeRes] = clamp(state.resources[tributeRes] + tributeAmt);
  ...
  function requestSpecificResource(hId, resKey, amount) {
    ...
    // Ally decides: free gift or exchange?
    const wantExchange = Math.random() < 0.55;
    if (wantExchange) {
      const otherRes = ['gold','army','people','faith','power'].filter(r => r !== resKey);
      const costResKey = rand(otherRes);
      const costAmt = Math.floor(amount * 0.6 + Math.random() * 3);
      ...
      state.resources.power = clamp(state.resources.power + 2);
      showToast(`📦 Casa ${h.name} vi invia ${amount} ${label} come segno di alleanza.`, 'good');
      updateHUD(); saveGame();
      toggleDiplomacy();
    }
  }
  ...
  function _scheduleKingChallengeCards() {
    // Turno 1 — introduzione narrativa, nessuna trattativa possibile
    state.eventQueue.push({
      id: 'king_ch_t1',
      speaker: 'Corvo Reale',
    ...
    // Turno 2 — Le casate si organizzano: alleati del Re si rivelano, alcuni tuoi alleati potrebbero vacillare
    state.eventQueue.push({
      id: 'king_ch_t2',
      speaker: 'Varys',
      ...
      leftText: 'Valutiamo la situazione', leftEffects: {},
      rightText: 'Mobilitiamo le riserve', rightEffects: { army: +6, gold: -8 },
      tags: ['king_challenge_pending'],
      onLeftChoose:  () => _revealKingAlliances(),
      onRightChoose: () => _revealKingAlliances(),
    });
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function makeTroops(count, icons, side) {
      let html = '';
      for (let i = 0; i < count; i++) {
        html += `<span class="troop-unit" id="th-${side}-t${i}" style="font-size:1.4rem;transition:opacity 0.6s ease ${i*20}ms,transform 0.4s ease">${icons[i % icons.length]}</span>`;
      }
      return html;
    }
    ...
    overlay.innerHTML = `
      ...
      <div style="position:relative;z-index:10;width:100%;max-width:540px;padding:0 1rem;display:flex;flex-direction:column;align-items:center;gap:0.5rem" id="throne-main">
        ...
        <div id="tb-btns" style="display:flex;gap:0.75rem;margin-top:0.3rem">
    ...
    const ROUND_DELAY = 3000; // più lento del normale (era 1600ms)
    ...
    function killTroops(side, count) {
      ...
      for (let i = alive - 1; i >= Math.max(0, alive - count); i--) {
        ...
        if (t) { t.classList.add('dying'); setTimeout(() => { if(t) t.style.display='none'; }, 600); }
      }
    ...
    function runPhaseRound(roundsLeft, cb) {
      ...
      // Danni più bassi per round (battaglia più lunga e drammatica)
      ...
      if (flash) { flash.style.opacity = '1'; setTimeout(() => { if(flash) flash.style.opacity='0'; }, 400); }
      ...
      Game._battleTimer = setTimeout(() => runPhaseRound(roundsLeft - 1, cb), ROUND_DELAY);
    }
      ...
      if (btns) btns.innerHTML = `
        <div style="text-align:center;width:100%">
          <div style="font-family:'EB Garamond',serif;font-size:0.9rem;color:#9a8a6a;margin-bottom:0.7rem;font-style:italic;line-height:1.5">
            Fine Fase ${completedPhase + 1} di 5 — ${adv}<br>
    ...
    // Expose startPhase so it can be called externally if the timer is lost
    Game._throneStartPhase0 = () => startPhase(0);
    Game._battleTimer = setTimeout(() => { if (Game._throneStartPhase0) { const fn = Game._throneStartPhase0; Game._throneStartPhase0 = null; fn(); } }, 1200);
  }
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, retreatCallback) {
    ...
    function makeTroops(count, icons, side) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        html += `<span class="troop-unit" id="${side}-t${i}" style="transition:opacity 0.4s ease ${i*25}ms">${icon}</span>`;
      }
      return html;
    }
    ...
    overlay.innerHTML = `
      <div style="text-align:center;margin-bottom:0.8rem">
        ...
        <div style="font-size:0.72rem;color:#9a8a6a;margin-top:0.25rem;letter-spacing:0.06em" id="battle-phase-label">Fase 1 di 3</div>
      </div>
    ...
    function killTroops(side, count) {
      ...
      for (let i = alive - 1; i >= Math.max(0, alive - count); i--) {
        ...
        if (t) {
          ...
          setTimeout(() => { if (t) t.style.display = 'none'; }, 450);
        }
      }
      if (side === 'player') pAlive = Math.max(0, pAlive - count);
      else                   eAlive = Math.max(0, eAlive - count);
    ...
    function runPhaseRounds(roundsLeft, phaseEvtApplied, afterRoundsCallback) {
      ...
      if (flash) { flash.style.opacity = '1'; setTimeout(() => { if (flash) flash.style.opacity = '0'; }, 280); }
      ...
      Game._battleTimer = setTimeout(() => runPhaseRounds(roundsLeft - 1, phaseEvtApplied, afterRoundsCallback), 1600);
    }
    ...
    Game._battleContinuePhase = function(nextPhase) {
      // Restore retreat button style for the next phase
      const btns = document.getElementById('battle-btns');
      if (btns) btns.innerHTML = `
        <button id="btn-retreat" onclick="Game._battleRetreat()" style="padding:0.65rem 1.4rem;background:transparent;border:1px solid rgba(201,168,76,0.5);border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#c9a84c">
          🏃 Ritirata
    ...
    // Start phase 0
    Game._battleTimer = setTimeout(() => startPhase(0), 800);
  }
  ...
  function _resolveThroneAttack(won, playerForce, kingForce, survived) {
    ...
    if (won) {
      ...
      // Old king's house becomes enemy
  ...
  function requestAllyArmy(hId) {
    ...
    if (!state.pendingWarTarget) { showToast('Devi prima dichiarare guerra o sfidare il Re.', 'warn'); return; }
  ...
  function _scheduleWarDeclarationCards(houseId, h, skipFirstTurn) {
    if (!skipFirstTurn) {
      // Turno 1 (già gestito da showWarDiplomacyChoice) — carta narrativa guerra senza tregua
      state.eventQueue.push({
    ...
    // Turno 2 — Le casate si organizzano: alleanze nemiche rivelate, alleati del giocatore valutano
    state.eventQueue.push({
      id: 'war_decl_t2_' + houseId,
      speaker: `Varys`,
      speakerRole: `Maestro dei Sussurri — rapporto alleanze`,
      portrait: '🕵️', icon: '🕵️',
      text: `Le spie riportano: le casate si stanno organizzando. Alcune si uniranno a Casa ${h.name}, altre valuteranno i propri interessi. Anche qualche vostro alleato potrebbe decidere di non schierarsi — o peggio. Aprite la Diplomazia per richiedere rinforzi prima che sia troppo tardi.`,
      leftText: 'Prendiamo nota', leftEffects: {},
      rightText: 'Mobilitiamo le riserve', rightEffects: { army: +5, gold: -6 },
      tags: ['war_pending'],
      onLeftChoose:  () => _revealEnemyAlliances(houseId, h),
      onRightChoose: () => _revealEnemyAlliances(houseId, h),
    });
...
const AudioManager = (() => {
  ...
  function createAudio(src) {
    ...
    // Handle track ending - restart from beginning
  ...
  function init() {
    ...
    track1.loop = true;
    track2.loop = false; // Track 2 should not loop, restart when needed
    track1.load();
    track2.load();
    buildMuteButton();
