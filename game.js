...
const Game = (() => {
  ...
  function _tutStep_charSelect() {
    _tutBlock(['char-grid']); // allow clicking chars but nothing else
    _tutPopup(
      'char-grid',
      ...
      'up',
      () => {
        // Don't unblock here — player still needs to click a char card
        _clearTutOverlay();
  ...
  function _tutOnCharDetail() {
    ...
    setTimeout(() => {
      _tutBlock(['char-detail-start']); // allow only the start button
      _tutPopup(
        'char-detail-start',
  ...
  function _tutStep_prologue() {
    _tutBlock(null); // block everything — only the Capito button in overlay works
    _tutPopup(
      null,
