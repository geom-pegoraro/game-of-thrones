...
const MultiplayerEngine = (() => {
    ...
    w.innerHTML = `
      <div style="width:100%;max-width:420px">
        ...
        <div class="mp-card">
          ...
          <div id="mp-charsec">
            <div style="font-family:'EB Garamond',serif;font-size:.85rem;color:#6b5e4a;margin-bottom:.5rem">Scegli la tua casata e il tuo personaggio. Le casate già prese dagli altri giocatori saranno bloccate.</div>
  ...
  function _refreshRoom() {
    ...
    plist.innerHTML = Object.values(_players).map(p => {
      ...
      return `<div class="mp-slot ${p.ready?'ready':''} ${isMe?'me':''}">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:.6rem;color:#9a8a6a">${cd?.name||'Non scelto'}</div>
        </div>
        <div style="font-size:.62rem">${p.ready?'<span style="color:#4ade80">✓ Pronto</span>':'<span style="color:#6b5e4a">attesa…</span>'}</div>
        ${canKick ? `<button onclick="MultiplayerEngine.kickPlayer('${p.peerId}')" title="Espelli dalla lobby"
          style="margin-left:.4rem;padding:.2rem .45rem;background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.4);border-radius:3px;font-size:.6rem;cursor:pointer;color:#f87171;font-family:'Cinzel',serif;letter-spacing:.04em;white-space:nowrap">✕ Kick</button>` : ''}
      </div>`;
    }).join('');
    ...
    if (csec && me?.charId) {
      ...
      csec.innerHTML = `
        <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.5rem">
          ...
          <div><div style="font-size:.8rem;color:#e8dcc8">${cd?.name||me.charId}</div>
          <div style="font-size:.62rem;color:#9a8a6a">${cd?.house||''}</div></div>
        </div>
        <button class="mp-btn-ghost" style="margin-top:0" onclick="MultiplayerEngine._openNativeCharSelect()">Cambia</button>
        ${!me.ready
          ? '<button class="mp-btn-green" onclick="MultiplayerEngine.setReady()">✓ Sono Pronto</button>'
          : '<div style="text-align:center;color:#4ade80;font-size:.82rem;margin-top:.5rem;font-family:\'Cinzel\',serif">✓ Pronto!</div>'}`;
    }

    if (act && _isHost) {
      const list = Object.values(_players);
      const allRdy = list.length >= 2 && list.every(p => p.ready && p.charId);
      act.innerHTML = allRdy
        ? '<button class="mp-btn-green" onclick="MultiplayerEngine.hostStart()" style="max-width:420px">⚔ Avvia la Partita!</button>'
        : `<div style="font-family:'EB Garamond',serif;font-size:.82rem;color:#6b5e4a;text-align:center;padding:.5rem">
             In attesa… ${list.filter(p=>p.ready).length}/${list.length} pronti
           </div>`;
    }
  }

  // ══════════════════════════════════════════════════════════════════════
  // SELEZIONE PERSONAGGIO — flusso nativo singleplayer
  // Il giocatore usa le schermate native (casata → personaggio → dettaglio).
  // Il bottone "⚔ Inizia la partita" viene intercettato in modalità MP:
  //   • comunica la scelta a tutti
  //   • mostra schermata "In attesa degli altri"
  //   • avvia tutti insieme quando tutti hanno confermato
  // ══════════════════════════════════════════════════════════════════════

  // Apre il flusso nativo di selezione personaggio con blocco casate occupate
  function _openNativeCharSelect() {
    if (typeof Game === 'undefined') return;

    // Salva l'eventuale scelta precedente per liberar la casata
    ...
    // Attiva la modalità MP sul motore di gioco e apre la schermata nativa
    Game._mpMode   = true;
    Game._mpEngine = MultiplayerEngine;
    Game.showCharacterSelect();

    // Dopo il render della griglia casate, applica i blocchi visivi
    setTimeout(_applyMpHouseBlocks, 200);

    // Installa l'hook sul bottone "⚔ Inizia la partita"
    // Sostituisce il comportamento nativo con la conferma MP
    _installStartHook();
  }

  // ── Blocca visivamente le casate già prese da altri giocatori ──
  function _applyMpHouseBlocks() {
    if (typeof Game === 'undefined') return;

    // Mappa houseId → nome giocatore che l'ha presa
    // _takenHouses contiene { houseName: peerId } dove houseName è il nome esteso
    // Le HOUSE_GROUPS usano id brevi (es. 'Stark', 'Lannister')
    // Costruiamo una mappa normalizzata per il confronto
    const takenByOthers = {};
    Object.entries(_takenHouses).forEach(([houseName, pid]) => {
      if (pid !== _myPeerId) takenByOthers[houseName.toLowerCase()] = _players[pid]?.name || '?';
    });

    // Cerca le card nella griglia e blocca quelle delle casate prese
    const cards = document.querySelectorAll('#char-grid .char-card');
    cards.forEach(card => {
      const nameEl = card.querySelector('.char-card-name');
      if (!nameEl) return;
      const houseText = nameEl.textContent.trim().toLowerCase();

      // Controlla se qualche chiave di takenByOthers è contenuta nel testo della card
      let takerName = null;
      Object.entries(takenByOthers).forEach(([hn, who]) => {
        // Match parziale: "casa stark" contiene "stark"
        const shortKey = hn.replace(/^casa\s*/i, '').trim();
        if (houseText.includes(shortKey) || hn === houseText) takerName = who;
      });

      // Blocca casate prese da altri
      if (takerName) {
        card.style.opacity = '0.38';
        card.style.pointerEvents = 'none';
        card.style.filter = 'grayscale(0.7)';
        if (!card.querySelector('.mp-house-taken-badge')) {
      ...
      // Blocca personaggi bloccati (non ancora sbloccati dal giocatore)
      ...
      if (charIdOnCard && typeof isCharLocked === 'function' && isCharLocked(charIdOnCard)) {
        ...
        if (!card.querySelector('.mp-char-locked-badge')) {
          const badge = document.createElement('div');
    ...
    // Re-osserva: se l'utente naviga nella lista personaggi della casa, ri-applica
    if (!window._mpHouseBlockObserver) {
      const grid = document.getElementById('char-grid');
      if (grid) {
        window._mpHouseBlockObserver = new MutationObserver(() => setTimeout(_applyMpHouseBlocks, 80));
        window._mpHouseBlockObserver.observe(grid, { childList: true, subtree: false });
      }
    }
  }

  // ── Ferma l'observer quando non serve più ──
  ...
  // ── Intercetta il bottone "⚔ Inizia la partita" in char-detail-overlay ──
  function _installStartHook() {
    // Polling: il bottone viene creato dinamicamente al click su un personaggio
    const poll = setInterval(() => {
      ...
      btn.addEventListener('click', (e) => {
        // Intercetta solo in modalità MP
        if (!Game._mpMode) return;
        e.stopImmediatePropagation();

        // Leggi il personaggio dalla overlay
        const overlay = document.getElementById('char-detail-overlay');
        if (!overlay) return;

        // Il charId è nell'URL del ritratto o nel data del bottone;
        // lo recuperiamo dall'ID univoco dell'img del ritratto
        ...
        if (typeof isCharLocked === 'function' && isCharLocked(charId)) {
          _toast('🔒 Personaggio non ancora sbloccato. Completane uno precedente.', 'warn');
          overlay.remove();
          return;
        }
        ...
        // Controlla che la casata non sia stata presa nel frattempo
        const shortHouse = char.house.replace(/^Casa\s*/i, '').trim().split('/')[0].trim().toLowerCase();
        const alreadyTaken = Object.entries(_takenHouses).some(([hn, pid]) => {
          if (pid === _myPeerId) return false;
          const shn = hn.replace(/^Casa\s*/i, '').trim().split('/')[0].trim().toLowerCase();
          return shn === shortHouse || hn.toLowerCase() === char.house.toLowerCase();
        });

        // "Senza Casa" / personaggi senza casata sono sempre liberi
        const isHouseless = ['Nessuna Casa','Senza Casa','Popolo Libero',"R'hllor",'Esilio / Casa Mormont'].includes(char.house);

        if (alreadyTaken && !isHouseless) {
          _toast('⚠ Questa casata è già stata scelta da un altro giocatore!', 'warn');
          overlay.remove();
          _applyMpHouseBlocks();
          return;
        }

        // Chiude la overlay nativa senza avviare il gioco
        overlay.remove();
        _stopHouseBlockObserver();

        // Registra la scelta
        _confirmMpCharChoice(charId, char.house);
      }, true); // capture=true per intercettare prima del listener nativo

      // Ri-installa l'hook se l'overlay cambia (cambio personaggio)
    }, 120);

    // Ferma il polling se l'utente torna indietro (overlay chiusa senza scelta)
    setTimeout(() => clearInterval(poll), 30000);
  }

  // ── Conferma la scelta e sincronizza con gli altri ──
  function _confirmMpCharChoice(charId, house) {
    const prev = _players[_myPeerId];
    if (prev?.house && prev.house !== house) delete _takenHouses[prev.house];

    _players[_myPeerId].charId = charId;
    _players[_myPeerId].house  = house;
    _players[_myPeerId].ready  = false; // reset: deve ancora premere "Pronto"
    _takenHouses[house] = _myPeerId;

    const msg = { type: 'CHAR_SEL', charId, house, peerId: _myPeerId };
    if (_isHost) {
      // L'host diffonde direttamente a tutti i guest, poi aggiorna la lista completa
      _bcast(msg);
      _bcastList();
    } else {
      // Il guest invia all'host; l'host in _hChar farà _bcastList
      _toHost(msg);
    }

    // Torna alla room MP (che ora mostrerà il personaggio scelto)
    Game.showScreen?.('screen-char-select'); // assicura che non rimanga schermata singleplayer
    setTimeout(() => {
      // Se la room esiste già, aggiorna solo; altrimenti la mostra
  ...
  function _hChar(msg, from) {
    ...
    if (p) {
      ...
      p.charId = msg.charId;
      p.house  = msg.house;
      p.ready  = false; // cambio personaggio = non più pronto
      _takenHouses[msg.house] = from;
    }
    if (_isHost) {
      // Relay del messaggio CHAR_SEL agli altri guest (escluso il mittente)
      Object.entries(_connections).forEach(([pid, c]) => {
        if (pid !== from) _send(c, msg);
      });
      _bcastList();
    }
    _refreshRoom();
    // Aggiorna i blocchi se l'utente è ancora nella schermata di selezione
    if (document.getElementById('char-grid')) _applyMpHouseBlocks();
  }
  ...
  function setReady() {
    const me = _players[_myPeerId];
    if (!me?.charId) { _toast('Scegli prima un personaggio!', 'warn'); return; }
    me.ready = true;
  ...
  function hostStart() {
    if (!Object.values(_players).every(p => p.ready && p.charId)) { _toast('Non tutti pronti!', 'warn'); return; }
    _bcast({ type: 'GAME_START' });
    _startGame();
    if (_isHost) _startPingLoop();
  }
  function _hStart() { _startGame(); }

  // ══════════════════════════════════════════════════════════════════
  // AVVIO PARTITA — usa il flusso nativo con il personaggio già scelto
  // ══════════════════════════════════════════════════════════════════
  function _startGame() {
    _el('mp-room')?.remove();
    _el('mp-lobby')?.remove();
    ...
    // Usa il flusso nativo: showCharacterSelect → click casata → click personaggio → click "Inizia"
    // Ma ora lo facciamo in automatico poiché la scelta è già stata fatta manualmente
    Game.showCharacterSelect();
    _autoClickNative(char, 0);

    setTimeout(() => { _injectHud(); _injectThroneBar(); _startSync(); }, 2400);
  }

  // Naviga automaticamente alla casata e al personaggio già scelti,
  // poi clicca "⚔ Inizia la partita" senza intercettarlo (non siamo più in lobby)
  function _autoClickNative(char, attempt) {
    if (attempt > 30) { _toast('⚠ Avvio manuale: scegli ' + char.name, 'warn'); return; }
    setTimeout(() => {
      // Prova a cliccare direttamente la char-card se visibile
      const cards = document.querySelectorAll('#char-grid .char-card');
      let found = false;
      cards.forEach(card => {
        if (found) return;
        ...
        if ((nameEl && nameEl.textContent.trim() === char.name) || charIdD === char.id) {
          card.click(); found = true;
        }
      });
      if (found) { setTimeout(() => _clickNativeStart(char, 0), 500); return; }

      // Naviga prima alla casata
      const houseSuffix = char.house.replace(/^Casa\s*/i, '').trim().split('/')[0].trim();
      let houseClicked = false;
      cards.forEach(card => {
        if (houseClicked) return;
        const nameEl = card.querySelector('.char-card-name');
        if (nameEl && nameEl.textContent.includes(houseSuffix)) { card.click(); houseClicked = true; }
      });
  ...
  function _clickNativeStart(char, attempt) {
    ...
    const overlay = document.getElementById('char-detail-overlay');
    if (!overlay) { setTimeout(() => _clickNativeStart(char, attempt + 1), 300); return; }
    ...
    if (btn) {
      // Rimuovi l'hook MP dal bottone (siamo in avvio reale, non in lobby)
      btn._mpHooked = false;
      btn.click();
      return;
    }
    setTimeout(() => _clickNativeStart(char, attempt + 1), 300);
  }
  ...
  function _startSync() {
    ...
    window._mpSync = setInterval(() => {
      ...
      const me = _players[_myPeerId];
      if (me) Object.assign(me, { resources:snap.resources, turn:snap.turn, isKing:snap.isKing, state:snap.gameOver?'dead':'playing' });
  ...
  function _hState(msg) {
    ...
    if (p) Object.assign(p, { resources:msg.snap.resources, turn:msg.snap.turn, isKing:msg.snap.isKing, state:msg.snap.gameOver?'dead':'playing' });
    _updateHud();
  }
  ...
  function _hRelay(msg, from) {
    ...
    if (dest) { const c = _connections[dest]; if (c) _send(c, msg.payload); }
    else Object.entries(_connections).forEach(([pid, c]) => { if (pid !== from) _send(c, msg.payload); });
  ...
  function _showPvpReinforcements(opponentPeerId, role) {
    ...
    const oppArmy = opp?.resources?.army || 50;
  ...
  function _launchPvpBattle(opponentPeerId, role) {
    ...
    _pvpBattleState = {
      ...
      myPlacement:   null,   // posizioni che io invio all'avversario
      oppPlacement:  null,   // posizioni ricevute dall'avversario
      battleStarted: false,
  ...
  function _hWarRes(msg, from) {
    ...
    if (!isMe) _toast(`${msg.winnerName} ha vinto la battaglia!`, 'good');
  }
  ...
  function _toHost(msg)       { const c = Object.values(_connections)[0]; if (c) _send(c, msg); }
  ...
  return {
    ...
    openCharPicker: _openNativeCharSelect,  // compat alias
    _openNativeCharSelect,
    selectChar, setReady, hostStart,
    sendLobbyChat,
    kickPlayer,
    // In-game
