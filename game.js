...
const Game = (() => {
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete) {
    ...
    const phases = [
      { icon: '🐎', text: 'La cavalleria carica verso le porte della città…', delay: 2000 },
      { icon: '🔥', text: 'Le mura risuonano di grida di battaglia…', delay: 2000 },
      { icon: '⚔️', text: 'Le forze si scontrano in uno scontro feroce…', delay: 2200 },
      { icon: '🩸', text: 'La battaglia infuria — il destino è appeso a un filo…', delay: 2400 },
    ];
    ...
    function nextPhase() {
      if (i >= phases.length) {
        ...
        document.getElementById('battle-phase').innerHTML = `
          <div style="font-size:3.5rem;margin-bottom:0.75rem">${won ? '👑' : '💀'}</div>
          <div style="font-family:'Cinzel Decorative',serif;font-size:1.5rem;color:${won ? '#c9a84c' : '#dc2626'};letter-spacing:0.08em;margin-bottom:0.5rem">
            ${won ? 'VITTORIA!' : 'SCONFITTA!'}
          </div>
          <div style="color:#e8dcc8;font-family:'EB Garamond',serif;font-size:1rem;line-height:1.6;max-width:300px;margin:0 auto">
            ${won ? 'Il nemico è stato sconfitto. Le sue terre e risorse sono ora vostre.' : 'Le vostre truppe si ritirano in disordine. La battaglia è perduta.'}
          </div>
          <div style="margin-top:0.75rem;padding:0.5rem 1rem;background:rgba(255,255,255,0.05);border-radius:4px;font-size:0.8rem;color:#9a8a6a">
            🗡 Vostre forze: <strong style="color:#4ade80">${Math.round(playerForce)}</strong>
            &nbsp;·&nbsp;
            🛡 Forze nemiche: <strong style="color:#f87171">${Math.round(kingForce)}</strong>
          </div>
          <div style="margin-top:1rem;font-size:0.7rem;color:#4a3f2e;letter-spacing:0.08em">Tocca per continuare</div>
        `;
        // Click to dismiss early too
  ...
  function buildTributeDemandCard(hId, h) {
    const demandAmt = Math.floor(12 + Math.random() * 14); // 12–26, reasonable
