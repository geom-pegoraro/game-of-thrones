...
const Game = (() => {
  ...
  const CHARACTERS = [
    ...
    },
    {
      ...
      icon: '🐉', emoji: '👑',
      difficulty: 'medium',
      startResources: { gold: 28, faith: 35, people: 40, army: 25, power: 55 },
      objective: 'Il Re Mendicante: accumula 3 alleanze e Potere >70 — riconquista il rispetto che ti spetta.',
      objectiveCheck: (s) => countAllies(s) >= 3 && s.resources.power > 70,
    ...
    },
    {
      ...
      objective: 'La Conquista: domina tutte le casate — conquista 3 o ottieni 4 alleanze entro il turno 60.',
      objectiveCheck: (s) => {
        const conquered = Object.values(s.houses).filter(h => h.suppressed).length;
        return (conquered >= 3 || countAllies(s) >= 4) && s.turn <= 60;
      },
    ...
    },
    {
      ...
      objective: 'Unisci il Nord: Popolo >70, Esercito >65 e almeno 3 alleanze — o siedi sul Trono di Spade.',
      objectiveCheck: (s) => s.king === 'jon' || (s.resources.people > 70 && s.resources.army > 65 && countAllies(s) >= 3),
    ...
    },
    {
      ...
      objective: 'Re del Nord: mantieni Stark + Tully alleati, Esercito >65 e vinci almeno 1 guerra al turno 45 — o conquista il Trono di Spade.',
      objectiveCheck: (s) => (s.king === 'robb') || (s.turn >= 45 && s.houses['Stark']?.status === 'ally' && s.houses['Tully']?.status === 'ally' && s.resources.army > 65 && s.decisionHistory.some(d => d.tags?.includes('war_victory'))),
    ...
    },
    {
      ...
      flavor: 'Una goccia di sangue in uno stagno si diffonde ovunque. Così il potere.',
    ...
    },
    {
      ...
      icon: '🗡️', emoji: '😈',
      difficulty: 'hard',
      startResources: { gold: 35, faith: 10, people: 18, army: 50, power: 40 },
      objective: 'Il Bastardo Spietato: vinci 3 guerre mantenendo il Popolo sopra 0 — o siedi sul Trono di Spade.',
      objectiveCheck: (s) => s.king === 'ramsay' || (s.decisionHistory.filter(d => d.tags?.includes('war_victory')).length >= 3 && s.resources.people > 0),
      startAllies: [], startEnemies: ['Stark', 'Baratheon'],
    ...
    },
    {
      ...
      icon: '🏹', emoji: '❄️',
      difficulty: 'medium',
      startResources: { gold: 15, faith: 30, people: 55, army: 40, power: 25 },
      objective: 'Figlia del Vento: mantieni almeno 2 alleanze e Popolo >65 per 40 turni.',
      objectiveCheck: (s) => s.turn >= 40 && countAllies(s) >= 2 && s.resources.people > 65,
      startAllies: [], startEnemies: ['Lannister'],
    ...
    },
    {
      ...
      icon: '🐕', emoji: '🔥',
      difficulty: 'medium',
      startResources: { gold: 42, faith: 8, people: 35, army: 52, power: 30 },
      objective: 'Sopravvissuto: raggiungi il turno 50 con Esercito >50 e Tesoro >50 — niente eroismi, solo sopravvivenza.',
      objectiveCheck: (s) => s.turn >= 50 && s.resources.army > 50 && s.resources.gold > 50,
      startAllies: [], startEnemies: [],
    ...
    },
    {
      ...
      objective: 'La Mano del Re: Potere >75, Fede >65 e Tesoro >60 al turno 50 — o siedi (o piazza tuo sangue) sul Trono di Spade.',
      objectiveCheck: (s) => s.king === 'otto' || s.isPlayerKing || (s.turn >= 50 && s.resources.power > 75 && s.resources.faith > 65 && s.resources.gold > 60),
    ...
    },
    {
      ...
      objective: 'La Regina Reggente: siedi sul Trono (o fai sedere tuo figlio) e mantieni Fede >70 e Potere >65.',
      objectiveCheck: (s) => (s.king === 'alicent' || s.isPlayerKing) && s.resources.faith > 70 && s.resources.power > 65,
  ...
  function showPreBattleOverlay(houseId, playerInitiated = true) {
    ...
    // Calculate forces preview (same logic as triggerHouseBattle)
    const loanedArmy = state.loanedArmy || 0;
    const playerForce = state.resources.army + loanedArmy;
    ...
    // Enemy supporter contributions (from warAllianceBonus breakdown — approximate)
    const enemySupporters = Object.entries(state.houses)
      .filter(([id, hh]) => id !== houseId && hh.status === 'enemy' && !hh.suppressed)
      .map(([, hh]) => {
  ...
  function triggerHouseBattle(houseId, playerInitiated) {
    ...
    // Reveal enemy alliances now — player has already had chance to request reinforcements
    // Only reveal if not already revealed for this house (warAllianceBonus might already be set)
