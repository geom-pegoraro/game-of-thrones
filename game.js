...
const Game = (() => {
  ...
  function _showHouseSelect() {
    ...
    const HOUSE_GROUPS = [
      ...
      { id: 'Targaryen', name: 'Casa Targaryen',   icon: '🐉', region: 'Esilio / Dragonstone',  chars: ['daenerys'],                                   color: 'rgba(220,38,38,0.12)',   border: 'rgba(220,38,38,0.45)'  },
  ...
  function drawNextCard() {
    ...
    const available = EVENTS.filter(e => {
      ...
      if (charId === 'custom' && state.customCardRole) {
        ...
        if (state.customCardRole === 'shadow' && e.forChars && e.forChars.every(c => ['tyrion','ned','catelyn','sansa','margaery','olenna','jaime','daenerys','cersei','stannis','robb','roose','ramsay','tywin'].includes(c))) return false;
      }
      return true;
    });
