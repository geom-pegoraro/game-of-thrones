...
const Game = (() => {
  ...
  function _showHouseCharacters(group) {
    ...
    chars.forEach(c => {
      ...
      // Show house crest if available, else emoji
      const crestSrc = group.crest;
      const iconHtml = crestSrc
        ? `<img src="${crestSrc}" alt="${group.name}" style="width:2.8rem;height:2.8rem;object-fit:contain;filter:drop-shadow(0 1px 4px rgba(0,0,0,0.7))" onerror="this.outerHTML='<span style=\\'font-size:2.2rem\\'>${c.icon}</span>'">`
  ...
  function showPreBattleOverlay(houseId, playerInitiated = true) {
    ...
    // Player portrait — tries <id>.png, fallback to emoji
    const playerPortrait = `
      <div style="width:3.8rem;height:3.8rem;border-radius:50%;border:2px solid rgba(74,222,128,0.5);overflow:hidden;display:flex;align-items:center;justify-content:center;background:rgba(74,222,128,0.06);flex-shrink:0">
        <img id="vs-portrait-player" src="${charId}.png" alt="${charFirstName}"
          style="width:100%;height:100%;object-fit:cover"
