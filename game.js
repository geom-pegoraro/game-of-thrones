...
const Game = (() => {
  ...
  function buildPrologue(char) {
    ...
    const prologueHeaderHtml = `
      <div style="
        position:relative;width:100%;margin-bottom:0.6rem;
      ...
      ">
        ...
        <div style="position:relative;padding:0.65rem;display:flex;align-items:center;gap:0.75rem">
          <!-- Ritratto re compatto -->
          <div style="width:3.5rem;height:3.5rem;border-radius:50%;overflow:hidden;border:2px solid rgba(192,132,252,0.6);box-shadow:0 0 12px rgba(192,132,252,0.2);background:rgba(0,0,0,0.5);flex-shrink:0">
            <img src="images/characters/${king.id}.png" alt="${king.name}"
              style="width:100%;height:100%;object-fit:cover;object-position:top"
              onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
            <span style="display:none;font-size:1.6rem;width:100%;height:100%;align-items:center;justify-content:center">${king.icon}</span>
          </div>
          <!-- Info re + casate alleate inline -->
          <div style="flex:1;min-width:0">
            <div style="font-family:'Cinzel Decorative',serif;font-size:0.75rem;color:#c084fc;letter-spacing:0.04em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${king.name}</div>
            <div style="font-size:0.6rem;color:#6b5e4a;font-family:'EB Garamond',serif;font-style:italic;margin-bottom:0.3rem">👑 Re Reggente dei Sette Regni</div>
            ${kingAlliesHtml ? `
              <div style="display:flex;gap:0.4rem;flex-wrap:wrap;align-items:center">
                <span style="font-family:'Cinzel',serif;font-size:0.5rem;color:#6b5e4a;text-transform:uppercase;letter-spacing:0.08em">Fedeli:</span>
                ${Object.entries(state.houses)
                  .filter(([, h]) => h.kingAlly && !h.suppressed)
                  .map(([, h]) => `<span style="font-size:0.85rem" title="${h.name}">${h.icon}</span>`).join('')}
              </div>` : ''}
          </div>
        </div>
      </div>`;
