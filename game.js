...
const MultiplayerEngine = (() => {
    ...
    w.innerHTML = `
      <div style="width:100%;max-width:420px">
        ...
        <div class="mp-card">
          ...
          <button class="mp-btn-blue" onclick="MultiplayerEngine.joinSession()">Unisciti</button>
        </div>
  ...
  function createSession() {
    ...
    _loadPeer(() => {
      const code = _genCode();
      _peer = new Peer('ia-' + code.toLowerCase(), PEER_CFG);
      _peer.on('open', id => {
        _myPeerId = id;
        _sessionCode = code;
      ...
      _peer.on('error', e => {
        if (e.type === 'unavailable-id') { if (_peer) { _peer.destroy(); _peer = null; } setTimeout(createSession, 200); }
        else _toast('❌ Errore: ' + e.type, 'warn');
      });
    });
  }
  ...
  function joinSession() {
    ...
    _loadPeer(() => {
      ...
      _peer.on('open', myId => {
        ...
        conn.on('error', () => _toast('❌ Connessione fallita. Verifica il codice.', 'warn'));
        setTimeout(() => { if (!ok) _toast('❌ Timeout — host non raggiungibile.', 'warn'); }, 12000);
      });
      _peer.on('error', e => _toast('❌ Errore peer: ' + e.type, 'warn'));
    });
  }
  ...
  function _confirmMpCharChoice(charId, house) {
    ...
    if (_isHost) _bcastList(); else _toHost(msg);
  ...
  function _showPvpReinforcements(opponentPeerId, role) {
    ...
    const allyRows = allies.length > 0
      ? allies.map(([id, h]) => {
          ...
          if (hasLoan) return `<div style="padding:.45rem .65rem;margin-bottom:.3rem;background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.25);border-radius:3px;font-family:'EB Garamond',serif;font-size:.88rem;color:#4ade80">${h.icon} Casa ${h.name} — ⚔ +${hasLoan.amount} forniti</div>`;
          return `<button onclick="Game.requestAllyArmy('${id}');document.getElementById('mp-pvp-reinf')?.remove();MultiplayerEngine._showPvpReinforcements('${opponentPeerId}','${role}')"
            ...
            ${h.icon} Casa ${h.name} — ⚔ ${Math.round(h.army)} <span style="color:#c9a84c;float:right">Chiedi rinforzi →</span>
          </button>`;
        }).join('')
  ...
  // ── Avvio battaglia sulla scacchiera (usa showStrategicBattle del singleplayer) ──
  function _launchPvpBattle(opponentPeerId, role) {
    ...
    const CHARS    = _chars();
    const oppChar  = CHARS.find(c => c.id === opp?.charId);
    const label    = `${oppChar?.icon||'⚔'} ${opp?.name||'?'}`;

    // Notifica l'avversario che la battaglia sta iniziando
    _sendToP(opponentPeerId, {
      type:'WAR_START', from:_myPeerId, fromName:_players[_myPeerId]?.name,
      myArmy, role,
    });

    // Lancia la scacchiera del singleplayer
    if (typeof Game !== 'undefined' && Game.showBattleAnimation) {
      Game.showBattleAnimation(myArmy, oppArmy, label, (won) => {
        _pvpBattleResult(won, opponentPeerId, role, myArmy, oppArmy);
      }, true);
    } else {
      // fallback semplificato
      _runSimpleBattle(opponentPeerId, role, myArmy, oppArmy);
    }
  ...
  function _hWarStart(msg, from) {
    // L'avversario ha avviato la battaglia — aspettiamo il risultato
    _toast(`⚔ La battaglia con ${msg.fromName} è iniziata!`, 'warn');
    // Mostra schermata spettatori per i non coinvolti
  ...
  function _becomNewHost() {
    ...
    otherPeers.forEach(pid => {
      try {
        const conn = _peer.connect(pid, { reliable: true });
        let done = false;
        conn.on('open', () => {
  ...
  function _hMigrationNewHost(msg, from) {
    ...
    if (!_connections[from]) {
      ...
      try {
        const conn = _peer.connect(from, { reliable: true });
        conn.on('open', () => {
