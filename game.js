...
const Game = (() => {
  ...
  function showCharacterSelect() {
    ...
    _injectTutorialButton(); // ensure tutorial btn exists on splash
  ...
  function initState(char) {
    ...
    // One very high (75-90%), one very low (5-12%), rest distributed randomly between 15-65%
    // Excludes player's starting allies (they start trusted) and king's house
    ...
    if (shuffledForBetrayal.length > 0) {
      // One very high
      betrayalPool.push({ id: shuffledForBetrayal[0], val: 75 + Math.floor(Math.random() * 16) }); // 75-90
      if (shuffledForBetrayal.length > 1) {
        // One very low
        betrayalPool.push({ id: shuffledForBetrayal[1], val: 5 + Math.floor(Math.random() * 8) }); // 5-12
      }
      // Rest: distributed 15-65
      for (let i = 2; i < shuffledForBetrayal.length; i++) {
        betrayalPool.push({ id: shuffledForBetrayal[i], val: 15 + Math.floor(Math.random() * 51) }); // 15-65
      }
    }
    // Player's starting allies get a lower base betrayal (10-25% — trusted but not blind)
    char.startAllies.forEach(id => {
      if (houses[id]) betrayalPool.push({ id, val: 10 + Math.floor(Math.random() * 16) });
    });
    // Apply to houses
    betrayalPool.forEach(({ id, val }) => {
      if (houses[id]) {
        houses[id].betrayalChance = val;     // base value — never changes
        houses[id].betrayalReduction = 0;    // accumulates from tribute offerings
      }
    });
    // King's house: no betrayal (they're the crown, not an ally)
  ...
  function renderDiplomacy() {
    ...
    Object.entries(state.houses).forEach(([hId, h]) => {
      ...
      let statusLabel = h.status === 'ally' ? '✅ Alleati' : h.status === 'enemy' ? '⚔ Nemici' : h.status === 'diffidente' ? '🕷 Diffidente' : '⚪ Neutrali';
      ...
      // Spy intel dots — one dot per successful spy mission
      const intelLevel = (state.spyIntel || {})[hId] || 0;
      const spyDots = intelLevel > 0
        ? `<span style="position:absolute;top:5px;right:6px;display:flex;gap:3px;align-items:center">
            ${Array.from({length: intelLevel}).map(() =>
              `<span style="width:7px;height:7px;border-radius:50%;background:#fbbf24;box-shadow:0 0 4px rgba(251,191,36,0.7);display:inline-block"></span>`
            ).join('')}
           </span>`
        : '';
      ...
      card.innerHTML = `
        ...
        ${hId === state.kingHouseAffiliation && !isSuppressed ? `<span style="display:inline-block;margin-top:0.2rem;font-family:'Cinzel',serif;font-size:0.58rem;color:#c084fc;background:rgba(192,132,252,0.12);border:1px solid rgba(192,132,252,0.4);border-radius:10px;padding:0.1rem 0.4rem;letter-spacing:0.06em">👑 Casa Regnante</span>` : (h.kingAlly && !isSuppressed ? `<span style="display:inline-block;margin-top:0.2rem;font-family:'Cinzel',serif;font-size:0.58rem;color:#fbbf24;background:rgba(251,191,36,0.12);border:1px solid rgba(251,191,36,0.3);border-radius:10px;padding:0.1rem 0.4rem;letter-spacing:0.06em">🛡 Fedele al Re</span>` : '')}
        <span class="house-army">⚔ Esercito: ${isSuppressed ? '—' : Math.round(h.army)}</span>
        ${allyArmyBtn}
      `;
      // Border color: purple = king's own house, gold = king's ally, green = player ally, red = enemy
      if (!isSuppressed) {
        if (hId === state.kingHouseAffiliation) {
          card.style.borderColor = 'rgba(192,132,252,0.7)';
          card.style.boxShadow   = '0 0 8px rgba(192,132,252,0.15)';
        } else if (h.status === 'diffidente') {
          card.style.borderColor = 'rgba(251,146,60,0.7)';  // orange — suspicious
          card.style.boxShadow   = '0 0 6px rgba(251,146,60,0.12)';
        } else if (h.kingAlly && h.status === 'neutral') {
    ...
    if (!legend) {
      ...
      legend.innerHTML = `
        ...
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#9a8a6a">·</span>
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#c084fc">🟣 Casa Regnante</span>
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#9a8a6a">·</span>
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#fbbf24">🟡 Fedeli al Re</span>
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#9a8a6a">·</span>
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#fb923c">🟠 Diffidente</span>
        <span style="font-size:0.65rem;font-family:'EB Garamond',serif;color:#9a8a6a">·</span>
    ...
    if (state.ravenTarget) {
      ...
      const isKingHouse  = state.ravenTarget === state.kingHouseAffiliation;
      const isCivilWar   = !!state.civilWar;

      // In civil war: only war button, everything else locked
      // King's house: never allow any interaction except war declaration
      if (isCivilWar || isKingHouse) {
        const isPendingWar = state.pendingWarDeclaration && state.pendingWarDeclaration.houseId === state.ravenTarget;
        const warOnlyBtn = isKingHouse
          ? `<div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#c084fc;padding:0.35rem 0.1rem;font-style:italic">👑 Casa Regnante — nessun contatto diplomatico ammesso.</div>`
          : !isAlly && !isPendingWar
            ? `<button class="btn-raven" onclick="Game.ravenAction('war')" style="border-color:rgba(239,68,68,0.5)">⚔ Dichiara Guerra</button>`
            : isPendingWar
        ...
        ravenActions.innerHTML = `
          ${isCivilWar && !isKingHouse ? `<div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#f87171;padding:0.35rem 0.1rem;font-style:italic">⚔ Guerra Civile — nessun trattato possibile. Solo la guerra decide.</div>` : ''}
          ${warOnlyBtn}
      ...
      // Alliance cooldown check — only for neutrals, never for enemies or king's house
      ...
      let allianceBtn = '';
      if (!isAlly && !isEnemy && !isKingHouse) {
        allianceBtn = allianceCooldownRemaining > 0
          ? `<button class="btn-raven" disabled style="opacity:0.4;cursor:not-allowed">🤝 Alleanza (tra ${allianceCooldownRemaining} turni)</button>`
          : `<button class="btn-raven" onclick="Game.ravenAction('alliance')">🤝 Proponi Alleanza</button>`;
      }

      // Tribute offering — allies only, to reduce their betrayal chance
      let tributeBtn = '';
      if (isAlly) {
        const offeredCount = (state.tributeOffered || {})[state.ravenTarget] || 0;
        const canAffordTribute = state.resources.gold >= 10 || state.resources.army >= 10 || state.resources.people >= 10 || state.resources.faith >= 10;
        tributeBtn = canAffordTribute
          ? `<button class="btn-raven" onclick="Game.showTributeOfferOverlay('${state.ravenTarget}')" style="border-color:rgba(74,222,128,0.45);color:#4ade80">🎁 Offri Tributo${offeredCount > 0 ? ` (×${offeredCount})` : ''}</button>`
          : `<button class="btn-raven" disabled style="opacity:0.4;cursor:not-allowed">🎁 Offri Tributo</button>`;
      }

      // Spy intel button — available for ANY house (not king's house), including allies
      let spyBtn = '';
      if (!isKingHouse) {
        const isSuspicious = tgt?.status === 'diffidente';
        const intelLevel = (state.spyIntel || {})[state.ravenTarget] || 0;
        if (intelLevel >= 2) {
          const bc  = tgt?.betrayalChance || 0;
          const red = tgt?.betrayalReduction || 0;
          spyBtn = `<button class="btn-raven" disabled style="opacity:0.65;cursor:default;border-color:rgba(251,191,36,0.4);color:#fbbf24;font-size:0.68rem">🕷 Fedeltà nota: ${Math.max(0, bc - red)}% tradimento</button>`;
        } else {
          const spyCost = intelLevel === 0 ? 12 : 18;
          const label   = intelLevel === 0 ? 'Info vaga' : 'Info precisa';
          const canAffordSpy = state.resources.gold >= spyCost;
          spyBtn = canAffordSpy
            ? `<button class="btn-raven" onclick="Game.sendSpy('${state.ravenTarget}')" style="border-color:rgba(251,191,36,0.45);color:#fbbf24">🕷 Invia spia (💰${spyCost} — ${label})</button>`
            : `<button class="btn-raven" disabled style="opacity:0.4;cursor:not-allowed">🕷 Spia (💰${spyCost} — fondi insufficienti)</button>`;
        }
      }

      // Diffidente status — show forgiveness tribute button
      let diffidenteBtn = '';
      if (tgt?.status === 'diffidente' && !isKingHouse) {
        const pardonCost = (state.diffidentePardonCost || {})[state.ravenTarget] || 20;
        const canAffordPardon = state.resources.gold >= pardonCost;
        diffidenteBtn = canAffordPardon
          ? `<button class="btn-raven" onclick="Game.payDiffidentePardon('${state.ravenTarget}')" style="border-color:rgba(251,191,36,0.6);color:#fbbf24;background:rgba(251,191,36,0.06)">🕷 Paga perdono — 💰${pardonCost} oro</button>`
          : `<button class="btn-raven" disabled style="opacity:0.4;cursor:not-allowed;font-size:0.68rem">🕷 Perdono richiesto: 💰${pardonCost} oro</button>`;
      }
      let truceBtn = '';
      if (isEnemy && !isKingHouse) {
        const truceCooldownTurn = (state.truceCooldowns || {})[state.ravenTarget] || 0;
        const truceCooldownRemaining = Math.max(0, truceCooldownTurn - state.turn);
        const truceRefusals = (state.truceRefusals || {})[state.ravenTarget] || 0;
        const truceCost = 10 + truceRefusals * 6; // crescente a ogni rifiuto
        if (truceCooldownRemaining > 0) {
          truceBtn = `<button class="btn-raven" disabled style="opacity:0.4;cursor:not-allowed">🕊 Tregua (tra ${truceCooldownRemaining} turni)</button>`;
        } else {
          const resLabel = truceRefusals === 0 ? `💰 ${truceCost} oro` : `💰 ${truceCost} oro (rifiuto ${truceRefusals}×)`;
          truceBtn = `<button class="btn-raven" onclick="Game.ravenAction('truce')" style="border-color:rgba(251,191,36,0.5);color:#fbbf24">🕊 Chiedi Tregua — ${resLabel}</button>`;
        }
      }

      // Resource request — allies only, with cooldown
      ...
      const resourceReqBtn = isAlly
        ? (reqCooldownRemaining > 0
          ? `<button class="btn-raven" disabled style="opacity:0.4;cursor:not-allowed">📦 Risorse (tra ${reqCooldownRemaining} turni)</button>`
          : `<button class="btn-raven" onclick="Game.ravenAction('request_resources')">📦 Chiedi Risorse</button>`)
        : '';

      // War declaration — not for allies, not if already pending
      const isPendingWar = state.pendingWarDeclaration && state.pendingWarDeclaration.houseId === state.ravenTarget;
      let warBtn = '';
      if (!isAlly) {
        warBtn = isPendingWar
      ...
      // Reinforce button — ally + active war
      const reinforceBtn = isAlly && state.pendingWarTarget
        ? `<button class="btn-raven" onclick="Game.requestAllyArmy('${state.ravenTarget}')">⚔ Richiedi Rinforzi</button>`
        : '';

      // Block most actions if diffidente
      const isDiffidente = tgt?.status === 'diffidente';
      if (isDiffidente) {
        allianceBtn = '';
        truceBtn    = '';
        if (warBtn) warBtn = `<button class="btn-raven" onclick="Game.ravenAction('war')" style="border-color:rgba(239,68,68,0.5)">⚔ Dichiara Guerra</button>`;
      }

      ravenActions.innerHTML = `
        ${allianceBtn}
        ${isDiffidente ? diffidenteBtn : truceBtn}
        ${isDiffidente ? `<div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#fbbf24;padding:0.35rem 0.1rem;font-style:italic">🕷 Diffidente — scoperta una tua spia. Nessun contatto finché non paghi il perdono.</div>` : ''}
        ${resourceReqBtn}
        ${tributeBtn}
        ${spyBtn}
        ${reinforceBtn}
  ...
  function showTruceOverlay(hId, h, truceCost, truceRefusals) {
    const existing = document.getElementById('truce-overlay');
    if (existing) existing.remove();

    // House accepts with probability based on player's strength vs house
    const playerForce = state.resources.army + (state.loanedArmy || 0);
    const ratio = playerForce / Math.max(1, h.army);
    // Higher ratio = house more inclined to accept; also scales with gold offered
    const baseAccept = Math.min(0.80, Math.max(0.20, 0.30 + ratio * 0.25));
    ...
    overlay.innerHTML = `
      <div style="background:#12121a;border:1px solid rgba(251,191,36,0.5);border-radius:6px;width:92%;max-width:420px;padding:1.6rem;font-family:'Cinzel',serif;">
        ...
        <div style="background:rgba(201,168,76,0.05);border:1px solid rgba(201,168,76,0.15);border-radius:4px;padding:0.8rem;margin-bottom:0.85rem;font-family:'EB Garamond',serif;font-size:0.92rem;color:#e8dcc8;line-height:1.6">
          ...
          <div style="margin-top:0.5rem;font-size:0.8rem;color:#9a8a6a">Probabilità di accettazione stimata: <strong style="color:#c9a84c">~${acceptPct}%</strong></div>
        </div>
        ...
        <div style="display:flex;gap:0.65rem">
          <button onclick="Game.acceptTruce('${hId}',${truceCost},${baseAccept})" style="flex:1;padding:0.7rem;background:linear-gradient(135deg,#78350f,#c9a84c);border:none;border-radius:2px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#0a0a0f">
            💰 Paga ${truceCost} oro
          </button>
  ...
  function ravenAction(action) {
    ...
    if (action === 'alliance') {
      ...
      const truceCost = 10 + truceRefusals * 6;
      ...
      showTruceOverlay(hId, h, truceCost, truceRefusals);
  ...
  // ══════════════════════════════════════════════
  function acceptTruce(hId, cost, acceptChance) {
    ...
    if (accepted) {
      h.status = 'neutral';
      // Tregua accettata — questa casata non è più un nemico "di partenza"
      // La logica "osserva e chiedi tributo" non si applica più
      h.startingEnemy = false;
      // Reset stato tribute startingEnemy per questa casata
      ...
      // Rimuovi eventuali minacce attive
      if (state.activeThreats) delete state.activeThreats[hId];
      // Cooldown before they can be asked again for full alliance (8 turns)
      if (!state.allianceCooldowns) state.allianceCooldowns = {};
      state.allianceCooldowns[hId] = state.turn + 8;
      // Reset truce refusals on success
      if (state.truceRefusals) state.truceRefusals[hId] = 0;
      showToast(`🕊 Casa ${h.name} accetta il patto. L'oro ha smussato i rancori — ora neutrali. Potete proporre alleanza tra 8 turni.`, 'good');
    } else {
  ...
  function requestAllyArmy(hId) {
    ...
    // ── Betrayal check — applies before any loan decision ──
    // Effective betrayal = base - reduction from tributes
    const baseBetrayal = h.betrayalChance || 0;
    const reduction    = h.betrayalReduction || 0;
    const effectiveBetrayal = Math.max(0, baseBetrayal - reduction) / 100;

    if (effectiveBetrayal > 0 && Math.random() < effectiveBetrayal) {
      // Betrayal! The ally sides with the enemy instead
      h.status = 'enemy';
      if (!state.allyLoanRefusals) state.allyLoanRefusals = {};
      state.allyLoanRefusals[hId] = true;

      // Add their troops to the war enemy bonus
      ...
      // Intel leak: if player had spy info on this house, show betrayal chance was known
      const intelLevel = (state.spyIntel || {})[hId] || 0;
      const intelNote = intelLevel >= 2
        ? ` Le vostre spie lo avevano segnalato.`
        : intelLevel === 1
          ? ` Avevate sentore che non fosse del tutto affidabile.`
          : ` Nessuna vostra spia aveva segnalato questo pericolo.`;

      showToast(`💔 TRADIMENTO! ${h.icon} Casa ${h.name} si è schierata con il nemico — +${betrayalContrib} truppe nemiche!${intelNote}`, 'warn');
      updateHUD(); saveGame();
      return;
    }

    // Allies are cautious — normal refusal check
  ...
  function checkEnemyTributeDemands() {
    ...
    Object.entries(state.houses).forEach(([hId, h]) => {
      ...
      if (h.startingEnemy) {
        ...
        // ── OTHER ENEMIES: original aggressive tribute demand ──
  ...
  function buildStartingEnemyTributeCard(hId, h, threatScore) {
    ...
    return {
      ...
      onLeftChoose: () => {
        // Rifiuto: la casata considera di attaccare (entra in activeThreats con probabilità)
        ...
        // Probabilità di attacco cresce con i rifiuti: 30% → 55% → 80%
        const attackChance = Math.min(0.80, 0.30 + (refusals - 1) * 0.25);
        if (Math.random() < attackChance) {
          if (!state.activeThreats) state.activeThreats = {};
          const attackIn = Math.max(2, 4 - refusals); // attaccano prima ad ogni rifiuto
  ...
  function _resolveHouseBattle(houseId, h, won, survived, enemySurvived, playerForce, enemyForce, desertNote, playerInitiated) {
    ...
    if (state.civilWar) {
      Object.entries(state.houses).forEach(([hId, nh]) => {
        ...
        const switchChance = (betrayal / 100) * 0.25; // max 25% per battle even at 100% betrayal
        if (Math.random() < switchChance) {
          nh.status = 'enemy';
          setTimeout(() => showToast(`💔 ${nh.icon} Casa ${nh.name} ha cambiato fazione — si schiera col Re!`, 'warn'), 800);
        }
      });
    }
  ...
  function _tutPopup(anchorId, text, arrowDir, onNext, nextLabel) {
    ...
    // Highlight anchor element if provided
    if (anchorId) {
      ...
      if (anchor) {
        ...
        const dirs = {
          up:    { text:'⬇', tx:'0px', ty:'-8px', top: r.top - 50,  left: r.left + r.width/2 - 18 },
        ...
        const popupTop  = arrowDir === 'up'   ? r.top - 180  :
                          arrowDir === 'down'  ? r.bottom + 60 :
                          r.top - 20;
        const popupLeft = Math.min(window.innerWidth - 300, Math.max(8, r.left + r.width/2 - 140));
        popup.style.cssText = `top:${Math.max(8, popupTop) + window.scrollY}px;left:${popupLeft}px;`;
        popup.innerHTML = `
          <div style="font-size:0.7rem;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem">📖 Tutorial</div>
          <div style="font-family:'EB Garamond',serif;font-size:0.95rem;color:#e8dcc8;line-height:1.55;margin-bottom:0.75rem">${text}</div>
          <button onclick="Game._tutNext()" style="width:100%;padding:0.5rem;background:linear-gradient(135deg,#78350f,#c9a84c);border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#0a0a0f">
            ${nextLabel || 'Capito →'}
          </button>`;
      ...
      const popup = document.createElement('div');
      popup.className = 'tut-popup';
      popup.style.cssText = 'top:50%;left:50%;transform:translate(-50%,-50%);max-width:320px;';
      popup.innerHTML = `
        <div style="font-size:0.7rem;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem">📖 Tutorial</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.95rem;color:#e8dcc8;line-height:1.55;margin-bottom:0.75rem">${text}</div>
        <button onclick="Game._tutNext()" style="width:100%;padding:0.5rem;background:linear-gradient(135deg,#78350f,#c9a84c);border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#0a0a0f">
  ...
  function _tutStep_charSelect() {
    _tutPopup(
      ...
      () => {
        _clearTutOverlay();
        // Wait for player to click a character — we'll hook into showCharacterDetail
  ...
  function _tutStep_game() {
    _tutGameSteps = [
      // Step 0: HUD risorse
      () => _tutPopup(null,
        '🎯 <strong>Le tue risorse</strong> sono mostrate come cerchi colorati in cima allo schermo.<br>💰 Oro · ✝ Fede · 👥 Popolo · ⚔ Esercito · 👑 Potere.<br><em>Se una arriva a 0 o a 100 — è game over!</em>',
        null, _tutGameNext, 'Capito →'),
      // Step 1: carta
      () => _tutPopup('main-card',
        '🃏 <strong>Le carte</strong> sono il cuore del gioco. Ogni turno appare una carta con un evento o una richiesta. Leggi bene il testo prima di scegliere!',
        'up', _tutGameNext, 'Capito →'),
      // Step 2: scelte
      () => _tutPopup('btn-left',
        '◄► <strong>Scegli</strong> premendo i pulsanti oppure <em>swipando la carta</em> a sinistra o destra. Gli effetti sulle risorse appaiono quando passi sopra al pulsante.',
        'up', _tutGameNext, 'Capito →'),
      // Step 3: obiettivo
      () => _tutPopup('objective-bar',
        '📜 <strong>Obiettivo</strong>: ogni personaggio ha un obiettivo unico mostrato in basso. Raggiungilo per vincere! Oppure conquista il Trono di Spade sconfiggendo il Re.',
        'up', _tutGameNext, 'Capito →'),
      // Step 4: diplomazia
      () => _tutPopup('diplomacy-panel' in document ? null : null,
        '⚔ <strong>Diplomazia</strong>: premi il pulsante "⚔ Diplomazia" in alto a destra per aprire il pannello. Lì puoi proporre alleanze, dichiarare guerra, inviare spie e molto altro.',
        null, _tutGameNext, 'Capito →'),
      // Step 5: casate
      () => _tutPopup(null,
        '🏰 <strong>Le casate</strong> hanno colori diversi:<br>🟢 Verde = alleata · 🔴 Rosso = nemica · 🟣 Viola = Casa Regnante · 🟡 Oro = fedele al Re · 🟠 Arancio = diffidente.<br>Costruisci alleanze per rafforzarti!',
        null, _tutGameNext, 'Capito →'),
      // Step 6: vittoria
      () => _tutPopup(null,
        '👑 <strong>Come vincere</strong>:<br>1. Raggiungi l\'obiettivo del personaggio, oppure<br>2. Accumula abbastanza forza e sfida il Re dal pannello Diplomazia.<br><em>Buona fortuna — il Trono di Spade ti aspetta!</em>',
        null, () => { _clearTutOverlay(); _tutorialActive = false; _tutorialPendingStep = null; }, 'Inizia a giocare! 🗡'),
    ];
    _tutGameIdx = 0;
    _tutGameSteps[0]();
  }
  ...
  function _tutGameNext() {
    ...
    if (_tutGameIdx < _tutGameSteps.length) {
      setTimeout(() => _tutGameSteps[_tutGameIdx](), 250);
    } else {
  ...
  // ── Hook into showCharacterDetail ──
  const _origShowCharDetail_tut = showCharacterDetail;
  function showCharacterDetail(charId) {
    _origShowCharDetail_tut(charId);
...
// Inject tutorial button on page load
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    if (typeof Game !== 'undefined' && Game._injectTutorialButton) {
      Game._injectTutorialButton();
    }
  }, 200);
});
