...
const Game = (() => {
  ...
  function buildPrologue(char) {
    ...
    // ── Costruisci header visivo: ritratto re + sfondo casata + alleati ──
    const kingH = state.houses[state.kingHouseAffiliation] || null; // fix: era undefined
    const kingAlliesHtml = Object.entries(state.houses)
      .filter(([, h]) => h.kingAlly && !h.suppressed)
      .map(([, h]) => `
        <div style="display:flex;flex-direction:column;align-items:center;gap:0.2rem">
          <div style="width:2rem;height:2rem;border-radius:50%;overflow:hidden;border:1px solid rgba(201,168,76,0.4);background:rgba(0,0,0,0.4)">
            ${h.crest
              ? `<img src="${h.crest}" style="width:100%;height:100%;object-fit:contain;padding:2px" onerror="this.outerHTML='<span style=\\'font-size:1.1rem;display:flex;align-items:center;justify-content:center;height:100%\\'>${h.icon}</span>'">`
              : `<span style="font-size:1.1rem;display:flex;align-items:center;justify-content:center;height:100%">${h.icon}</span>`}
          </div>
          <span style="font-size:0.55rem;color:#9a8a6a;font-family:'Cinzel',serif">${h.name}</span>
        </div>`).join('');
    ...
    const prologueHeaderHtml = `
      ...
      ">
        <!-- Sfondo stemma casata re -->
        ${kingH?.crest ? `<img src="${kingH.crest}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:contain;opacity:0.06;pointer-events:none;filter:blur(2px)">` : ''}
        <div style="position:relative;padding:1rem;display:flex;flex-direction:column;align-items:center;gap:0.75rem">
          <!-- Ritratto re -->
          <div style="width:5.5rem;height:5.5rem;border-radius:50%;overflow:hidden;border:2px solid rgba(192,132,252,0.6);box-shadow:0 0 20px rgba(192,132,252,0.2);background:rgba(0,0,0,0.5)">
            <img src="images/characters/${king.id}.png" alt="${king.name}"
              style="width:100%;height:100%;object-fit:cover;object-position:top"
              onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
            <span style="display:none;font-size:2.5rem;width:100%;height:100%;align-items:center;justify-content:center">${king.icon}</span>
          </div>
          <!-- Nome e casata re -->
          <div style="text-align:center">
            <div style="font-family:'Cinzel Decorative',serif;font-size:0.85rem;color:#c084fc;letter-spacing:0.05em">${king.name}</div>
            <div style="font-size:0.65rem;color:#6b5e4a;font-family:'EB Garamond',serif;font-style:italic;margin-top:0.2rem">👑 Re Reggente dei Sette Regni</div>
          </div>
          <!-- Casate alleate al re -->
          ${kingAlliesHtml ? `
            <div style="width:100%;border-top:1px solid rgba(201,168,76,0.15);padding-top:0.6rem">
              <div style="font-family:'Cinzel',serif;font-size:0.55rem;color:#6b5e4a;letter-spacing:0.1em;text-transform:uppercase;text-align:center;margin-bottom:0.5rem">Casate fedeli alla Corona</div>
              <div style="display:flex;justify-content:center;gap:0.75rem;flex-wrap:wrap">${kingAlliesHtml}</div>
            </div>` : ''}
        </div>
      </div>`;
...
const LOCKED_CHARS = {
  'aegon_t':    { unlockBy: ['daenerys','rhaenyra'], reason: 'Completa l\'obiettivo di Daenerys o Rhaenyra' },
  'tywin':      { unlockBy: ['cersei','jaime'],      reason: 'Completa l\'obiettivo di Cersei o Jaime' },
  ...
  'littlefinger':{ unlockBy: ['tyrion'],             reason: 'Completa l\'obiettivo di Tyrion Lannister' },
