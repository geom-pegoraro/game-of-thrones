...
const Game = (() => {
  ...
  function _tutBlock(allowIds) {
    ...
    blocker.style.cssText = 'position:fixed;inset:0;z-index:899;pointer-events:none;';

    const handler = (e) => {
      // Always allow clicks inside tut-overlay (Capito button, etc.)
      if (e.target.closest('#tut-overlay')) return;
      
      // Allow clicks on explicitly allowed elements or their children
      for (const id of ids) {
        const el = document.getElementById(id);
        if (el && (el === e.target || el.contains(e.target) || e.target.closest(`#${id}`))) {
    ...
    // Use pointer-events: auto on the blocker and handle manually
    blocker.style.pointerEvents = 'auto';
    blocker.addEventListener('click', handler, true);
    blocker.addEventListener('mousedown', handler, true);
  ...
  function _tutStep_game() {
    _tutGameSteps = [
      ...
      // 2 — Scelta obbligatoria (sblocca solo carta e pulsanti, hint non bloccante)
      () => {
        _tutBlock(['btn-left','btn-right','main-card']);
        _tutPulse('btn-left', 'btn-right');
        // Show a non-blocking hint inside the card's empty area
        document.getElementById('tut-blocker')?.remove(); // remove old blocker first
        // Re-add blocker allowing card interactions
        _tutBlock(['btn-left','btn-right','main-card']);
        // Inject a non-blocking hint into card-effects-preview (pointer-events:none so it won't block clicks)
        ...
        if (preview) {
          preview.innerHTML = `<div id="tut-choice-hint" style="pointer-events:none;text-align:center;padding:0.5rem 0.75rem;background:rgba(201,168,76,0.1);border:1px solid rgba(201,168,76,0.4);border-radius:6px;font-family:'Cinzel',serif;font-size:0.68rem;color:#c9a84c;letter-spacing:0.06em;line-height:1.5">
            📖 TUTORIAL<br>
            <span style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#e8dcc8">◄► <strong>Fai una scelta!</strong> Premi un pulsante o swipa la carta.</span>
          </div>`;
        }
      ...
      // 6 — Casate (chiude il pannello diplomazia prima)
      () => {
        // Close diplomacy panel after the tutorial-forced open
        const panel = document.getElementById('diplomacy-panel');
  ...
  function abandonGame() {
    ...
    if (!confirm('Vuoi davvero abbandonare la partita e tornare alla home?')) return;
    if (typeof AudioManager !== 'undefined') AudioManager.playMainFromWar();
    localStorage.removeItem('ia_save');
    state = {};
