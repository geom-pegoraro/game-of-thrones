...
const Game = (() => {
  ...
  function ravenAction(action) {
    ...
    if (action === 'alliance') {
      ...
      if (h.status === 'ally') {
        ...
        // 3-turn war declaration delay — reveal alliances
        h.status = 'enemy';
        state.ravenTarget = null;
        
        // Schedule the actual war for 3 turns later
        if (!state.pendingWarDeclaration) {
          state.pendingWarDeclaration = {
            houseId: warTargetId,
            revealTurn: state.turn + 3,
            declaredTurn: state.turn,
          };
          // Queue the 3-turn alliance reveal card
          _scheduleWarDeclarationCards(warTargetId, h);
        }
        
        updateHUD(); toggleDiplomacy(); saveGame();
        showToast(`⚔ Guerra dichiarata a Casa ${h.name}! In 3 turni si riveleranno le alleanze e inizierà la battaglia.`, 'warn');
        return;
      }
  ...
  function challengeKing() {
    ...
    if (state.resources.army <= 70) { showToast('⚔ Devi avere Esercito >70 per sfidare il Re!', 'warn'); return; }
  ...
  function _scheduleKingChallengeCards() {
    // Turno 1 — la notizia si diffonde, puoi chiedere rinforzi dalla Diplomazia
    state.eventQueue.push({
      id: 'king_ch_t1',
      speaker: 'Corvo Reale',
    ...
    // Turno 2 — Varys rivela le forze del Re e i suoi alleati
    state.eventQueue.push({
      id: 'king_ch_t2',
      speaker: 'Varys',
      speakerRole: 'Maestro dei Sussurri — rapporto segreto',
      portrait: '🕵️', icon: '🕵️',
      text: `Le spie riportano: ${state.kingName} ha mobilitato tutte le casate a lui fedeli. I loro eserciti marciano verso la capitale. Alcune casate neutrali stanno aspettando di vedere chi avrà il sopravvento prima di schierarsi. Aprite la Diplomazia per garantirvi altri rinforzi.`,
      leftText: 'Valutiamo la situazione', leftEffects: {},
      rightText: 'Mobilitiamo le riserve', rightEffects: { army: +6, gold: -8 },
      tags: ['king_challenge_pending'],
      onResolve: () => _revealKingAlliances(),
    });

    // Turno 3 — la battaglia è inevitabile
    state.eventQueue.push({
      id: 'king_ch_t3',
      ...
      onResolve: () => _startKingBattle(),
    });
  }

  function _revealKingAlliances() {
    // Casate nemiche contribuiscono al Re
    const supporters = [];
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function showThronePhaseBreak(completedPhase) {
      if (pForce <= 0 || eForce <= 0 || completedPhase >= 4) { finalizeThroneBattle(); return; }
    ...
    function finalizeThroneBattle() {
      ...
      const survived = won ? Math.max(5, Math.round(pForce * 0.80)) : Math.max(5, Math.round(pForce * 0.25));
    ...
    Game._battleTimer = setTimeout(() => startPhase(0), 1200);
  }
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, retreatCallback) {
    ...
    function startPhase(phaseNum) {
      ...
      if (phaseLabel) phaseLabel.textContent = `Fase ${phaseNum + 1} di 3`;

      const headline = document.getElementById('battle-headline');
      if (headline) headline.textContent = ['⚔ FASE 1 — PRIMO SCONTRO', '🔥 FASE 2 — MISCHIA TOTALE', '💀 FASE 3 — SCONTRO FINALE'][phaseNum] || '⚔ BATTAGLIA';

      // Pick a phase event (different each phase)
      ...
      // Kill extra troops if event caused major force change
      ...
      // Run 2 rounds of combat, then show Continue/Retreat
      Game._battleTimer = setTimeout(() => {
        runPhaseRounds(ROUNDS_PER_PHASE, true, () => {
          // After rounds: show pause — Continue or Retreat
    ...
    function showPhaseBreak(completedPhase) {
      if (pForce <= 0 || eForce <= 0) { finalizeBattle(); return; }
      if (completedPhase >= 2) { finalizeBattle(); return; }
    ...
    function finalizeBattle() {
        ...
        setTimeout(() => {
          if (logEl) {
            ...
            logEl.textContent = won
              ? `Il campo di battaglia è vostro. Superstiti: ${survived}.`
      ...
      if (btns) {
        setTimeout(() => {
          btns.innerHTML = `
            <button onclick="if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();document.getElementById('battle-anim-overlay').remove();Game._battleResolveFn(${won},${survived})"
      ...
      Game._battleResolveFn = (won, survived) => onComplete(won, survived);
    }
  ...
  function _scheduleWarDeclarationCards(houseId, h) {
    // Turn 1: Declaration card (immediate next turn flavor)
    state.eventQueue.push({
      id: 'war_decl_t1_' + houseId,
      speaker: `Messaggero di guerra`,
      speakerRole: `Corvi inviati a tutte le casate`,
      portrait: '📜', icon: '📜',
      text: `La dichiarazione di guerra a Casa ${h.name} si è diffusa in tutto il regno. Le altre casate stanno ora valutando le proprie posizioni. Tra 2 turni scoprirete chi si schiera con loro.`,
    ...
    // Turn 2: Spy report — some houses declare support for the enemy
    state.eventQueue.push({
      id: 'war_decl_t2_' + houseId,
      speaker: `Varys`,
      speakerRole: `Maestro dei Sussurri — rapporto alleanze`,
      portrait: '🕵️', icon: '🕵️',
      text: `Le spie riportano: alcune casate hanno stretto accordi segreti con Casa ${h.name}. Non daranno il loro intero esercito, ma rinforzi arriveranno al nemico. Usate la Diplomazia per preparare i vostri alleati.`,
      leftText: 'Prendiamo nota', leftEffects: {},
      rightText: 'Mobilitiamo le riserve', rightEffects: { army: +5, gold: -6 },
      tags: ['war_pending'],
      onResolve: () => _revealEnemyAlliances(houseId, h),
    });

    // Turn 3: Battle begins
    state.eventQueue.push({
  ...
  function _revealEnemyAlliances(houseId, h) {
    ...
    if (supporters.length > 0) {
      ...
      showToast(`✅ Le spie confermano: nessuna casata si unirà a Casa ${h.name}. Combatterete soli.`, 'good');
    }
  }
...
Game._battleRetreatFinish = function(survived) {
  // Default implementation - will be overridden by specific battle contexts
  updateHUD();
  saveGame();
  checkGameOver();
