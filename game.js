...
const Game = (() => {
  ...
  const TROOP_TYPES = [
    {
      ...
      unitIcon: '🪓',
      passives: [
        { icon: '🌾', name: 'Figli della Terra', desc: 'Nessun costo di mantenimento. Il popolo combatte per dovere, non per denaro.' },
    ...
    },
    {
      ...
      passives: [
        ...
        { icon: '💪', name: 'Corazza di Scaglie', desc: 'HP 4.0× — quasi impossibile da abbattere in battaglia.' },
      ],
    },
  ];
  ...
  const CHARACTERS = [
    ...
    },
    {
      ...
      icon: '🌹', emoji: '👵',
      difficulty: 'hard',
      startResources: { gold: 68, faith: 42, people: 55, army: 30, power: 65 },
  ...
  const EVENTS = [
    ...
    },
    {
      ...
      rightText: 'Assoldali per tre stagioni', rightEffects: { gold: -14, army: +16 },
      minTurn: 2,
    },
    {
    ...
    },
    {
      ...
      rightText: 'Apri le rotte commerciali', rightEffects: { gold: +16, power: -8 },
      minTurn: 6,
    },
    {
    ...
    },
    {
      ...
      rightText: 'Combattiamo insieme', rightEffects: { army: +16, people: -5, faith: -8 },
      minTurn: 2,
    },
    {
    ...
    },
    {
      ...
      rightText: 'Il Signore della Luce vuole così', rightEffects: { faith: +16, army: +14, people: -16 },
      minTurn: 8,
    },
    {
    ...
    },
    {
      ...
      leftText: 'Riscatto in oro', leftEffects: { gold: +16, power: -5 },
    ...
    },
    {
      ...
      leftText: 'Vendilo a Braavos', leftEffects: { gold: +16 },
    ...
    },
    {
      ...
      rightText: 'Paga e raduna i Lannister', rightEffects: { gold: -14, army: +16, power: +5 },
      minTurn: 4,
    },
    ...
    },
    {
      ...
      rightText: "Rivela la verità al mondo", rightEffects: { faith: +16, power: -12, people: +8 },
      minTurn: 4,
    },
    {
...

    {
      ...
      rightText: 'Usa i vecchi canali', rightEffects: { gold: +16, faith: -10, power: -5 },
      minTurn: 2,
    },
    {
    ...
    },
    {
      ...
      leftText: 'Vendi alla miglior offerta', leftEffects: { gold: +16, power: -3 },
  ...
  function showArmyPanel(tab) {
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:500;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.75);backdrop-filter:blur(4px);';
    overlay.innerHTML = `
      <div style="background:#0e0e16;border:1px solid rgba(74,222,128,0.35);border-radius:10px;width:92%;max-width:480px;max-height:88vh;display:flex;flex-direction:column;animation:tut-pop 0.25s ease">
        <!-- Header -->
  ...
  function recruitTroop(typeId) {
    ...
    updateHUD(); saveGame();
    showArmyPanel(); // aggiorna il pannello
  }
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    function makeUnit(side, col, row, angle, stacks, troopType) {
      ...
      const typeStats = {
        ...
        drago:            { hpMult: 4.0,  dmgMult: 3.0,  spd: 1.2,  r: 6,    color: '#f87171', icon: '🐉' },
    ...
    for (let row = ROWS - 1; row >= ROWS - 2 && placed < visibleList.length; row--) {
      for (let col = 0; col < COLS && placed < visibleList.length; col++) {
        ...
        // ── Passiva: Infiltrazione — Sellaspe e Compagnia Dorata spawn dietro le linee ──
        if (entry.passive === 'infiltrato' || entry.passive === 'infiltrato_elite') {
          // Posiziona nella zona nemica (righe 0-1) invece che in fondo
          const infiltrateRow = Math.floor(Math.random() * 2); // riga 0 o 1
          const infiltrateCol = Math.floor(Math.random() * COLS);
          const pos = snapToCell(infiltrateCol, infiltrateRow);
          u.x = pos.x; u.y = pos.y;
          u.angle = Math.PI / 2; // punta verso il basso (verso il centro)
          u.infiltrating = true;
    ...
    // Ogni casata usa SOLO tipi presenti in TROOP_TYPES (milizia, sellaspe, dothraki, immacolati, compagnia_dorata)
    const HOUSE_ROSTERS = {
      Stark:      ['milizia', 'milizia', 'sellaspe'],              // uomini del nord, fedeli e numerosi
      Lannister:  ['compagnia_dorata', 'sellaspe', 'sellaspe'],    // oro = mercenari d'elite
      Tyrell:     ['sellaspe', 'milizia', 'milizia'],              // arcieri/difensori di Altogarden
      Baratheon:  ['sellaspe', 'sellaspe', 'milizia'],             // cavalieri pesanti baratheon
      Tully:      ['milizia', 'sellaspe', 'milizia'],              // difensori dei fiumi
      Martell:    ['sellaspe', 'dothraki', 'milizia'],             // veloci e letali di Dorne
      Greyjoy:    ['sellaspe', 'milizia', 'sellaspe'],             // predatori del mare
      Frey:       ['milizia', 'milizia', 'sellaspe'],              // truppe mediocri
      Bolton:     ['sellaspe', 'sellaspe', 'milizia'],             // spietati cacciatori
      Targaryen:  ['immacolati', 'dothraki', 'immacolati'],        // eserciti di Essos (già corretti)
      Hightower:  ['immacolati', 'sellaspe', 'milizia'],           // guardie di Oldtown
      _king:      ['compagnia_dorata', 'immacolati', 'sellaspe'],  // guardia reale d'elite
    };
    ...
    ov.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:4px;width:${BW+20}px;max-height:100vh;overflow:hidden">
        ...
        <!-- Legenda truppe -->
        <div style="display:flex;flex-wrap:wrap;gap:4px 8px;justify-content:center;padding:3px 4px;border-top:1px solid rgba(201,168,76,.12);margin-top:2px">
          <span style="font-size:.5rem;color:#6b5e4a;letter-spacing:.06em;text-transform:uppercase;width:100%;text-align:center;margin-bottom:1px">Legenda truppe</span>
          <span style="font-size:.55rem;color:#a3a3a3">🪓 Milizia — fragile</span>
          <span style="font-size:.55rem;color:#fbbf24">⚔️ Sellaspe — standard</span>
          <span style="font-size:.55rem;color:#fb923c">🐴 Dothraki — veloce</span>
          <span style="font-size:.55rem;color:#60a5fa">🛡️ Immacolati — resistente</span>
          <span style="font-size:.55rem;color:#fde047">💰 C.Dorata — élite</span>
          <span style="font-size:.55rem;color:#f87171">🐉 Drago — leggendario</span>
        </div>
      </div>
    `;
    document.body.appendChild(ov);
    ...
    function startCountdown(){
      ...
      cdTimer=setInterval(()=>{
        ...
        if(countdown<=0){clearInterval(cdTimer);startBattle();}
      },1000);
    }
