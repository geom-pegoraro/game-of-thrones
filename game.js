...
// ── DEV TOOL: Army booster (remove before production) ──
(function() {
  function injectDevBtn() {
    if (document.getElementById('dev-army-btn')) return;
    const btn = document.createElement('button');
    btn.id = 'dev-army-btn';
    btn.title = '[DEV] +50 Esercito';
    btn.textContent = '⚔+50';
    btn.style.cssText = [
      'position:fixed', 'bottom:2.4rem', 'left:0.75rem', 'z-index:200',
      'background:rgba(239,68,68,0.18)', 'border:1px solid rgba(239,68,68,0.5)',
      'border-radius:3px', 'font-family:Cinzel,serif', 'font-size:0.62rem',
      'font-weight:700', 'letter-spacing:0.05em', 'cursor:pointer',
      'color:#f87171', 'padding:0.25rem 0.45rem',
    ...
    btn.addEventListener('click', () => {
      const s = Game._getState && Game._getState();
      if (s && s.resources) {
        s.resources.army = Math.min((s.resourceCap || 100), s.resources.army + 50);
        Game._updateHUD && Game._updateHUD();
        Game._saveGame && Game._saveGame();
        // small toast feedback
        const t = document.getElementById('toast');
        if (t) { t.textContent = '⚔ [DEV] +50 Esercito'; t.className = 'toast'; setTimeout(() => t.classList.add('hidden'), 2000); }
      }
    });
    document.body.appendChild(btn);
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', injectDevBtn);
  } else {
    injectDevBtn();
  }
  window.addEventListener('load', injectDevBtn);
  setTimeout(injectDevBtn, 400);
})();
Game._battleRetreatFinish = function(survived) {
