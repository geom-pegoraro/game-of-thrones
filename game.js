...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    Game._battleTimer = setTimeout(() => { if (Game._throneStartPhase0) { const fn = Game._throneStartPhase0; Game._throneStartPhase0 = null; fn(); } }, THRONE_SPEED_CONFIG[_tSpeed].phaseStartDelay);
  }
  ...
  function requestAllyArmy(hId) {
    ...
    // Accetta se c'è una guerra in qualsiasi fase di preparazione o imminente
  ...
  function _scheduleWarDeclarationCards(houseId, h, skipFirstTurn) {
    ...
    // Turno 2 — Varys avvisa: il nemico sta cercando alleati. Il giocatore sceglie come reagire.
    state.eventQueue.push({
      ...
      tags: ['war_pending'],
      onLeftChoose: () => {
        // Salta direttamente alla battaglia — prima rivela le alleanze nemiche
        _revealEnemyAlliances(houseId, h);
        // Sostituisce il turno 3 con una battaglia immediata
        state.eventQueue = state.eventQueue.filter(c => c.id !== 'war_decl_t3_' + houseId);
        state.pendingWarDeclaration = null;
        setTimeout(() => triggerHouseBattle(houseId, true), 500);
      },
      onRightChoose: () => {
        // Rivela le alleanze nemiche poi apre la diplomazia sui rinforzi
        _revealEnemyAlliances(houseId, h);
    ...
    overlay.innerHTML = `
      <div style="background:#12121a;border:2px solid rgba(239,68,68,0.4);border-radius:6px;width:92%;max-width:450px;max-height:90vh;overflow-y:auto;padding:1.75rem;font-family:'Cinzel',serif;">
        ...
        <div style="display:flex;gap:0.65rem">
          <button onclick="document.getElementById('war-diplo-reinf-overlay').remove()" style="flex:1;padding:0.7rem;background:linear-gradient(135deg,#7f1d1d,#dc2626);border:none;border-radius:2px;font-family:'Cinzel',serif;font-size:0.73rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#fff">
