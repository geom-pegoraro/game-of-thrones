...
const Game = (() => {
  ...
  function _tutPopup(anchorId, text, arrowDir, onNext, nextLabel) {
    ...
    // Wait for browser layout before reading getBoundingClientRect
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
  ...
  function _positionTutPopup(overlay, anchorId, text, arrowDir, nextLabel) {
    ...
    if (anchorId) {
      ...
      if (anchor) {
        ...
        // Position popup, then adjust if it overflows viewport
        const popupTop  = arrowDir === 'up'   ? r.top - 200 :
                          arrowDir === 'down'  ? r.bottom + 60 :
                          r.top - 20;
        const popupLeft = Math.min(window.innerWidth - 310, Math.max(8, r.left + r.width/2 - 140));
        ...
        // After popup is in DOM, check if it goes off-screen and clamp
        requestAnimationFrame(() => {
          const pr = popup.getBoundingClientRect();
          if (pr.bottom > window.innerHeight - 8) {
            popup.style.top = Math.max(8, window.innerHeight - pr.height - 16 + window.scrollY) + 'px';
          }
          if (pr.right > window.innerWidth - 8) {
            popup.style.left = Math.max(8, window.innerWidth - pr.width - 16) + 'px';
          }
        });
      }
    } else {
  ...
  function _tutBlock(allowIds) {
    ...
    blocker.style.cssText = 'position:fixed;inset:0;z-index:899;background:rgba(0,0,0,0.01);';
    ...
    blocker.addEventListener('click', e => {
      ...
      if (popup) {
        popup.style.animation = 'none'; void popup.offsetWidth; popup.style.animation = 'tut-shake 0.3s ease';
        if (!document.getElementById('tut-shake-style')) {
          const s = document.createElement('style');
          s.id = 'tut-shake-style';
          s.textContent = '@keyframes tut-shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}75%{transform:translateX(6px)}}';
          document.head.appendChild(s);
        }
      }
    }, true);
  ...
  function _tutStep_game() {
    _tutGameSteps = [
      ...
      // 1 — Carta
      () => { _tutBlock(null); _tutPopup('main-card',
      ...
      // 2 — Scelta obbligatoria (sblocca solo carta e pulsanti)
      () => {
        _tutBlock(['btn-left','btn-right','main-card']);
        _tutPulse('btn-left', 'btn-right');
        _tutPopup('btn-left',
          '◄► <strong>Ora fai una scelta!</strong> Premi un pulsante o <em>swipa la carta</em>.<br><span style="color:#fbbf24">Il tutorial continuerà dopo la tua scelta.</span>',
          'up', null, null);
      ...
      // 4 — Obiettivo
      () => { _tutBlock(null); _tutPopup('objective-bar',
      ...
      // 5 — Diplomazia obbligatoria (sblocca solo btn-diplo)
      () => {
        _tutBlock(['btn-diplo']);
        _tutPulse('btn-diplo');
        _tutPopup('btn-diplo',
          '⚔ <strong>Apri la Diplomazia!</strong> Premi il pulsante evidenziato in alto a destra.<br><span style="color:#fbbf24">Il tutorial continuerà quando apri il pannello.</span>',
          'down', null, null);
      ...
      // 9 — Finale obbligatorio con messaggio
      () => { _tutBlock(null); _tutPopup(null,
        ...
        null,
        () => { _tutUnblock(); _clearTutOverlay(); _tutorialActive = false; _tutorialPendingStep = null; },
