...
const Game = (() => {
  ...
  function _injectArmyFAB() {
    ...
    fab.innerHTML = '⚔';
    fab.style.cssText = [
      'position:fixed', 'bottom:3.2rem', 'right:0.75rem', 'z-index:300',
    ...
    fab.addEventListener('mouseenter', () => { fab.style.transform='scale(1.1)'; fab.style.borderColor='rgba(74,222,128,0.9)'; });
    fab.addEventListener('mouseleave', () => { fab.style.transform='scale(1)'; fab.style.borderColor='rgba(74,222,128,0.55)'; });
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    if (Object.keys(recruitedTroops).length > 0) {
      TROOP_TYPES.forEach(t => {
        ...
        for (let i = 0; i < count; i++) playerUnitsList.push({ type: t.id });
      });
      if (state.hasDragon && !playerUnitsList.find(u => u.type === 'drago')) {
        playerUnitsList.push({ type: 'drago' });
      }
    } else {
      const n = Math.min(UNIT_CAP, Math.max(1, Math.floor(playerForce / 5)));
      for (let i = 0; i < n; i++) playerUnitsList.push({ type: 'sellaspe' });
    }
    ...
    for (let row = ROWS - 1; row >= ROWS - 2 && placed < visibleList.length; row--) {
      for (let col = 0; col < COLS && placed < visibleList.length; col++) {
        playerUnits.push(makeUnit('p', col, row, -Math.PI / 2, 1, visibleList[placed].type));
        placed++;
      }
    }
