...
const Game = (() => {
  ...
  function makeChoice(side) {
    ...
    // ── Animazione uscita carta con GSAP ──
    const el = document.getElementById('main-card');
    ...
    function _afterExit() {
      checkGameOver();
      if (!state.gameOver && !pauseFlow) { updateHUD(); drawNextCard(); saveGame(); }
      else if (!state.gameOver && pauseFlow) { updateHUD(); saveGame(); }
    }
    ...
    if (typeof gsap !== 'undefined') {
      ...
      gsap.to(el, {
        ...
        ease: 'power2.in',
        onComplete: () => {
          gsap.set(el, { x: 0, rotation: 0, opacity: 1, clearProps: 'all' });
          el.style.transform = '';
          _afterExit();
        }
      });
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _spawnSparks(el, containerId) {
      if (!el || typeof anime === 'undefined') return;
      const cont = document.getElementById(containerId);
      if (!cont) return;
      const rect = el.getBoundingClientRect();
      const cRect = cont.getBoundingClientRect();
      const colors = ['#f87171','#fbbf24','#c9a84c','#ff4444','#ffaa00'];
      for (let i = 0; i < 7; i++) {
        const s = document.createElement('div');
        s.style.cssText = `position:absolute;width:${3+Math.random()*4}px;height:${3+Math.random()*4}px;border-radius:50%;background:${colors[Math.floor(Math.random()*colors.length)]};pointer-events:none;z-index:30;left:${rect.left-cRect.left+rect.width/2}px;top:${rect.top-cRect.top+rect.height/2}px;`;
        cont.appendChild(s);
        anime({ targets: s,
          translateX: (Math.random()-0.5)*90, translateY: (Math.random()-1)*70,
          opacity: [1,0], scale: [1,0],
          duration: 450+Math.random()*250, easing: 'easeOutCubic',
          complete: () => s.remove() });
      }
    }
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      function animDie(t, prefix) {
        if (!t || t.classList.contains('dying')) return;
        t.classList.add('dying');
        _spawnSparks(t, 'throne-battle-overlay');
        if (typeof anime !== 'undefined') {
          t.textContent = '💀';
          anime({ targets: t,
            scale: [1.2, 0.15], rotate: [0, (Math.random()-0.5)*60],
            opacity: [1, 0.25], translateY: [0, 14],
            duration: 550, easing: 'easeInCubic' });
        } else {
          t.textContent = '💀'; t.style.opacity = '0.35'; t.style.transform = 'scale(0.75)';
        }
      }

      for (let i = TROOP_COUNT_TH-1; i >= pShouldBeAlive; i--) animDie(document.getElementById(`th-p-t${i}`), 'th-p');
      for (let i = TROOP_COUNT_TH-1; i >= eShouldBeAlive; i--) animDie(document.getElementById(`th-e-t${i}`), 'th-e');
      pAlive = pShouldBeAlive;
      eAlive = eShouldBeAlive;

      // Shake campo con Anime.js
      if (typeof anime !== 'undefined') {
        const field = document.getElementById('tb-field');
        if (field) anime({ targets: field,
          translateX: [-6,6,-5,5,-3,3,0], duration: 400, easing: 'easeInOutSine' });
      }
    }
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat, bgConfig) {
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      function animDie(t) {
        ...
        // Scintille
        if (typeof anime !== 'undefined') {
          const cont = document.getElementById('battle-anim-overlay');
          if (cont) {
            const rect = t.getBoundingClientRect();
            const cRect = cont.getBoundingClientRect();
            const colors = ['#f87171','#fbbf24','#c9a84c','#ff4444'];
            for (let i = 0; i < 6; i++) {
              const s = document.createElement('div');
              s.style.cssText = `position:absolute;width:${3+Math.random()*4}px;height:${3+Math.random()*4}px;border-radius:50%;background:${colors[Math.floor(Math.random()*colors.length)]};pointer-events:none;z-index:30;left:${rect.left-cRect.left+rect.width/2}px;top:${rect.top-cRect.top+rect.height/2}px;`;
              cont.appendChild(s);
              anime({ targets: s,
                translateX: (Math.random()-0.5)*80, translateY: (Math.random()-1)*60,
                opacity: [1,0], scale: [1,0],
                duration: 400+Math.random()*200, easing: 'easeOutCubic',
                complete: () => s.remove() });
            }
          }
          t.textContent = '💀';
          anime({ targets: t,
            scale: [1.2, 0.15], rotate: [0, (Math.random()-0.5)*55],
            opacity: [1, 0.25], translateY: [0, 12],
      ...
      for (let i = TROOP_COUNT-1; i >= pShouldBeAlive; i--) animDie(document.getElementById(`p-t${i}`));
      for (let i = TROOP_COUNT-1; i >= eShouldBeAlive; i--) animDie(document.getElementById(`e-t${i}`));
      pAlive = pShouldBeAlive;
      eAlive = eShouldBeAlive;
      ...
      if (typeof anime !== 'undefined') {
        const field = document.getElementById('battle-field');
        if (field) anime({ targets: field,
          translateX: [-5,5,-4,4,-2,2,0], duration: 380, easing: 'easeInOutSine' });
      }
    }
  ...
  function initSwipe() {
    ...
    function onMove(e) {
      ...
      const drag = currentX * 0.9;
      const rot  = Math.max(-22, Math.min(22, currentX * 0.08));
      const lift = Math.min(0, -Math.abs(currentX) * 0.04);

      if (typeof gsap !== 'undefined') {
        gsap.set(card, { x: drag, rotation: rot, y: lift, clearProps: 'transition' });
      } else {
