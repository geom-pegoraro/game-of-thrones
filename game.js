  ...
  const EVENTS = [
...

    {
      ...
      onRightChoose: () => {
        const roll = Math.random();
        if (roll > 0.6) {
          state.resources.faith = Math.min(state.resources.faith + 18, 100);
          state.resources.power = Math.min(state.resources.power + 10, 100);
          showToast('🥚 L\'uovo era autentico. La leggenda del drago alimenta la vostra fama. +18 fede, +10 potere.', 'good');
        } else {
