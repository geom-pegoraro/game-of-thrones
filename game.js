...
const Game = (() => {
  ...
  function renderCard(card) {
    ...
    // Visual override for ultimatum/tribute demand cards — red pulsing border
    const isUltimatum = card.tags?.includes('tribute_demand') || card.tags?.includes('house_attack_final');
    el.style.borderColor = isUltimatum ? 'rgba(239,68,68,0.8)' : '';
    el.style.boxShadow   = isUltimatum
      ? '0 10px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(239,68,68,0.4), 0 0 20px rgba(239,68,68,0.2)'
      : '';

    // Add/remove ultimatum flash animation
    let flashEl = document.getElementById('card-ultimatum-flash');
    if (isUltimatum) {
      if (!flashEl) {
        flashEl = document.createElement('div');
        flashEl.id = 'card-ultimatum-flash';
        flashEl.style.cssText = `
          position:absolute;inset:0;border-radius:8px;pointer-events:none;z-index:5;
          background:linear-gradient(135deg,rgba(239,68,68,0.06) 0%,transparent 60%);
          animation:ultimatum-pulse 2s ease-in-out infinite;
        `;
        if (!document.getElementById('ultimatum-style')) {
          const s = document.createElement('style');
          s.id = 'ultimatum-style';
          s.textContent = `
            @keyframes ultimatum-pulse {
      ...
      let badge = document.getElementById('card-ultimatum-badge');
      if (!badge) {
        badge = document.createElement('div');
        badge.id = 'card-ultimatum-badge';
        badge.style.cssText = `
          position:absolute;top:-10px;left:50%;transform:translateX(-50%);
          background:linear-gradient(135deg,#7f1d1d,#dc2626);
          color:#fff;font-family:'Cinzel',serif;font-size:0.62rem;font-weight:700;
          letter-spacing:0.12em;text-transform:uppercase;
          padding:0.2rem 0.7rem;border-radius:20px;z-index:10;
          box-shadow:0 2px 8px rgba(239,68,68,0.5);white-space:nowrap;
        `;
      ...
      const badge = document.getElementById('card-ultimatum-badge');
