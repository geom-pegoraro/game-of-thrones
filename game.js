...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _bc_drawSoldier(ctx, x, y, flip, phase, state, alpha) {
      ...
      let bodyBob=0, legSw=0, armSw=0, swordA=0;
      if (state==='march') { bodyBob=Math.sin(t*9+phase)*1.5; legSw=Math.sin(t*9+phase)*20; armSw=Math.sin(t*9+phase)*15; }
      else if (state==='attack') { bodyBob=Math.sin(t*16)*2; swordA=Math.sin(t*14)*50-15; armSw=Math.sin(t*16)*28; }
      else { bodyBob=Math.sin(t*3+phase)*0.8; armSw=Math.sin(t*3+phase)*4; }

      ctx.save(); ctx.globalAlpha=alpha;
      if (flip) {
        ctx.translate(x, y); ctx.scale(-1, 1); // specchia soldato nemico verso sinistra
      } else {
        ctx.translate(x, y);
      }
      ctx.fillStyle='#0d0d0d'; ctx.strokeStyle='#222'; ctx.lineWidth=0.5;

      // Testa + elmo
      ctx.beginPath(); ctx.arc(0,-32+bodyBob,5,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(0,-36+bodyBob,6.5,4,0,Math.PI,0); ctx.fill();
      ctx.fillRect(-3,-34+bodyBob,6,2.5);

      // Corpo
      ctx.beginPath(); ctx.roundRect(-4,-26+bodyBob,8,13,2); ctx.fill();

      // Braccio sx + spada
      ctx.save(); ctx.translate(-4,-22+bodyBob); ctx.rotate((armSw+swordA*0.3)*Math.PI/180);
      ctx.fillRect(-5,0,4,2.5);
      ctx.save(); ctx.translate(-5,0); ctx.rotate((-35+swordA)*Math.PI/180);
      ctx.fillStyle='#1c1c1c'; ctx.fillRect(-1,-16,2,16); ctx.fillRect(-3.5,-1,7,2); ctx.fillRect(-1,1,2,4);
      ctx.restore(); ctx.restore();

      // Braccio dx + scudo
      ctx.save(); ctx.translate(4,-22+bodyBob); ctx.rotate(-armSw*Math.PI/180);
      ctx.fillRect(0,0,5,2.5);
      ctx.save(); ctx.translate(7,-5); ctx.fillStyle='#111';
      ctx.beginPath(); ctx.roundRect(-3,0,5,10,[2,2,4,4]); ctx.fill();
      ctx.fillStyle='#1c1c1c'; ctx.fillRect(-0.8,2,1.6,6);
      ctx.restore(); ctx.restore();

      // Gambe
      ctx.save(); ctx.translate(-2,-13+bodyBob); ctx.rotate(legSw*Math.PI/180);
      ctx.fillRect(-2,0,4,12); ctx.fillRect(-2.5,10,5,3); ctx.restore();
      ctx.save(); ctx.translate(2,-13+bodyBob); ctx.rotate(-legSw*Math.PI/180);
      ctx.fillRect(-2,0,4,12); ctx.fillRect(-2,10,5,3); ctx.restore();
      ctx.restore();
    }
