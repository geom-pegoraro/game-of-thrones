...
const Game = (() => {
  ...
  function drawNextCard() {
    ...
    if (state.pendingWarTarget || state.pendingWarDeclaration || state.pendingKingChallenge) {
      currentCard = {
        id: 'war_wait',
        speaker: 'Varys', speakerRole: 'Maestro dei Sussurri — Rapporto riservato',
        portrait: `<img src="images/characters/varys.png" alt="Varys" style="width:5rem;height:5rem;object-fit:cover;object-position:top;border-radius:50%;border:2px solid rgba(251,191,36,0.4)" onerror="this.outerHTML='🕷'">`,
        icon: '🕷',
        text: 'Le truppe sono in marcia. I corvi volano tra le casate. «I miei informatori mi dicono che il nemico si prepara — ma anche i vostri alleati osservano. Il prossimo turno porterà nuove informazioni dal fronte.» — Varys',
  ...
  function renderCard(card) {
    ...
    // Portrait: supporta sia emoji/testo che HTML (es. ritratto Varys)
    const portraitEl = document.getElementById('card-portrait');
    const p = card.portrait || '📜';
    if (p.startsWith('<')) { portraitEl.innerHTML = p; }
    else { portraitEl.textContent = p; }
  ...
  function makeChoice(side) {
    ...
    state.turn++;
    state.tributeThisTurn = 0; // reset donazioni per turno
  ...
  function _showSpyReportOverlay(h, hId, narrative, isPrecise, effective) {
    ...
    const riskLabel = effective >= 60 ? '🔴 ALTO RISCHIO' : effective >= 35 ? '🟡 RISCHIO MODERATO' : '🟢 CASATA FEDELE';
    const riskColor = effective >= 60 ? '#f87171' : effective >= 35 ? '#fbbf24' : '#4ade80';

    // Ritratto Varys
    const varysSrc = 'images/characters/varys.png';
    ...
    overlay.innerHTML = `
      <div style="background:#12121a;border:1px solid rgba(251,191,36,0.45);border-radius:6px;width:92%;max-width:420px;padding:1.5rem;font-family:'Cinzel',serif;">

        <!-- Header Varys con ritratto -->
        <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.85rem;padding-bottom:0.75rem;border-bottom:1px solid rgba(251,191,36,0.15)">
          <div style="width:3rem;height:3rem;border-radius:50%;overflow:hidden;border:2px solid rgba(251,191,36,0.5);background:rgba(0,0,0,0.4);flex-shrink:0">
            <img src="${varysSrc}" alt="Varys" style="width:100%;height:100%;object-fit:cover;object-position:top"
              onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
            <span style="display:none;font-size:1.6rem;width:100%;height:100%;align-items:center;justify-content:center">🕷</span>
          </div>
          <div>
            <div style="font-family:'Cinzel Decorative',serif;color:#fbbf24;font-size:0.85rem">Varys — Maestro dei Sussurri</div>
            <div style="font-size:0.6rem;color:#6b5e4a;font-family:'EB Garamond',serif;font-style:italic">${isPrecise ? 'Analisi completa' : 'Prima ricognizione'} · ${h.name}</div>
          </div>
        </div>

        <!-- Stemma + stato casata -->
        <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.75rem;padding:0.6rem;background:rgba(251,191,36,0.04);border:1px solid rgba(251,191,36,0.12);border-radius:4px">
          <span style="font-size:1.6rem">${h.icon}</span>
          <div style="flex:1">
            <div style="font-family:'Cinzel',serif;font-size:0.8rem;color:#e8dcc8">Casa ${h.name}</div>
            <div style="font-size:0.68rem;color:${riskColor};font-family:'Cinzel',serif;margin-top:0.1rem">${riskLabel}</div>
          </div>
        </div>

        <!-- Rapporto narrativo -->
        <div style="background:rgba(251,191,36,0.04);border-left:3px solid rgba(251,191,36,0.4);padding:0.75rem 0.85rem;margin-bottom:0.85rem;font-family:'EB Garamond',serif;font-size:0.92rem;color:#e8dcc8;line-height:1.65;font-style:italic;border-radius:0 4px 4px 0">
          «${narrative}»
        </div>

        <!-- Barra fedeltà (sempre visibile, precisione varia) -->
        <div style="margin-bottom:0.9rem">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.35rem">
            <span style="font-size:0.62rem;color:#9a8a6a;letter-spacing:0.08em;text-transform:uppercase">Fedeltà stimata</span>
            <span style="font-size:0.72rem;color:${barColor};font-family:'Cinzel',serif">${isPrecise ? loyalty+'%' : '~'+Math.round(loyalty/10)*10+'%'}</span>
          </div>
          <div style="height:8px;background:rgba(255,255,255,0.06);border-radius:4px;overflow:hidden;margin-bottom:0.3rem">
            <div style="height:100%;width:${isPrecise ? loyalty : Math.round(loyalty/10)*10}%;background:${barColor};border-radius:4px;transition:width 1s ease;box-shadow:0 0 6px ${barColor}60"></div>
          </div>
          ${isPrecise
            ? `<div style="display:flex;justify-content:space-between;font-size:0.65rem;color:#6b5e4a;font-family:'EB Garamond',serif">
                <span>⚠ Rischio tradimento: <strong style="color:${effective>=40?'#f87171':'#9a8a6a'}">${effective}%</strong></span>
                <span>✅ Fedeltà: <strong style="color:${barColor}">${loyalty}%</strong></span>
               </div>`
            : `<div style="font-size:0.65rem;color:#6b5e4a;font-family:'EB Garamond',serif;text-align:center;font-style:italic">
                Stima approssimativa — invia una seconda spia per dati precisi
               </div>`}
        </div>
  ...
  function showTributeOfferOverlay(hId) {
    ...
    // ── Limita abuso donazioni ──
    if (!state.tributeCooldown) state.tributeCooldown = {};
    if (!state.tributeThisTurn) state.tributeThisTurn = 0;

    const cooldownUntil = state.tributeCooldown[hId] || 0;
    if (cooldownUntil > state.turn) {
      showToast(`🎁 Casa ${h.name} non accetta altri doni per altri ${cooldownUntil - state.turn} turni.`, 'warn');
      return;
    }
    if (state.tributeThisTurn >= 2) {
      showToast('🎁 Hai già donato risorse a due casate questo turno. Aspetta il prossimo.', 'warn');
      return;
    }
    // Controlla se il giocatore ha risorse molto alte — avviso se sta abusando
    const highResCount = ['gold','faith','people','army'].filter(r => state.resources[r] > 80).length;
    if (highResCount === 0) {
      showToast('🎁 Le tue risorse non sono abbastanza elevate per offrire tributi generosi.', 'warn');
      return;
    }
  ...
  function acceptTributeOffer(hId, resKey) {
    ...
    // Verifica minimo 30 per non morire donando
    if (state.resources[resKey] < 30) { showToast('Troppo poche risorse per donare in sicurezza.', 'warn'); return; }
    ...
    if (!state.tributeCooldown) state.tributeCooldown = {};
    state.tributeCooldown[hId] = state.turn + 5; // cooldown 5 turni
    state.tributeThisTurn = (state.tributeThisTurn || 0) + 1;
