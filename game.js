...
const Game = (() => {
  ...
  function sendSpy(hId) {
    ...
    // Discovery chance: 15% base, +10% for hostile/enemy houses, +8% per existing intel level
    // (harder to infiltrate deeper)
    const discoveryBase = 0.15 + (h.status === 'enemy' ? 0.10 : 0) + currentLevel * 0.08;
    const discovered = Math.random() < discoveryBase;

    if (discovered) {
      // Spia scoperta — casata diventa diffidente
      const wasPrevStatus = h.status;
      h.status = 'diffidente';
      _resetBetrayalReduction(hId); // intel lost too — they know

      // Pardon cost: higher if they were an ally or enemy
      const pardonCost = wasPrevStatus === 'ally'  ? 30 :
                         wasPrevStatus === 'enemy' ? 25 : 20;
      if (!state.diffidentePardonCost) state.diffidentePardonCost = {};
      state.diffidentePardonCost[hId] = pardonCost;

      // Show discovery overlay
      _showSpyDiscoveredOverlay(h, hId, pardonCost, wasPrevStatus);
      updateHUD(); saveGame();
      toggleDiplomacy();
      return;
    }

    // Success — advance intel level
  ...
  function _showSpyDiscoveredOverlay(h, hId, pardonCost, prevStatus) {
    ...
    overlay.innerHTML = `
      <div style="background:#12121a;border:2px solid rgba(251,146,60,0.6);border-radius:6px;width:92%;max-width:400px;padding:1.6rem;font-family:'Cinzel',serif;">
        ...
        <div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#9a8a6a;margin-bottom:0.9rem">
          Potete pagare il perdono in qualsiasi momento dal pannello Diplomazia selezionando Casa ${h.name}.
        </div>
