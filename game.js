...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    let _bc_soldiers = [], _bc_blood = [], _bc_sparks = [];
    ...
    function _bc_loop(fieldId) {
      ...
      // Avanzamento fluido
      _bc_soldiers.forEach(s=>{ if(s.alive){ const dx=s.tx-s.x; s.x+=dx*0.035; }});
    ...
    function _bc_advance(progress) {
      ...
      // Gap diminuisce con il progresso — i sopravvissuti avanzano verso il centro
      const gap=Math.max(10, 22+(1-progress)*(W*.30));
      // Giocatori: i primi (indice 0) sono in prima linea (più vicini al centro)
      pS.forEach((s,i)=>{ s.tx=Math.min(center-gap-i*20, center-12); });
      // Nemici: i primi sono in prima linea (più vicini al centro)
      eS.forEach((s,i)=>{ s.tx=Math.max(center+gap+i*20, center+12); });
    }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => _bc_advance(0.2), 400);
    }, 80);
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat, bgConfig) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => _bc_advance(0.25), 400);
    }, 80);
