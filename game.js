...
const Game = (() => {
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    const pCount = Math.min(16, Math.max(1, Math.floor(playerForce / 10)));
    const eCount = Math.min(16, Math.max(1, Math.floor(enemyForce / 10)));

    const COLS = 4, ROWS = 8;
    const CW = 64, CH = 44;
    const BW = COLS * CW;   // 256
    const BH = ROWS * CH;   // 352
    const UR = 18;
    const HP_BASE = 5;
    const SPEED_PX = 34;
    const ATK_RANGE = UR * 2.5;
    const ATK_INTERVAL = 2200; // più lenti

    // Stemma casata giocatore
    const playerHouseId = state.character?.house?.split(' ').pop() || '';
    const playerHouseDef = HOUSES_DEF.find(h => h.id === playerHouseId || state.character?.house?.includes(h.id));
    const playerCrest = playerHouseDef?.crest || null;
    const playerIcon  = playerHouseDef?.icon || '⚔';

    // Stemma casata nemica (dalla pendingWarTarget o king)
    ...
    const enemyIconChar = enemyHouseDef?.icon || '⚔';
    ...
    function spawnParticles(x, y, type) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        particles.push({
          ...
          r: type === 'blood' ? 2 + Math.random() * 2.5 : 1.5 + Math.random() * 2,
          color: type === 'blood'
            ? `rgba(${160+Math.floor(Math.random()*60)},${Math.floor(Math.random()*20)},${Math.floor(Math.random()*10)},`
            : `rgba(${220+Math.floor(Math.random()*35)},${180+Math.floor(Math.random()*60)},${Math.floor(Math.random()*40)},`,
    ...
    function nearestCell(x, y) {
      const col = Math.max(0, Math.min(COLS - 1, Math.round((x - CW/2) / CW)));
      const row = Math.max(0, Math.min(ROWS - 1, Math.round((y - CH/2) / CH)));
      return { col, row };
    }
    ...
    function makeUnit(side, col, row, angle, stacks) {
      ...
      return {
        ...
        angle,
        hp: HP_BASE * stacks,
        maxHp: HP_BASE * stacks,
        stacks, dmg: stacks,
        alive: true, dead: false, deadAt: 0,
        lastAtk: 0,
      };
    }
    ...
    const CPU_STRATS = [
      ...
      (n) => [[1,0],[2,0],[0,1],[3,1],[1,1],[2,1],[0,0],[3,0]].slice(0,n),
      (n) => { const u=[]; for(let i=0;i<n;i++){const c=i%2===0?0:COLS-1;u.push([c,Math.floor(i/2)]);} return u; },
      (n) => { const u=[]; for(let i=0;i<n;i++) u.push([i%COLS,i<4?0:1]); return u; },
      (n) => { const u=[]; for(let i=0;i<n;i++) u.push([1+i%2,Math.floor(i/2)]); return u; },
    ];
    const cpuStrat = CPU_STRATS[Math.floor(Math.random() * CPU_STRATS.length)];
    const enemyUnits = [];
    cpuStrat(eCount).forEach(([col, row]) => enemyUnits.push(makeUnit('e', col, row, Math.PI / 2)));
    ...
    ov.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:5px;width:${BW+24}px">
        <div style="display:flex;justify-content:space-between;width:100%;font-size:.58rem;letter-spacing:.1em;text-transform:uppercase">
          <span style="color:#4ade80">🛡 TUE FORZE <strong>${Math.round(playerForce)}</strong></span>
          <span id="sb-phase" style="color:#9a8a6a">POSIZIONAMENTO</span>
          <span style="color:#f87171">⚔ ${enemyLabel} <strong>${Math.round(enemyForce)}</strong></span>
        </div>

        <!-- Barre HP in tempo reale -->
        <div style="display:flex;gap:6px;width:100%;align-items:center">
          <div style="flex:1;height:6px;background:rgba(74,222,128,.15);border-radius:3px;overflow:hidden">
            <div id="sb-pbar" style="height:100%;width:100%;background:linear-gradient(90deg,#16a34a,#4ade80);border-radius:3px;transition:width .3s"></div>
          </div>
          <div style="font-size:.6rem;color:#9a8a6a;min-width:28px;text-align:center" id="sb-units">
            <span id="sb-pu" style="color:#4ade80">${pCount}</span><span style="color:#6b5e4a">vs</span><span id="sb-eu" style="color:#f87171">${eCount}</span>
          </div>
          <div style="flex:1;height:6px;background:rgba(248,113,113,.15);border-radius:3px;overflow:hidden">
            <div id="sb-ebar" style="height:100%;width:100%;background:linear-gradient(90deg,#f87171,#dc2626);border-radius:3px;transition:width .3s;margin-left:auto"></div>
          </div>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div id="sb-msg" style="font-size:.6rem;color:#9a8a6a;line-height:1.4;text-align:center">Posiziona le truppe nei quadrati.<br>Sovrapponi 2 cerchietti per fonderli.</div>
        </div>

        <div style="position:relative;width:${BW}px;height:${BH}px;border:1px solid rgba(201,168,76,.3);border-radius:4px;overflow:hidden;background:#08080f">
          ...
          <div style="position:absolute;left:0;right:0;top:50%;height:1px;background:rgba(201,168,76,.3);pointer-events:none"></div>
          <div style="position:absolute;top:3px;left:5px;font-size:.46rem;color:#f87171;opacity:.7">${enemyLabel.toUpperCase()}</div>
          <div style="position:absolute;bottom:3px;left:5px;font-size:.46rem;color:#4ade80;opacity:.7">LE TUE TRUPPE</div>
        </div>

        <div id="sb-btns" style="display:flex;gap:10px;margin-top:3px">
          ${allowRetreat ? '<button id="sb-ret" style="padding:.45rem 1rem;background:transparent;border:1px solid rgba(201,168,76,.4);border-radius:3px;font-family:Cinzel,serif;font-size:.62rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;color:#c9a84c">🏃 Ritirata</button>' : ''}
        </div>
      </div>
    `;
    ...
    // Precarica stemmi
    const pImg = playerCrest ? (() => { const i=new Image(); i.src=playerCrest; return i; })() : null;
    const eImg = enemyCrest  ? (() => { const i=new Image(); i.src=enemyCrest;  return i; })() : null;

    function drawGrid() {
      gCtx.clearRect(0,0,BW,BH);
      gCtx.strokeStyle='rgba(239,68,68,.10)'; gCtx.lineWidth=1;
      for(let c=1;c<COLS;c++){gCtx.beginPath();gCtx.moveTo(c*CW,0);gCtx.lineTo(c*CW,BH/2);gCtx.stroke();}
      ...
      for(let c=1;c<COLS;c++){gCtx.beginPath();gCtx.moveTo(c*CW,BH/2);gCtx.lineTo(c*CW,BH);gCtx.stroke();}
      for(let r=ROWS/2+1;r<ROWS;r++){gCtx.beginPath();gCtx.moveTo(0,r*CH);gCtx.lineTo(BW,r*CH);gCtx.stroke();}
    }
    drawGrid();

    const fogPts = Array.from({length:40},()=>({
      x:Math.random()*BW, y:Math.random()*BH/2,
      r:22+Math.random()*28, vx:(Math.random()-.5)*.3, vy:(Math.random()-.5)*.2,
      a:.3+Math.random()*.26
    }));
    function drawFog() {
      fCtx.clearRect(0,0,BW,BH);
      if(fogLifted)return;
      fCtx.fillStyle='rgba(6,3,3,.58)'; fCtx.fillRect(0,0,BW,BH/2);
      fogPts.forEach(p=>{
        p.x+=p.vx; p.y+=p.vy;
        if(p.x<-40)p.x=BW+5; if(p.x>BW+40)p.x=-5;
        if(p.y<-10)p.y=BH/2+5; if(p.y>BH/2+10)p.y=-5;
        const g=fCtx.createRadialGradient(p.x,p.y,0,p.x,p.y,p.r);
        g.addColorStop(0,`rgba(52,40,36,${p.a})`); g.addColorStop(1,'rgba(52,40,36,0)');
    ...
    function updateBars() {
      ...
      const pb = document.getElementById('sb-pbar');
      const eb = document.getElementById('sb-ebar');
      const pu = document.getElementById('sb-pu');
      const eu = document.getElementById('sb-eu');
    ...
    function drawUnit(u) {
      if(u.dead) {
        ...
        if(age>4)return;
        mCtx.save();
        mCtx.globalAlpha=Math.max(0,.65-age*.16);
        mCtx.font=`${UR*1.2}px serif`;
      ...
      if(crestImg && crestImg.complete && crestImg.naturalWidth>0) {
        ...
        mCtx.drawImage(crestImg, -r*0.85, -r*0.85, r*1.7, r*1.7);
        mCtx.restore();
      }

      // Corpo cerchio
      mCtx.beginPath(); mCtx.arc(0,0,r,0,Math.PI*2);
      mCtx.fillStyle = iP?'rgba(8,30,16,.90)':'rgba(30,8,8,.90)';
      mCtx.fill();
      mCtx.strokeStyle = isSel?'#fbbf24':col;
      mCtx.lineWidth = isSel?2.8:1.8;
      mCtx.stroke();

      // ── Semicerchio "testa" sul perimetro (fronte scura) ──
      mCtx.rotate(u.angle);
      // Semicerchio anteriore — colore più scuro/intenso indica la fronte
      const headColor = iP ? 'rgba(16,200,80,.65)' : 'rgba(240,60,60,.65)';
      mCtx.beginPath();
      mCtx.arc(0, 0, r+2, -Math.PI/2.2, Math.PI/2.2);
      mCtx.strokeStyle = headColor;
      mCtx.lineWidth = 3.5;
      mCtx.stroke();

      // ── Solo punta triangolare della freccia ──
      mCtx.beginPath();
      mCtx.moveTo(r+4, 0);
      mCtx.lineTo(r-2, -5);
      mCtx.lineTo(r-2, 5);
      mCtx.closePath();
      mCtx.fillStyle = isSel?'#fbbf24':col;
      mCtx.fill();

      mCtx.restore();

      // HP bar sottile sotto il cerchio
      const bw=r*2, bx=u.x-r, by=u.y+r+4;
      ...
      // Numero HP / stacks al centro
      mCtx.fillStyle='rgba(255,255,255,.88)';
      mCtx.font=`bold ${u.stacks>1?11:10}px sans-serif`;
      mCtx.textAlign='center'; mCtx.textBaseline='middle';
    ...
    function render() {
      ...
      mCtx.fillStyle='rgba(5,5,12,.3)'; mCtx.fillRect(0,0,BW,BH);
    ...
    function canvasPos(e) {
      ...
      // Su touch: touches può essere vuoto al pointerup → usa clientX diretto
    ...
    function hitPlayer(pos, extra=6) {
      for(const u of playerUnits){
        if(u.dead||!u.alive)continue;
        const r=UR+(u.stacks-1)*1.8;
        const dx=pos.x-u.x, dy=pos.y-u.y;
        if(dx*dx+dy*dy<(r+extra)*(r+extra))return u;
      }
      return null;
    }

    // Touch drag con setPointerCapture per grip stabile su mobile
    mainCv.addEventListener('pointerdown',e=>{
      if(phase!=='placement')return;
      const pos=canvasPos(e);
      // Hitbox più grande su touch (pointerType=touch)
      const extra = e.pointerType==='touch' ? 14 : 6;
      const h=hitPlayer(pos, extra);
      if(h){
        dragUnit=h; selectedId=h.id;
        dragOX=pos.x-h.x; dragOY=pos.y-h.y;
    ...
    mainCv.addEventListener('pointermove',e=>{
      ...
      const pos=canvasPos(e);
      // Snap live alla cella più vicina durante il drag
      const rawX=Math.max(UR,Math.min(BW-UR,pos.x-dragOX));
      const rawY=Math.max(BH/2+UR,Math.min(BH-UR,pos.y-dragOY));
      const {col,row}=nearestCell(rawX,rawY);
      const safeRow=Math.max(ROWS/2,Math.min(ROWS-1,row));
      const snapped=snapToCell(col,safeRow);
      dragUnit.x=snapped.x; dragUnit.y=snapped.y;
      e.preventDefault();
    },{passive:false});
    mainCv.addEventListener('pointerup',e=>{
      if(dragUnit&&phase==='placement'){
        const {col,row}=nearestCell(dragUnit.x,dragUnit.y);
        const safeRow=Math.max(ROWS/2,Math.min(ROWS-1,row));
        const snapped=snapToCell(col,safeRow);
        const other=playerUnits.find(u=>u!==dragUnit&&u.alive&&!u.dead&&
          Math.abs(u.x-snapped.x)<CW*.4&&Math.abs(u.y-snapped.y)<CH*.4);
        if(other){
    ...
    mainCv.addEventListener('pointercancel',e=>{
      try{ mainCv.releasePointerCapture(e.pointerId); }catch(_){}
      dragUnit=null; mainCv.style.cursor='grab';
    });
    ...
    function startBattle(){
      phase='battle';
      fogLifted=true;
      // Nascondi griglia scacchiera
      const _gCv=document.getElementById('sb-grid');
      if(_gCv) _gCv.style.opacity='0';
      ...
      function angleToUnit(from,to){return Math.atan2(to.y-from.y,to.x-from.x);}
      function dmgMult(atkAngle,victimAngle){
        let diff=atkAngle-(victimAngle+Math.PI);
      ...
      function tick(ts){
        ...
        [...ap,...ae].forEach(u=>{
          ...
          let near=opps[0],nearD=dist(u,opps[0]);
          opps.forEach(op=>{const d=dist(u,op);if(d<nearD){nearD=d;near=op;}});

          const ta=angleToUnit(u,near);
          let da=ta-u.angle;
          ...
          if(nearD>ATK_RANGE){
            ...
            if(ts-u.lastAtk>=ATK_INTERVAL){
              u.lastAtk=ts;
              const atkAngle=angleToUnit(u,near);
              const mult=dmgMult(atkAngle,near.angle);
              near.hp-=u.dmg*mult;

              // Particelle: scintille per colpo frontale, sangue per fianco/schiena
              if(mult===1.0) spawnParticles(near.x,near.y,'spark');
              else           spawnParticles(near.x,near.y,'blood');

              if(near.hp<=0){
                near.hp=0; near.alive=false; near.dead=true; near.deadAt=Date.now();
                // Esplosione sangue alla morte
                spawnParticles(near.x,near.y,'blood');
                spawnParticles(near.x,near.y,'blood');
              }
              updateBars();
            }
    ...
    function endBattle(playerWon){
      ...
      if(msgEl)msgEl.innerHTML=playerWon
        ?`<span style="color:#4ade80">${Math.round(survived)} truppe sopravvissute.</span>`
        :`<span style="color:#f87171">Sconfitti. ${Math.round(enemySurvived)} nemici rimasti.</span>`;
      if(btnsEl)btnsEl.innerHTML=`<button id="sb-finish" style="padding:.6rem 2rem;background:linear-gradient(135deg,${playerWon?'#14532d,#16a34a':'#7f1d1d,#dc2626'});border:none;border-radius:3px;font-family:Cinzel,serif;font-size:.72rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;color:#fff">${playerWon?'👑 Continua':'💀 Continua'}</button>`;
      document.getElementById('sb-finish')?.addEventListener('click',()=>{cleanup();onComplete(playerWon,survived,enemySurvived);});
