...
const MultiplayerEngine = (() => {
    ...
    w.innerHTML = `
      <div style="width:100%;max-width:420px">
        ...
        <div class="mp-card">
          ...
          <div id="mp-charsec">
            <div style="font-family:'EB Garamond',serif;font-size:.82rem;color:#6b5e4a;font-style:italic">Premi "✓ Sono Pronto" — l'host avvierà la selezione quando tutti sono pronti.</div>
          </div>
        </div>
        <div class="mp-card">
  ...
  function _refreshRoom() {
    ...
    // Sezione personaggio (visibile solo dopo la fase charPick)
    if (csec) {
      if (me?.charId) {
        ...
        csec.innerHTML = `
          ...
          <button class="mp-btn-ghost" style="margin-top:0" onclick="MultiplayerEngine._openNativeCharSelect()">↺ Cambia personaggio</button>`;
      } else if (anyCharPick) {
        // Fase selezione già avviata — mostra bottone per scegliere
        csec.innerHTML = `
        ...
        csec.innerHTML = `<div style="font-family:'EB Garamond',serif;font-size:.82rem;color:#6b5e4a;font-style:italic">In attesa che l'host avvii la selezione…</div>`;
      }
    }

    // Pulsanti azione
    if (act) {
      if (!allReady && !anyCharPick) {
        ...
        if (!me?.ready) {
          ...
          // Host: se tutti pronti mostra "Avvia Selezione"
          if (_isHost && allReady) {
            act.innerHTML = `<button class="mp-btn-gold" onclick="MultiplayerEngine.hostStartCharPick()" style="max-width:420px">⚔ Avvia Selezione Personaggi</button>`;
          }
        }
        // Controlla separatamente per l'host con tutti pronti
        if (_isHost && allReady) {
          act.innerHTML = `<button class="mp-btn-gold" onclick="MultiplayerEngine.hostStartCharPick()" style="max-width:420px">⚔ Avvia Selezione Personaggi</button>`;
        }
