...
const Game = (() => {
  ...
  const CHARACTERS = [
    ...
    },
    {
      ...
      objective: "Il Mercenario d'Oro: Tesoro >65 e Esercito >50 entro il turno 50.",
      objectiveCheck: (s) => s.turn >= 50 && s.resources.gold > 65 && s.resources.army > 50,
      startAllies: [], startEnemies: [],
  ...
  const EVENTS = [
    ...
    // ── CARTE REGNO DIVISO (post-trono, casate non ancora fedeli) ──
    {
      ...
      rightText: 'Invia ambasciatori con doni', rightEffects: { gold: -10, power: +8, people: +6 },
      minTurn: 2,
      maxUses: 2,
    },
    {
      ...
      rightText: 'Prepara l\'offensiva militare', rightEffects: { army: +8, gold: -8, power: +6, people: -8 },
      minTurn: 3,
      maxUses: 3,
    },
    {
      ...
      leftText: 'Invia la flotta — mostra forza', leftEffects: { army: -8, gold: -6, power: +10 },
    ...
    },
    {
      ...
      leftText: 'Delegittimalo pubblicamente', leftEffects: { power: +8, people: -6, faith: -5 },
      rightText: 'Prepara le difese costiere', rightEffects: { army: +6, gold: -10, power: +4 },
      minTurn: 6,
      maxUses: 1,
    },
    {
      ...
      leftText: 'Prenderne atto — agire diplomaticamente', leftEffects: { power: +5, gold: +6 },
      rightText: 'Convocare un Consiglio Generale delle Casate', rightEffects: { gold: -10, people: +10, power: +8, faith: +6 },
      minTurn: 5,
      maxUses: 2,
    },
  ...
  function _showHouseSelect() {
    ...
    backWrapper.innerHTML = `
      <button onclick="Game.goBackToSplash()" style="background:transparent;border:1px solid rgba(201,168,76,0.3);border-radius:3px;padding:0.45rem 0.9rem;font-family:'Cinzel',serif;font-size:0.68rem;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#9a8a6a;transition:all 0.2s">
        ◄ Menu Principale
      </button>`;
    ...
    const HOUSE_GROUPS = [
      ...
      { id: 'Targaryen', name: 'Casa Targaryen',   icon: '🐉', crest: null,            region: 'Esilio / Dragonstone',  chars: ['daenerys','viserys','rhaenyra','aegon_t'],     color: 'rgba(220,38,38,0.12)',   border: 'rgba(220,38,38,0.45)'  },
      { id: 'Baratheon', name: 'Casa Baratheon',   icon: '🦌', crest: null,            region: 'Capo della Tempesta',   chars: ['stannis','melisandre','davos'],                color: 'rgba(251,191,36,0.12)',  border: 'rgba(251,191,36,0.4)'  },
      { id: 'Tyrell',    name: 'Casa Tyrell',      icon: '🌹', crest: null,            region: 'Altogarden',            chars: ['margaery','olenna'],                           color: 'rgba(74,222,128,0.1)',   border: 'rgba(74,222,128,0.4)'  },
      { id: 'Martell',   name: 'Casa Martell',     icon: '☀️', crest: null,            region: 'Dorne',                 chars: ['oberyn'],                                     color: 'rgba(251,146,60,0.12)',  border: 'rgba(251,146,60,0.4)'  },
      { id: 'Greyjoy',   name: 'Casa Greyjoy',     icon: '🐙', crest: null,            region: 'Isole di Ferro',        chars: ['theon'],                                      color: 'rgba(99,102,241,0.1)',   border: 'rgba(99,102,241,0.4)'  },
      { id: 'Tully',     name: 'Casa Tully',       icon: '🐟', crest: null,            region: 'Acque del Nera',        chars: ['catelyn'],                                    color: 'rgba(59,130,246,0.1)',   border: 'rgba(59,130,246,0.4)'  },
      { id: 'Bolton',    name: 'Casa Bolton',      icon: '🩸', crest: null,            region: 'Forte Terrore',         chars: ['roose','ramsay'],                             color: 'rgba(127,29,29,0.15)',   border: 'rgba(239,68,68,0.4)'   },
      { id: 'Hightower', name: 'Casa Hightower',   icon: '🕯️', crest: null,            region: 'Oldtown',               chars: ['otto','alicent'],                             color: 'rgba(251,191,36,0.1)',   border: 'rgba(251,191,36,0.35)' },
  ...
  function _showHouseCharacters(group) {
    ...
    // Back buttons row: ◄ Menu Principale  |  ◄ Torna alle Casate
    const backWrapper = document.createElement('div');
    backWrapper.style.cssText = 'width:100%;padding:0 0.5rem;margin-bottom:0.75rem;box-sizing:border-box;display:flex;gap:0.5rem;';
    backWrapper.innerHTML = `
      <button onclick="Game.goBackToSplash()" style="background:transparent;border:1px solid rgba(201,168,76,0.2);border-radius:3px;padding:0.45rem 0.9rem;font-family:'Cinzel',serif;font-size:0.68rem;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;color:#6b5e4a;transition:all 0.2s">
        ◄ Menu
      </button>
        ...
        Object.entries(state.houses).forEach(([id, h]) => {
          ...
          if (Math.random() < 0.25) {
            ...
            state.eventQueue.unshift({
              ...
              leftText: 'Accetta la sfida — prepara la guerra', leftEffects: { power: +5 },
      ...
      sec.innerHTML = `
        ...
        <div style="background:rgba(192,132,252,0.06);border:2px solid rgba(192,132,252,0.5);border-radius:6px;padding:0.8rem;cursor:pointer;position:relative"
          ...
          <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem">
            <span style="width:2.2rem;height:2.2rem;display:flex;align-items:center;justify-content:center;flex-shrink:0">${houseIcon(kingH,'2rem')}</span>
            <div>
