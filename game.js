...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _bc_loop(fieldId) {
      ...
      gr.addColorStop(0,'rgba(8,3,3,0)'); gr.addColorStop(1,'rgba(25,8,4,0.4)');
    ...
    function _bc_advance(progress) {
      ...
      const gap=22+(1-progress)*(W*.28);
      pS.forEach((s,i)=>{ s.tx=Math.min(center-gap+i*22,center-28); });
      eS.forEach((s,i)=>{ s.tx=Math.max(center+gap-i*22-22,center+4); });
    }
    ...
    function _bc_kill(side, count) {
      ...
      alive.slice(-Math.min(count,alive.length)).forEach((s,idx)=>{
        setTimeout(()=>{
          s.alive=false; s.dieA=0;
          for(let i=0;i<14;i++) _bc_blood.push(_bc_createBlood(s.x,gnd-18));
          for(let i=0;i<5;i++) _bc_sparks.push({x:s.x+(Math.random()-.5)*15,y:gnd-22,vx:(Math.random()-.5)*7,vy:-Math.random()*6-2,alpha:1,r:2+Math.random()*2,color:['#ff4','#f80','#f44'][Math.floor(Math.random()*3)],g:0.42});
        }, idx*180);
      });
    }
