...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _bc_drawSoldier(ctx, x, y, flip, phase, state, alpha, s) {
      ...
      // ── Inizializza campi animazione per-soldato al primo utilizzo ──
      if (s._atkT === undefined) {
        s._atkT      = Math.random() * 6.28;   // timer personale
        s._atkType   = Math.floor(Math.random() * 5); // tipo colpo 0-4
        s._atkSpeed  = 0.85 + Math.random() * 0.7;    // velocità personale
        s._stagger   = 0;                              // rinculo (0-1)
        s._blockT    = Math.random() * 6.28;           // timer parata
        s._isBlocking= false;
        s._stepOff   = Math.random() * 2.2;            // offset passo
        s._breathOff = Math.random() * 6.28;           // offset respiro
      }

      let bob=0, legF=0, legB=0, sw=0, sw2=0, cloak=0, lean=0, crouchY=0, shieldA=0;

      if (state === 'march') {
        // Marcia con respiro naturale per-soldato
        const mt = t * 8 + phase + s._stepOff;
        bob   = Math.sin(mt) * 2.2;
        legF  = Math.sin(mt) * 30;
        legB  = -legF * 0.88;
        cloak = Math.sin(t * 5.5 + phase) * 8;
        lean  = Math.sin(mt) * 2.5;
        s._stagger = Math.max(0, s._stagger - 0.04);

      } else if (state === 'attack') {
        // Avanza timer personale
        s._atkT += 0.016 * s._atkSpeed * 60 / 60;

        // Ogni 2.8-4.2s cambia tipo di colpo
        if (Math.sin(s._atkT * 0.4 + s._blockT) > 0.97) {
          s._atkType = (s._atkType + 1 + Math.floor(Math.random()*3)) % 5;
          s._atkSpeed = 0.8 + Math.random() * 0.8;
          // Chance di passare a parata
          s._isBlocking = Math.random() < 0.22;
        }

        // Rinculo dissipato
        s._stagger = Math.max(0, s._stagger - 0.05);

        const at = s._atkT * s._atkSpeed;

        if (s._isBlocking) {
          // ── PARATA: piegamento ginocchia, braccio alto, cloak fermo ──
          crouchY = 4 + Math.sin(at * 3) * 1.5;
          bob     = Math.sin(at * 4 + s._breathOff) * 1.2;
          lean    = -8 + Math.sin(at * 2) * 2;   // indietro
          legF    = Math.sin(at * 4) * 8;
          legB    = -legF * 0.6;
          sw      = -80 + Math.sin(at * 3) * 12; // spada alta in guardia
          sw2     = 35;  // braccio dx fuori
          cloak   = Math.sin(at * 3) * 5;
          shieldA = 1;
        } else {
          // ── 5 TIPI DI COLPO ──
          switch (s._atkType) {

            case 0: // Fendente dall'alto — ampio arco verticale
              sw  = Math.sin(at * 2.8) * 80 - 20;   // da -100° a +60°
              bob = Math.abs(Math.sin(at * 2.8)) * 3.5 - 1;
              lean= Math.sin(at * 2.8) * 12;
              legF= Math.sin(at * 2.8) * 22;
              legB= -legF * 0.75;
              crouchY = Math.max(0, Math.sin(at * 2.8 + 0.5) * 5);
              cloak = Math.sin(at * 2.5) * 15;
              break;

            case 1: // Stoccata — spada orizzontale, passo in avanti
              sw  = -60 + Math.sin(at * 4.5) * 15;   // quasi orizzontale
              bob = Math.sin(at * 4.5 + s._breathOff) * 1.5;
              lean= 14 + Math.sin(at * 4.5) * 5;     // fortemente in avanti
              legF= Math.sin(at * 4.5) * 18;
              legB= -legF * 0.6;
              cloak = Math.sin(at * 4) * 8;
              break;

            case 2: // Colpo laterale — spazzata da destra a sinistra
              sw  = 30 + Math.sin(at * 3.2) * 55;    // da -25° a +85°
              sw2 = Math.sin(at * 3.2) * 20;          // braccio dx contrappeso
              bob = Math.sin(at * 3.2) * 2.8;
              lean= Math.sin(at * 3.2 + 1) * 9;
              legF= Math.sin(at * 3.2) * 20;
              legB= -legF * 0.8;
              cloak = Math.sin(at * 3) * 18;
              break;

            case 3: // Colpo breve — sequenza rapida di finti
              sw  = Math.sin(at * 6) * 30 + Math.sin(at * 3.1) * 25;
              bob = Math.abs(Math.sin(at * 6)) * 2;
              lean= Math.sin(at * 6) * 6;
              legF= Math.sin(at * 5) * 14;
              legB= -legF * 0.7;
              cloak = Math.sin(at * 5.5) * 10;
              break;

            case 4: // Spallata + taglio ascendente
              sw  = -20 + Math.sin(at * 2.5) * 60;
              bob = -Math.abs(Math.sin(at * 2.5)) * 4 + 2; // abbassarsi poi slancio
              lean= 6 + Math.sin(at * 2.5) * 10;
              legF= Math.sin(at * 2.5) * 25;
              legB= -legF * 0.9;
              crouchY = Math.max(0, -Math.sin(at * 2.5) * 6);
              cloak = Math.sin(at * 2.2) * 14;
              break;
          }

          // Rinculo visivo quando stagger > 0
          if (s._stagger > 0) {
            lean -= s._stagger * 18;
            crouchY += s._stagger * 5;
            bob += s._stagger * 3;
          }
        }

      } else { // idle
        // Respiro lento e naturale
        bob   = Math.sin(t * 2.2 + s._breathOff) * 1.5;
        cloak = Math.sin(t * 1.8 + phase) * 3;
        sw    = -70 + Math.sin(t * 1.5 + s._breathOff) * 5; // spada a riposo alta
        lean  = Math.sin(t * 1.2 + s._breathOff) * 2;
        s._stagger = Math.max(0, s._stagger - 0.03);
      }

      const cy = crouchY; // offset verticale inginocchiamento
      ...
      ctx.closePath(); ctx.fill();
      // Dettaglio corazza (linea centrale)
      ...
      // BRACCIO SX con spada
      ctx.save(); ctx.translate(-7, -22 + bob); ctx.rotate((lean * 0.6 + sw * .12) * Math.PI / 180);
      ...
      ctx.lineTo(1.5, 34); ctx.lineTo(0, 40);
      ctx.lineTo(-1.5, 34); ctx.closePath(); ctx.fill();
      // Riflesso lama
      ...
      // BRACCIO DX
      ctx.save(); ctx.translate(7, -22 + bob); ctx.rotate((-lean * 0.5 + sw2 * 0.08) * Math.PI / 180);
      ctx.beginPath(); ctx.roundRect(-2, 0, 4.5, 11, 3); ctx.fill();
      ctx.save(); ctx.translate(0, 11); ctx.rotate((15 + sw2 * 0.5) * Math.PI / 180);
    ...
    function _bc_loop(fieldId) {
      ...
      _bc_soldiers.forEach(s=>{
        if (!s.alive) return;
        if (!_bc_contacted || Math.abs(s.tx-s.x)>6) s.state='march';
    ...
    Game._battleRetreatFn = () => {
      ...
      document.getElementById('tb-btns').innerHTML = `
        <button onclick="if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();_bc_stop();document.getElementById('throne-battle-overlay').remove();Game._battleRetreatFinish(${survived})"
    ...
    function runPhaseRound(roundsLeft, cb) {
      ...
      const pDmg = eForce * (0.04 + Math.random() * 0.05 + _underdogBonus);
      const eDmg = pForce * (0.04 + Math.random() * 0.05);
    ...
    function finalizeThroneBattle() {
      ...
      if (btns) {
        setTimeout(() => {
          btns.innerHTML = `
            <button onclick="${won
              ? `_bc_stop();document.getElementById('throne-battle-overlay').remove();Game._battleCompleteFn(${survived})`
              : `if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();_bc_stop();document.getElementById('throne-battle-overlay').remove();Game._battleCompleteFn(${survived})`
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat, bgConfig) {
    ...
    function runPhaseRounds(roundsLeft, phaseEvtApplied, afterRoundsCallback) {
      ...
      const pDmg = eForce * (0.04 + Math.random() * 0.05 + underdogBonus);
      const eDmg = pForce * (0.04 + Math.random() * 0.05);
      const prevP = pForce, prevE = eForce;
      pForce = Math.max(0, pForce - pDmg);
      eForce = Math.max(0, eForce - eDmg);
    ...
    function finalizeBattle() {
      ...
      if (btns) {
        setTimeout(() => {
          btns.innerHTML = `
            <button onclick="${won ? "if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();" : ''}_bc_stop();document.getElementById('battle-anim-overlay').remove();Game._battleResolveFn(${won},${survived},${enemySurvived})"
