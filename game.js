...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function startPhase(phaseNum) {
      console.log('startPhase called with phaseNum:', phaseNum);
      phase = phaseNum;
      if (pForce <= 0 || eForce <= 0) { 
        console.log('Battle ending - forces depleted:', { pForce, eForce });
      ...
      if (headline) { 
        headline.textContent = phaseData.title;
        console.log('Set headline to:', phaseData.title);
      }
      if (phaseLabel) {
        phaseLabel.textContent = `Fase ${phaseNum + 1}`;
        console.log('Set phase label to:', `Fase ${phaseNum + 1}`);
      }
    ...
    console.log('Starting throne battle with delay:', _initialDelay);
    Game._battleTimer = setTimeout(() => {
      console.log('Calling startPhase(0)');
      startPhase(0);
    }, _initialDelay);
  }
...
Game._tutNext = function() {
  if (Game._tutNext && typeof Game._tutNext === 'function') {
    Game._tutNext();
  }
};
