...
const Game = (() => {
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    // ── Numero cerchietti: 1 ogni 10 truppe, max 5 ──
    ...
    // ── Costanti campo ──
    const COLS = 4, ROWS = 4;
    const CW = 64, CH = 48;          // cell width/height
    const BW = COLS * CW;            // 256px
    const BH = ROWS * CH;            // 192px
    const UR = 18;                   // unit radius
    const HP_MAX = 5;
    const BASE_DMG = 1;
    const SPEED_PX = 45;             // px/sec
    const ATK_RANGE = UR * 2.4;
    const ATK_INTERVAL = 900;        // ms tra attacchi
    ...
    // ── Crea unità ──
    function makeUnit(id, side, x, y, angle) {
      return { id, side, x, y, angle, hp: HP_MAX, alive: true,
               dead: false, deadAt: 0, lastAtk: 0 };
    }

    const playerUnits = [];
    for (let i = 0; i < pCount; i++) {
      // Posizione default: distribuiti nella riga inferiore
      const col = i % COLS, row = ROWS - 1 - Math.floor(i / COLS);
      playerUnits.push(makeUnit('p'+i, 'p',
        col * CW + CW/2, row * CH + CH/2, -Math.PI/2));
    }

    // CPU strategia: sceglie tra 5 pattern ogni partita
    const enemyUnits = [];
    const CPU_PATTERNS = [
      // 0: linea centrale
      (i,n) => [Math.floor(n/2-i+n/2) % COLS * CW + CW/2, 0*CH + CH/2, Math.PI/2],
      // 1: triangolo
      (i,n) => [
        (i === 0 ? 1.5 : i % 2 === 0 ? 1 : 2) * CW + CW/2,
        (i === 0 ? 0 : 1) * CH + CH/2, Math.PI/2
      ],
      // 2: dispersione totale
      (i,n) => [(i % COLS) * CW + CW/2, Math.floor(i/COLS) * CH + CH/2, Math.PI/2],
      // 3: flanchi
      (i,n) => [
        (i < Math.ceil(n/2) ? 0 : COLS-1) * CW + CW/2,
        Math.floor(i % Math.ceil(n/2)) * CH + CH/2, Math.PI/2
      ],
      // 4: concentrato centro
      (i,n) => [
        (1 + i % 2) * CW + CW/2,
        Math.floor(i/2) * CH + CH/2, Math.PI/2
      ],
    ];
    const cpuPat = CPU_PATTERNS[Math.floor(Math.random() * CPU_PATTERNS.length)];
    for (let i = 0; i < eCount; i++) {
      const [x,y,a] = cpuPat(i, eCount);
      enemyUnits.push(makeUnit('e'+i, 'e',
        Math.max(UR, Math.min(BW-UR, x)),
        Math.max(UR, Math.min(BH-UR, y)), a));
    }
    ...
    ov.id = 'sb-overlay';
    ov.style.cssText = 'position:fixed;inset:0;z-index:950;background:#07070e;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:"Cinzel",serif;';
    ov.innerHTML = `
      <div style="width:${BW+24}px;display:flex;flex-direction:column;gap:6px;align-items:center">

        <!-- Intestazione -->
        <div style="display:flex;justify-content:space-between;width:100%;font-size:0.62rem;letter-spacing:.1em;text-transform:uppercase">
          <span style="color:#4ade80">⚔ TUE FORZE <strong style="color:#fff">${Math.round(playerForce)}</strong></span>
          <span id="sb-phase" style="color:#9a8a6a">POSIZIONAMENTO</span>
          <span style="color:#f87171">⚔ ${enemyLabel} <strong style="color:#fff">${Math.round(enemyForce)}</strong></span>
        </div>

        <!-- Campo nemico -->
        <div style="position:relative;width:${BW}px;height:${BH}px;border:1px solid rgba(239,68,68,.45);border-radius:3px;overflow:hidden;background:#120608">
          <canvas id="sb-ec" width="${BW}" height="${BH}" style="position:absolute;inset:0"></canvas>
          <canvas id="sb-fog" width="${BW}" height="${BH}" style="position:absolute;inset:0;pointer-events:none"></canvas>
          <div style="position:absolute;top:3px;left:5px;font-size:.5rem;color:#f87171;letter-spacing:.08em;opacity:.8">👁 ${enemyLabel.toUpperCase()}</div>
        </div>

        <!-- Centro HUD -->
        <div style="display:flex;align-items:center;gap:14px;height:38px">
          <div id="sb-cd" style="font-family:'Cinzel Decorative',serif;font-size:1.5rem;color:#c9a84c;min-width:2.2rem;text-align:center">20</div>
          <div id="sb-msg" style="font-size:.65rem;color:#9a8a6a;text-align:center;line-height:1.4">Trascina le truppe nei quadranti.<br>Considera i fianchi — colpire di lato fa ×1.5, da dietro ×2.</div>
        </div>

        <!-- Campo giocatore -->
        <div style="position:relative;width:${BW}px;height:${BH}px;border:1px solid rgba(74,222,128,.45);border-radius:3px;overflow:hidden;background:#06100a">
          <canvas id="sb-grid" width="${BW}" height="${BH}" style="position:absolute;inset:0;pointer-events:none"></canvas>
          <canvas id="sb-pc" width="${BW}" height="${BH}" style="position:absolute;inset:0;cursor:grab"></canvas>
          <div style="position:absolute;top:3px;left:5px;font-size:.5rem;color:#4ade80;letter-spacing:.08em;opacity:.8">🛡 LE TUE TRUPPE</div>
        </div>
        ...
        <div id="sb-btns" style="display:flex;gap:10px;margin-top:4px">
          ...
          <button id="sb-go" style="padding:.5rem 1.6rem;background:linear-gradient(135deg,#7f1d1d,#dc2626);border:none;border-radius:3px;font-family:Cinzel,serif;font-size:.7rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;color:#fff">⚔ BATTAGLIA!</button>
        </div>
      </div>
    `;
    document.body.appendChild(ov);

    const pCv = document.getElementById('sb-pc');
    const eCv = document.getElementById('sb-ec');
    ...
    function drawGrid() {
      ...
      gridCtx.strokeStyle = 'rgba(74,222,128,.12)';
      gridCtx.lineWidth = 1;
      for (let c=1;c<COLS;c++) {
        gridCtx.beginPath(); gridCtx.moveTo(c*CW,0); gridCtx.lineTo(c*CW,BH); gridCtx.stroke();
      }
      for (let r=1;r<ROWS;r++) {
        gridCtx.beginPath(); gridCtx.moveTo(0,r*CH); gridCtx.lineTo(BW,r*CH); gridCtx.stroke();
      }
    }
    drawGrid();

    // ── Nebbia particelle ──
    const fogPts = Array.from({length:55},()=>({
    ...
    function drawFog(lifted) {
      fogCtx.clearRect(0,0,BW,BH);
      if (lifted) return;
      fogCtx.fillStyle = 'rgba(8,4,4,.52)';
      fogCtx.fillRect(0,0,BW,BH);
      fogPts.forEach(p=>{
        p.x+=p.vx; p.y+=p.vy;
        if(p.x<-40)p.x=BW+10; if(p.x>BW+40)p.x=-10;
        if(p.y<-40)p.y=BH+10; if(p.y>BH+40)p.y=-10;
        const g=fogCtx.createRadialGradient(p.x,p.y,0,p.x,p.y,p.r);
        g.addColorStop(0,`rgba(65,52,44,${p.a})`);
        g.addColorStop(1,'rgba(65,52,44,0)');
        fogCtx.fillStyle=g;
        fogCtx.beginPath();fogCtx.arc(p.x,p.y,p.r,0,Math.PI*2);fogCtx.fill();
      });
    }

    // ── Disegna singola unità ──
    function drawUnit(ctx, u, isPlayer) {
      if (u.dead) {
        const age = (Date.now()-u.deadAt)/1000;
        if (age>4) return;
        ctx.save();
        ctx.globalAlpha = Math.max(0,.7-age*.18);
        ctx.font = `${UR*1.3}px serif`;
        ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.fillText('💀',u.x,u.y);
        ctx.restore();
        return;
      }
      const col = isPlayer ? '#4ade80' : '#f87171';
      const selCol = '#fbbf24';
      ...
      // Corpo
      ctx.beginPath(); ctx.arc(0,0,UR,0,Math.PI*2);
      ctx.fillStyle = isPlayer ? 'rgba(8,32,16,.88)' : 'rgba(32,8,8,.88)';
      ctx.fill();
      ctx.strokeStyle = isSel ? selCol : col;
      ctx.lineWidth = isSel ? 2.8 : 2;
      ctx.stroke();

      // Linea fronte (ruotata secondo angle)
      ctx.rotate(u.angle);
      ctx.beginPath(); ctx.moveTo(-UR,0); ctx.lineTo(UR,0);
      ctx.strokeStyle = isSel ? selCol : col;
      ctx.lineWidth = 2.5; ctx.stroke();
      // Freccia fronte
      ctx.beginPath();
      ctx.moveTo(UR,0); ctx.lineTo(UR-7,-5); ctx.lineTo(UR-7,5);
      ctx.closePath(); ctx.fillStyle = isSel ? selCol : col; ctx.fill();

      ctx.restore();

      // HP bar
      const bx=u.x-UR, by=u.y-UR-7, bw=UR*2;
      ctx.fillStyle='rgba(0,0,0,.5)'; ctx.fillRect(bx,by,bw,4);
      ctx.fillStyle = u.hp>=4?'#4ade80':u.hp>=2?'#fbbf24':'#f87171';
      ctx.fillRect(bx,by,bw*(u.hp/HP_MAX),4);

      // HP numero
      ctx.fillStyle='rgba(255,255,255,.7)';
      ctx.font='bold 9px sans-serif';
      ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(u.hp,u.x,u.y);
    }
    ...
    function render() {
      ...
      playerUnits.forEach(u=>drawUnit(pCtx,u,true));
      enemyUnits.forEach(u=>drawUnit(eCtx,u,false));
      drawFog(fogLifted);
    }

    // ── Input drag placement ──
    ...
    function hitPlayer(pos) {
      for(const u of playerUnits) {
        ...
        if(dx*dx+dy*dy<(UR+5)*(UR+5)) return u;
      }
      return null;
    }
    pCv.addEventListener('pointerdown',e=>{
      if(phase!=='placement') return;
      const pos=canvasPos(e,pCv);
      const h=hitPlayer(pos);
      if(h){dragUnit=h;selectedId=h.id;dragOX=pos.x-h.x;dragOY=pos.y-h.y;pCv.style.cursor='grabbing';}
      e.preventDefault();
    });
    pCv.addEventListener('pointermove',e=>{
      ...
      dragUnit.y=Math.max(UR,Math.min(BH-UR,pos.y-dragOY));
      e.preventDefault();
    });
    pCv.addEventListener('pointerup',()=>{dragUnit=null;pCv.style.cursor='grab';});
    pCv.addEventListener('pointercancel',()=>{dragUnit=null;});

    // ── Countdown e avvio ──
    function startCountdown() {
      phase='countdown';
      const goBtn=document.getElementById('sb-go');
      if(goBtn){goBtn.disabled=true;goBtn.style.opacity='.45';}
      document.getElementById('sb-phase').textContent='COUNTDOWN';
      document.getElementById('sb-msg').textContent='Le forze si preparano all\'assalto...';
      cdTimer=setInterval(()=>{
        countdown--;
    ...
    document.getElementById('sb-go').addEventListener('click', startCountdown);
    ...
    function startBattle() {
      phase='battle';
      fogLifted=true; // nebbia si alza allo scadere del countdown
      ...
      // Scala mondo unificata: giocatore campo invertito verticalmente
      // pUnit.y → worldY = BH*2 - pUnit.y  (giocatore parte dal basso)
      // eUnit.y → worldY = eUnit.y          (nemico parte dall'alto)
      // Il nemico ha y=0 top, il giocatore ha y=BH bottom ma le coordinate
      // sono già nello stesso range 0..BH. Usiamo coordinate dirette con
      // offset verticale: worldY del giocatore = BH + pUnit.y (campo doppio),
      // worldY nemico = eUnit.y. Poi la distanza è calcolata in questo spazio.

      function wx(u){return u.x;}
      function wy(u){return u.side==='p' ? BH + u.y : u.y;}

      function dist(a,b){
        const dx=wx(a)-wx(b), dy=wy(a)-wy(b);
        return Math.sqrt(dx*dx+dy*dy);
      }
      function angleToUnit(from,to){
        return Math.atan2(wy(to)-wy(from), wx(to)-wx(from));
      }

      // Moltiplicatore danno basato su angolo relativo alla fronte della vittima
      // attackAngle = angolo da attaccante a vittima
      // victimAngle = fronte della vittima
      // Se attacca dalla direzione opposta alla fronte → frontale (×1)
      // Se attacca di lato → ×1.5
      // Se attacca dalla stessa direzione della fronte → schiena (×2)
      function dmgMult(attackAngle, victimFront) {
        // L'attaccante arriva da attackAngle; la vittima guarda verso victimFront
        // "frontale" = attaccante viene dall'opposto di victimFront
        let diff = attackAngle - (victimFront + Math.PI);
      ...
      function battleTick(ts) {
        ...
        const alive_p=playerUnits.filter(u=>u.alive);
        const alive_e=enemyUnits.filter(u=>u.alive);

        // Controlla fine
        if(alive_p.length===0||alive_e.length===0) {
          endBattle(alive_p.length>0, playerUnits, enemyUnits, playerForce, enemyForce);
          return;
        }

        // Ogni unità viva: trova nemico più vicino, ruota, avanza o attacca
        const allUnits=[...playerUnits,...enemyUnits];
        allUnits.forEach(u=>{
          if(!u.alive) return;
          const opps = u.side==='p' ? alive_e : alive_p;
          if(!opps.length) return;

          // Nemico più vicino
          let nearest=opps[0], nearD=dist(u,opps[0]);
          opps.forEach(op=>{const d=dist(u,op);if(d<nearD){nearD=d;nearest=op;}});

          // Ruota verso nemico (lerp angolo)
          const targetAngle=angleToUnit(u,nearest);
          let da=targetAngle-u.angle;
          while(da>Math.PI)da-=2*Math.PI; while(da<-Math.PI)da+=2*Math.PI;
          u.angle+=da*Math.min(1,dt*5);

          if(nearD>ATK_RANGE) {
            // Avanza
            const step=SPEED_PX*dt;
            u.x+=Math.cos(targetAngle)*step;
            u.y+=Math.sin(targetAngle)*step;
            // Clamp nel proprio campo
            u.x=Math.max(UR,Math.min(BW-UR,u.x));
            u.y=Math.max(UR,Math.min(BH-UR,u.y));
          } else {
            // Attacca se cooldown passato
            if(ts-u.lastAtk>=ATK_INTERVAL) {
              u.lastAtk=ts;
              // Calcola angolo da u a nearest (come lo vede nearest)
              const atkAngle=angleToUnit(u,nearest);
              const mult=dmgMult(atkAngle, nearest.angle);
              const dmg=BASE_DMG*mult;
              nearest.hp-=dmg;
              // Flash visivo
              nearest._hit=(nearest._hit||0)+1;
              if(nearest.hp<=0) {
                nearest.hp=0;
                nearest.alive=false;
                nearest.dead=true;
                nearest.deadAt=Date.now();
              }
            }
          }
        });
        ...
        render();
        raf=requestAnimationFrame(battleTick);
      }
    ...
    // ── Fine battaglia ──
    function endBattle(playerWon, pUnits, eUnits, pForce, eForce) {
      phase='result';
      cancelAnimationFrame(raf);

      // Calcola superstiti proporzionali
      const pAlive=pUnits.filter(u=>u.alive).length;
      const eAlive=eUnits.filter(u=>u.alive).length;
      const pTot=pUnits.length, eTot=eUnits.length;

      const pSurvRatio = pTot>0 ? pAlive/pTot : 0;
      const eSurvRatio = eTot>0 ? eAlive/eTot : 0;

      const survived     = Math.max(1, Math.round(pForce * pSurvRatio));
      const enemySurvived= Math.max(0, Math.round(eForce * eSurvRatio));

      const phLabel=document.getElementById('sb-phase');
      const msg=document.getElementById('sb-msg');
      const cd=document.getElementById('sb-cd');
      const btns=document.getElementById('sb-btns');

      if(phLabel) phLabel.textContent=playerWon?'VITTORIA':'SCONFITTA';
      if(cd) {cd.textContent=playerWon?'🏆':'💀';cd.style.color=playerWon?'#4ade80':'#f87171';}
      if(msg) {
        msg.innerHTML=playerWon
          ?`<span style="color:#4ade80;font-size:.8rem">Vittoria! ${Math.round(survived)} forze sopravvissute.</span>`
          :`<span style="color:#f87171;font-size:.8rem">Sconfitta. Le truppe nemiche resistono (${Math.round(enemySurvived)} rimaste).</span>`;
      }
      if(btns) {
        btns.innerHTML=`<button onclick="window._sbFinish&&window._sbFinish()" style="padding:.6rem 2rem;background:linear-gradient(135deg,${playerWon?'#14532d,#16a34a':'#7f1d1d,#dc2626'});border:none;border-radius:3px;font-family:Cinzel,serif;font-size:.72rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;color:#fff">${playerWon?'👑 Continua':'💀 Continua'}</button>`;
      }
      ...
      // Continua render per vedere i caduti
      function resultLoop(){
        render();
        if(phase==='result') requestAnimationFrame(resultLoop);
      }
    ...
    function cleanup() {
      ...
      phase='done';
      document.getElementById('sb-overlay')?.remove();
      delete window._sbFinish;
      delete window._sbReady;
    }
