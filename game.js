...
const Game = (() => {
  ...
  const EVENTS = [
...

    {
      ...
      rightText: 'Qualche compromesso è necessario', leftEffects: { army: +8, faith: -8 },
      minTurn: 2,
    },
    {
    ...
    },
    {
      ...
      rightText: 'Mantieni le tasse, usa la forza se serve', leftEffects: { gold: +8, people: -14, army: -5 },
      minTurn: 5,
    },
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    // ═══════════════════════════════════════════════════
    // CANVAS BATTLE ENGINE — soldati silhouette animati
    // ═══════════════════════════════════════════════════
    let _bc_canvas = null, _bc_ctx = null, _bc_raf = null;
    let _bc_soldiers = [], _bc_blood = [], _bc_sparks = [], _bc_arrows = [];
    let _bc_running = false;
    let _bc_arrowTimer = 0;
    let _bc_killQueue = [];   // {side, count} da eseguire solo quando in clash
    let _bc_killTimer = 0;    // frame tra un kill e l'altro
    let _bc_contacted = false; // true dopo il primo contatto fisico

    function _bc_drawSoldier(ctx, x, y, flip, phase, state, alpha, s) {
      const now = Date.now() / 1000;

      // ── Init per-soldier fields ──
      if (s._t === undefined) {
        s._t         = Math.random() * 20;      // timer personale continuo
        s._atkType   = Math.floor(Math.random() * 5);
        s._atkSpeed  = 0.7 + Math.random() * 0.6;
        s._nextTypeT = s._t + 3 + Math.random() * 4;
        s._stagger   = 0;
        s._breathOff = Math.random() * 6.28;
        s._stepOff   = Math.random() * 6.28;
        s._isBlocking= false;
        // Valori interpolati correnti (inizializzati in marcia)
        s._cur = { bob:0, legF:0, legB:0, sw:-60, sw2:0, cloak:0, lean:0, crouchY:0 };
      }

      // Avanza timer personale sempre
      s._t += 0.016 * s._atkSpeed;

      // Cambio tipo colpo graduale solo in attack
      if (state === 'attack' && s._t > s._nextTypeT) {
        s._atkType   = Math.floor(Math.random() * 5);
        s._atkSpeed  = 0.7 + Math.random() * 0.6;
        s._isBlocking= Math.random() < 0.18;
        s._nextTypeT = s._t + 2.5 + Math.random() * 3.5;
      }

      // Stagger decade sempre
      s._stagger = Math.max(0, s._stagger - 0.04);

      // ── Calcola TARGET per lo stato corrente ──
      let tgt = { bob:0, legF:0, legB:0, sw:-60, sw2:0, cloak:0, lean:0, crouchY:0 };
      const at = s._t;

      if (state === 'march') {
        const mt = at * 5.5 + s._stepOff;
        tgt.bob   = Math.sin(mt) * 2.5;
        tgt.legF  = Math.sin(mt) * 28;
        tgt.legB  = -tgt.legF * 0.85;
        tgt.cloak = Math.sin(at * 4 + s._breathOff) * 7;
        tgt.lean  = Math.sin(mt) * 3;
        tgt.sw    = -65 + Math.sin(at * 1.5 + s._breathOff) * 8;

      } else if (state === 'attack') {

        if (s._isBlocking) {
          tgt.crouchY = 5 + Math.sin(at * 2.5) * 1.5;
          tgt.bob     = Math.sin(at * 3 + s._breathOff) * 1.2;
          tgt.lean    = -10 + Math.sin(at * 1.8) * 3;
          tgt.legF    = Math.sin(at * 3) * 7;
          tgt.legB    = -tgt.legF * 0.6;
          tgt.sw      = -85 + Math.sin(at * 2.5) * 10;
          tgt.sw2     = 30 + Math.sin(at * 2) * 8;
          tgt.cloak   = Math.sin(at * 2.5) * 5;
        } else {
          switch (s._atkType) {
            case 0: { // Fendente dall'alto
              const p = Math.sin(at * 2.2);
              tgt.sw     = p * 75 - 15;
              tgt.bob    = Math.abs(p) * 3;
              tgt.lean   = p * 11;
              tgt.legF   = p * 20;
              tgt.legB   = -tgt.legF * 0.75;
              tgt.crouchY= Math.max(0, p * 4);
              tgt.cloak  = p * 13;
              break; }
            case 1: { // Stoccata orizzontale
              const p = Math.sin(at * 3.5);
              tgt.sw     = -58 + p * 14;
              tgt.bob    = p * 1.5;
              tgt.lean   = 12 + p * 5;
              tgt.legF   = p * 16;
              ...
              tgt.cloak  = p * 7;
              break; }
            case 2: { // Spazzata laterale
              const p = Math.sin(at * 2.7);
              tgt.sw     = 25 + p * 52;
              tgt.sw2    = p * 18;
              tgt.bob    = p * 2.5;
              tgt.lean   = Math.sin(at * 2.7 + 0.9) * 9;
              tgt.legF   = p * 19;
              tgt.legB   = -tgt.legF * 0.8;
              tgt.cloak  = p * 17;
              break; }
            case 3: { // Finti rapidi
              const p = Math.sin(at * 5.2) * 0.6 + Math.sin(at * 2.6) * 0.4;
              tgt.sw     = p * 40;
              tgt.bob    = Math.abs(p) * 2;
              tgt.lean   = p * 7;
              tgt.legF   = Math.sin(at * 4.5) * 13;
              tgt.legB   = -tgt.legF * 0.7;
              tgt.cloak  = p * 10;
              break; }
            case 4: { // Spallata + taglio ascendente
              const p = Math.sin(at * 2.0);
              tgt.sw     = -18 + p * 58;
              tgt.bob    = -Math.abs(p) * 3.5 + 1.5;
              tgt.lean   = 5 + p * 10;
              tgt.legF   = p * 23;
              tgt.legB   = -tgt.legF * 0.88;
              tgt.crouchY= Math.max(0, -p * 5);
              tgt.cloak  = p * 13;
              break; }
          }
          // Stagger: rinculo visivo
          if (s._stagger > 0) {
            tgt.lean   -= s._stagger * 16;
            tgt.crouchY+= s._stagger * 4;
            tgt.bob    += s._stagger * 3;
          }
        }

      } else { // idle
        tgt.bob    = Math.sin(at * 1.8 + s._breathOff) * 1.5;
        tgt.cloak  = Math.sin(at * 1.5 + s._breathOff) * 3;
        tgt.sw     = -68 + Math.sin(at * 1.2 + s._breathOff) * 5;
        tgt.lean   = Math.sin(at * 1.0 + s._breathOff) * 2;
      }

      // ── Lerp smooth tra cur e tgt ──
      // Velocità di interpolazione: più bassa = più fluido
      const lerpSpeed = state === 'march' ? 0.10 : state === 'attack' ? 0.13 : 0.07;
      const cur = s._cur;
      for (const k in tgt) {
        cur[k] += (tgt[k] - cur[k]) * lerpSpeed;
      }

      const bob=cur.bob, legF=cur.legF, legB=cur.legB,
            sw=cur.sw, sw2=cur.sw2, cloak=cur.cloak,
            lean=cur.lean, cy=cur.crouchY;

      const sc   = (s._scale  !== undefined) ? s._scale  : 1.0;
      const yOff = (s._yOff   !== undefined) ? s._yOff   : 0;

      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.shadowColor = 'rgba(0,0,0,0.5)'; ctx.shadowBlur = 4;
      if (flip) { ctx.translate(x, y + cy + yOff); ctx.scale(-sc, sc); }
      else      { ctx.translate(x, y + cy + yOff); ctx.scale( sc, sc); }
      ...
      ctx.beginPath();
      ctx.moveTo(-4, 0);
      ctx.bezierCurveTo(-12 + cloak*.5, 10, -10 + cloak*.7, 24, -6 + cloak*.3, 36);
      ctx.lineTo(5 + cloak*.2, 36);
      ctx.bezierCurveTo(8 + cloak*.5, 24, 6 + cloak*.3, 10, 4, 0);
      ctx.closePath(); ctx.fill();
      ctx.restore();

      // GAMBA POST
      ctx.save(); ctx.translate(3, -11 + bob); ctx.rotate(legB * Math.PI / 180);
      ctx.beginPath();
      ctx.moveTo(-3,0); ctx.lineTo(3,0); ctx.lineTo(3.5,13);
      ctx.arc(0,13,3.5,0,Math.PI); ctx.lineTo(-3,0); ctx.fill();
      ctx.save(); ctx.rotate(-legB*.35*Math.PI/180);
      ctx.beginPath();
      ctx.moveTo(-2.5,13); ctx.lineTo(2.5,13); ctx.lineTo(3,24);
      ctx.lineTo(5,24); ctx.lineTo(5,27); ctx.lineTo(-3,27); ctx.lineTo(-3,24);
      ctx.lineTo(-2.5,24); ctx.closePath(); ctx.fill();
      ctx.restore(); ctx.restore();

      // GAMBA ANT
      ctx.save(); ctx.translate(-3, -11 + bob); ctx.rotate(legF * Math.PI / 180);
      ctx.beginPath();
      ctx.moveTo(-3.5,0); ctx.lineTo(3.5,0); ctx.lineTo(4,13);
      ctx.arc(0,13,4,0,Math.PI); ctx.lineTo(-3.5,0); ctx.fill();
      ctx.save(); ctx.rotate(-legF*.35*Math.PI/180);
      ctx.beginPath();
      ctx.moveTo(-3,13); ctx.lineTo(3,13); ctx.lineTo(3.5,24);
      ctx.lineTo(6,24); ctx.lineTo(6,28); ctx.lineTo(-3.5,28);
      ctx.lineTo(-3.5,24); ctx.lineTo(-3,24); ctx.closePath(); ctx.fill();
      ctx.restore(); ctx.restore();

      // BUSTO
      ctx.save(); ctx.translate(0, bob); ctx.rotate(lean * Math.PI / 180);
      ctx.beginPath();
      ...
      ctx.fillStyle='#0a0a0a';
      ctx.beginPath(); ctx.ellipse(-8,-23,4,3,-.4,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.ellipse( 8,-23,4,3, .4,0,Math.PI*2); ctx.fill();
      ctx.restore();

      // BRACCIO SX + SPADA
      ctx.save(); ctx.translate(-7, -22 + bob); ctx.rotate((lean*.6 + sw*.1) * Math.PI / 180);
      ctx.beginPath(); ctx.roundRect(-2.5,0,5,12,3); ctx.fill();
      ctx.save(); ctx.translate(0,12); ctx.rotate((-55 + sw) * Math.PI / 180);
      ctx.beginPath(); ctx.arc(0,0,3.5,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.roundRect(-2,0,4,10,1); ctx.fill();
      ctx.beginPath(); ctx.roundRect(-8,9,16,3,1); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(-3,12); ctx.lineTo(3,12);
      ctx.lineTo(1.5,34); ctx.lineTo(0,40); ctx.lineTo(-1.5,34);
      ctx.closePath(); ctx.fill();
      ctx.strokeStyle='rgba(255,255,255,0.15)'; ctx.lineWidth=1;
      ctx.beginPath(); ctx.moveTo(1,13); ctx.lineTo(0.5,36); ctx.stroke();
      ctx.restore(); ctx.restore();

      // BRACCIO DX
      ctx.save(); ctx.translate(7, -22 + bob); ctx.rotate((-lean*.5 + sw2*.08) * Math.PI / 180);
      ctx.beginPath(); ctx.roundRect(-2,0,4.5,11,3); ctx.fill();
      ctx.save(); ctx.translate(0,11); ctx.rotate((15 + sw2*.5) * Math.PI / 180);
      ctx.beginPath(); ctx.roundRect(-2,0,4,8,2); ctx.fill();
      ctx.restore(); ctx.restore();

      // ELMO
      ctx.save(); ctx.translate(0, -28 + bob);
      ctx.beginPath(); ctx.arc(0,0,7,Math.PI,0); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(-7,0); ctx.lineTo(7,0); ctx.lineTo(6,5);
      ctx.lineTo(1.5,5); ctx.lineTo(1,7); ctx.lineTo(-1,7);
      ctx.lineTo(-1.5,5); ctx.lineTo(-6,5); ctx.closePath(); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(-2,-7); ctx.bezierCurveTo(-3,-13,-1,-15,0,-13);
      ctx.bezierCurveTo(1,-15,3,-13,2,-7);
      ctx.closePath(); ctx.fill();
      ctx.restore();
    ...
    function _bc_createBlood(x,y) {
      return { x, y, vx:(Math.random()-.5)*5, vy:-Math.random()*6-1,
               r:2+Math.random()*4, alpha:0.95, gravity:0.38,
               streak:Math.random()>0.45 };
    }

    function _bc_spawnArrow(W, H) {
      // Freccia con vera parabola: parte bassa, sale, poi scende sul nemico
      const fromLeft = Math.random() > 0.5;
      const gnd = H - 30;
      const startX = fromLeft ? -5 : W + 5;
      const startY = gnd - 8 - Math.random()*12; // parte quasi a terra
      const speed = 4.0 + Math.random() * 1.8;
      _bc_arrows.push({
        x: startX,
        y: startY,
        vx: fromLeft ? speed : -speed,
        vy: -(speed * 0.85 + Math.random() * 0.3), // negativo: sale prima
        gravity: 0.13, // gravità più forte → arco pronunciato
        alpha: 0.85,
    ...
    function _bc_drawArrow(ctx, a) {
      ctx.save();
      ctx.globalAlpha = a.alpha;
      const angle = Math.atan2(a.vy, a.vx);
      ctx.translate(a.x, a.y);
      ctx.rotate(angle);
      // Asta freccia — silhouette nera con drop-shadow
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(-a.len, 0); ctx.lineTo(-a.len + 5, -4);
      ctx.moveTo(-a.len, 0); ctx.lineTo(-a.len + 5, 4);
      ctx.stroke();
      ctx.restore();
    }

    function _bc_initField(fieldId, pN, eN) {
      _bc_blood=[]; _bc_sparks=[]; _bc_arrows=[]; _bc_arrowTimer=0;
      _bc_killQueue=[]; _bc_killTimer=0; _bc_contacted=false;
      const field=document.getElementById(fieldId);
      if (!field) return;
      field.querySelectorAll('.bc-canvas').forEach(c=>c.remove());
      _bc_canvas=document.createElement('canvas');
      _bc_canvas.className='bc-canvas';
      _bc_canvas.style.cssText='position:absolute;inset:0;width:100%;height:100%;z-index:1';
      _bc_canvas.width=field.offsetWidth||300;
      _bc_canvas.height=field.offsetHeight||180;
      field.insertBefore(_bc_canvas,field.firstChild);
      _bc_ctx=_bc_canvas.getContext('2d');
      const W=_bc_canvas.width;
      // Dividi in prima fila (F1) e seconda fila (F2) per effetto prospettiva
      const f1 = Math.ceil(pN / 2);   // prima fila — più grandi, in primo piano
      const f2 = pN - f1;             // seconda fila — più piccoli, leggermente arretrati
      const sp1 = Math.min(26, (W * 0.38) / Math.max(1, f1)); // spaziatura F1
      const sp2 = Math.min(22, (W * 0.36) / Math.max(1, f2)); // spaziatura F2
      _bc_soldiers=[];
      // GIOCATORE — prima fila (scala 1.0, y normale)
      for(let i=0;i<f1;i++) {
        const startX = -35 - i*sp1;
        const targetX = 10 + i*sp1;
        _bc_soldiers.push({id:'p'+i,x:startX,tx:targetX,side:'p',alive:true,phase:Math.random()*6.28,dieA:0,alpha:1,state:'march',_row:0,_scale:1.0,_yOff:0});
      }
      // GIOCATORE — seconda fila (scala 0.78, leggermente più in alto = più lontana)
      for(let i=0;i<f2;i++) {
        const startX = -35 - i*sp2;
        const targetX = 14 + i*sp2 + sp1*0.5;
        _bc_soldiers.push({id:'p2_'+i,x:startX,tx:targetX,side:'p',alive:true,phase:Math.random()*6.28,dieA:0,alpha:0.82,state:'march',_row:1,_scale:0.78,_yOff:-9});
      }
      // NEMICO — prima fila
      const ef1 = Math.ceil(eN / 2);
      const ef2 = eN - ef1;
      const es1 = Math.min(26, (W * 0.38) / Math.max(1, ef1));
      const es2 = Math.min(22, (W * 0.36) / Math.max(1, ef2));
      for(let i=0;i<ef1;i++) {
        const startX = W + 35 + i*es1;
        const targetX = W - 30 - i*es1;
        _bc_soldiers.push({id:'e'+i,x:startX,tx:targetX,side:'e',alive:true,phase:Math.random()*6.28,dieA:0,alpha:1,state:'march',_row:0,_scale:1.0,_yOff:0});
      }
      for(let i=0;i<ef2;i++) {
        const startX = W + 35 + i*es2;
        const targetX = W - 34 - i*es2 - es1*0.5;
        _bc_soldiers.push({id:'e2_'+i,x:startX,tx:targetX,side:'e',alive:true,phase:Math.random()*6.28,dieA:0,alpha:0.82,state:'march',_row:1,_scale:0.78,_yOff:-9});
      }
      _bc_running=true;
      _bc_loop(fieldId);
    }

    function _bc_loop(fieldId) {
      if (!_bc_running||!_bc_ctx) return;
      _bc_raf=requestAnimationFrame(()=>_bc_loop(fieldId));
      const W=_bc_canvas.width, H=_bc_canvas.height, gnd=H-30;
      _bc_ctx.clearRect(0,0,W,H);

      // Nebbia
      const gr=_bc_ctx.createLinearGradient(0,0,0,H);
      gr.addColorStop(0,'rgba(30,10,8,0.15)'); gr.addColorStop(1,'rgba(45,18,8,0.35)');
      _bc_ctx.fillStyle=gr; _bc_ctx.fillRect(0,0,W,H);

      // Clash glow al centro
      const pF=_bc_soldiers.filter(s=>s.side==='p'&&s.alive), eF=_bc_soldiers.filter(s=>s.side==='e'&&s.alive);
      let inClash=false;
      if (pF.length&&eF.length) {
        const pMax=Math.max(...pF.map(s=>s.x)), eMin=Math.min(...eF.map(s=>s.x));
        if (eMin-pMax<30) {
          inClash=true;
          const cg=_bc_ctx.createRadialGradient(W/2,gnd-20,0,W/2,gnd-20,45);
          cg.addColorStop(0,'rgba(255,100,0,0.28)'); cg.addColorStop(1,'rgba(255,100,0,0)');
          _bc_ctx.fillStyle=cg; _bc_ctx.fillRect(0,0,W,H);
          if (Math.random()<0.14) {
            const sc=4+Math.floor(Math.random()*4);
            for(let i=0;i<sc;i++) _bc_sparks.push({x:W/2+(Math.random()-.5)*30,y:gnd-22+(Math.random()-.5)*10,vx:(Math.random()-.5)*9,vy:-Math.random()*8-2,alpha:1,r:1+Math.random()*2.5,color:['#ff4','#fa0','#f44','#fff','#aef','#fc8'][Math.floor(Math.random()*6)],g:0.42});
          }
        }
      }

      // ── AVANZATA CONTINUA verso il centro ──
      // Ogni frame i frontalieri di prima fila avanzano verso il nemico
      // finché non si toccano fisicamente (gap = 0)
      if (pF.length && eF.length) {
        const pFront = pF.filter(s=>(s._row||0)===0);
        const eFront = eF.filter(s=>(s._row||0)===0);
        if (pFront.length && eFront.length) {
          const pLead = pFront.reduce((a,b)=>a.x>b.x?a:b); // più avanzato giocatore
          const eLead = eFront.reduce((a,b)=>a.x<b.x?a:b); // più avanzato nemico
          const dist = eLead.x - pLead.x;
          if (dist > 18) {
            // Non ancora a contatto — avanza di 0.6px/frame (costante, no lerp)
            const step = Math.min(2.8, dist * 0.07);
            pFront.forEach(s=>{ s.tx = s.x + step; });
            eFront.forEach(s=>{ s.tx = s.x - step; });
            // Seconda fila segue con ritardo
            pF.filter(s=>(s._row||0)===1).forEach(s=>{ s.tx = s.x + step*0.8; });
            eF.filter(s=>(s._row||0)===1).forEach(s=>{ s.tx = s.x - step*0.8; });
          } else {
            // A contatto: blocca le tx dove sono
            _bc_contacted = true;
            pFront.forEach(s=>{ s.tx = s.x; });
            eFront.forEach(s=>{ s.tx = s.x; });
          }
        }
      }

      // Avanzamento fluido (lerp verso tx)
      _bc_soldiers.forEach(s=>{ if(s.alive){ const dx=s.tx-s.x; s.x+=dx*0.12; }});

      // ── KILL QUEUE: esegui morti solo quando in clash ──
      if (inClash && _bc_killQueue.length > 0) {
        _bc_killTimer++;
        if (_bc_killTimer >= 18) { // ~0.3s a 60fps tra un kill e l'altro
          _bc_killTimer = 0;
          const job = _bc_killQueue.shift();
          _bc_killNow(job.side, 1, gnd); // uccidi 1 alla volta per effetto drammatico
          if (job.count > 1) _bc_killQueue.unshift({side:job.side, count:job.count-1});
        }
      }

      // Stato soldati — transizione graduale march→attack via _stateBlend
      _bc_soldiers.forEach(s=>{
        if (!s.alive) return;
        const isMoving = !_bc_contacted || Math.abs(s.tx - s.x) > 5;
        const wantAttack = inClash && !isMoving;
        // _stateBlend: 0=march, 1=attack — lerp lento per evitare scatti
        if (s._stateBlend === undefined) s._stateBlend = 0;
        s._stateBlend += (wantAttack ? 1 : isMoving ? -1 : 0) * 0.03;
        s._stateBlend = Math.max(0, Math.min(1, s._stateBlend));
        // Lo stato effettivo usato da drawSoldier
        if      (isMoving)           s.state = 'march';
        else if (s._stateBlend > 0.5) s.state = 'attack';
        else if (inClash)             s.state = 'attack';
        else                          s.state = 'idle';
      });

      // Sangue
      _bc_blood=_bc_blood.filter(b=>b.alpha>0.04);
      _bc_blood.forEach(b=>{
        b.vy+=b.gravity; b.x+=b.vx; b.y+=b.vy; b.alpha-=0.018;
        _bc_ctx.save(); _bc_ctx.globalAlpha=b.alpha; _bc_ctx.fillStyle='#6a0000';
        if (b.streak) { _bc_ctx.beginPath(); _bc_ctx.ellipse(b.x,b.y,b.r*.4,b.r*2,Math.atan2(b.vy,b.vx),0,Math.PI*2); _bc_ctx.fill(); }
        else { _bc_ctx.beginPath(); _bc_ctx.arc(b.x,b.y,b.r,0,Math.PI*2); _bc_ctx.fill(); }
        _bc_ctx.restore();
      });

      // Scintille
      _bc_sparks=_bc_sparks.filter(s=>s.alpha>0.04);
      _bc_sparks.forEach(s=>{
        s.vy+=s.g; s.x+=s.vx; s.y+=s.vy; s.alpha-=0.045;
        _bc_ctx.save(); _bc_ctx.globalAlpha=s.alpha; _bc_ctx.fillStyle=s.color;
        _bc_ctx.beginPath(); _bc_ctx.arc(s.x,s.y,s.r,0,Math.PI*2); _bc_ctx.fill();
        _bc_ctx.restore();
      });

      // Frecce
      _bc_arrowTimer++;
      if (_bc_arrowTimer % 55 === 0) _bc_spawnArrow(W, H); // ogni ~55 frame = ~1s a 60fps
      _bc_arrows = _bc_arrows.filter(a => a.x > -20 && a.x < W+20 && a.alpha > 0.1);
      for (const a of _bc_arrows) {
        a.vy += a.gravity;
        a.x  += a.vx;
        a.y  += a.vy;
        _bc_drawArrow(_bc_ctx, a);
      }

      // Soldati vivi — seconda fila disegnata prima (sta dietro)
      const _alive = _bc_soldiers.filter(s=>s.alive).sort((a,b)=>(b._row||0)-(a._row||0));
      _alive.forEach(s=>_bc_drawSoldier(_bc_ctx,s.x,gnd-4,s.side==='e',s.phase,s.state,s.alpha,s));

      // Soldati morti (cadono a terra)
      _bc_soldiers.filter(s=>!s.alive).forEach(s=>{
        s.dieA=Math.min(s.dieA+3.5,90); s.alpha=Math.max(s.alpha-.006,0.18);
        _bc_ctx.save(); _bc_ctx.translate(s.x,gnd-3);
        _bc_ctx.rotate((s.side==='p'?-1:1)*s.dieA*Math.PI/180);
        _bc_ctx.globalAlpha=s.alpha; _bc_ctx.fillStyle='#0d0d0d';
        _bc_ctx.beginPath(); _bc_ctx.ellipse(0,0,11,3.5,0,0,Math.PI*2); _bc_ctx.fill();
        if (s.dieA>55) { _bc_ctx.globalAlpha=s.alpha*.55; _bc_ctx.fillStyle='#4a0000'; _bc_ctx.beginPath(); _bc_ctx.ellipse(s.side==='p'?-9:9,2,7,2.5,0,0,Math.PI*2); _bc_ctx.fill(); }
        _bc_ctx.restore();
      });
    }

    function _bc_advance(_unused) { return; // avanzata ora gestita dal loop
      const W=0, center=0;
      const pS=[], eS=[];
      // Gap = distanza del soldato in prima linea dal centro
      // Diminuisce man mano che la battaglia progredisce
      const gap=Math.max(0, (1-progress)*(W*.30));
      // Prima fila avanza al centro, seconda fila segue poco dietro
      const pF1 = pS.filter(s=>(s._row||0)===0), pF2 = pS.filter(s=>(s._row||0)===1);
      const eF1 = eS.filter(s=>(s._row||0)===0), eF2 = eS.filter(s=>(s._row||0)===1);
      const sp1t=Math.min(26,(W*.38)/Math.max(1,pF1.length));
      const sp2t=Math.min(22,(W*.36)/Math.max(1,pF2.length));
      pF1.forEach((s,i)=>{ s.tx = center - gap - i*sp1t; });
      pF2.forEach((s,i)=>{ s.tx = center - gap - 10 - i*sp2t - sp1t*0.5; });
      const es1t=Math.min(26,(W*.38)/Math.max(1,eF1.length));
      const es2t=Math.min(22,(W*.36)/Math.max(1,eF2.length));
      eF1.forEach((s,i)=>{ s.tx = center + gap + i*es1t; });
      eF2.forEach((s,i)=>{ s.tx = center + gap + 10 + i*es2t + es1t*0.5; });
    }

    // Accoda morti — verranno eseguite solo quando i soldati sono in clash
    function _bc_kill(side, count) {
      if (!_bc_canvas || count <= 0) return;
      _bc_killQueue.push({side, count});
    }

    // Esecuzione immediata di 1 morte (chiamata dal loop quando in clash)
    function _bc_killNow(side, count, gnd) {
      const alive = _bc_soldiers.filter(s=>s.side===side&&s.alive);
      const sorted = alive.slice().sort((a,b)=>
        side==='p' ? b.x-a.x : a.x-b.x
      );
      sorted.slice(0, Math.min(count, sorted.length)).forEach(s=>{
        s.alive=false; s.dieA=0;
        _bc_soldiers.filter(o=>o.alive&&o.side===s.side&&Math.abs(o.x-s.x)<32)
          .forEach(o=>{o._stagger=(o._stagger||0)+0.55;});
        for(let i=0;i<14;i++) _bc_blood.push(_bc_createBlood(s.x, gnd-18));
        for(let i=0;i<5;i++) _bc_sparks.push({
          x:s.x+(Math.random()-.5)*18, y:gnd-24,
          vx:(Math.random()-.5)*8, vy:-Math.random()*7-2,
          alpha:1, r:2+Math.random()*2.5,
          color:['#ff4','#f80','#f44','#fff'][Math.floor(Math.random()*4)], g:0.42
        });
      });
    }

    function _bc_stop() { _bc_running=false; if(_bc_raf){cancelAnimationFrame(_bc_raf);_bc_raf=null;} }
    Game._bc_stop = _bc_stop; // esposto globalmente per i pulsanti onclick
    // ═══════════════════════════════════════════════════

    const TROOP_COUNT_TH = 7;
    ...
    overlay.innerHTML = `
      ...
      <div style="position:relative;z-index:10;width:100%;max-width:540px;padding:0 1rem;display:flex;flex-direction:column;align-items:center;gap:0.5rem" id="throne-main">
        ...
        <div style="width:100%;background:linear-gradient(180deg,#2e1510 0%,#1e0e08 55%,#2e1a0a 100%);border:1px solid rgba(201,168,76,0.3);border-radius:4px;padding:0.6rem 0.5rem;min-height:80px;position:relative;overflow:hidden" id="tb-field">
    ...
    // ── Init canvas soldati battaglia trono ──
    setTimeout(() => {
      _bc_initField('tb-field', TROOP_COUNT_TH, TROOP_COUNT_TH);
    }, 80);
    ...
    let phase  = 0;
    const ROUNDS_PER_PHASE = 6;
    const usedEventIds = new Set();

    // Speed state per throne battle (shared via closure)
    const THRONE_SPEED_CONFIG = {
      1: { roundDelay: 2800, phaseStartDelay: 800, flashDuration: 500, showDelta: true  },
      2: { roundDelay: 700,  phaseStartDelay: 300, flashDuration: 150, showDelta: false },
    };
    let _tSpeed = 1;
    ...
    Game._battleRetreatFn = () => {
      ...
      document.getElementById('tb-btns').innerHTML = `
        <button onclick="if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();Game._bc_stop();document.getElementById('throne-battle-overlay').remove();Game._battleRetreatFinish(${survived})"
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      const pToKill = Math.max(0, pAlive - pShouldBeAlive);
      const eToKill = Math.max(0, eAlive - eShouldBeAlive);

      if (pToKill > 0) _bc_kill('p', pToKill);
      if (eToKill > 0) _bc_kill('e', eToKill);
      const progress = Math.min(0.9, 0.2 + phase * 0.14);
    ...
    function runPhaseRound(roundsLeft, cb) {
      ...
      const pDmg = eForce * (0.025 + Math.random() * 0.035 + _underdogBonus);
      const eDmg = pForce * (0.025 + Math.random() * 0.035);
    ...
    function finalizeThroneBattle() {
      ...
      if (btns) {
        setTimeout(() => {
          btns.innerHTML = `
            <button onclick="${won
              ? `Game._bc_stop();document.getElementById('throne-battle-overlay').remove();Game._battleCompleteFn(${survived})`
              : `if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();Game._bc_stop();document.getElementById('throne-battle-overlay').remove();Game._battleCompleteFn(${survived})`
    ...
    // Assicura callback disponibile prima che la battaglia finisca
    Game._battleCompleteFn = (survived) => onComplete(playerForce > kingForce, survived, Math.max(0, Math.round(eForce)));
    // Start phase 0 — aspetta il contatto fisico prima di far partire i round
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat, bgConfig) {
    ...
    // ── Init canvas soldati battaglia casate ──
    setTimeout(() => {
      _bc_initField('battle-field', TROOP_COUNT, TROOP_COUNT);
    }, 80);

    // ── Speed state ──
    let _speed = 1; // 1 = slow (x1), 2 = fast (x2)
    const SPEED_CONFIG = {
      1: { roundDelay: 2800, phaseStartDelay: 800, flashDuration: 500, particles: 8,  showDelta: true  },
      2: { roundDelay: 700,  phaseStartDelay: 300, flashDuration: 150, particles: 3,  showDelta: false },
    };
    Game._battleToggleSpeed = function() {
      _speed = _speed === 1 ? 2 : 1;
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      // Avanzata proporzionale alle perdite
      const totalDead = (TROOP_COUNT - pShouldBeAlive) + (TROOP_COUNT - eShouldBeAlive);
      const progress = Math.min(1.0, 0.35 + totalDead / (TROOP_COUNT * 2) * 0.65);
    ...
    function runPhaseRounds(roundsLeft, phaseEvtApplied, afterRoundsCallback) {
      ...
      const pDmg = eForce * (0.025 + Math.random() * 0.035 + underdogBonus);
      const eDmg = pForce * (0.025 + Math.random() * 0.035);
      const prevP = pForce, prevE = eForce;
      pForce = Math.max(0, pForce - pDmg);
      eForce = Math.max(0, eForce - eDmg);
    ...
    function finalizeBattle() {
      ...
      if (btns) {
        setTimeout(() => {
          btns.innerHTML = `
            <button onclick="${won ? "if(typeof AudioManager!=='undefined')AudioManager.playMainFromWar();" : ''}Game._bc_stop();document.getElementById('battle-anim-overlay').remove();Game._battleResolveFn(${won},${survived},${enemySurvived})"
      ...
      // Callback assegnato subito — il bottone lo usa via onclick inline
      if (!Game._battleResolveFn || typeof Game._battleResolveFn !== 'function') {
        Game._battleResolveFn = (won, survived, enemySurvived) => onComplete(won, survived, enemySurvived);
      }
    }

    // Assicura che _battleResolveFn sia sempre disponibile prima che la battaglia finisca
    Game._battleResolveFn = (won, survived, enemySurvived) => onComplete(won, survived, enemySurvived);
