...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _bc_drawSoldier(ctx, x, y, flip, phase, state, alpha) {
      const t = Date.now() / 1000;
      let bob=0, legF=0, legB=0, sw=0, cloak=0, lean=0;
      if (state==='march') {
        bob=Math.sin(t*8+phase)*2; legF=Math.sin(t*8+phase)*28; legB=-legF*0.9;
        cloak=Math.sin(t*6+phase)*7; lean=Math.sin(t*8+phase)*3;
      } else if (state==='attack') {
        bob=Math.sin(t*13)*2.5; sw=Math.sin(t*10)*35+5;
        lean=Math.sin(t*13)*7; cloak=Math.sin(t*9)*12;
        legF=Math.sin(t*13)*15; legB=-legF*0.7;
      } else {
        bob=Math.sin(t*2.5+phase)*1; cloak=Math.sin(t*2+phase)*3;
      }

      ctx.save();
      ctx.globalAlpha=alpha;
      ctx.shadowColor='rgba(0,0,0,0.5)'; ctx.shadowBlur=4;
      if (flip){ctx.translate(x,y);ctx.scale(-1,1);}
      else ctx.translate(x,y);
      ctx.fillStyle='#0a0a0a';

      // MANTELLO — forma a campana dietro il corpo
      ...
      ctx.arc(0,13,3.5,0,Math.PI); ctx.lineTo(-3,0);
      ctx.fill();
      ...
      ctx.arc(0,13,4,0,Math.PI); ctx.lineTo(-3.5,0);
      ctx.fill();
      ...
      // BUSTO con corazza (trapezio largo)
      ...
      // BRACCIO con spada (dietro schiena durante marcia, davanti durante attacco)
      ctx.save(); ctx.translate(-7,-22+bob); ctx.rotate((lean+sw*.15)*Math.PI/180);
      // braccio
      ctx.beginPath(); ctx.roundRect(-2.5,0,5,12,3); ctx.fill();
      ctx.save(); ctx.translate(0,12); ctx.rotate((-60+sw)*Math.PI/180);
      // mano
      ctx.beginPath(); ctx.arc(0,0,3.5,0,Math.PI*2); ctx.fill();
      // SPADA a due mani — impugnatura
      ...
      ctx.lineTo(1.5,34); ctx.lineTo(0,39);
      ctx.lineTo(-1.5,34); ctx.closePath(); ctx.fill();
      ctx.restore(); ctx.restore();

      // BRACCIO DX (al fianco, leggermente mosso)
      ctx.save(); ctx.translate(7,-22+bob); ctx.rotate((-lean)*Math.PI/180);
      ctx.beginPath(); ctx.roundRect(-2,0,4.5,11,3); ctx.fill();
      ctx.save(); ctx.translate(0,11); ctx.rotate(15*Math.PI/180);
      ctx.beginPath(); ctx.roundRect(-2,0,4,8,2); ctx.fill();
      ctx.restore(); ctx.restore();

      // ELMO con cimiero e nasale — forma caratteristica medievale
      ctx.save(); ctx.translate(0,-28+bob);
      // calotta rotonda
      ctx.beginPath(); ctx.arc(0,0,7,Math.PI,0); ctx.fill();
      // frontale piatto con nasale
      // frontale piatto con nasale (più corto)
      ctx.beginPath();
    ...
    function _bc_initField(fieldId, pN, eN) {
      ...
      _bc_soldiers=[];
      for(let i=0;i<pN;i++) _bc_soldiers.push({id:'p'+i,x:8+i*sp,tx:8+i*sp,side:'p',alive:true,phase:Math.random()*6.28,dieA:0,alpha:1,state:'march'});
      for(let i=0;i<eN;i++) _bc_soldiers.push({id:'e'+i,x:W-28-i*sp,tx:W-28-i*sp,side:'e',alive:true,phase:Math.random()*6.28,dieA:0,alpha:1,state:'march'});
      _bc_running=true;
      _bc_loop(fieldId);
    }
    ...
    function _bc_loop(fieldId) {
      ...
      if (pF.length&&eF.length) {
        ...
        if (eMin-pMax<50) {
          ...
          if (Math.random()<0.07) {
            for(let i=0;i<3;i++) _bc_sparks.push({x:W/2+(Math.random()-.5)*25,y:gnd-22,vx:(Math.random()-.5)*7,vy:-Math.random()*6-2,alpha:1,r:1.5+Math.random()*2,color:['#ff4','#fa0','#f44','#fff'][Math.floor(Math.random()*4)],g:0.45});
          }
        }
      }

      // Avanzamento fluido
      _bc_soldiers.forEach(s=>{ if(s.alive){ const dx=s.tx-s.x; s.x+=dx*0.028; }});
      ...
      // Soldati vivi
      _bc_soldiers.filter(s=>s.alive).forEach(s=>_bc_drawSoldier(_bc_ctx,s.x,gnd-4,s.side==='e',s.phase,s.state,s.alpha));
    ...
    function _bc_advance(progress) {
      ...
      const gap=Math.max(0, 2+(1-progress)*(W*.28));
      const sp=16; // spaziatura tra soldati
      ...
      sorted.slice(0,Math.min(count,sorted.length)).forEach((s,idx)=>{
        setTimeout(()=>{
          ...
          setTimeout(()=>{
            ...
            const prog=Math.min(0.92, 0.25+totalDead/20*0.67);
            _bc_advance(prog);
          }, 250);
        }, idx*200);
      });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => _bc_advance(0.15), 300);
      setTimeout(() => _bc_advance(0.45), 2500);
    }, 80);
