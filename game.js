...
const Game = (() => {
  ...
  function updateHUD() {
    ...
    Object.keys(bars).forEach(key => {
      const v = Math.max(0, Math.min(cap, r[key]));
      document.getElementById(bars[key]).style.width = (v / cap * 100) + '%';
      const loanNote = key === 'army' && state.loanedArmy ? ` (+${state.loanedArmy})` : '';
      document.getElementById(vals[key]).textContent = Math.round(v) + loanNote;
    });
  ...
  function makeChoice(side) {
    ...
    // Show effects popup AFTER choice is made
    showChoiceEffectsPopup(side, effects);
    ...
    // Check game over — delayed to let popup show first
    setTimeout(() => {
  ...
  function showChoiceEffectsPopup(side, effects) {
      ...
      .map(([k, v]) => {
        ...
        return `
          <div style="display:flex;align-items:center;justify-content:space-between;
                      padding:0.55rem 0.75rem;margin-bottom:0.3rem;
                      background:${pos ? 'rgba(74,222,128,0.07)' : 'rgba(248,113,113,0.07)'};
                      border:1px solid ${pos ? 'rgba(74,222,128,0.25)' : 'rgba(248,113,113,0.25)'};
                      border-radius:4px;">
            <span style="font-family:'Cinzel',serif;font-size:0.82rem;color:#e8dcc8">
              ${icons[k]} ${labels[k]}
            </span>
            <span style="font-family:'Cinzel',serif;font-size:1rem;font-weight:700;color:${color}">
              ${arrow} ${pos ? '+' : ''}${v}
            </span>
          </div>`;
      }).join('');
    ...
    popup.innerHTML = `
      ...
      ">
        <div style="
          ...
          color:#9a8a6a;margin-bottom:0.5rem;
        ">Conseguenze della scelta</div>
        <div style="
          font-family:'EB Garamond',serif;font-size:0.9rem;color:#c9a84c;
          margin-bottom:0.85rem;font-style:italic;line-height:1.4;
          border-bottom:1px solid rgba(201,168,76,0.2);padding-bottom:0.65rem;
        ">«${choiceLabel}»</div>
        ${rows}
        <div style="text-align:center;margin-top:0.85rem">
          <div style="font-size:0.65rem;color:#4a3f2e;letter-spacing:0.08em">
    ...
    // Auto-dismiss after 2.4s OR on tap/click
    const dismiss = () => {
  ...
  function checkGameOver() {
    ...
    if (r.gold <= 0) {
      return triggerEnd(false, '💸', 'Le Casse Sono Vuote',
        `Il tesoro reale è esaurito fino all'ultimo Stag. I mercenari hanno abbandonato le vostre fila reclamando paghe arretrate. ${isRuler ? 'Il Gran Consiglio si è riunito in vostro nome — e ha votato la vostra deposizione.' : 'Senza oro, la vostra influenza si è dissolta come nebbia al sole.'} Il vostro nome sarà ricordato come monito ai governanti avidi.`
      );
    }
    if (r.gold >= cap) {
      return triggerEnd(false, '🏦', 'L\'Avidità del Drago',
        `Le vostre casse traboccano di ricchezze accumulate senza distribuirne una moneta. La Banca di Ferro ha finanziato una coalizione di rivali. ${isRuler ? 'Un colpo di stato ben finanziato vi ha strappato il trono.' : 'I vostri stessi alleati vi hanno tradito, attratti dal bottino.'} L\'avidità è la rovina dei re.`
      );
    }
    // ── FAITH ──
    if (r.faith <= 0) {
      return triggerEnd(false, '⛪', 'La Maledizione dei Sette',
        `I Sette si sono voltati contro di voi. Il Septon Supremo ha pronunciato la scomunica. ${isRuler ? 'La guardia reale ha rifiutato di combattere contro la volontà dei Sette.' : 'Nessun signore osa ospitarvi.'} Siete fuggiti nella notte, soli.`
      );
    }
    if (r.faith >= cap) {
      return triggerEnd(false, '🔥', 'Il Fanatismo dei Fedeli',
        `La Fede Militante ha preso il controllo delle strade. ${isRuler ? 'Il Septon Supremo ha dichiarato che la purificazione richiede il vostro sacrificio.' : 'I fanatici hanno rivolto le loro spade contro di voi.'} Il fuoco della Fede consuma tutto.`
      );
    }
    // ── PEOPLE ──
    if (r.people <= 0) {
      return triggerEnd(false, '🔥', 'La Grande Rivolta',
        `Le strade bruciano. La gente affamata ha sfondato i cancelli. ${isRuler ? 'Le vostre guardie hanno disertato rifiutandosi di massacrare i propri fratelli.' : 'La folla ha assaltato la vostra dimora.'} Avete dimenticato che i re regnano per grazia del popolo.`
      );
    }
    if (r.people >= cap) {
      return triggerEnd(false, '🎭', 'L\'Idolo Spodestato',
        `Eravate troppo amati. ${isRuler ? 'Il Gran Consiglio vi ha "elevato" a simbolo sacro, privandovi di ogni potere reale.' : 'La vostra popolarità ha spaventato il Trono. Un sicario dei Molti Volti è stato ingaggiato.'} Anche il sole tramonta.`
      );
    }
    // ── ARMY ──
    if (r.army <= 0) {
      return triggerEnd(false, '💀', 'La Disfatta Totale',
        `Il vostro esercito non esiste più. ${isRuler ? 'Le porte sono state aperte senza combattere. Il Re conquistatore è entrato a cavallo nella Sala del Trono.' : 'I vostri nemici hanno fatto irruzione nel castello all\'alba.'} La storia ricorda solo i vincitori.`
      );
    }
    if (r.army >= cap) {
      return triggerEnd(false, '⚔', 'Il Condottiero Spodestato',
        `Il vostro esercito ha terrorizzato ogni casata. ${isRuler ? 'I vostri generali hanno dichiarato che un re con un esercito simile è un tiranno. Vi hanno presentato una scelta: abdicare o essere rimosso.' : 'Una coalizione di casate ha lanciato un attacco preventivo.'} Il potere militare assoluto è preludio alla caduta.`
      );
    }
    // ── POLITICAL POWER ──
    if (r.power <= 0) {
      return triggerEnd(false, '🕯️', 'L\'Ombra Svanita',
        `Il vostro potere politico si è dissolto. ${isRuler ? 'Il Gran Maester ha convocato il Gran Consiglio senza informarvi.' : 'Le grandi casate vi ignorano come si ignora un servo.'} Uno spettro alla corte, senza voce.`
      );
    }
    if (r.power >= cap) {
      return triggerEnd(false, '👁️', 'Il Tiranno Assoluto',
        `Avete accumulato potere senza precedenti. ${isRuler ? 'Una congiura silenziosa vi ha eliminati prima che il vostro controllo diventasse eterno.' : 'Siete la minaccia che tutti temono — quindi tutti vi hanno affrontato insieme.'} Nessuno può tenere il mondo nel pugno senza che si rivolti.`
      );
    }
    // ── WIN ──
  ...
  function checkEnemyTributeDemands() {
    ...
    Object.entries(state.houses).forEach(([hId, h]) => {
      ...
      if (h.suppressed) return; // conquered house
      if (state.tributeCooldowns[hId] && state.turn < state.tributeCooldowns[hId]) return;
      // Only attack if their army is at least 40% of player's
      if (h.army < playerArmy * 0.4) return;
      // ~15% chance per turn to send tribute demand
      if (Math.random() > 0.15) return;

      state.tributeCooldowns[hId] = state.turn + 6; // cooldown 6 turns
      state.eventQueue.push(buildTributeDemandCard(hId, h));
    });
  }

  function buildTributeDemandCard(hId, h) {
    const demandAmt = Math.floor(10 + Math.random() * 15); // 10–25
    ...
    return {
      ...
      speaker: `Casa ${h.name}`,
      speakerRole: 'Ultimatum dei nemici',
      portrait: h.icon, icon: h.icon,
      text: `${h.icon} Casa ${h.name} invia un ultimatum: «Pagate ${demandAmt} ${resLabels[demandRes]} come tributo o preparatevi alla guerra. Avete tre turni per decidere.»`,
      leftText: `Rifiuta l'ultimatum`, leftEffects: {},
      rightText: `Paga il tributo (${demandAmt} ${resLabels[demandRes]})`, rightEffects: { [demandRes]: -demandAmt },
      tags: ['tribute_demand'],
      tributeHouseId: hId,
      tributeRes: demandRes,
      tributeAmt: demandAmt,
      onLeftChoose: () => scheduleHouseAttack(hId, 3),
    };
  }
  ...
  function scheduleHouseAttack(hId, inTurns) {
    ...
    state.eventQueue.push({
      id: 'house_attack_warning_' + hId,
      speaker: `Casa ${h.name}`,
      speakerRole: '⚠ Dichiarazione di guerra',
      portrait: '⚔️', icon: '⚔️',
      text: `${h.icon} Casa ${h.name} ha radunato le sue truppe. Gli esploratori riferiscono che le loro armate si stanno avvicinando. La battaglia è imminente — preparatevi.`,
      leftText: 'Preparati alla difesa', leftEffects: { army: +5, gold: -5 },
      rightText: 'Attendi la battaglia', leftEffects: {},
      tags: ['house_attack_incoming'],
      minTurn: state.turn + inTurns,
  ...
  function triggerHouseBattle(houseId, playerInitiated) {
    ...
    showBattleAnimation(playerForce, enemyForce, () => {
      _resolveHouseBattle(houseId, h, won, playerForce, enemyForce, desertNote, playerInitiated);
    });
  }

  function _resolveHouseBattle(houseId, h, won, playerForce, enemyForce, desertNote, playerInitiated) {
    ...
    if (won) {
      ...
      // Add house resources to player (capped at new cap+100)
      ...
      state.resources.army   = Math.min(newCap, state.resources.army   + bonusArmy   - Math.floor(margin * 0.15));
      ...
      showHouseBattleResult(true, h, bonusGold, bonusArmy, bonusPeople, bonusPower, newCap, desertNote);
  ...
  function showHouseBattleResult(won, h, bonusGold, bonusArmy, bonusPeople, bonusPower, newCap, desertNote) {
    const overlay = document.createElement('div');
    overlay.className = 'war-overlay';
    overlay.innerHTML = `
...
window.addEventListener('load', () => {
  ...
  if (badge) badge.textContent = 'v1.8.0';
