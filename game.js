...
const Game = (() => {
  ...
  // ═══════════════════════════════════════════════════════
  // SOLDIER SYSTEM — silhouette SVG con Anime.js
  // ═══════════════════════════════════════════════════════
  const _SOLDIER_SVG = `<svg viewBox="0 0 22 42" xmlns="http://www.w3.org/2000/svg" fill="#111">
  <!-- testa con elmo -->
  <ellipse cx="11" cy="5" rx="4" ry="4.5" fill="#0a0a0a"/>
  <rect x="7" y="2" width="8" height="3" rx="1.5" fill="#0a0a0a"/>
  <!-- corpo -->
  <rect x="8" y="9" width="6" height="11" rx="1" fill="#0a0a0a"/>
  <!-- braccio sinistro (con spada) -->
  <g class="arm-l">
    <rect x="3" y="10" width="5" height="2.5" rx="1" fill="#0a0a0a"/>
    <!-- spada -->
    <g class="sword">
      <rect x="-1" y="8" width="1.5" height="9" rx="0.5" fill="#1a1a1a"/>
      <rect x="-2" y="15.5" width="3.5" height="1.2" rx="0.4" fill="#1a1a1a"/>
    </g>
  </g>
  <!-- braccio destro (con scudo) -->
  <g class="arm-r">
    <rect x="14" y="10" width="5" height="2.5" rx="1" fill="#0a0a0a"/>
    <rect x="18" y="8" width="3.5" height="7" rx="1" fill="#0f0f0f"/>
  </g>
  <!-- gamba sinistra -->
  <g class="leg-l">
    <rect x="8" y="20" width="3" height="10" rx="1" fill="#0a0a0a"/>
    <rect x="7.5" y="29" width="3.5" height="2.5" rx="1" fill="#0a0a0a"/>
  </g>
  <!-- gamba destra -->
  <g class="leg-r">
    <rect x="11" y="20" width="3" height="10" rx="1" fill="#0a0a0a"/>
    <rect x="11" y="29" width="3.5" height="2.5" rx="1" fill="#0a0a0a"/>
  </g>
</svg>`;

  function _createSoldier(id, side, xPos) {
    const el = document.createElement('div');
    el.className = `soldier ${side==='p'?'player-s':'enemy-s'}`;
    el.id = id;
    el.style.cssText = `left:${xPos}px; bottom:32px;`;
    el.innerHTML = _SOLDIER_SVG;
    return el;
  }

  function _spawnBlood(field, x, y, count=8) {
    for (let i=0; i<count; i++) {
      ...
      d.className = 'blood-drop' + (isStreak?' streak':'');
      d.style.cssText = `
        left:${x}px; top:${y}px;
        --bs:${size}px;
        --bx:${Math.cos(angle)*dist}px;
        --by:${Math.sin(angle)*dist - 10}px;
  ...
  function _clashSpark(field, x, y) {
    const sparks = ['✦','⚡','✸','✷'];
    const s = document.createElement('div');
    s.className = 'clash-spark-el';
  ...
  function _initSoldierBattle(fieldId, pCount, eCount) {
    const field = document.getElementById(fieldId);
    if (!field) return null;
    const W = field.offsetWidth || 300;
    const soldiers = [];

    // Posizioni iniziali: giocatore a sinistra, nemici a destra
    const pSpacing = Math.min(22, (W*0.42) / pCount);
    const eSpacing = Math.min(22, (W*0.42) / eCount);

    for (let i=0; i<pCount; i++) {
      const x = 6 + i * pSpacing;
      const s = _createSoldier(`${fieldId}-p-${i}`, 'p', x);
      field.appendChild(s);
      soldiers.push({el:s, side:'p', idx:i, x, alive:true, targetX: x});
    }
    for (let i=0; i<eCount; i++) {
      const x = W - 28 - i * eSpacing;
      const s = _createSoldier(`${fieldId}-e-${i}`, 'e', x);
      field.appendChild(s);
      soldiers.push({el:s, side:'e', idx:i, x, alive:true, targetX: x});
    }

    // Avvia marcia
    soldiers.forEach(s => s.el.classList.add('marching'));
    return soldiers;
  }

  function _marchSoldiers(soldiers, fieldId, progress) {
    // progress 0→1: truppe avanzano verso il centro
    const field = document.getElementById(fieldId);
    if (!field) return;
    const W = field.offsetWidth || 300;
    const center = W / 2;
    ...
    soldiers.forEach(s => {
      if (!s.alive) return;
      const pSoldiers = soldiers.filter(x=>x.side==='p'&&x.alive);
      const eSoldiers = soldiers.filter(x=>x.side==='e'&&x.alive);
      const gap = 30 + (1-progress) * (W*0.3); // distanza dal centro

      if (s.side==='p') {
        const rank = pSoldiers.indexOf(s);
        s.targetX = Math.min(center - gap + rank*22, center - 28);
      } else {
        const rank = eSoldiers.indexOf(s);
        s.targetX = Math.max(center + gap - rank*22 - 22, center + 6);
      }

      if (typeof anime !== 'undefined') {
        anime({ targets: s.el, left: s.targetX, duration: 800, easing:'easeInOutSine' });
      } else {
  ...
  function _killSoldierAnim(soldiers, fieldId, side, count) {
    const field = document.getElementById(fieldId);
    if (!field) return;
    const toKill = soldiers
      .filter(s=>s.side===side && s.alive)
      .slice(-count);

    toKill.forEach((s, idx) => {
      setTimeout(()=>{
        s.alive = false;
        s.el.classList.remove('marching','attacking');
        ...
        _spawnBlood(field,
          rect.left - fRect.left + 11,
          rect.top  - fRect.top  + 20,
          6 + Math.floor(Math.random()*5)
        );

        // Anima caduta con Anime.js se disponibile
        if (typeof anime !== 'undefined') {
          anime({ targets: s.el,
            rotate: s.side==='p' ? -95 : 95,
            translateY: 12,
            opacity: 0.25,
            duration: 580,
            easing: 'easeInCubic'
  ...
  function _clashAnimation(soldiers, fieldId) {
    const field = document.getElementById(fieldId);
    if (!field) return;
    const W = field.offsetWidth || 300;

    // Flash
    const flash = field.querySelector('.battle-clash-flash');
    if (flash) { flash.classList.remove('flash'); void flash.offsetWidth; flash.classList.add('flash'); }

    // Spark al centro
    _clashSpark(field, W/2, 45);

    // Soldati vivi attaccano
    soldiers.filter(s=>s.alive).forEach(s=>{
      s.el.classList.remove('marching');
      s.el.classList.add('attacking');
      setTimeout(()=>{
        s.el.classList.remove('attacking');
        s.el.classList.add('marching');
      }, 350);
    });

    // Shake campo
    if (typeof anime !== 'undefined') {
      anime({targets:field, translateX:[-4,4,-3,3,-2,2,0], duration:350, easing:'easeInOutSine'});
    }
  }
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat, bgConfig) {
    ...
    let pForce = playerForce;
    let eForce = kingForce;
    let pAlive = TROOP_COUNT;
    let eAlive = TROOP_COUNT;
    let phase = 0;
    const ROUNDS_PER_PHASE = 2;

    // Inizializza soldati SVG dopo che l'overlay è nel DOM
    let _soldiers = [];
    requestAnimationFrame(() => {
      _soldiers = _initSoldierBattle('battle-field', TROOP_COUNT, TROOP_COUNT) || [];
      // Marcia iniziale verso il centro
      setTimeout(() => _marchSoldiers(_soldiers, 'battle-field', 0.3), 200);
    });
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      // Usa il nuovo sistema soldati SVG se disponibile
      if (_soldiers.length > 0) {
        if (pToKill > 0) _killSoldierAnim(_soldiers, 'battle-field', 'p', pToKill);
        if (eToKill > 0) _killSoldierAnim(_soldiers, 'battle-field', 'e', eToKill);
        // Clash + avanzata
        setTimeout(() => _clashAnimation(_soldiers, 'battle-field'), 100);
        setTimeout(() => _marchSoldiers(_soldiers, 'battle-field', 0.6 + phase*0.1), 400);
      }
