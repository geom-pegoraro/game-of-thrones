...
const Game = (() => {
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    function makeUnit(side, col, row, angle, stacks, troopType) {
      ...
      // Statistiche base modificate per tipo truppa
      const typeStats = {
        milizia:          { hpMult: 0.7,  dmgMult: 0.7,  spd: 0.85, r: 0,    color: '#a3a3a3', icon: '🪓' },
        sellaspe:         { hpMult: 1.0,  dmgMult: 1.0,  spd: 1.0,  r: 1,    color: '#fbbf24', icon: '⚔️' },
        dothraki:         { hpMult: 0.85, dmgMult: 1.4,  spd: 1.6,  r: 1,    color: '#fb923c', icon: '🐴' },
        immacolati:       { hpMult: 1.3,  dmgMult: 1.1,  spd: 0.9,  r: 2,    color: '#60a5fa', icon: '🛡️' },
        compagnia_dorata: { hpMult: 1.5,  dmgMult: 1.3,  spd: 0.95, r: 3,    color: '#fde047', icon: '💰' },
        drago:            { hpMult: 3.0,  dmgMult: 2.5,  spd: 1.2,  r: 6,    color: '#f87171', icon: '🐉' },
        // Tipi nemici casate/re
        guardie_stark:    { hpMult: 1.1,  dmgMult: 1.0,  spd: 0.95, r: 1,    color: '#94a3b8', icon: '🐺' },
        cavalieri_lann:   { hpMult: 1.2,  dmgMult: 1.2,  spd: 1.0,  r: 2,    color: '#c9a84c', icon: '🦁' },
        arcieri_tyrell:   { hpMult: 0.8,  dmgMult: 1.3,  spd: 1.1,  r: 1,    color: '#4ade80', icon: '🌹' },
        cavalieri_bar:    { hpMult: 1.2,  dmgMult: 1.1,  spd: 1.1,  r: 1,    color: '#fbbf24', icon: '🦌' },
        lancieri_tully:   { hpMult: 1.0,  dmgMult: 1.0,  spd: 1.0,  r: 0,    color: '#60a5fa', icon: '🐟' },
        serpi_martell:    { hpMult: 0.9,  dmgMult: 1.4,  spd: 1.3,  r: 1,    color: '#fb923c', icon: '☀️' },
        saccheggiatori:   { hpMult: 0.85, dmgMult: 1.3,  spd: 1.5,  r: 0,    color: '#818cf8', icon: '🐙' },
        soldati_frey:     { hpMult: 0.8,  dmgMult: 0.9,  spd: 0.9,  r: 0,    color: '#9ca3af', icon: '🌉' },
        sguernitori:      { hpMult: 0.9,  dmgMult: 1.2,  spd: 1.1,  r: 1,    color: '#ef4444', icon: '🩸' },
        guardie_re:       { hpMult: 1.4,  dmgMult: 1.3,  spd: 1.0,  r: 3,    color: '#c9a84c', icon: '👑' },
      };
      const ts = typeStats[troopType] || typeStats['sellaspe'];
      ...
      return {
        ...
        stacks, dmg: dmgVal,
        troopType: troopType || (side === 'p' ? 'sellaspe' : 'soldati_frey'),
        troopColor: ts.color,
        troopIcon: ts.icon,
    ...
    // ── Unità giocatore dai tipi di truppa reclutati ──
    const UNIT_CAP = 12;
    const playerUnitsList = [];
    const recruitedTroops = state.troops || {};

    if (Object.keys(recruitedTroops).length > 0) {
      TROOP_TYPES.forEach(t => {
        const count = recruitedTroops[t.id] || 0;
        for (let i = 0; i < count; i++) playerUnitsList.push({ type: t.id, passive: t.battlePassive || null });
      });
      if (state.hasDragon && !playerUnitsList.find(u => u.type === 'drago')) {
        playerUnitsList.push({ type: 'drago', passive: null });
      }
    } else {
      const n = Math.min(UNIT_CAP, Math.max(1, Math.floor(playerForce / 5)));
      for (let i = 0; i < n; i++) playerUnitsList.push({ type: 'sellaspe', passive: null });
    }
    ...
    // ── CPU: tipi truppa per casata/re — narrativamente coerenti ──
    // Ogni casata ha un roster fisso che rispecchia il suo arco narrativo
    const HOUSE_ROSTERS = {
      Stark:      ['milizia', 'milizia', 'sellaspe'],
      Lannister:  ['compagnia_dorata', 'sellaspe', 'sellaspe'],
      Tyrell:     ['sellaspe', 'milizia', 'milizia'],
      Baratheon:  ['sellaspe', 'sellaspe', 'milizia'],
      Tully:      ['milizia', 'sellaspe', 'milizia'],
      Martell:    ['sellaspe', 'dothraki', 'milizia'],
      Greyjoy:    ['sellaspe', 'milizia', 'sellaspe'],
      Frey:       ['milizia', 'milizia', 'sellaspe'],
      Bolton:     ['sellaspe', 'sellaspe', 'milizia'],
      Targaryen:  ['immacolati', 'dothraki', 'immacolati'],
      Hightower:  ['immacolati', 'sellaspe', 'milizia'],
      _king:      ['compagnia_dorata', 'immacolati', 'sellaspe'],
    };

    // Determina il roster nemico
    const isKingBattle = !enemyHouseId || enemyLabel?.toLowerCase().includes('re') || enemyLabel?.toLowerCase().includes('king');
    const rosterKey = isKingBattle ? '_king' : (enemyHouseId || '_king');
    const enemyRoster = HOUSE_ROSTERS[rosterKey] || HOUSE_ROSTERS['soldati_frey'];
    ...
    CPU_STRATS[Math.floor(Math.random() * CPU_STRATS.length)](eVisible)
      .forEach(([col, row], i) => {
        // Assegna il tipo ciclando sul roster della casata
        const troopType = enemyRoster[i % enemyRoster.length];
        enemyUnits.push(makeUnit('e', col, row, Math.PI / 2, 1, troopType));
      });
    ...
    ov.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:4px;width:${BW+20}px;max-height:100vh;overflow:hidden">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:2px 0">
          ...
          ${(() => {
            const troops = state.troops || {};
            const parts = TROOP_TYPES.filter(t => (troops[t.id]||0) > 0)
              .map(t => `<span style="color:${t.battleColor}">${t.icon}${troops[t.id]}</span>`).join(' ');
            const dragon = state.hasDragon ? `<span style="color:#f87171">🐉×1</span>` : '';
            return (parts || dragon) ? `${parts} ${dragon}` : '⚔ Milizia';
          })()}</div>
        </div>
    ...
    function drawUnit(u) {
      ...
      } else if(tt==='immacolati'||tt==='compagnia_dorata') {
        // Elite: quadrato ruotato (diamante)
        const s=r*0.85;
        mCtx.moveTo(0,-s); mCtx.lineTo(s,0); mCtx.lineTo(0,s); mCtx.lineTo(-s,0);
        mCtx.closePath();
        mCtx.fillStyle=iP?'rgba(8,20,40,.94)':'rgba(30,20,8,.94)';
      } else if(tt==='dothraki') {
        // Veloci/aggressivi: triangolo
        mCtx.moveTo(0,-r); mCtx.lineTo(r*0.87,r*0.5); mCtx.lineTo(-r*0.87,r*0.5);
        mCtx.closePath();
        mCtx.fillStyle=iP?'rgba(35,15,5,.94)':'rgba(35,10,5,.94)';
      } else {
        // Default: cerchio
  ...
  function _tutStep_game() {
    _tutGameSteps = [
      ...
      // 8 — Esercito
      () => _tutPopup(null,
        '⚔ <strong>Esercito</strong>:<br>La risorsa ⚔ rappresenta la tua forza militare. In battaglia ogni <strong>5 punti Esercito = 1 cerchietto</strong> sul campo (max 16).<br><br>Gestisci bene il tuo esercito — troppo alto o troppo basso può costarti la partita!<br><br>🐉 <strong>Drago</strong>: ottenibile <em>solo</em> tramite eventi speciali (uovo di drago).',
        null, _tutGameNext, 'Capito →'),
...
const MultiplayerEngine = (() => {
  ...
  // ── Stato interno ──
  let _peer        = null;   // istanza PeerJS locale
  let _myId        = null;   // ID peer locale
  let _isHost      = false;
  let _connections = {};     // { peerId: DataConnection }
  let _players     = {};     // { peerId: { name, charId, house, ready, resources, turn, king, isKing, throneTimer, state:'playing'|'war'|'dead' } }
  let _myPeerId    = null;
  let _myCharId    = null;
  let _lobbyReady  = false;
  let _sessionCode = null;   // 6 char code visibile ai giocatori
  let _warSpectators = [];   // peerId spettatori guerra attiva
  let _throneTimer = null;   // intervallo 5 min trono
  let _throneStart = null;   // timestamp quando si è salito sul trono
  let _throneHolderPeerId = null;
  let _throneWarBlocked   = false; // se i 5 min sono scaduti durante una guerra
  const THRONE_WIN_MS = 5 * 60 * 1000; // 5 minuti

  // Case bloccate: man mano che i giocatori scelgono personaggi
  let _takenHouses = {}; // { house: peerId }

  // ── Carica PeerJS da CDN ──
  function _loadPeerJS(cb) {
    if (window.Peer) { cb(); return; }
    const s = document.createElement('script');
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/peerjs/1.5.4/peerjs.min.js';
    s.onload  = cb;
    s.onerror = () => _mpToast('❌ Impossibile caricare PeerJS — verifica la connessione.', 'warn');
    document.head.appendChild(s);
  }

  // ── Helpers UI ──
  function _mpToast(msg, type) {
    if (typeof Game !== 'undefined' && Game._showToast) Game._showToast(msg, type || '');
  ...
  // ═══════════════════════════════════════
  // INIEZIONE PULSANTE NELLA SPLASH SCREEN
  // ═══════════════════════════════════════
  function injectLobbyButton() {
    ...
    btn.style.cssText = 'margin-top:0.5rem;border-color:rgba(248,113,113,0.5);color:#f87171;';
    btn.addEventListener('click', () => showLobbyScreen());
    const content = splash.querySelector('.splash-content');
    if (content) content.appendChild(btn);
  }
  ...
  function showLobbyScreen() {
    _el('mp-lobby-overlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'mp-lobby-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:600;background:#0a0a0f;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;overflow-y:auto;padding:1.5rem 1rem 2rem;animation:fadeIn 0.3s ease;';
    ...
    if (!document.getElementById('mp-style')) {
      ...
      s.textContent = `
        ...
        .mp-card {
          ...
          width:100%; max-width:420px; margin-bottom:0.75rem;
          font-family:'Cinzel',serif;
        }
        .mp-btn-gold {
          width:100%; padding:0.75rem;
          ...
          letter-spacing:0.12em; text-transform:uppercase;
          cursor:pointer; color:#0a0a0f; margin-top:0.5rem;
        }
        .mp-btn-red {
          ...
          letter-spacing:0.1em; text-transform:uppercase;
          cursor:pointer; color:#fff; margin-top:0.4rem;
        }
        .mp-btn-ghost {
          width:100%; padding:0.65rem;
          ...
          letter-spacing:0.1em; text-transform:uppercase;
          cursor:pointer; color:#c9a84c; margin-top:0.4rem;
        }
        .mp-player-slot {
        ...
        .mp-player-slot.ready { border-color:rgba(74,222,128,0.5); }
        .mp-player-slot.me    { border-color:rgba(201,168,76,0.7); }
        .mp-code {
          font-family:'Cinzel Decorative',serif;
          font-size:2rem; color:#c9a84c; letter-spacing:0.3em;
        ...
        .mp-code:hover { background:rgba(201,168,76,0.12); }
        .mp-war-live {
          ...
          font-family:'Cinzel',serif; font-size:0.68rem; color:#fca5a5;
          z-index:700; letter-spacing:0.06em;
        ...
        .mp-throne-bar {
          ...
          display:flex; align-items:center; justify-content:space-between;
          font-family:'Cinzel',serif; font-size:0.65rem; color:#c084fc;
          z-index:699; letter-spacing:0.04em;
        }
        .mp-hud {
          ...
          z-index:698; gap:0.3rem; flex-wrap:wrap;
        }
        .mp-enemy-card {
          ...
          min-width:50px; font-size:0.55rem;
          cursor:pointer; transition:border-color 0.2s;
        }
        .mp-enemy-card:hover { border-color:rgba(201,168,76,0.5); }
        .mp-enemy-card.is-king { border-color:rgba(192,132,252,0.6); }
        .mp-spectator-bar {
          position:fixed; bottom:0; left:0; right:0;
          ...
          padding:0.6rem 1rem;
          display:flex; align-items:center; gap:0.5rem;
          font-family:'Cinzel',serif; font-size:0.7rem; color:#fca5a5;
          z-index:800; letter-spacing:0.04em;
        }
      `;
      document.head.appendChild(s);
    }
    ...
    overlay.innerHTML = `
      <div style="width:100%;max-width:420px">
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.1rem;color:#c9a84c;text-align:center;margin-bottom:0.3rem;letter-spacing:0.05em">⚔ Multiplayer</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.88rem;color:#6b5e4a;text-align:center;margin-bottom:1.2rem;font-style:italic">Fino a 4 giocatori · Stessa rete o Internet</div>

        <!-- CREA PARTITA -->
        <div class="mp-card">
          <div style="font-size:0.7rem;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.6rem">👑 Crea una Partita</div>
          <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#9a8a6a;margin-bottom:0.75rem">Diventa l'host. Condividi il codice con gli altri giocatori.</div>
          <input id="mp-host-name" placeholder="Il tuo nome (es. Daenerys)" maxlength="20"
            style="width:100%;padding:0.55rem 0.7rem;background:#0a0a0f;border:1px solid rgba(201,168,76,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.78rem;color:#e8dcc8;box-sizing:border-box;margin-bottom:0.3rem">
          <button class="mp-btn-gold" onclick="MultiplayerEngine.createSession()">Crea Partita</button>
        </div>

        <!-- UNISCITI -->
        <div class="mp-card">
          <div style="font-size:0.7rem;color:#60a5fa;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.6rem">🗡 Unisciti a una Partita</div>
          <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#9a8a6a;margin-bottom:0.75rem">Inserisci il codice a 6 lettere dell'host.</div>
          <input id="mp-guest-name" placeholder="Il tuo nome" maxlength="20"
            style="width:100%;padding:0.55rem 0.7rem;background:#0a0a0f;border:1px solid rgba(60,130,246,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.78rem;color:#e8dcc8;box-sizing:border-box;margin-bottom:0.4rem">
          <input id="mp-join-code" placeholder="Codice (es. STARK7)" maxlength="8"
            style="width:100%;padding:0.55rem 0.7rem;background:#0a0a0f;border:1px solid rgba(60,130,246,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.78rem;color:#e8dcc8;box-sizing:border-box;text-transform:uppercase;letter-spacing:0.2em"
            oninput="this.value=this.value.toUpperCase()">
          <button class="mp-btn-gold" style="background:linear-gradient(135deg,#1e3a5f,#3b82f6,#1e3a5f);color:#fff"
            onclick="MultiplayerEngine.joinSession()">Unisciti</button>
        </div>

        <!-- TORNA -->
        <div style="width:100%;max-width:420px">
          <button class="mp-btn-ghost" onclick="MultiplayerEngine.closeLobby()">← Torna al Menu</button>
        </div>
      </div>`;
  ...
  function createSession() {
    const name = (_el('mp-host-name')?.value || '').trim() || 'Anonimo';
    _isHost = true;
    ...
    _loadPeerJS(() => {
      ...
      _peer.on('open', (id) => {
        _myPeerId  = id;
        _myId      = id;
        _sessionCode = _generateCode(id);
        _players[id] = { name, charId: null, house: null, ready: false, peerId: id, isMe: true,
                         resources: null, turn: 0, king: false, state: 'lobby' };
        _showRoomScreen();
      });
      _peer.on('connection', (conn) => _onGuestConnect(conn));
      _peer.on('error', (e) => _mpToast('❌ Errore PeerJS: ' + e.type, 'warn'));
    });
  }
  ...
  function joinSession() {
    const name = (_el('mp-guest-name')?.value || '').trim() || 'Anonimo';
    const code = (_el('mp-join-code')?.value || '').trim().toUpperCase();
    if (code.length < 4) { _mpToast('Inserisci un codice valido', 'warn'); return; }
    ...
    _loadPeerJS(() => {
      ...
      _peer.on('open', (id) => {
        ...
        // Converti codice → peerId host
        const hostId = _codeToHostId(code);
        const conn = _peer.connect(hostId, { reliable: true });
        conn.on('open', () => {
          _connections[hostId] = conn;
          _setupConnListeners(conn);
          _send(conn, { type: 'JOIN_REQUEST', name, peerId: id });
          _mpToast('🔗 Connesso all\'host!', 'good');
          _showRoomScreen();
        });
        conn.on('error', (e) => _mpToast('❌ Connessione fallita: ' + e, 'warn'));
      });
      _peer.on('error', (e) => _mpToast('❌ Errore peer: ' + e.type, 'warn'));
    });
  }

  // ═══════════════════════════════════════
  // GESTIONE CONNESSIONE NUOVI GUEST (HOST)
  // ═══════════════════════════════════════
  function _onGuestConnect(conn) {
    const numPlayers = Object.keys(_players).length;
    if (numPlayers >= 4) {
      conn.on('open', () => _send(conn, { type: 'ROOM_FULL' }));
      return;
    }
    conn.on('open', () => {
  ...
  // ═══════════════════════════════════════
  // SETUP LISTENER MESSAGGI
  // ═══════════════════════════════════════
  function _setupConnListeners(conn) {
    conn.on('data', (msg) => _onMessage(msg, conn.peer));
    conn.on('close', () => _onPeerDisconnect(conn.peer));
  }

  // ═══════════════════════════════════════
  // ROUTER MESSAGGI IN ENTRATA
  // ═══════════════════════════════════════
  function _onMessage(msg, fromPeerId) {
    switch (msg.type) {
      case 'JOIN_REQUEST':       _handleJoinRequest(msg, fromPeerId); break;
      case 'PLAYER_LIST':        _handlePlayerList(msg); break;
      case 'CHAR_SELECTED':      _handleCharSelected(msg, fromPeerId); break;
      case 'PLAYER_READY':       _handlePlayerReady(msg, fromPeerId); break;
      case 'GAME_START':         _handleGameStart(msg); break;
      case 'STATE_UPDATE':       _handleStateUpdate(msg, fromPeerId); break;
      case 'DIPLO_REQUEST':      _handleDiploRequest(msg, fromPeerId); break;
      case 'DIPLO_RESPONSE':     _handleDiploResponse(msg, fromPeerId); break;
      case 'WAR_DECLARATION':    _handleWarDeclaration(msg, fromPeerId); break;
      case 'WAR_UPDATE':         _handleWarUpdate(msg, fromPeerId); break;
      case 'WAR_RESULT':         _handleWarResult(msg, fromPeerId); break;
      case 'SPECTATE_WAR':       _handleSpectateWar(msg, fromPeerId); break;
      case 'SPECTATE_FRAME':     _handleSpectateFrame(msg, fromPeerId); break;
      case 'THRONE_CLAIMED':     _handleThroneClaimed(msg, fromPeerId); break;
      case 'THRONE_LOST':        _handleThroneLost(msg, fromPeerId); break;
      case 'PLAYER_WIN':         _handlePlayerWin(msg, fromPeerId); break;
      case 'PLAYER_DEAD':        _handlePlayerDead(msg, fromPeerId); break;
      case 'CHAT':               _handleChat(msg, fromPeerId); break;
      case 'RELAY':              _handleRelay(msg, fromPeerId); break;
      case 'ROOM_FULL':          _mpToast('❌ Stanza piena (max 4 giocatori)', 'warn'); break;
    }
  }

  // ═══════════════════════════════════════
  // HANDLER: JOIN REQUEST (solo host)
  // ═══════════════════════════════════════
  function _handleJoinRequest(msg, fromPeerId) {
    if (!_isHost) return;
    _players[fromPeerId] = { name: msg.name, charId: null, house: null, ready: false,
                              peerId: fromPeerId, isMe: false, resources: null, turn: 0, king: false, state: 'lobby' };
    // Invia lista aggiornata a tutti
  ...
  function _broadcastPlayerList() {
    const list = Object.values(_players).map(p => ({
      ...
      king:    p.king,
      state:   p.state,
      takenHouses: _takenHouses,
    }));
    _broadcast({ type: 'PLAYER_LIST', players: list, sessionCode: _sessionCode });
  }

  function _handlePlayerList(msg) {
    // Guest riceve lista dal host
    _sessionCode = msg.sessionCode;
    msg.players.forEach(p => {
      if (p.peerId === _myPeerId) return; // me stesso, non sovrascrivo
      _players[p.peerId] = { ...(_players[p.peerId] || {}), ...p };
    });
    _takenHouses = msg.players[0]?.takenHouses || _takenHouses;
    _refreshRoomUI();
  }
  ...
  function _showRoomScreen() {
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:610;background:#0a0a0f;display:flex;flex-direction:column;align-items:center;overflow-y:auto;padding:1.5rem 1rem 2rem;animation:fadeIn 0.3s ease;';

    const hostLabel = _isHost ? '<span style="color:#c9a84c;font-size:0.65rem">(tu sei l\'host)</span>' : '<span style="color:#60a5fa;font-size:0.65rem">(ospite)</span>';
    const codeHtml  = _isHost && _sessionCode
      ? `<div class="mp-code" title="Clicca per copiare" onclick="MultiplayerEngine._copyCode()">${_sessionCode}</div>
         <div style="font-family:'EB Garamond',serif;font-size:0.78rem;color:#6b5e4a;text-align:center;margin-bottom:0.75rem">
           📋 Clicca il codice per copiarlo — condividilo con gli altri giocatori
         </div>`
      : `<div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#4ade80;text-align:center;margin:0.5rem 0 0.75rem">
           ✅ Connesso alla stanza ${_sessionCode || '...'}
         </div>`;

    overlay.innerHTML = `
      <div style="width:100%;max-width:420px">
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.05rem;color:#c9a84c;text-align:center;margin-bottom:0.2rem">
          ⚔ Sala del Trono ${hostLabel}
        </div>
        ${codeHtml}

        <!-- Lista giocatori -->
        <div class="mp-card">
          <div style="font-size:0.68rem;color:#9a8a6a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.6rem">Giocatori connessi</div>
          <div id="mp-player-list"></div>
        </div>

        <!-- Selezione personaggio -->
        <div class="mp-card">
          <div style="font-size:0.68rem;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.5rem">Il tuo personaggio</div>
          <div id="mp-char-section">
            <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#6b5e4a">Scegli un personaggio da una casata non ancora presa.</div>
        ...
        <!-- Chat veloce -->
        <div class="mp-card">
          <div style="font-size:0.68rem;color:#9a8a6a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.5rem">📢 Chat</div>
          <div id="mp-chat-log" style="max-height:100px;overflow-y:auto;margin-bottom:0.5rem;font-family:'EB Garamond',serif;font-size:0.82rem;color:#9a8a6a;line-height:1.6"></div>
          <div style="display:flex;gap:0.4rem">
            <input id="mp-chat-input" placeholder="Messaggio..." maxlength="80"
              style="flex:1;padding:0.45rem 0.6rem;background:#0a0a0f;border:1px solid rgba(201,168,76,0.25);border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;color:#e8dcc8"
              onkeydown="if(event.key==='Enter')MultiplayerEngine.sendChat()">
            <button onclick="MultiplayerEngine.sendChat()" style="padding:0.45rem 0.75rem;background:rgba(201,168,76,0.15);border:1px solid rgba(201,168,76,0.4);border-radius:3px;font-family:'Cinzel',serif;font-size:0.7rem;cursor:pointer;color:#c9a84c">Invia</button>
          </div>
        </div>

        <!-- Azioni -->
        <div id="mp-room-actions" style="width:100%;max-width:420px"></div>
        <button class="mp-btn-ghost" onclick="MultiplayerEngine._leaveRoom()" style="max-width:420px;width:100%;margin-top:0.25rem">← Lascia la stanza</button>
      </div>`;
  ...
  function _refreshRoomUI() {
    // Player list
    const listEl = _el('mp-player-list');
    if (!listEl) return;
    const me = _players[_myPeerId];
    listEl.innerHTML = Object.values(_players).map(p => {
      const isMe    = p.peerId === _myPeerId;
      const charDef = p.charId ? (typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === p.charId) : null) : null;
      const icon    = charDef ? charDef.icon : '❓';
      const charName= charDef ? charDef.name : 'Non scelto';
      const readyMark = p.ready ? '<span style="color:#4ade80">✓ Pronto</span>' : '<span style="color:#6b5e4a">In attesa…</span>';
      return `<div class="mp-player-slot ${p.ready ? 'ready' : ''} ${isMe ? 'me' : ''}">
        <span style="font-size:1.4rem">${icon}</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:0.75rem;color:#e8dcc8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.name}${isMe ? ' <span style="color:#c9a84c;font-size:0.6rem">(tu)</span>' : ''}</div>
          <div style="font-size:0.62rem;color:#9a8a6a">${charName}</div>
        </div>
        <div style="font-size:0.62rem">${readyMark}</div>
      </div>`;
    }).join('');

    // Char section
    const charEl = _el('mp-char-section');
    if (charEl && me) {
      if (me.charId) {
        const charDef = typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === me.charId) : null;
        charEl.innerHTML = `
          <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem">
            <span style="font-size:1.8rem">${charDef?.icon || '❓'}</span>
            <div>
              <div style="font-size:0.8rem;color:#e8dcc8">${charDef?.name || me.charId}</div>
              <div style="font-size:0.62rem;color:#9a8a6a">${charDef?.house || ''}</div>
            </div>
          </div>
          <button class="mp-btn-ghost" onclick="MultiplayerEngine.openCharPicker()" style="margin-top:0">Cambia</button>
          ${!me.ready
            ? `<button class="mp-btn-gold" onclick="MultiplayerEngine.setReady()">✓ Sono Pronto</button>`
            : `<div style="text-align:center;color:#4ade80;font-size:0.8rem;margin-top:0.5rem">✓ Pronto!</div>`}`;
      }
    }

    // Actions (solo host: avvia quando tutti pronti)
    const actEl = _el('mp-room-actions');
    if (actEl && _isHost) {
      const allReady = Object.values(_players).length >= 2
        && Object.values(_players).every(p => p.ready && p.charId);
      actEl.innerHTML = allReady
        ? `<button class="mp-btn-gold" onclick="MultiplayerEngine.hostStartGame()" style="background:linear-gradient(135deg,#14532d,#16a34a,#14532d)">⚔ Avvia la Partita!</button>`
        : `<div style="font-family:'EB Garamond',serif;font-size:0.82rem;color:#6b5e4a;text-align:center;padding:0.5rem">
             In attesa che tutti scelgano un personaggio e premano "Pronto"…
             <br><span style="font-size:0.72rem">${Object.values(_players).filter(p=>p.ready).length}/${Object.values(_players).length} pronti</span>
           </div>`;
    }
  }

  // ═══════════════════════════════════════
  // PICKER PERSONAGGI (escludi case già prese)
  // ═══════════════════════════════════════
  function openCharPicker() {
    _el('mp-char-picker')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'mp-char-picker';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:700;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;padding:1rem;';

    if (typeof CHARACTERS === 'undefined') {
      overlay.innerHTML = `<div class="mp-card" style="max-width:380px"><div style="color:#f87171;text-align:center">Personaggi non disponibili.<br>Assicurati di caricare game.js prima.</div><button class="mp-btn-ghost" onclick="document.getElementById('mp-char-picker').remove()">Chiudi</button></div>`;
      document.body.appendChild(overlay);
      return;
    }

    // Raggruppa per casata
    const houses = {};
    CHARACTERS.forEach(c => {
      if (!houses[c.house]) houses[c.house] = [];
      houses[c.house].push(c);
    });

    // Casate prese da altri giocatori
    const myHouse = _players[_myPeerId]?.house;
    const takenByOthers = Object.entries(_takenHouses)
      .filter(([, pid]) => pid !== _myPeerId)
      .map(([h]) => h);

    let html = `
      <div style="background:#0e0e16;border:1px solid rgba(201,168,76,0.4);border-radius:8px;width:100%;max-width:420px;max-height:88vh;overflow-y:auto;padding:1.2rem;font-family:'Cinzel',serif;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem">
          <div style="font-family:'Cinzel Decorative',serif;font-size:0.9rem;color:#c9a84c">Scegli Personaggio</div>
          <button onclick="document.getElementById('mp-char-picker').remove()" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:50%;width:26px;height:26px;font-size:0.85rem;cursor:pointer;color:#9a8a6a">✕</button>
        </div>
        <div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#6b5e4a;margin-bottom:0.75rem">
          🚫 Le casate in rosso sono già prese da altri giocatori.<br>Puoi scegliere qualsiasi personaggio di una casata libera.
        </div>`;

    Object.entries(houses).forEach(([houseName, chars]) => {
      const isTaken = takenByOthers.includes(houseName) && houseName !== myHouse;
      const takerName = isTaken
        ? Object.values(_players).find(p => p.house === houseName)?.name || '?'
        : null;

      html += `<div style="margin-bottom:0.75rem">
        <div style="font-size:0.65rem;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.35rem;padding:0.2rem 0.4rem;border-radius:3px;
          ${isTaken ? 'color:#f87171;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2)' : 'color:#c9a84c;background:rgba(201,168,76,0.06);border:1px solid rgba(201,168,76,0.15)'}">
          ${houseName}${isTaken ? ` — 🔒 Presa da ${takerName}` : ''}
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:0.35rem">`;

      chars.forEach(c => {
        const disabled = isTaken;
        html += `<button onclick="${disabled ? '' : `MultiplayerEngine.selectChar('${c.id}','${houseName}')`}"
          style="display:flex;flex-direction:column;align-items:center;padding:0.4rem 0.55rem;
            background:${disabled ? 'rgba(30,30,30,0.4)' : '#12121a'};
            border:1px solid ${disabled ? 'rgba(80,80,80,0.2)' : 'rgba(201,168,76,0.2)'};
            border-radius:5px;cursor:${disabled ? 'not-allowed' : 'pointer'};
            opacity:${disabled ? '0.35' : '1'};min-width:64px;transition:border-color 0.15s"
          ${disabled ? 'disabled' : `onmouseenter="this.style.borderColor='rgba(201,168,76,0.6)'" onmouseleave="this.style.borderColor='rgba(201,168,76,0.2)'"`}>
          <span style="font-size:1.3rem">${c.icon}</span>
          <span style="font-family:'Cinzel',serif;font-size:0.52rem;color:#e8dcc8;margin-top:0.15rem;max-width:64px;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${c.name.split(' ')[0]}</span>
        </button>`;
      });
  ...
  function selectChar(charId, house) {
    ...
    if (prev && prev !== house) {
      // Libera la casa precedente
      delete _takenHouses[prev];
    }
    ...
    const msg = { type: 'CHAR_SELECTED', charId, house, peerId: _myPeerId };
    if (_isHost) {
      _broadcastPlayerList();
    } else {
      _sendToHost(msg);
  ...
  function _handleCharSelected(msg, fromPeerId) {
    if (_players[fromPeerId]) {
      const prev = _players[fromPeerId].house;
      if (prev) delete _takenHouses[prev];
      _players[fromPeerId].charId = msg.charId;
      _players[fromPeerId].house  = msg.house;
      _takenHouses[msg.house] = fromPeerId;
    }
    if (_isHost) _broadcastPlayerList();
    _refreshRoomUI();
  }
  ...
  function setReady() {
    const me = _players[_myPeerId];
    if (!me?.charId) { _mpToast('Scegli prima un personaggio!', 'warn'); return; }
    me.ready = true;
    const msg = { type: 'PLAYER_READY', peerId: _myPeerId };
    if (_isHost) { _broadcastPlayerList(); }
    else         { _sendToHost(msg); }
    _refreshRoomUI();
  }

  function _handlePlayerReady(msg, fromPeerId) {
    if (_players[fromPeerId]) _players[fromPeerId].ready = true;
    if (_isHost) _broadcastPlayerList();
    _refreshRoomUI();
  }
  ...
  function hostStartGame() {
    const allReady = Object.values(_players).every(p => p.ready && p.charId);
    if (!allReady) { _mpToast('Non tutti sono pronti!', 'warn'); return; }

    const playerOrder = Object.keys(_players);
    _broadcast({ type: 'GAME_START', playerOrder });
    _startLocalGame();
  }

  function _handleGameStart(msg) {
    _startLocalGame();
  }
  ...
  function _startLocalGame() {
    _el('mp-room-overlay')?.remove();
    _el('mp-lobby-overlay')?.remove();
    _myCharId = _players[_myPeerId].charId;

    // Avvia il gioco single-player normale con il personaggio scelto
    if (typeof Game !== 'undefined' && Game._getState) {
      // Selezioniamo il personaggio e avviamo direttamente
      const char = CHARACTERS.find(c => c.id === _myCharId);
      if (!char) { _mpToast('Personaggio non trovato!', 'warn'); return; }

      // Simula il percorso normale: seleziona personaggio → prologo → gioco
      Game._mpMode = true; // flag per sapere che siamo in multiplayer
      Game._mpEngine = MultiplayerEngine;

      // Usa il percorso interno: imposta il personaggio e avvia
      const s = Game._getState();
      // Triggera la selezione automatica
      _autoStartWithChar(char);
    }

    // Mostra HUD multiplayer
    setTimeout(() => {
      _injectMpHud();
      _injectThroneBar();
    }, 1200);
  ...
  function _autoStartWithChar(char) {
    // Naviga alle schermate in sequenza simulando le scelte utente
    if (typeof Game === 'undefined') return;
    // Usa showCharacterDetail internamente → simula click su "Inizia"
    // Invece, usiamo l'API pubblica per farlo in modo pulito
    Game.showCharacterSelect();
    setTimeout(() => {
      // Clicca programmaticamente il personaggio scelto
      const cards = document.querySelectorAll('.char-card');
      cards.forEach(card => {
        if (card.dataset.charId === char.id) {
          card.click();
          setTimeout(() => {
            const startBtn = document.querySelector('#char-detail-overlay button[onclick*="startGame"], #char-detail-overlay .btn-primary');
            if (startBtn) startBtn.click();
            else {
              // fallback: cerca pulsante "Inizia"
              const btns = document.querySelectorAll('#char-detail-overlay button');
              btns.forEach(b => { if (b.textContent.includes('Inizia') || b.textContent.includes('Avvia')) b.click(); });
            }
          }, 400);
        }
      });
  ...
  // ═══════════════════════════════════════
  // SYNC STATO PERIODICO
  // ═══════════════════════════════════════
  function _startStateSync() {
    // Ogni 3 secondi invia il proprio stato agli altri giocatori
    if (window._mpSyncInterval) clearInterval(window._mpSyncInterval);
    window._mpSyncInterval = setInterval(() => {
      if (typeof Game === 'undefined') return;
      const s = Game._getState();
      ...
      if (_players[_myPeerId]) {
        _players[_myPeerId].resources = snapshot.resources;
        _players[_myPeerId].turn      = snapshot.turn;
        _players[_myPeerId].king      = snapshot.isKing;
        _players[_myPeerId].isKing    = snapshot.isKing;
        _players[_myPeerId].state     = snapshot.gameOver ? 'dead' : 'playing';
      }

      const broadcastMsg = { type: 'STATE_UPDATE', snapshot };
      if (_isHost) _broadcast(broadcastMsg);
      else         _sendToHost({ ...broadcastMsg, type: 'RELAY', payload: broadcastMsg });

      // Aggiorna HUD multiplayer
      _updateMpHud();

      // Controlla vittoria trono
      _checkThroneTimer(snapshot);

    }, 3000);
  }

  function _handleStateUpdate(msg, fromPeerId) {
    const snap = msg.snapshot;
    if (_players[snap.peerId]) {
      _players[snap.peerId].resources = snap.resources;
      _players[snap.peerId].turn      = snap.turn;
      _players[snap.peerId].king      = snap.isKing;
      _players[snap.peerId].isKing    = snap.isKing;
      _players[snap.peerId].state     = snap.gameOver ? 'dead' : 'playing';
    }

    // Se ha il trono notifica
    if (snap.isKing && snap.peerId !== _myPeerId) {
      _handleThroneClaimed({ peerId: snap.peerId }, snap.peerId);
    }
    if (!snap.isKing && _throneHolderPeerId === snap.peerId) {
      _handleThroneLost({ peerId: snap.peerId }, snap.peerId);
    }
  ...
  // ═══════════════════════════════════════
  // RELAY (guest → host → tutti)
  // ═══════════════════════════════════════
  function _handleRelay(msg, fromPeerId) {
    if (!_isHost) return;
    // Ritrasmetti a tutti gli altri
    Object.entries(_connections).forEach(([pid, conn]) => {
      if (pid !== fromPeerId) _send(conn, msg.payload);
    });
    // Processa anche localmente
    _onMessage(msg.payload, fromPeerId);
  }

  // ═══════════════════════════════════════
  // TRONO — TIMER 5 MINUTI
  // ═══════════════════════════════════════
  function _checkThroneTimer(snapshot) {
    if (!snapshot.isKing) {
      if (_throneHolderPeerId === _myPeerId) {
        // Ho perso il trono
        _throneHolderPeerId = null;
        _throneStart        = null;
        clearInterval(_throneTimer);
        _throneTimer = null;
        _mpToast('👑 Hai perso il Trono!', 'warn');
        _broadcast({ type: 'THRONE_LOST', peerId: _myPeerId });
        _updateThroneBar();
      }
      return;
    }
    // Sono io sul trono
    if (_throneHolderPeerId !== _myPeerId) {
      _throneHolderPeerId = _myPeerId;
      _throneStart = Date.now();
      _mpToast('👑 Hai conquistato il Trono! Mantienilo per 5 minuti per vincere!', 'good');
      _broadcast({ type: 'THRONE_CLAIMED', peerId: _myPeerId, name: _players[_myPeerId]?.name });
      _startThroneCountdown();
    }
  }

  function _startThroneCountdown() {
    clearInterval(_throneTimer);
    _throneTimer = setInterval(() => {
      const elapsed = Date.now() - _throneStart;
      _updateThroneBar(elapsed);
      if (elapsed >= THRONE_WIN_MS) {
        clearInterval(_throneTimer);
        _throneTimer = null;
        // Controlla se c'è una guerra in corso
        const s = typeof Game !== 'undefined' ? Game._getState() : null;
        const inWar = s && (s.inActiveBattle || s.pendingWarTarget);
        if (inWar) {
          _throneWarBlocked = true;
          _mpToast('👑 5 minuti completati! Esito della guerra in corso determinerà il vincitore!', 'good');
          _updateThroneBar(elapsed, true);
        } else {
          _declareVictory();
        }
      }
  ...
  function _declareVictory() {
    _mpToast('🏆 HAI VINTO! 5 minuti sul trono completati!', 'good');
    _broadcast({ type: 'PLAYER_WIN', peerId: _myPeerId, name: _players[_myPeerId]?.name });
    _showVictoryOverlay(_players[_myPeerId]?.name || 'Tu');
  }

  function _handleThroneClaimed(msg, fromPeerId) {
    if (fromPeerId === _myPeerId) return;
    _throneHolderPeerId = fromPeerId;
    const pName = _players[fromPeerId]?.name || '?';
    _mpToast(`👑 ${pName} ha conquistato il Trono! Hai 5 minuti per scalzarlo.`, 'warn');
    _updateThroneBar();
  }

  function _handleThroneLost(msg, fromPeerId) {
    if (_throneHolderPeerId === fromPeerId) {
      _throneHolderPeerId = null;
      _updateThroneBar();
    }
  }

  function _handlePlayerWin(msg, fromPeerId) {
    const pName = _players[fromPeerId]?.name || '?';
    _showVictoryOverlay(pName, fromPeerId === _myPeerId);
  }

  function _handlePlayerDead(msg, fromPeerId) {
    if (_players[fromPeerId]) _players[fromPeerId].state = 'dead';
    const pName = _players[fromPeerId]?.name || '?';
    _mpToast(`💀 ${pName} è stato eliminato.`, 'warn');
    _updateMpHud();
  }

  // ═══════════════════════════════════════
  // DIPLOMAZIA TRA GIOCATORI
  // ═══════════════════════════════════════
  function sendDiploRequest(targetPeerId, action) {
    // action: 'alliance' | 'war'
    const me = _players[_myPeerId];
    const msg = { type: 'DIPLO_REQUEST', from: _myPeerId, fromName: me?.name, action, charId: me?.charId };
    _sendToPlayer(targetPeerId, msg);
  }

  function _handleDiploRequest(msg, fromPeerId) {
    const fromName = msg.fromName || _players[fromPeerId]?.name || '?';
    const fromChar = msg.charId ? (typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === msg.charId) : null) : null;
    const actionLabel = msg.action === 'alliance' ? '🤝 ALLEANZA' : '⚔ GUERRA';
    const actionColor = msg.action === 'alliance' ? '#4ade80' : '#f87171';

    _el('mp-diplo-popup')?.remove();
    const popup = document.createElement('div');
    popup.id = 'mp-diplo-popup';
    popup.style.cssText = 'position:fixed;inset:0;z-index:850;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.8);backdrop-filter:blur(4px);animation:fadeIn 0.25s ease';
    popup.innerHTML = `
      <div style="background:linear-gradient(160deg,#1a1508,#12100a);border:2px solid ${actionColor}44;border-radius:8px;width:88%;max-width:360px;padding:1.5rem;font-family:'Cinzel',serif;text-align:center">
        <div style="font-size:2rem;margin-bottom:0.4rem">${fromChar?.icon || '⚔'}</div>
        <div style="font-family:'Cinzel Decorative',serif;color:${actionColor};font-size:0.9rem;margin-bottom:0.4rem">${actionLabel}</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.9rem;color:#9a8a6a;margin-bottom:1.2rem;line-height:1.6;font-style:italic">
          «<strong style="color:#e8dcc8">${fromName}</strong> ${msg.action === 'alliance' ? 'ti propone un\'alleanza. Accetti?' : 'ti dichiara guerra! Sei pronto a combattere?'}»
        </div>
        <div style="display:flex;gap:0.75rem">
          <button onclick="MultiplayerEngine._respondDiplo('${fromPeerId}','${msg.action}','accept')" style="flex:1;padding:0.7rem;background:linear-gradient(135deg,#14532d,#16a34a);border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;cursor:pointer;color:#fff">
            ${msg.action === 'alliance' ? '✓ Accetta' : '⚔ Combatti'}
          </button>
          <button onclick="MultiplayerEngine._respondDiplo('${fromPeerId}','${msg.action}','refuse')" style="flex:1;padding:0.7rem;background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.4);border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;cursor:pointer;color:#f87171">
            ${msg.action === 'alliance' ? '✗ Rifiuta' : '🏃 Fuggi (−10 ⚔)'}
          </button>
        </div>
      </div>`;
    document.body.appendChild(popup);
    ...
    setTimeout(() => { _el('mp-diplo-popup')?.remove(); }, 30000);
  }

  function _respondDiplo(targetPeerId, action, response) {
    _el('mp-diplo-popup')?.remove();
    const me = _players[_myPeerId];
    _sendToPlayer(targetPeerId, { type: 'DIPLO_RESPONSE', from: _myPeerId, fromName: me?.name, action, response });

    if (action === 'war' && response === 'refuse') {
      // Penalità fuga
      const s = typeof Game !== 'undefined' ? Game._getState() : null;
      if (s) s.resources.army = Math.max(1, (s.resources.army || 0) - 10);
      _mpToast('🏃 Sei fuggito. −10 Esercito', 'warn');
    }
    if (action === 'alliance' && response === 'accept') {
      _mpToast(`🤝 Hai stretto alleanza con ${_players[targetPeerId]?.name || '?'}`, 'good');
    }
    if (action === 'war' && response === 'accept') {
      _mpToast(`⚔ La guerra è dichiarata contro ${_players[targetPeerId]?.name || '?'}!`, 'warn');
      _startPvpWar(targetPeerId);
    }
  }

  function _handleDiploResponse(msg, fromPeerId) {
    _el('mp-diplo-popup')?.remove();
    const fromName = msg.fromName || _players[fromPeerId]?.name || '?';
    if (msg.action === 'alliance') {
      if (msg.response === 'accept') _mpToast(`🤝 ${fromName} ha accettato l'alleanza!`, 'good');
      else                           _mpToast(`✗ ${fromName} ha rifiutato l'alleanza.`, 'warn');
    } else if (msg.action === 'war') {
      if (msg.response === 'accept') {
        _mpToast(`⚔ ${fromName} accetta la guerra!`, 'warn');
        _startPvpWar(fromPeerId);
      } else {
        _mpToast(`🏃 ${fromName} è fuggito! Guadagni +5 Esercito`, 'good');
        const s = typeof Game !== 'undefined' ? Game._getState() : null;
        if (s) s.resources.army = Math.min(100, (s.resources.army || 0) + 5);
      }
    }
  }

  // ═══════════════════════════════════════
  // GUERRA PvP TRA GIOCATORI
  // ═══════════════════════════════════════
  function _startPvpWar(opponentPeerId) {
    const me  = _players[_myPeerId];
    const opp = _players[opponentPeerId];
    if (!me || !opp) return;

    _mpToast(`⚔ Preparati alla battaglia contro ${opp.name}!`, 'warn');

    // Annuncia agli spettatori
    _broadcast({
      type: 'WAR_DECLARATION',
      p1: { peerId: _myPeerId,   name: me.name,  charId: me.charId,  army: me.resources?.army || 50 },
      p2: { peerId: opponentPeerId, name: opp.name, charId: opp.charId, army: opp.resources?.army || 50 },
    });

    // Mostra overlay di battaglia PvP
    _showPvpBattleOverlay(opponentPeerId);
  }

  function _handleWarDeclaration(msg, fromPeerId) {
    const involvedPeerIds = [msg.p1.peerId, msg.p2.peerId];
    if (involvedPeerIds.includes(_myPeerId)) return; // sono io, gestito da _startPvpWar
    // Sono spettatore → mostra avviso con opzione di guardare
    _showWarSpectatorPrompt(msg);
  }

  function _showPvpBattleOverlay(opponentPeerId) {
    _el('mp-pvp-battle')?.remove();
    const opp   = _players[opponentPeerId];
    const me    = _players[_myPeerId];
    const myArmy = me?.resources?.army || 50;
    const oppArmy= opp?.resources?.army || 50;
    const myChar = typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === me?.charId) : null;
    const oppChar= typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === opp?.charId) : null;

    const overlay = document.createElement('div');
    overlay.id = 'mp-pvp-battle';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:800;background:#0a0a0f;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:1.5rem;font-family:"Cinzel",serif;';

    overlay.innerHTML = `
      <div style="font-family:'Cinzel Decorative',serif;font-size:1rem;color:#f87171;margin-bottom:1.2rem;letter-spacing:0.05em;text-shadow:0 0 20px rgba(239,68,68,0.5)">⚔ BATTAGLIA ⚔</div>

      <!-- VS screen -->
      <div style="display:flex;align-items:center;gap:1.5rem;margin-bottom:1.5rem;width:100%;max-width:380px;justify-content:center">
        <div style="text-align:center;flex:1">
          <div style="font-size:2.5rem">${myChar?.icon || '⚔'}</div>
          <div style="font-size:0.75rem;color:#e8dcc8;margin-top:0.3rem">${me?.name}</div>
          <div style="font-size:0.62rem;color:#9a8a6a">${myChar?.house || ''}</div>
          <div style="margin-top:0.4rem;font-size:0.7rem;color:#4ade80">⚔ ${myArmy}</div>
        </div>
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.2rem;color:#f87171">VS</div>
        <div style="text-align:center;flex:1">
          <div style="font-size:2.5rem">${oppChar?.icon || '⚔'}</div>
          <div style="font-size:0.75rem;color:#e8dcc8;margin-top:0.3rem">${opp?.name}</div>
          <div style="font-size:0.62rem;color:#9a8a6a">${oppChar?.house || ''}</div>
          <div style="margin-top:0.4rem;font-size:0.7rem;color:#f87171">⚔ ${oppArmy}</div>
        </div>
      </div>

      <!-- Barra HP animata -->
      <div id="mp-battle-log" style="width:100%;max-width:380px;height:180px;overflow-y:auto;background:#12121a;border:1px solid rgba(239,68,68,0.2);border-radius:6px;padding:0.75rem;font-family:'EB Garamond',serif;font-size:0.85rem;color:#e8dcc8;line-height:1.7;margin-bottom:1rem"></div>

      <!-- Progress bars -->
      <div style="width:100%;max-width:380px;margin-bottom:1rem">
        <div style="display:flex;justify-content:space-between;font-size:0.65rem;color:#9a8a6a;margin-bottom:0.25rem">
          <span>⚔ ${me?.name}</span><span>${opp?.name} ⚔</span>
        </div>
        <div style="display:flex;height:8px;border-radius:4px;overflow:hidden;background:rgba(255,255,255,0.05)">
          <div id="mp-bar-me"  style="background:#4ade80;transition:width 0.5s ease;width:${myArmy}%"></div>
          <div id="mp-bar-opp" style="background:#f87171;transition:width 0.5s ease;width:${oppArmy}%"></div>
        </div>
      </div>

      <div id="mp-battle-result" style="display:none;text-align:center;margin-top:0.5rem"></div>

      <!-- Spettatori -->
      <div id="mp-spectator-count" style="font-size:0.62rem;color:#6b5e4a;margin-top:0.5rem;text-align:center"></div>`;

    document.body.appendChild(overlay);

    // Avvia simulazione battaglia
    _simulatePvpBattle(opponentPeerId, myArmy, oppArmy, overlay);
  }

  function _simulatePvpBattle(opponentPeerId, myArmy, oppArmy, overlay) {
    const log      = _el('mp-battle-log');
    const barMe    = _el('mp-bar-me');
    const barOpp   = _el('mp-bar-opp');
    const resultEl = _el('mp-battle-result');

    let myHp  = myArmy;
    let oppHp = oppArmy;
    const myBonus  = 0.5 + Math.random() * 0.5;
    const oppBonus = 0.5 + Math.random() * 0.5;
    let round = 0;
    ...
    logLine(`⚔ La battaglia ha inizio!`);

    const battleInterval = setInterval(() => {
      round++;
      const myAtk  = Math.round((5 + Math.random() * 10) * myBonus);
      const oppAtk = Math.round((5 + Math.random() * 10) * oppBonus);
      ...
      const totalHp = myArmy + oppArmy;
      if (barMe)  barMe.style.width  = Math.round((myHp  / totalHp) * 100) + '%';
      if (barOpp) barOpp.style.width = Math.round((oppHp / totalHp) * 100) + '%';

      logLine(`Turno ${round}: <span style="color:#4ade80">${me?.name}</span> attacca per <strong>${myAtk}</strong> · <span style="color:#f87171">${opp?.name}</span> risponde con <strong>${oppAtk}</strong>`);

      // Invia aggiornamento agli spettatori
      _broadcast({
        type: 'WAR_UPDATE',
        p1Hp: myHp, p2Hp: oppHp, round,
        p1Name: me?.name, p2Name: opp?.name,
      });

      if (myHp <= 0 || oppHp <= 0 || round >= 12) {
        clearInterval(battleInterval);
        const iWon = myHp > oppHp;
        const s = typeof Game !== 'undefined' ? Game._getState() : null;

        if (iWon) {
          logLine(`<span style="color:#c9a84c">👑 HAI VINTO LA BATTAGLIA!</span>`);
          if (s) {
            s.resources.army = Math.min(100, myHp + 5);
            s.resources.power = Math.min(100, (s.resources.power || 50) + 10);
          }
        } else {
          logLine(`<span style="color:#f87171">💀 HAI PERSO LA BATTAGLIA.</span>`);
          if (s) {
            s.resources.army = Math.max(1, Math.round(myHp * 0.5));
            s.resources.power = Math.max(1, (s.resources.power || 50) - 8);
          }
        }

        const resultMsg = { type: 'WAR_RESULT', winnerId: iWon ? _myPeerId : opponentPeerId };
        _broadcast(resultMsg);

        // Controlla se i 5 min erano scaduti durante questa guerra
        if (_throneWarBlocked && iWon && _players[_myPeerId]?.isKing) {
          _throneWarBlocked = false;
          _declareVictory();
        }
        ...
        if (resultEl) {
          ...
          resultEl.innerHTML = `
            <div style="font-family:'Cinzel Decorative',serif;font-size:1rem;color:${iWon ? '#c9a84c' : '#f87171'};margin-bottom:0.75rem">
              ${iWon ? '🏆 Vittoria!' : '💀 Sconfitta'}
            </div>
            <button onclick="document.getElementById('mp-pvp-battle')?.remove()" style="padding:0.65rem 1.5rem;background:linear-gradient(135deg,#78350f,#c9a84c,#78350f);border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.78rem;font-weight:700;letter-spacing:0.1em;cursor:pointer;color:#0a0a0f">Torna al Gioco</button>`;
        }
      }
    }, 900);
  }

  function _handleWarDeclaration_spectator(msg) { }

  function _handleWarUpdate(msg, fromPeerId) {
    // Aggiorna schermata spettatori se aperta
    const specLog = _el('mp-spec-log');
    if (!specLog) return;
    const line = document.createElement('div');
    line.style.cssText = 'font-family:"EB Garamond",serif;font-size:0.82rem;color:#e8dcc8;line-height:1.6';
    line.innerHTML = `T${msg.round}: <span style="color:#4ade80">${msg.p1Name}</span> ${msg.p1Hp} HP · <span style="color:#f87171">${msg.p2Name}</span> ${msg.p2Hp} HP`;
    specLog.appendChild(line);
    specLog.scrollTop = specLog.scrollHeight;
  }

  function _handleWarResult(msg, fromPeerId) {
    const winner = _players[msg.winnerId];
    if (winner) _mpToast(`🏆 ${winner.name} ha vinto la battaglia!`, 'good');
  }

  // ═══════════════════════════════════════
  // SPETTATORI
  // ═══════════════════════════════════════
  function _showWarSpectatorPrompt(msg) {
    const p1Name = msg.p1?.name || '?';
    const p2Name = msg.p2?.name || '?';
    _el('mp-spec-prompt')?.remove();
    const popup = document.createElement('div');
    popup.id = 'mp-spec-prompt';
    popup.style.cssText = 'position:fixed;bottom:5rem;left:50%;transform:translateX(-50%);z-index:799;background:#12121a;border:1px solid rgba(239,68,68,0.4);border-radius:8px;padding:0.85rem 1rem;font-family:"Cinzel",serif;width:88%;max-width:340px;animation:fadeIn 0.3s ease';
    popup.innerHTML = `
      <div style="font-size:0.68rem;color:#f87171;letter-spacing:0.08em;margin-bottom:0.3rem">⚔ GUERRA IN CORSO</div>
      <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#e8dcc8;margin-bottom:0.6rem">
        <strong>${p1Name}</strong> vs <strong>${p2Name}</strong>
      </div>
      <div style="display:flex;gap:0.5rem">
        <button onclick="MultiplayerEngine._openSpectatorView()" style="flex:1;padding:0.5rem;background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.4);border-radius:3px;font-family:'Cinzel',serif;font-size:0.65rem;cursor:pointer;color:#fca5a5">👁 Guarda</button>
        <button onclick="document.getElementById('mp-spec-prompt')?.remove()" style="padding:0.5rem 0.75rem;background:transparent;border:1px solid rgba(201,168,76,0.25);border-radius:3px;font-family:'Cinzel',serif;font-size:0.65rem;cursor:pointer;color:#9a8a6a">No</button>
      </div>`;
    document.body.appendChild(popup);
    setTimeout(() => popup.remove(), 15000);
  }

  function _openSpectatorView() {
    _el('mp-spec-prompt')?.remove();
    _el('mp-spectator-overlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'mp-spectator-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:790;background:rgba(10,10,15,0.97);display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:1.5rem 1rem;font-family:"Cinzel",serif;';
    overlay.innerHTML = `
      <div style="font-family:'Cinzel Decorative',serif;font-size:0.9rem;color:#f87171;margin-bottom:0.75rem">👁 Modalità Spettatore</div>
      <div id="mp-spec-log" style="width:100%;max-width:380px;flex:1;overflow-y:auto;background:#12121a;border:1px solid rgba(239,68,68,0.2);border-radius:6px;padding:0.75rem;font-family:'EB Garamond',serif;font-size:0.85rem;color:#e8dcc8;line-height:1.7;margin-bottom:1rem"></div>
      <button onclick="document.getElementById('mp-spectator-overlay')?.remove()" style="padding:0.6rem 1.5rem;background:transparent;border:1px solid rgba(201,168,76,0.35);border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;cursor:pointer;color:#c9a84c">Torna al gioco</button>`;
    document.body.appendChild(overlay);
    // Segnala all'host che stiamo guardando
    _sendToHost({ type: 'SPECTATE_WAR', peerId: _myPeerId });
  }

  function _handleSpectateWar(msg, fromPeerId) {
    if (!_warSpectators.includes(fromPeerId)) _warSpectators.push(fromPeerId);
  }

  function _handleSpectateFrame(msg, fromPeerId) {
    // frame aggiornamento battaglia (non usato in questa implementazione base)
  }

  // ═══════════════════════════════════════
  // HUD MULTIPLAYER IN GAME
  // ═══════════════════════════════════════
  function _injectMpHud() {
    _el('mp-hud')?.remove();
    const hud = document.createElement('div');
    hud.id = 'mp-hud';
    hud.className = 'mp-hud';
    hud.innerHTML = `
      <div style="font-size:0.6rem;color:#c9a84c;letter-spacing:0.06em">⚔ AVVERSARI</div>
      <div id="mp-enemy-cards" style="display:flex;gap:0.3rem;flex-wrap:wrap"></div>
      <button onclick="MultiplayerEngine._showMpActions()" style="padding:0.3rem 0.6rem;background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.58rem;cursor:pointer;color:#fca5a5;flex-shrink:0">
        ⚔ Azioni PvP
      </button>`;
  ...
  function _updateMpHud() {
    const cardsEl = _el('mp-enemy-cards');
    if (!cardsEl) return;
    const others = Object.values(_players).filter(p => p.peerId !== _myPeerId);
    cardsEl.innerHTML = others.map(p => {
      const charDef = typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === p.charId) : null;
      const stateColor = p.state === 'dead' ? '#6b5e4a' : p.isKing ? '#c084fc' : '#9a8a6a';
      const stateBadge = p.state === 'dead' ? '💀' : p.isKing ? '👑' : '';
      const army = p.resources?.army ? Math.round(p.resources.army) : '?';
      return `<div class="mp-enemy-card ${p.isKing ? 'is-king' : ''}" onclick="MultiplayerEngine._showPlayerActions('${p.peerId}')" title="${p.name}">
        <span style="font-size:1.1rem">${charDef?.icon || '⚔'}</span>
        <span style="font-size:0.55rem;color:#e8dcc8;max-width:50px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.name}</span>
        <span style="font-size:0.5rem;color:${stateColor}">${stateBadge} ⚔${army}</span>
      </div>`;
    }).join('');
  }
  ...
  function _injectThroneBar() {
    ...
    bar.id  = 'mp-throne-bar';
    bar.className = 'mp-throne-bar';
    bar.innerHTML = `<span id="mp-throne-label" style="flex:1">👑 Nessuno sul Trono</span><span id="mp-throne-timer" style="font-size:0.7rem;color:#c084fc"></span>`;
    document.body.appendChild(bar);
  }

  function _updateThroneBar(elapsed, completed) {
    const label = _el('mp-throne-label');
    const timer = _el('mp-throne-timer');
    if (!label) return;

    if (!_throneHolderPeerId) {
      label.textContent = '👑 Nessuno sul Trono';
    ...
    label.innerHTML = `👑 <strong style="color:${isMine ? '#c9a84c' : '#c084fc'}">${holderName}</strong> tiene il Trono`;

    if (elapsed !== undefined && timer) {
      const remaining = Math.max(0, THRONE_WIN_MS - elapsed);
  ...
  // AZIONI PvP (popup)
  // ═══════════════════════════════════════
  function _showMpActions() {
    _el('mp-actions-popup')?.remove();
    const others = Object.values(_players).filter(p => p.peerId !== _myPeerId && p.state !== 'dead');
    if (others.length === 0) { _mpToast('Nessun avversario disponibile', 'warn'); return; }

    const popup = document.createElement('div');
    popup.id = 'mp-actions-popup';
    popup.style.cssText = 'position:fixed;inset:0;z-index:820;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,0.7);animation:fadeIn 0.2s ease';
    popup.innerHTML = `
      <div style="background:#0e0e16;border:1px solid rgba(201,168,76,0.35);border-radius:10px 10px 0 0;width:100%;max-width:480px;padding:1rem;font-family:'Cinzel',serif;max-height:60vh;overflow-y:auto">
        <div style="font-family:'Cinzel Decorative',serif;font-size:0.9rem;color:#c9a84c;margin-bottom:0.75rem;text-align:center">⚔ Azioni verso Giocatori</div>
        ${others.map(p => {
          const charDef = typeof CHARACTERS !== 'undefined' ? CHARACTERS.find(c => c.id === p.charId) : null;
          return `
          <div style="background:#12121a;border:1px solid rgba(201,168,76,0.15);border-radius:5px;padding:0.6rem 0.75rem;margin-bottom:0.4rem">
            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.4rem">
              <span style="font-size:1.2rem">${charDef?.icon || '⚔'}</span>
              <div>
                <div style="font-size:0.75rem;color:#e8dcc8">${p.name}</div>
                <div style="font-size:0.58rem;color:#9a8a6a">⚔ ${p.resources?.army ? Math.round(p.resources.army) : '?'} · ${charDef?.house || ''}</div>
              </div>
              ${p.isKing ? '<span style="margin-left:auto;font-size:0.7rem;color:#c084fc">👑 Re</span>' : ''}
            </div>
            <div style="display:flex;gap:0.35rem">
              <button onclick="MultiplayerEngine.sendDiploRequest('${p.peerId}','alliance');document.getElementById('mp-actions-popup')?.remove()"
                style="flex:1;padding:0.42rem;background:rgba(74,222,128,0.1);border:1px solid rgba(74,222,128,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.6rem;cursor:pointer;color:#4ade80">🤝 Alleanza</button>
              <button onclick="MultiplayerEngine.sendDiploRequest('${p.peerId}','war');document.getElementById('mp-actions-popup')?.remove()"
                style="flex:1;padding:0.42rem;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.6rem;cursor:pointer;color:#f87171">⚔ Dichiara Guerra</button>
            </div>
          </div>`;
        }).join('')}
        <button onclick="document.getElementById('mp-actions-popup')?.remove()" style="width:100%;padding:0.55rem;background:transparent;border:1px solid rgba(201,168,76,0.25);border-radius:3px;font-family:'Cinzel',serif;font-size:0.68rem;cursor:pointer;color:#9a8a6a;margin-top:0.25rem">← Chiudi</button>
      </div>`;
    popup.addEventListener('click', e => { if (e.target === popup) popup.remove(); });
    document.body.appendChild(popup);
  }

  function _showPlayerActions(peerId) { _showMpActions(); }

  // ═══════════════════════════════════════
  // VITTORIA OVERLAY
  // ═══════════════════════════════════════
  function _showVictoryOverlay(winnerName, isMe) {
    _el('mp-victory-overlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'mp-victory-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:900;background:rgba(10,10,15,0.96);display:flex;flex-direction:column;align-items:center;justify-content:center;animation:fadeIn 0.5s ease';
    overlay.innerHTML = `
      <div style="text-align:center;padding:2rem;font-family:'Cinzel',serif">
        <div style="font-size:4rem;margin-bottom:0.5rem">${isMe ? '🏆' : '👑'}</div>
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.3rem;color:#c9a84c;margin-bottom:0.5rem;letter-spacing:0.05em">
          ${isMe ? 'HAI VINTO!' : `${winnerName} ha vinto`}
        </div>
        <div style="font-family:'EB Garamond',serif;font-size:1rem;color:#9a8a6a;margin-bottom:1.5rem;font-style:italic;max-width:320px;line-height:1.6">
          ${isMe
            ? '«Avete mantenuto il Trono di Spade per 5 minuti. I Sette Regni vi appartengono.»'
            : `«${winnerName} ha tenuto il Trono per 5 minuti senza essere scalzato. Il regno è suo.»`}
        </div>
        <button onclick="MultiplayerEngine._endSession()" style="padding:0.75rem 2rem;background:linear-gradient(135deg,#78350f,#c9a84c,#78350f);border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.8rem;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;cursor:pointer;color:#0a0a0f">Torna al Menu</button>
      </div>`;
    document.body.appendChild(overlay);
  }
  ...
  // ═══════════════════════════════════════
  function sendChat() {
    const input = _el('mp-chat-input');
    const text = input?.value?.trim();
    if (!text) return;
    input.value = '';
    const me = _players[_myPeerId];
    const msg = { type: 'CHAT', from: _myPeerId, name: me?.name || '?', text };
    _appendChat(msg);
    if (_isHost) _broadcast(msg);
    else         _sendToHost({ type: 'RELAY', payload: msg });
  }

  function _handleChat(msg, fromPeerId) {
    _appendChat(msg);
    // Se non siamo nella room, mostra toast
    if (!_el('mp-room-overlay')) _mpToast(`💬 ${msg.name}: ${msg.text}`);
  }

  function _appendChat(msg) {
    const log = _el('mp-chat-log');
    if (!log) return;
    const line = document.createElement('div');
    line.innerHTML = `<span style="color:#c9a84c">${msg.name}:</span> <span style="color:#e8dcc8">${msg.text}</span>`;
    log.appendChild(line);
    log.scrollTop = log.scrollHeight;
  }

  // ═══════════════════════════════════════
  // UTILITIES INVIO MESSAGGI
  // ═══════════════════════════════════════
  ...
  function _broadcast(msg) {
    Object.values(_connections).forEach(conn => _send(conn, msg));
  }

  function _sendToHost(msg) {
    const hostConn = Object.values(_connections)[0];
    if (hostConn) _send(hostConn, msg);
  }

  function _sendToPlayer(peerId, msg) {
    if (_isHost) {
      const conn = _connections[peerId];
      if (conn) _send(conn, msg);
    } else {
      // Guest manda all'host con destinazione
      _sendToHost({ type: 'RELAY', payload: { ...msg, _to: peerId } });
    }
  }

  // ═══════════════════════════════════════
  // GENERAZIONE CODICE SESSIONE
  // ═══════════════════════════════════════
  function _generateCode(peerId) {
    const alpha = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    let code = '';
    for (let i = 0; i < 6; i++) {
      const idx = (peerId.charCodeAt(i % peerId.length) + i * 7) % alpha.length;
      code += alpha[idx];
    }
    return code;
  }

  function _codeToHostId(code) {
    // PeerJS genera ID casuali — il guest deve conoscere l'ID host completo
    // Il codice viene trasmesso → In fase join, chiediamo l'ID peer completo
    // Per semplicità UX, l'host condivide il suo Peer ID direttamente come codice
    return code; // il codice IS il peer ID (abbreviato nella UI ma completo internamente)
  }

  function _copyCode() {
    if (!_sessionCode) return;
    // Il codice vero da condividere è il Peer ID completo dell'host
    const shareText = `IA-MP:${_myPeerId}`;
    navigator.clipboard?.writeText(shareText).then(() => _mpToast('📋 Codice copiato!', 'good')).catch(() => {
      // fallback
      prompt('Copia questo codice e condividilo:', shareText);
    });
  }
  ...
  // DISCONNECT & CLEANUP
  // ═══════════════════════════════════════
  function _onPeerDisconnect(peerId) {
    const p = _players[peerId];
    if (p) {
      _mpToast(`⚠ ${p.name} si è disconnesso.`, 'warn');
      p.state = 'dead';
    }
  ...
  function _endSession() {
    ...
    clearInterval(_throneTimer);
    _throneTimer = null;
    clearInterval(window._mpSyncInterval);
    window._mpSyncInterval = null;
    _el('mp-lobby-overlay')?.remove();
    _el('mp-room-overlay')?.remove();
    _el('mp-hud')?.remove();
    _el('mp-throne-bar')?.remove();
  ...
  // ═══════════════════════════════════════
  // NOTA: Join by code
  // Il sistema usa il Peer ID dell'host come codice di sessione.
  // L'host condivide il suo Peer ID tramite il pulsante "Copia Codice".
  // Il guest incolla il formato "IA-MP:<peerid>" nel campo codice.
  // ═══════════════════════════════════════

  // Override joinSession per gestire formato IA-MP:
  const _origJoinSession = joinSession;
  function joinSessionWithCode() {
    let code = (_el('mp-join-code')?.value || '').trim();
    // Supporta sia il codice grezzo che il formato "IA-MP:xxx"
    if (code.startsWith('IA-MP:')) code = code.slice(6);
    if (_el('mp-join-code')) _el('mp-join-code').value = code;
    joinSession();
  }

  // Patch il pulsante join dopo l'init
  setTimeout(() => {
    const joinBtn = document.querySelector('button[onclick="MultiplayerEngine.joinSession()"]');
    if (joinBtn) joinBtn.setAttribute('onclick', 'MultiplayerEngine.joinSessionWithCode()');
  }, 100);

  // ═══════════════════════════════════════
  // INIT — inietta pulsante in splash
  // ═══════════════════════════════════════
  window.addEventListener('load', () => {
    setTimeout(injectLobbyButton, 300);
  });

  // Se la splash è già caricata
  if (document.readyState === 'complete') setTimeout(injectLobbyButton, 300);

  return {
    // UI
    createSession,
    joinSession: joinSessionWithCode,
    closeLobby,
    openCharPicker,
    selectChar,
    setReady,
    ...
    sendChat,
    sendDiploRequest,
    // Internals esposti per handler HTML
    _respondDiplo,
    _showPlayerActions,
    _showMpActions,
    _openSpectatorView,
