...
const Game = (() => {
  ...
  const TROOP_TYPES = [
    {
      ...
      cost: { people: 6 },
      upkeep: {},                    // nessun mantenimento
      force: 4,                      // forza per unità reclutata
      maxUnits: 8,
      battleColor: '#a3a3a3',
      unitIcon: '🪓',
    },
    ...
    },
    {
      ...
      cost: {},                       // solo da carta evento
      upkeep: { gold: 4, faith: 2 },
      force: 40,                      // forza enorme ma unico
      maxUnits: 1,
      battleColor: '#f87171',
      unitIcon: '🐉',
      onlyFromCard: true,             // non reclutabile normalmente
    },
  ];
  ...
  // ══════════════════════════════════════════════
  // PANNELLO ESERCITO — UI
  // ══════════════════════════════════════════════
  function showArmyPanel() {
    document.getElementById('army-panel-overlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'army-panel-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:500;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);';

    const available = getAvailableUnits();
    const totalForce = getArmyForce();
    const upkeep = getArmyUpkeep();
    const upkeepStr = Object.entries(upkeep).map(([r,v]) => {
      const ic = { gold:'💰', people:'👥', faith:'✝', power:'👑', army:'⚔' }[r] || r;
      return `−${v}${ic}`;
    }).join(' ') || 'nessuno';

    // Sezione unità attive
    const activeUnits = state.armyUnits || [];
    const activeHtml = activeUnits.length === 0
      ? `<div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#6b5e4a;font-style:italic;padding:0.5rem 0">Nessuna unità reclutata — stai usando milizia implicita (⚔ ${state.resources.army})</div>`
      : activeUnits.map(slot => {
          const def = ARMY_UNITS.find(u => u.id === slot.typeId);
          if (!def) return '';
          return `
            <div style="display:flex;align-items:center;gap:0.5rem;padding:0.4rem 0.5rem;background:rgba(74,222,128,0.06);border:1px solid rgba(74,222,128,0.2);border-radius:4px;margin-bottom:0.3rem">
              <span style="font-size:1.2rem">${def.icon}</span>
              <div style="flex:1">
                <div style="font-family:'Cinzel',serif;font-size:0.72rem;color:#e8dcc8">${def.name} ×${slot.count}</div>
                <div style="font-family:'EB Garamond',serif;font-size:0.7rem;color:#9a8a6a">⚔ Forza ${def.force * slot.count} · Mantenim: ${Object.entries(def.upkeep||{}).map(([r,v])=>`${v}${({gold:'💰',people:'👥',faith:'✝',power:'👑'})[r]||r}`).join(' ')||'gratuito'}/5t</div>
              </div>
              <button onclick="Game.dismissUnit('${def.id}')" style="padding:0.25rem 0.55rem;background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.3);border-radius:3px;font-size:0.65rem;cursor:pointer;color:#f87171;font-family:'Cinzel',serif">Congeda</button>
            </div>`;
        }).join('');

    // Sezione drago
    const dragonHtml = state.hasDragon
      ? `<div style="display:flex;align-items:center;gap:0.5rem;padding:0.4rem 0.5rem;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.35);border-radius:4px;margin-bottom:0.3rem">
           <span style="font-size:1.4rem">🐉</span>
           <div style="flex:1">
             <div style="font-family:'Cinzel',serif;font-size:0.72rem;color:#fca5a5">Drago — Abilità speciale in battaglia</div>
             <div style="font-family:'EB Garamond',serif;font-size:0.7rem;color:#9a8a6a">+35 Forza · Mantenim: −4💰 −2✝/5t</div>
           </div>
           <span style="font-size:0.65rem;color:#fca5a5;font-family:'Cinzel',serif">UNICO</span>
         </div>`
      : '';

    // Sezione reclutamento
    const recruitHtml = available.map(def => {
      const current = getUnitCount(def.id);
      const full = current >= def.maxCount;
      const costStr = Object.entries(def.cost).map(([r,v]) => `${v}${({gold:'💰',people:'👥',faith:'✝',power:'👑',army:'⚔'})[r]||r}`).join(' ');
      const canAfford = Object.entries(def.cost).every(([r,v]) => (state.resources[r]||0) >= v);
      const btnStyle = full
        ? 'opacity:0.35;cursor:not-allowed;'
        : canAfford
          ? 'cursor:pointer;border-color:rgba(201,168,76,0.5);color:#c9a84c;'
          : 'opacity:0.5;cursor:not-allowed;';
      return `
        <div style="display:flex;align-items:center;gap:0.5rem;padding:0.4rem 0.5rem;background:rgba(201,168,76,0.04);border:1px solid rgba(201,168,76,0.15);border-radius:4px;margin-bottom:0.25rem">
          <span style="font-size:1.1rem">${def.icon}</span>
          <div style="flex:1">
            <div style="font-family:'Cinzel',serif;font-size:0.7rem;color:#e8dcc8">${def.name} <span style="color:#6b5e4a">(${current}/${def.maxCount})</span></div>
            <div style="font-family:'EB Garamond',serif;font-size:0.68rem;color:#9a8a6a">${def.desc}</div>
            <div style="font-size:0.65rem;color:#c9a84c;margin-top:0.1rem">Costo: ${costStr} · Forza: +${def.force} · Mantenim: ${Object.entries(def.upkeep||{}).map(([r,v])=>`${v}${({gold:'💰',people:'👥',faith:'✝',power:'👑'})[r]||r}`).join(' ')||'gratuito'}/5t</div>
          </div>
          <button ${full||!canAfford?'disabled':''} onclick="Game.recruitUnit('${def.id}')"
            style="padding:0.3rem 0.6rem;background:transparent;border:1px solid rgba(201,168,76,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.62rem;font-weight:700;letter-spacing:0.06em;${btnStyle}">
            ${full ? 'Max' : canAfford ? 'Recluta' : 'Fondi'}
          </button>
        </div>`;
    }).join('');

    overlay.innerHTML = `
      <div style="background:#0e0e16;border:1px solid rgba(201,168,76,0.4);border-radius:10px 10px 0 0;width:100%;max-width:480px;padding:1.2rem 1.1rem 1.5rem;max-height:85vh;overflow-y:auto">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.9rem">
          <div>
            <div style="font-family:'Cinzel Decorative',serif;font-size:0.9rem;color:#c9a84c">⚔ Gestione Esercito</div>
            <div style="font-family:'EB Garamond',serif;font-size:0.75rem;color:#9a8a6a;margin-top:0.1rem">Forza totale: <strong style="color:#4ade80">${totalForce}</strong> · Mantenim ogni 5 turni: <strong style="color:#f87171">${upkeepStr}</strong></div>
          </div>
          <button onclick="document.getElementById('army-panel-overlay').remove()" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:50%;width:28px;height:28px;font-size:0.9rem;cursor:pointer;color:#9a8a6a">✕</button>
        </div>

        <div style="font-family:'Cinzel',serif;font-size:0.62rem;color:#6b5e4a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem">🗡 Truppe Attive</div>
        ${dragonHtml}${activeHtml}

        <div style="height:1px;background:rgba(201,168,76,0.15);margin:0.7rem 0"></div>
        <div style="font-family:'Cinzel',serif;font-size:0.62rem;color:#6b5e4a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem">+ Recluta Nuove Truppe</div>
        ${recruitHtml}

        <div style="margin-top:0.6rem;padding:0.5rem 0.65rem;background:rgba(201,168,76,0.05);border:1px solid rgba(201,168,76,0.15);border-radius:4px;font-family:'EB Garamond',serif;font-size:0.75rem;color:#9a8a6a;line-height:1.5">
          🐉 <em>Il Drago non si recluta — si trova. Cerca il mercante di Asshai con l'uovo misterioso...</em>
        </div>
      </div>`;
  ...
  function showScreen(id) {
    ...
    // Mostra FAB esercito solo in gioco
    const fab = document.getElementById('army-fab');
    if (fab) {
      if (id === 'screen-game') {
  ...
  function _injectArmyFAB() {
    document.getElementById('army-fab')?.remove();
    const fab = document.createElement('button');
    fab.id = 'army-fab';
    fab.innerHTML = '⚔';
    fab.title = 'Gestisci Esercito';
    fab.style.cssText = [
      'position:fixed', 'bottom:1.1rem', 'right:1.1rem', 'z-index:300',
      'width:3rem', 'height:3rem', 'border-radius:50%',
      'background:linear-gradient(135deg,#1a1508,#3d2e0a)',
    ...
    fab.addEventListener('mouseenter', () => { fab.style.transform = 'scale(1.08)'; fab.style.borderColor = 'rgba(74,222,128,0.9)'; });
    fab.addEventListener('mouseleave', () => { fab.style.transform = 'scale(1)'; fab.style.borderColor = 'rgba(74,222,128,0.5)'; });
    fab.addEventListener('click', () => Game.showArmyPanel());
  ...
  function updateHUD() {
    ...
    // ── Aggiorna badge forza sul FAB esercito ──
    const fab = document.getElementById('army-fab');
    if (fab) {
      const force = getTroopForce();
      const hasTroops = state.troops && Object.keys(state.troops).length > 0;
      const dragonNote = state.hasDragon ? '🐉' : '';
      fab.innerHTML = `${dragonNote || '⚔'}<span style="position:absolute;top:-4px;right:-4px;background:#16a34a;color:#fff;font-size:0.45rem;font-family:'Cinzel',serif;font-weight:700;border-radius:8px;padding:1px 4px;min-width:16px;text-align:center;line-height:1.4">${force}</span>`;
      fab.style.position = 'fixed'; // assicura position per il badge assoluto
      fab.style.overflow = 'visible';
      fab.style.borderColor = state.hasDragon ? 'rgba(248,113,113,0.7)' : hasTroops ? 'rgba(74,222,128,0.7)' : 'rgba(74,222,128,0.4)';
    }
  }
  ...
  // ══════════════════════════════════════════════
  // ══════════════════════════════════════════════
  // PANNELLO ESERCITO — recluta e gestisci le truppe
  // ══════════════════════════════════════════════
  function showArmyPanel() {
    document.getElementById('army-panel-overlay')?.remove();
    const char = state.character;
    ...
    // Calcola riepilogo upkeep
    const resIcons = { gold:'💰', faith:'✝', people:'👥', army:'⚔', power:'👑' };
    const upkeepLines = Object.entries(upkeep)
      .filter(([, v]) => v > 0)
      .map(([res, v]) => `${resIcons[res]}${v}`)
      .join(' ');

    // Costruisce le righe di ogni tipo truppa
    const troopRows = TROOP_TYPES.map(t => {
      if (t.onlyFromCard) {
        // Drago — visibile solo se hasDragon
        if (!state.hasDragon) return '';
        const count = troops[t.id] || 0;
        return `
          <div style="display:flex;align-items:center;gap:0.5rem;padding:0.6rem 0.75rem;background:rgba(248,113,113,0.08);border:1px solid rgba(248,113,113,0.35);border-radius:5px;margin-bottom:0.4rem">
            <span style="font-size:1.4rem">${t.icon}</span>
            <div style="flex:1">
              <div style="font-family:'Cinzel',serif;font-size:0.72rem;color:#f87171;font-weight:700">${t.name} <span style="color:#fbbf24;font-size:0.65rem">★ LEGGENDARIO</span></div>
              <div style="font-family:'EB Garamond',serif;font-size:0.78rem;color:#9a8a6a">${t.desc}</div>
              <div style="font-size:0.65rem;color:#fb923c;margin-top:0.15rem">⚔ Forza: ${t.force} · Mantenim.: 💰${t.upkeep.gold} ✝${t.upkeep.faith}/5t</div>
            </div>
            <div style="text-align:center;min-width:2.5rem">
              <div style="font-family:'Cinzel Decorative',serif;font-size:1.1rem;color:#f87171">${count}</div>
              <div style="font-size:0.55rem;color:#6b5e4a">nel campo</div>
            </div>
          </div>`;
      }

      // Controlla se il personaggio può reclutare questo tipo
      const allowed = !t.allowedChars || t.allowedChars.includes(charId);
      const count = troops[t.id] || 0;
      const atMax = count >= t.maxUnits;

      // Calcola costo e se può permettersi
      const costParts = Object.entries(t.cost).map(([res, v]) => `${resIcons[res]}${v}`).join(' ');
      const canAffordRecruit = Object.entries(t.cost).every(([res, v]) => state.resources[res] >= v);
      const upkeepParts = Object.entries(t.upkeep || {}).map(([res, v]) => `${resIcons[res]}${v}`).join(' ') || '—';

      const recruitBtn = !allowed
        ? `<button disabled style="padding:0.3rem 0.6rem;font-size:0.6rem;font-family:'Cinzel',serif;background:transparent;border:1px solid rgba(100,100,100,0.3);border-radius:3px;color:#4a4a4a;cursor:not-allowed">Non disponibile</button>`
        : atMax
        ? `<button disabled style="padding:0.3rem 0.6rem;font-size:0.6rem;font-family:'Cinzel',serif;background:transparent;border:1px solid rgba(100,100,100,0.3);border-radius:3px;color:#6b5e4a;cursor:not-allowed">Max (${t.maxUnits})</button>`
        : !canAffordRecruit
        ? `<button disabled style="padding:0.3rem 0.6rem;font-size:0.6rem;font-family:'Cinzel',serif;background:transparent;border:1px solid rgba(239,68,68,0.3);border-radius:3px;color:#6b5e4a;cursor:not-allowed">Fondi insuff.</button>`
        : `<button onclick="Game.recruitTroop('${t.id}')" style="padding:0.3rem 0.7rem;font-size:0.6rem;font-family:'Cinzel',serif;font-weight:700;background:rgba(74,222,128,0.1);border:1px solid rgba(74,222,128,0.4);border-radius:3px;color:#4ade80;cursor:pointer;letter-spacing:0.05em">+ Recluta</button>`;

      const dismissBtn = count > 0
        ? `<button onclick="Game.dismissTroop('${t.id}')" style="padding:0.3rem 0.6rem;font-size:0.6rem;font-family:'Cinzel',serif;background:transparent;border:1px solid rgba(239,68,68,0.3);border-radius:3px;color:#f87171;cursor:pointer;margin-left:0.3rem">− Congeda</button>`
        : '';

      const borderColor = !allowed ? 'rgba(60,60,60,0.3)'
        : count > 0 ? `${t.battleColor}44`
        : 'rgba(201,168,76,0.15)';

      return `
        <div style="padding:0.6rem 0.75rem;background:#12121a;border:1px solid ${borderColor};border-radius:5px;margin-bottom:0.4rem;opacity:${allowed?'1':'0.45'}">
          <div style="display:flex;align-items:center;gap:0.5rem">
            <span style="font-size:1.3rem">${t.icon}</span>
            <div style="flex:1">
              <div style="font-family:'Cinzel',serif;font-size:0.72rem;color:#e8dcc8;font-weight:700">${t.name}
                ${count > 0 ? `<span style="color:${t.battleColor};margin-left:0.3rem">×${count}</span>` : ''}
              </div>
              <div style="font-family:'EB Garamond',serif;font-size:0.75rem;color:#9a8a6a;line-height:1.3">${t.desc}</div>
              <div style="font-size:0.62rem;color:#6b5e4a;margin-top:0.1rem">
                ⚔ Forza: <strong style="color:#c9a84c">${t.force}</strong> ·
                Costo: <strong style="color:#fbbf24">${costParts||'—'}</strong> ·
                Mantenim.: <strong style="color:#fb923c">${upkeepParts||'gratis'}/5t</strong> ·
                Max: ${t.maxUnits}
              </div>
            </div>
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:500;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);';
    overlay.innerHTML = `
      <div style="background:#0e0e16;border:1px solid rgba(74,222,128,0.35);border-radius:10px 10px 0 0;width:100%;max-width:480px;padding:1.2rem 1.1rem 1.6rem;animation:tut-pop 0.25s ease;max-height:85vh;overflow-y:auto">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.9rem">
          <div>
            <div style="font-family:'Cinzel Decorative',serif;font-size:0.9rem;color:#4ade80">⚔ Esercito</div>
            <div style="font-family:'EB Garamond',serif;font-size:0.78rem;color:#9a8a6a;margin-top:0.1rem">
              Forza totale: <strong style="color:#4ade80">${totalForce}</strong>
              ${upkeepLines ? ` · Mantenim.: <strong style="color:#fb923c">${upkeepLines}/5t</strong>` : ''}
            </div>
          </div>
          <button onclick="document.getElementById('army-panel-overlay').remove()" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:50%;width:28px;height:28px;font-size:0.9rem;cursor:pointer;color:#9a8a6a">✕</button>
        </div>
        <div style="height:1px;background:rgba(74,222,128,0.15);margin-bottom:0.8rem"></div>
        ${troopRows}
        <div style="margin-top:0.6rem;padding:0.5rem 0.65rem;background:rgba(74,222,128,0.04);border:1px solid rgba(74,222,128,0.12);border-radius:4px;font-family:'EB Garamond',serif;font-size:0.78rem;color:#6b5e4a;line-height:1.5">
          💡 Le truppe reclutate si riflettono sulla risorsa Esercito. Se non puoi pagare il mantenimento, le truppe più costose disertano.
          ${state.hasDragon ? '<br>🐉 Hai un drago nell\'esercito — in battaglia usa l\'abilità speciale.' : ''}
        </div>
      </div>`;
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
    document.body.appendChild(overlay);
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    let _uid = 0;
    function makeUnit(side, col, row, angle, stacks) {
      stacks = stacks || 1;
      const pos = snapToCell(col, row);
      return {
        id: 'u' + (_uid++), side,
        x: pos.x, y: pos.y, angle,
        hp: HP_BASE * stacks, maxHp: HP_BASE * stacks,
        stacks, dmg: stacks,
        alive: true, dead: false, deadAt: 0, lastAtk: 0,
        engagedId: null,  // id del nemico a cui è agganciato
      };
    }

    // ── Posizionamento iniziale giocatore: ultime 2 righe ──
    // Cap fisso: max 12 cerchietti visibili. Le truppe in eccesso vengono fuse
    // automaticamente nei cerchietti esistenti come stacks extra
    const UNIT_CAP = 12;
    const visibleCount = Math.min(UNIT_CAP, pCount);
    const extraStacks = pCount - visibleCount; // cerchietti in eccesso da distribuire
    const playerUnits = [];
    let placed = 0;
    for (let row = ROWS - 1; row >= ROWS - 2 && placed < visibleCount; row--) {
      for (let col = 0; col < COLS && placed < visibleCount; col++) {
        playerUnits.push(makeUnit('p', col, row, -Math.PI / 2));
        placed++;
      }
    }
    // Distribuisci gli stacks extra in modo uniforme sui cerchietti creati
    if (extraStacks > 0 && playerUnits.length > 0) {
      for (let i = 0; i < extraStacks; i++) {
        const u = playerUnits[i % playerUnits.length];
        u.stacks++; u.hp += HP_BASE; u.maxHp += HP_BASE; u.dmg++;
      }
    }

    // ── CPU: 5 pattern intelligenti ──
    // Cap CPU uguale al giocatore: max 12, extra fusi
    const CPU_CAP = 12;
    const eVisible = Math.min(CPU_CAP, eCount);
    const eExtra = eCount - eVisible;
    const CPU_STRATS = [
      // Linea compatta prime 2 righe
      (n) => { const u=[]; for(let i=0;i<n;i++) u.push([i%COLS, Math.floor(i/COLS)]); return u; },
      // Triangolo: punta al centro, ali ai lati
      (n) => {
        const pos=[[2,0],[3,0],[1,1],[4,1],[0,1],[5,1],[2,1],[3,1],[0,0],[5,0]];
        return pos.slice(0,n);
      },
      // Flanchi concentrati
      (n) => { const u=[]; for(let i=0;i<n;i++) u.push([i%2===0?0:COLS-1, Math.floor(i/2)]); return u; },
      // Dispersione totale su 2 righe
    ...
    CPU_STRATS[Math.floor(Math.random() * CPU_STRATS.length)](eVisible)
      .forEach(([col, row]) => enemyUnits.push(makeUnit('e', col, row, Math.PI / 2)));
    // Distribuisci stacks extra CPU
    if (eExtra > 0 && enemyUnits.length > 0) {
      for (let i = 0; i < eExtra; i++) {
        const u = enemyUnits[i % enemyUnits.length];
        u.stacks++; u.hp += HP_BASE; u.maxHp += HP_BASE; u.dmg++;
      }
    }
    ...
    ov.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:4px;width:${BW+20}px;max-height:100vh;overflow:hidden">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:2px 0">
          ...
          <div id="sb-msg" style="font-size:.58rem;color:#9a8a6a;line-height:1.35;text-align:center">Posiziona le truppe. Trascina nei quadrati.<br>Sovrapponi cerchietti per fonderli.</div>
        </div>
    ...
    function drawUnit(u) {
      ...
      const iP = u.side==='p';
      const col = iP ? '#4ade80' : '#f87171';
      const r = UR + (u.stacks-1)*1.5;
      const isSel = selectedId===u.id;

      mCtx.save();
      mCtx.translate(u.x, u.y);

      // Stemma casata (trasparenza leggera)
      const crestImg = iP ? pImg : eImg;
      if(crestImg && crestImg.complete && crestImg.naturalWidth>0) {
        mCtx.save();
        mCtx.globalAlpha=0.16;
        mCtx.drawImage(crestImg, -r*.82, -r*.82, r*1.64, r*1.64);
        mCtx.restore();
      }

      // Corpo
      mCtx.beginPath(); mCtx.arc(0,0,r,0,Math.PI*2);
      mCtx.fillStyle=iP?'rgba(8,28,14,.92)':'rgba(28,8,8,.92)';
      mCtx.fill();
      mCtx.strokeStyle=isSel?'#fbbf24':col;
      mCtx.lineWidth=isSel?2.5:1.6; mCtx.stroke();

      // Semicerchio "testa" — arco spesso indica la fronte
      mCtx.rotate(u.angle);
      mCtx.beginPath();
      mCtx.arc(0,0,r+2.5,-Math.PI/2.3,Math.PI/2.3);
      mCtx.strokeStyle=iP?'rgba(16,200,80,.7)':'rgba(240,55,55,.7)';
      mCtx.lineWidth=3.2; mCtx.stroke();

      // Solo punta triangolare (niente linea)
      mCtx.beginPath();
      mCtx.moveTo(r+4,0); mCtx.lineTo(r-2,-4.5); mCtx.lineTo(r-2,4.5);
      mCtx.closePath();
      mCtx.fillStyle=isSel?'#fbbf24':col; mCtx.fill();

      mCtx.restore();

      // HP bar sotto il cerchio
      ...
      mCtx.font=`bold ${u.stacks>1?10:9}px sans-serif`;
      mCtx.textAlign='center'; mCtx.textBaseline='middle';
      mCtx.fillText(u.stacks>1?`${u.stacks}×`:u.hp, u.x, u.y);
    }
    ...
    mainCv.addEventListener('pointerdown',e=>{
      ...
      if(h){dragUnit=h;selectedId=h.id;dragOX=pos.x-h.x;dragOY=pos.y-h.y;mainCv.style.cursor='grabbing';}
      e.preventDefault();
    },{passive:false});
    ...
    mainCv.addEventListener('pointerup',()=>{
      if(dragUnit&&phase==='placement'){
        // Snap alla cella più vicina nella zona giocatore
        const {col,row}=nearestPlayerCell(dragUnit.x,dragUnit.y);
        const snapped=snapToCell(col,row);
        // Fusione se c'è già un'unità in quella cella
        const other=playerUnits.find(u=>u!==dragUnit&&u.alive&&!u.dead&&
        ...
        if(other){
          ...
          dragUnit.x=snapped.x; dragUnit.y=snapped.y;
        }
      ...
      function tick(ts){
        ...
        allAlive.forEach(u=>{
          ...
          if(targetDist>ATK_RANGE){
            ...
            u.x+=Math.cos(ta)*SPEED_PX*dt;
            u.y+=Math.sin(ta)*SPEED_PX*dt;
            u.x=Math.max(UR,Math.min(BW-UR,u.x));
            u.y=Math.max(UR,Math.min(BH-UR,u.y));
          } else {
            // In range → attacca il bersaglio agganciato, sempre lui
            if(ts-u.lastAtk>=ATK_INTERVAL){
              u.lastAtk=ts;
              const atkA=angleToUnit(u,target);
              const mult=dmgMult(atkA,target.angle);
              target.hp-=u.dmg*mult;
              spawnParticles(target.x,target.y,mult===1.0?'spark':'blood');
              if(target.hp<=0){
                target.hp=0; target.alive=false; target.dead=true; target.deadAt=Date.now();
                u.engagedId=null; // bersaglio morto → libero per prossimo
...
const AudioManager = (() => {
  ...
  function _buildArmyFab() {
    if (document.getElementById('army-fab')) return;
    const fab = document.createElement('button');
    fab.id = 'army-fab';
    fab.title = 'Gestisci il tuo esercito';
    fab.innerHTML = '⚔';
    fab.style.cssText = [
      'position:fixed', 'bottom:3.2rem', 'right:0.75rem', 'z-index:200',
      'width:2.4rem', 'height:2.4rem',
      'background:linear-gradient(135deg,rgba(127,29,29,0.85),rgba(153,27,27,0.9))',
      'border:1px solid rgba(239,68,68,0.5)', 'border-radius:50%',
      'font-size:1rem', 'cursor:pointer', 'color:#fca5a5',
      'display:flex', 'align-items:center', 'justify-content:center',
      'box-shadow:0 2px 8px rgba(0,0,0,0.5)',
      'transition:opacity 0.2s, transform 0.15s',
      'opacity:0', 'pointer-events:none',  // nascosto finché non si è in gioco
    ].join(';');
