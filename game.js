...
const MultiplayerEngine = (() => {
    ...
    w.innerHTML = `
      <div style="width:100%;max-width:420px">
        ...
        <div class="mp-card">
          ...
          <div id="mp-charsec">
            <div style="font-family:'EB Garamond',serif;font-size:.85rem;color:#6b5e4a;margin-bottom:.5rem">Scegli un personaggio da una casata libera.</div>
            <button class="mp-btn-gold" onclick="MultiplayerEngine.openCharPicker()">Scegli Personaggio</button>
          </div>
        </div>
        <div class="mp-card">
  ...
  function _refreshRoom() {
    ...
    if (csec && me?.charId) {
      ...
      csec.innerHTML = `
        ...
        <button class="mp-btn-ghost" style="margin-top:0" onclick="MultiplayerEngine.openCharPicker()">Cambia</button>
        ${!me.ready
  ...
  function openCharPicker() {
    _el('mp-cpick')?.remove();
    const CHARS = _chars();
    if (!CHARS.length) { _toast('Personaggi non ancora disponibili, riprova.', 'warn'); return; }
    const myHouse = _players[_myPeerId]?.house;
    const takenByOthers = Object.entries(_takenHouses)
      .filter(([, pid]) => pid !== _myPeerId).map(([h]) => h);
    const houses = {};
    CHARS.forEach(c => { if (!houses[c.house]) houses[c.house] = []; houses[c.house].push(c); });

    let inner = '';
    Object.entries(houses).forEach(([hn, chars]) => {
      const taken = takenByOthers.includes(hn);
      const taker = taken ? (Object.values(_players).find(p => p.house === hn)?.name || '?') : null;
      inner += `<div style="margin-bottom:.85rem">
        <div style="font-size:.63rem;letter-spacing:.1em;text-transform:uppercase;padding:.22rem .5rem;border-radius:3px;margin-bottom:.35rem;
          ${taken?'color:#f87171;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2)':'color:#c9a84c;background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.15)'}">
          ${hn}${taken?` — 🔒 ${taker}`:''}
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:.35rem">`;
      chars.forEach(c => {
        inner += `<button class="mp-char-btn" ${taken?'disabled':''}
          onclick="${taken?'':`MultiplayerEngine.selectChar('${c.id}','${hn.replace(/'/g,"\\'")}');document.getElementById('mp-cpick')?.remove()`}">
          <span style="font-size:1.25rem">${c.icon}</span>
          <span style="font-size:.5rem;color:#e8dcc8;max-width:62px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:.1rem">${c.name.split(' ')[0]}</span>
        </button>`;
      });
      inner += `</div></div>`;
    });
    ...
    const ov = document.createElement('div');
    ov.id = 'mp-cpick';
    ov.style.cssText = 'position:fixed;inset:0;z-index:700;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;padding:1rem;animation:mp-in .2s ease';
    ov.innerHTML = `<div style="background:#0e0e16;border:1px solid rgba(201,168,76,.4);border-radius:8px;width:100%;max-width:420px;max-height:88vh;overflow-y:auto;padding:1.2rem;font-family:'Cinzel',serif">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem">
        <div style="font-family:'Cinzel Decorative',serif;font-size:.9rem;color:#c9a84c">Scegli Personaggio</div>
        <button onclick="document.getElementById('mp-cpick')?.remove()" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:50%;width:26px;height:26px;font-size:.85rem;cursor:pointer;color:#9a8a6a">✕</button>
      </div>
      <div style="font-family:'EB Garamond',serif;font-size:.8rem;color:#6b5e4a;margin-bottom:.75rem">Le casate in rosso sono già prese. Scegli da una casata libera.</div>
      ${inner}
    </div>`;
    ov.addEventListener('click', e => { if (e.target === ov) ov.remove(); });
    document.body.appendChild(ov);
  ...
  function selectChar(charId, house) {
    const prev = _players[_myPeerId]?.house;
    if (prev && prev !== house) delete _takenHouses[prev];
  ...
  function _hChar(msg, from) {
    ...
    if (p) { if (p.house) delete _takenHouses[p.house]; p.charId=msg.charId; p.house=msg.house; _takenHouses[msg.house]=from; }
    if (_isHost) _bcastList(); _refreshRoom();
  }
  ...
  // ══════════════════════════════
  // AVVIO PARTITA — AUTO-SELEZIONA PERSONAGGIO
  // salta schermata casata/personaggio: parte direttamente
  // ══════════════════════════════
  function _startGame() {
    _el('mp-room')?.remove();
    _el('mp-lobby')?.remove();
    ...
    // ── Avvio diretto: bypassa completamente le schermate di selezione ──
    // Usiamo l'API interna per costruire lo stato come se l'utente avesse
    // selezionato il personaggio attraverso il percorso normale.
    // showCharacterDetail → startFromPrologue → startGame
    Game.showCharacterSelect();
    _autoClick(char, 0);

    setTimeout(() => { _injectHud(); _injectThroneBar(); _startSync(); }, 2200);
  }

  function _autoClick(char, attempt) {
    if (attempt > 25) { _toast('⚠ Avvio manuale: scegli ' + char.name, 'warn'); return; }
    setTimeout(() => {
      // 1. cerca card per nome nella griglia
      const cards = document.querySelectorAll('.char-card');
      let found = false;
      cards.forEach(card => {
        const nameEl = card.querySelector('.char-card-name,.char-name,[class*="name"]');
        if (!found && nameEl && nameEl.textContent.trim() === char.name) { card.click(); found = true; }
        if (!found && card.dataset.charId === char.id) { card.click(); found = true; }
      });
      if (found) { setTimeout(() => _clickStart(char, 0), 500); return; }
      // 2. potrebbe essere nascosta — cerca prima la casa
      const houseSuffix = char.house.replace(/Casa\s*/i,'').trim().split('/')[0].trim();
      let houseClicked = false;
      document.querySelectorAll('.house-card,[data-house],[class*="house"]').forEach(hc => {
        if (!houseClicked && (hc.textContent||'').includes(houseSuffix)) { hc.click(); houseClicked = true; }
      });
      _autoClick(char, attempt + 1);
    }, attempt === 0 ? 350 : 280);
  }

  function _clickStart(char, attempt) {
    if (attempt > 15) return;
    const ov = _el('char-detail-overlay') || document.querySelector('[id*="char-detail"]');
    if (!ov) { setTimeout(() => _clickStart(char, attempt+1), 300); return; }
    const btns = Array.from(ov.querySelectorAll('button'));
    const start = btns.find(b => /avvia|inizia|gioca|start|scegli/i.test(b.textContent));
    if (start) { start.click(); return; }
    setTimeout(() => _clickStart(char, attempt+1), 300);
  }
  ...
  function _leave()  { _end(); showLobby(); }
  function _end() {
    clearInterval(_throneTimer); clearInterval(window._mpSync);
    _throneTimer = null; window._mpSync = null;
    ...
    ['mp-lobby','mp-room','mp-hud','mp-throne-bar','mp-win','mp-actions',
     ...
     'mp-pvp-reinf','mp-gchat','mp-pchat','mp-spec-ov','mp-spec-pr','mp-notif'].forEach(id => _el(id)?.remove());
  }
  ...
  return {
    ...
    openCharPicker, selectChar, setReady, hostStart,
    sendLobbyChat,
    // In-game
