...
const Game = (() => {
    ...
    function buildHouseCard(hId, h, section) {
      ...
      card.innerHTML = `
        ...
        <div style="flex:1;min-width:0">
          <div style="font-family:'Cinzel',serif;font-size:0.75rem;color:#e8dcc8;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Casa ${h.name}</div>
  ...
  function _openHousePopup(hId) {
    ...
    if (isSuppressed) {
      ...
      if (isAlly) {
        ...
        if (isWarPending) {
          const hasResp = (state.allyLoans&&state.allyLoans[hId]) || (state.allyLoanRefusals&&state.allyLoanRefusals[hId]);
          if (hasResp) {
            const loanInfo = state.allyLoans&&state.allyLoans[hId];
            actions += `<button class="btn-raven" disabled style="opacity:0.5">${loanInfo?'⚔ Rinforzi: +'+loanInfo.amount:'✗ Rifiutato'}</button>`;
          } else {
      ...
      <div style="background:#0e0e16;border:1px solid rgba(201,168,76,0.4);border-radius:10px 10px 0 0;width:100%;max-width:480px;padding:1.2rem 1.1rem 1.4rem;animation:tut-pop 0.25s ease;max-height:80vh;overflow-y:auto">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.85rem">
          <div style="display:flex;align-items:center;gap:0.6rem">
            ...
            <div>
              <div style="font-family:'Cinzel Decorative',serif;font-size:0.88rem;color:#c9a84c">Casa ${h.name}${spyDotsHtml}</div>
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function killTroops(side, forceLost, totalForce) {
      const prefix = side === 'player' ? 'th-p' : 'th-e';
      const alive = side === 'player' ? pAlive : eAlive;
      const pctRemaining = Math.max(0, (totalForce - forceLost) / totalForce);
      const shouldBeAlive = Math.round(pctRemaining * TROOP_COUNT_TH);
      const toKill = Math.max(0, alive - shouldBeAlive);
      for (let i = alive - 1; i >= Math.max(0, alive - toKill); i--) {
      ...
      if (side === 'player') pAlive = Math.max(0, alive - toKill);
      else                   eAlive = Math.max(0, alive - toKill);
    }
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat) {
    ...
    function killTroops(side, forceLost, totalForce) {
      // Kill proportionally: how many of the 10 visual units should die
      const prefix = side === 'player' ? 'p' : 'e';
      const alive = side === 'player' ? pAlive : eAlive;
      // How many visual units should be alive now based on force percentage
      const pctRemaining = Math.max(0, (totalForce - forceLost) / totalForce);
      const shouldBeAlive = Math.round(pctRemaining * TROOP_COUNT);
      const toKill = Math.max(0, alive - shouldBeAlive);
      for (let i = alive - 1; i >= Math.max(0, alive - toKill); i--) {
        const t = document.getElementById(`${prefix}-t${i}`);
        if (t && !t.classList.contains('dying')) {
  ...
  function acceptAllyLoan(hId, amount, isPact, resKey, resCost) {
    ...
    // Riapri overlay guerra con dati aggiornati
    const warOverlay = document.getElementById('war-diplo-reinf-overlay');
    if (warOverlay) {
      const warTarget = state.pendingWarDeclaration?.houseId;
      warOverlay.remove();
      setTimeout(() => {
        if (state.pendingKingChallenge) Game._openKingChallengeDiplomacy();
        else if (warTarget) Game._openWarDiplomacy(warTarget);
      }, 150);
    }
  }
...
// Inject tutorial button immediately — Game is already defined at this point
(function() {
  ...
  // Try immediately (if DOM is ready) and also on load
