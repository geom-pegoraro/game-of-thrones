...
const Game = (() => {
  ...
  function warDiploDirectWar(houseId) {
    ...
    if (!h) return;
    h.status = 'enemy';
    showEnemyToast(houseId, 'Avete scelto la guerra senza compromessi. La casata risponderà con tutta la sua forza.');
  ...
  function showStrategicBattle(playerForce, enemyForce, enemyLabel, onComplete, allowRetreat) {
    ...
    // ── Cerchietti: 1 ogni 10 truppe, cap 16 ──
    ...
    // ── Costanti campo unico 4×8 (4 colonne, 8 righe — metà per lato) ──
    const COLS = 4, ROWS = 8;
    const CW = 64, CH = 44;
    ...
    const ATK_INTERVAL = 1800; // ms tra attacchi — più lenti
    const MERGE_DIST = UR * 1.4; // distanza fusione
    ...
    function makeUnit(side, col, row, angle, stacks) {
      ...
      return {
        ...
        stacks,           // numero cerchietti fusi
        dmg: stacks,      // danno base = numero fusi
        alive: true, dead: false, deadAt: 0,
        lastAtk: 0,
      };
    }

    // Giocatore: distribuiti nelle 4 righe inferiori (righe 4-7)
    const playerUnits = [];
    let placed = 0;
    ...
    // ── CPU: 5 pattern intelligenti, sempre diversi ──
    const CPU_STRATS = [
      // 0: linea compatta prima fila
      (n) => {
        const units = [];
        for (let i = 0; i < n; i++) units.push([i % COLS, Math.floor(i / COLS)]);
        return units;
      },
      // 1: triangolo offensivo — punta centrale
      (n) => {
        const positions = [[1,0],[2,0],[0,1],[3,1],[1,1],[2,1],[0,0],[3,0]];
        return positions.slice(0, n);
      },
      // 2: flanchi — concentra sulle ali
      (n) => {
        const units = [];
        for (let i = 0; i < n; i++) {
      ...
      // 3: dispersione totale
      (n) => {
        const units = [];
        const rows = [0,1,0,1,0,1,0,1];
        for (let i = 0; i < n; i++) units.push([i % COLS, rows[i] || 0]);
        return units;
      },
      // 4: massed center — tutto al centro
      (n) => {
        const units = [];
        for (let i = 0; i < n; i++) units.push([1 + i % 2, Math.floor(i / 2)]);
        return units;
      },
    ];
    const cpuStrat = CPU_STRATS[Math.floor(Math.random() * CPU_STRATS.length)];
    const cpuPositions = cpuStrat(eCount);
    const enemyUnits = [];
    cpuPositions.forEach(([col, row]) => {
      enemyUnits.push(makeUnit('e', col, row, Math.PI / 2));
    });
    ...
    ov.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;gap:6px;width:${BW+24}px">
        <!-- Header -->
        <div style="display:flex;justify-content:space-between;width:100%;font-size:.6rem;letter-spacing:.1em;text-transform:uppercase">
        ...
        <div style="display:flex;align-items:center;gap:12px">
          <div id="sb-cd" style="font-family:'Cinzel Decorative',serif;font-size:1.6rem;color:#c9a84c;min-width:2.2rem;text-align:center">20</div>
          <div id="sb-msg" style="font-size:.62rem;color:#9a8a6a;line-height:1.4;text-align:center">Posiziona le truppe. Trascina nei quadrati.<br>Sovrapponi 2 cerchietti per fonderli.</div>
        </div>
        ...
        <div style="position:relative;width:${BW}px;height:${BH}px;border:1px solid rgba(201,168,76,.3);border-radius:4px;overflow:hidden;background:#08080f">
          ...
          <canvas id="sb-main" width="${BW}" height="${BH}" style="position:absolute;inset:0;cursor:grab"></canvas>
          <canvas id="sb-fog"  width="${BW}" height="${BH}" style="position:absolute;inset:0;pointer-events:none"></canvas>
          <!-- Linea di mezzeria -->
          <div style="position:absolute;left:0;right:0;top:50%;height:1px;background:rgba(201,168,76,.35);pointer-events:none"></div>
          <div style="position:absolute;top:3px;left:5px;font-size:.48rem;color:#f87171;opacity:.7">${enemyLabel.toUpperCase()}</div>
          <div style="position:absolute;bottom:3px;left:5px;font-size:.48rem;color:#4ade80;opacity:.7">LE TUE TRUPPE</div>
        </div>

        <!-- Pulsanti -->
        <div id="sb-btns" style="display:flex;gap:10px;margin-top:4px">
          ${allowRetreat ? '<button id="sb-ret" style="padding:.5rem 1.1rem;background:transparent;border:1px solid rgba(201,168,76,.4);border-radius:3px;font-family:Cinzel,serif;font-size:.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;color:#c9a84c">🏃 Ritirata</button>' : ''}
        </div>
      </div>
    `;
    ...
    function drawGrid() {
      ...
      // Metà superiore (nemico) bordi rossi
      gCtx.strokeStyle = 'rgba(239,68,68,.12)';
      gCtx.lineWidth = 1;
      for (let c=1;c<COLS;c++){
        gCtx.beginPath();gCtx.moveTo(c*CW,0);gCtx.lineTo(c*CW,BH/2);gCtx.stroke();
      }
      for (let r=1;r<ROWS/2;r++){
        gCtx.beginPath();gCtx.moveTo(0,r*CH);gCtx.lineTo(BW,r*CH);gCtx.stroke();
      }
      // Metà inferiore (giocatore) bordi verdi
      gCtx.strokeStyle = 'rgba(74,222,128,.12)';
      for (let c=1;c<COLS;c++){
        gCtx.beginPath();gCtx.moveTo(c*CW,BH/2);gCtx.lineTo(c*CW,BH);gCtx.stroke();
      }
      for (let r=ROWS/2+1;r<ROWS;r++){
        gCtx.beginPath();gCtx.moveTo(0,r*CH);gCtx.lineTo(BW,r*CH);gCtx.stroke();
      }
    }
    drawGrid();

    // ── Nebbia (solo metà superiore) ──
    ...
    function drawFog() {
      ...
      fogPts.forEach(p=>{
        ...
        g.addColorStop(0,`rgba(55,42,38,${p.a})`);
        g.addColorStop(1,'rgba(55,42,38,0)');
        fCtx.fillStyle=g;
    ...
    function drawUnit(u) {
      ...
      const r = UR + (u.stacks-1)*2; // cerchio più grande se fuso
      ...
      mCtx.fillStyle = iP?'rgba(8,32,16,.88)':'rgba(32,8,8,.88)';
      mCtx.fill();
      mCtx.strokeStyle = isSel?'#fbbf24':col;
      mCtx.lineWidth = isSel?3:2;
      mCtx.stroke();

      // Linea fronte
      mCtx.rotate(u.angle);
      mCtx.beginPath(); mCtx.moveTo(-r,0); mCtx.lineTo(r,0);
      mCtx.strokeStyle = isSel?'#fbbf24':col;
      mCtx.lineWidth=2.5; mCtx.stroke();
      // Freccia
      mCtx.beginPath();
      mCtx.moveTo(r,0); mCtx.lineTo(r-6,-4); mCtx.lineTo(r-6,4);
      mCtx.closePath(); mCtx.fillStyle=isSel?'#fbbf24':col; mCtx.fill();
      mCtx.restore();

      // HP bar
      const bw=r*2, bx=u.x-r, by=u.y-r-8;
      mCtx.fillStyle='rgba(0,0,0,.5)'; mCtx.fillRect(bx,by,bw,4);
      mCtx.fillStyle=u.hp>u.maxHp*.6?'#4ade80':u.hp>u.maxHp*.3?'#fbbf24':'#f87171';
      mCtx.fillRect(bx,by,bw*(u.hp/u.maxHp),4);
    ...
    function render() {
      ...
      // Nemico: non disegnare se nebbia ancora presente
      enemyUnits.forEach(u=>{
        if (!fogLifted && !u.dead) return; // nascondi durante nebbia
        drawUnit(u);
      });
      // Morti nemici visibili anche con nebbia (feedback)
      if (!fogLifted) {
        enemyUnits.filter(u=>u.dead).forEach(u=>drawUnit(u));
      }
    ...
    function hitPlayer(pos) {
      for(const u of playerUnits){
        ...
        const r=UR+(u.stacks-1)*2;
        const dx=pos.x-u.x,dy=pos.y-u.y;
        if(dx*dx+dy*dy<(r+6)*(r+6))return u;
      }
      return null;
    }
    ...
    mainCv.addEventListener('pointermove',e=>{
      ...
      const pos=canvasPos(e);
      // Limita alla metà inferiore durante placement
    ...
    mainCv.addEventListener('pointerup',e=>{
      if(dragUnit && phase==='placement'){
        // Snap al centro cella più vicino (solo metà inferiore)
        ...
        // Fusione: controlla se c'è già un'unità in quella cella
        const other = playerUnits.find(u=>u!==dragUnit&&u.alive&&!u.dead&&
          Math.abs(u.x-snapped.x)<CW*.4&&Math.abs(u.y-snapped.y)<CH*.4);
        if(other){
          // Fondi: somma HP, stacks, dmg; rimuovi dragUnit
          other.stacks += dragUnit.stacks;
          other.hp     += dragUnit.hp;
          other.maxHp  += dragUnit.maxHp;
          other.dmg    += dragUnit.dmg;
          dragUnit.alive=false; dragUnit.dead=true; dragUnit.deadAt=0; // sparisce subito
          selectedId=other.id;
        } else {
    ...
    function startBattle(){
      ...
      // Coordinate già unificate nel canvas unico
      function dist(a,b){
        const dx=a.x-b.x,dy=a.y-b.y;
        return Math.sqrt(dx*dx+dy*dy);
      }
      function angleToUnit(from,to){
        return Math.atan2(to.y-from.y,to.x-from.x);
      }
      // Danno in base al lato colpito rispetto alla fronte della vittima
      function dmgMult(atkAngle,victimAngle){
        // frontale = attacchi dalla direzione opposta alla fronte della vittima
        let diff=atkAngle-(victimAngle+Math.PI);
        while(diff>Math.PI)diff-=2*Math.PI;
        while(diff<-Math.PI)diff+=2*Math.PI;
        const a=Math.abs(diff);
        if(a<Math.PI/3) return 1.0;    // frontale
        if(a<2*Math.PI/3) return 1.5;  // fianco
        return 2.0;                      // schiena
      }
    ...
    function endBattle(playerWon){
      ...
      const pTot=Math.max(1,playerUnits.length);
      const eTot=Math.max(1,enemyUnits.length);
      const survived     =Math.max(1,Math.round(playerForce*(pAlive/pTot)));
      const enemySurvived=Math.max(0,Math.round(enemyForce*(eAlive/eTot)));
      ...
      document.getElementById('sb-finish')?.addEventListener('click',()=>{
        cleanup();
        onComplete(playerWon,survived,enemySurvived);
      });
    ...
    // ── Avvia placement loop + countdown automatico ──
