...
const Game = (() => {
  ...
  function _injectArmyButton() {
    // Rimuovi FAB tondo legacy e qualsiasi bottone esercito precedente
    document.getElementById('army-btn')?.remove();
    document.getElementById('army-btn')?.remove();

    // Trova il bottone diplomazia nell'HTML statico e inserisci subito dopo
    const diploBtn = document.getElementById('btn-diplo');
    if (!diploBtn) return;

    const btn = document.createElement('button');
    btn.id = 'army-btn';
    btn.innerHTML = '⚔ &nbsp;ESERCITO';
    btn.style.cssText = [
      'display:block', 'width:100%',
      'padding:0.52rem 1rem',
      'background:linear-gradient(135deg,rgba(10,28,10,0.95),rgba(20,50,20,0.95))',
    ...
    btn.addEventListener('mouseenter', () => {
      btn.style.background = 'linear-gradient(135deg,rgba(20,50,20,0.98),rgba(30,80,30,0.98))';
      btn.style.borderColor = 'rgba(74,222,128,0.8)';
    });
    btn.addEventListener('mouseleave', () => {
      btn.style.background = 'linear-gradient(135deg,rgba(10,28,10,0.95),rgba(20,50,20,0.95))';
      btn.style.borderColor = 'rgba(74,222,128,0.45)';
    });
    btn.addEventListener('click', () => Game.showArmyPanel());
    diploBtn.parentNode.insertBefore(btn, diploBtn.nextSibling);
  }
  ...
  function updateHUD() {
    ...
    // ── Aggiorna testo bottone esercito ──
    const armyBtn = document.getElementById('army-btn');
    if (armyBtn) {
      const force = getTroopForce();
      const hasDragon = state.hasDragon;
      const dragonIcon = hasDragon ? '🐉 ' : '';
      armyBtn.innerHTML = `${dragonIcon}⚔ &nbsp;ESERCITO <span style="font-size:0.65rem;opacity:0.8;margin-left:0.3rem">(${force})</span>`;
      armyBtn.style.borderColor = hasDragon ? 'rgba(248,113,113,0.6)' : 'rgba(74,222,128,0.45)';
      armyBtn.style.color = hasDragon ? '#fca5a5' : '#4ade80';
    }
  }
  ...
  function _tutStep_game() {
    _tutGameSteps = [
      ...
      // 8 — Esercito e Truppe
      () => _tutPopup(null,
        '⚔ <strong>Esercito &amp; Truppe</strong>:<br>In battaglia ogni <strong>5 punti Esercito = 1 cerchietto</strong> sul campo (max 16).<br><br>Puoi reclutare truppe speciali: Dothraki 🐴, Immacolati 🛡️, Compagnia Dorata 💰 e altre — ognuna con abilità uniche.<br><br>🐉 <strong>Drago</strong>: ottenibile <em>solo</em> tramite eventi speciali (uovo di drago).',
        null, _tutGameNext, 'Capito →'),
