  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _bc_drawSoldier(ctx, x, y, flip, phase, state, alpha) {
      ...
      if (state==='march') {
        ...
        bob=Math.sin(t*13)*2.5; sw=Math.sin(t*11)*65-10;
      ...
      ctx.beginPath();
      ctx.moveTo(-7,0); ctx.lineTo(7,0); ctx.lineTo(6,6);
      ctx.lineTo(1.5,6); ctx.lineTo(1,10); ctx.lineTo(-1,10);
      ctx.lineTo(-1.5,6); ctx.lineTo(-6,6); ctx.closePath(); ctx.fill();
      // cimiero (pennacchio)
      ctx.beginPath();
      ctx.moveTo(-2,-7); ctx.bezierCurveTo(-4,-18,-1,-22,0,-20);
      ctx.bezierCurveTo(1,-22,4,-18,2,-7);
      ctx.closePath(); ctx.fill();
      ctx.restore();
    ...
    function _bc_spawnArrow(W, H) {
      // Freccia da sinistra (giocatore) o da destra (nemico)
      const fromLeft = Math.random() > 0.5;
      const gnd = H - 30;
      // Parte da un'altezza casuale nella parte alta del campo
      const startY = gnd - 15 - Math.random() * 40;
      // Angolo leggero verso il basso (traiettoria parabolica)
      const angle = fromLeft
        ? (15 + Math.random() * 20) * Math.PI / 180   // sinistra→destra, leggermente in basso
        : (160 + Math.random() * 20) * Math.PI / 180; // destra→sinistra
    ...
    function _bc_initField(fieldId, pN, eN) {
      ...
      _bc_canvas.height=field.offsetHeight||130;
    ...
    function _bc_advance(progress) {
      ...
      const gap=Math.max(8, 18+(1-progress)*(W*.28));
      const sp=20; // spaziatura tra soldati
