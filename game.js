...
const Game = (() => {
  ...
  function drawNextCard() {
    ...
    const available = EVENTS.filter(e => {
      ...
      if (charId === 'custom' && state.customCardRole) {
        ...
        if (state.customCardRole === 'noble' && e.forChars && e.forChars.every(c => ['arya','littlefinger','bronn','theon','tormund','oberyn'].includes(c))) return false;
        if (state.customCardRole === 'shadow' && e.forChars && e.forChars.every(c => ['tyrion','ned','catelyn','sansa','margaery','olenna','jaime','daenerys','cersei','stannis','robb'].includes(c))) return false;
      }
      return true;
    });
  ...
  function renderCard(card) {
    ...
    if (hasSpecial) {
      if (!badge) {
        ...
        badge.style.cssText = `
          position:absolute;bottom:-11px;top:auto;left:50%;transform:translateX(-50%);
  ...
  function _openHousePopup(hId) {
    ...
    if (isSuppressed) {
      ...
      if (isAlly) {
        ...
        if (isWarPending) {
          const hasLoan    = state.allyLoans && state.allyLoans[hId];
          const hasRefused = state.allyLoanRefusals && state.allyLoanRefusals[hId] === 'refused';
          if (hasLoan) {
            ...
            actions += `<button class="btn-raven" onclick="Game.requestAllyArmy('${hId}');document.getElementById('house-popup')?.remove()" style="border-color:rgba(251,146,60,0.5);color:#fb923c">⚔ Chiedi di nuovo (hanno rifiutato)</button>`;
          } else {
  ...
  function _openKingChallengeDiplomacy() {
    ...
    const allyRows = allies.length > 0
      ? allies.map(([id, hh]) => {
          ...
          const hasRefused = state.allyLoanRefusals && state.allyLoanRefusals[id] === 'refused';
          if (hasLoan) {
          ...
          if (hasRefused) {
            return `<button onclick="Game.requestAllyArmy('${id}');document.getElementById('war-diplo-reinf-overlay').remove();setTimeout(()=>Game._openKingChallengeDiplomacy(),200)" style="width:100%;display:flex;align-items:center;justify-content:space-between;padding:0.5rem 0.65rem;margin-bottom:0.3rem;background:rgba(251,146,60,0.06);border:1px solid rgba(251,146,60,0.3);border-radius:3px;font-family:'EB Garamond',serif;font-size:0.88rem;color:#e8dcc8;cursor:pointer;text-align:left">
              <span>${hh.icon} Casa ${hh.name} — ⚔ ${Math.round(hh.army)}</span>
              <span style="color:#fb923c;font-family:'Cinzel',serif;font-size:0.65rem">✗ Rifiutato — riprova →</span>
            </button>`;
          }
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      // Enemy: dies from left (far from center, index 0) → survivors stay on the right
      for (let i = 0; i < TROOP_COUNT_TH - eShouldBeAlive; i++) {
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat) {
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      // Enemy side — dies from left (far from center, index 0) → keep survivors on the right
      for (let i = 0; i < TROOP_COUNT - eShouldBeAlive; i++) {
  ...
  function _resolveThroneAttack(won, playerForce, kingForce, survived) {
    ...
    if (won) {
      ...
      state.resources.army  = Math.min(cap - 1, Math.max(5, survived ?? Math.max(5, state.resources.army - 10)));
  ...
  function requestAllyArmy(hId) {
    ...
    // If previously refused, clear it and let them try again
    if (state.allyLoanRefusals && state.allyLoanRefusals[hId] === 'refused') {
      delete state.allyLoanRefusals[hId];
    }
    ...
    const refusalChance = Math.min(0.55, 0.20 + hostility / 200 + (h.army < 60 ? 0.10 : 0));
    if (Math.random() < refusalChance) {
      ...
      if (!state.allyLoanRefusals) state.allyLoanRefusals = {};
      state.allyLoanRefusals[hId] = 'refused'; // mark so the button shows "Rifiutato" state
      showModal(
  ...
  function _refuseAllyLoan(hId) {
    document.getElementById('ally-army-overlay')?.remove();
    if (!state.allyLoanRefusals) state.allyLoanRefusals = {};
    state.allyLoanRefusals[hId] = 'refused';
    document.getElementById('war-diplo-reinf-overlay')?.remove();
    const panel = document.getElementById('diplomacy-panel');
    if (panel && !panel.classList.contains('hidden')) renderDiplomacy();
    setTimeout(() => {
  ...
  function _openWarDiplomacy(houseId) {
    ...
    const allyRows = allies.length > 0
      ? allies.map(([id, hh]) => {
          ...
          if (hasRefused) {
            return `<button onclick="Game.requestAllyArmy('${id}');document.getElementById('war-diplo-reinf-overlay').remove();setTimeout(()=>Game._openWarDiplomacy('${houseId}'),200)" style="width:100%;display:flex;align-items:center;justify-content:space-between;padding:0.5rem 0.65rem;margin-bottom:0.3rem;background:rgba(251,146,60,0.06);border:1px solid rgba(251,146,60,0.3);border-radius:3px;font-family:'EB Garamond',serif;font-size:0.88rem;color:#e8dcc8;cursor:pointer;text-align:left">
              <span>${hh.icon} Casa ${hh.name} — ⚔ ${Math.round(hh.army)}</span>
              <span style="color:#fb923c;font-family:'Cinzel',serif;font-size:0.65rem">✗ Rifiutato — riprova →</span>
            </button>`;
          }
  ...
  function _tutStep_game() {
    _tutGameSteps = [
      ...
      // 0 — Risorse: cerchi colorati
      () => _tutPopup('resource-gauges' in window ? null : null,
        '🎯 <strong>Le tue risorse</strong> sono i 5 cerchi in cima.<br>💰 Oro · ✝ Fede · 👥 Popolo · ⚔ Esercito · 👑 Potere.<br><strong style="color:#f87171">Attenzione:</strong> se una tocca 0 <em>o</em> 100 — game over!',
        null, _tutGameNext, 'Capito →'),

      // 1 — Carte: punta sulla carta
      () => _tutPopup('main-card',
        '🃏 <strong>Le carte</strong> arrivano ogni turno. Mostrano eventi, richieste di casate o decreti del Re.<br>Passa sopra i pulsanti per vedere come cambiano le risorse prima di scegliere.',
        'up', _tutGameNext, 'Capito →'),

      // 2 — Scelte interattive: aspetta che il giocatore faccia davvero una scelta
      () => {
        _tutPopup('btn-left',
          '◄► <strong>Ora fai una scelta!</strong> Premi uno dei due pulsanti oppure <em>swipa la carta</em> a sinistra o destra.<br><span style="color:#fbbf24">Il tutorial continuerà dopo la tua scelta.</span>',
          'up', null, null);
        // Override: il tutorial avanza quando il giocatore fa una scelta
        _tutorialPendingStep = 'wait_choice';
      },

      // 3 — Badge colori carte
      () => _tutPopup(null,
        '🎨 <strong>Carte colorate</strong> — riconoscile subito:<br>🔴 Rosso = pericolo/attacco · 🟡 Oro = Decreto del Re · 🟢 Verde = richiesta alleato · 🟣 Viola = colpo di stato · 🟠 Arancione = guerra in preparazione',
        null, _tutGameNext, 'Capito →'),

      // 4 — Obiettivo
      () => _tutPopup('objective-bar',
        '📜 <strong>Obiettivo personaggio</strong>: è mostrato in basso. Ogni personaggio ha una missione unica.<br>Raggiungerla è <em>uno dei modi per vincere</em> — ma non l\'unico.',
        'up', _tutGameNext, 'Capito →'),

      // 5 — Diplomazia interattiva: aspetta apertura reale
      () => {
        _tutPopup('btn-diplo',
          '⚔ <strong>Apri la Diplomazia</strong> — premi il pulsante in alto a destra.<br><span style="color:#fbbf24">Il tutorial continuerà quando apri il pannello.</span>',
          'down', null, null);
        _tutorialPendingStep = 'wait_diplo';
      },

      // 6 — Casate nel pannello (mostrato dopo apertura diplomazia)
      () => _tutPopup(null,
        '🏰 <strong>Le casate</strong> sono divise per colore:<br>🟢 Tuoi alleati · 🔴 Nemici · 🟣 Casa Regnante · 🟡 Fedeli al Re · 🟠 Diffidenti<br><br>Clicca una casata per vedere le azioni disponibili: alleanza, guerra, spie, tributi.',
        null, _tutGameNext, 'Capito →'),

      // 7 — Spie (Varys)
      () => _tutPopup(null,
        '🕷 <strong>Spie di Varys</strong>: clicca una casata → "Invia spia".<br>• 💰12: info vaga sulla fedeltà<br>• 💰18: percentuale precisa<br><strong style="color:#fb923c">Attenzione:</strong> la spia può essere scoperta — la casata diventa diffidente o nemica!',
        null, _tutGameNext, 'Capito →'),

      // 8 — Guerra e Re
      () => _tutPopup(null,
        '⚔ <strong>Come sfidare il Re</strong>:<br>1. Hai <strong>Esercito &gt;80</strong><br>2. Hai <strong>almeno 2 alleati</strong><br>3. Apri Diplomazia → clicca Casa Regnante → "Sfida il Re"<br><br>Prima della battaglia puoi chiedere rinforzi ai tuoi alleati.',
        null, _tutGameNext, 'Capito →'),

      // 9 — Come vincere (finale)
      () => _tutPopup(null,
        '👑 <strong>Come vincere</strong> — scegli la tua strada:<br>1. <strong>Obiettivo</strong>: raggiungi la missione del personaggio<br>2. <strong>Conquista</strong>: sconfiggi il Re e ottieni la fedeltà di <em>tutte</em> le casate vive<br><br><em>I Sette Regni ti aspettano. Buona fortuna!</em>',
  ...
  // Called from makeChoice when tutorial is waiting for a card choice
  ...
  // Called from toggleDiplomacy when tutorial is waiting for diplomacy open
