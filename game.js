...
const Game = (() => {
  ...
  function showThroneAttackAnimation(playerForce, kingForce, onComplete) {
    ...
    function _bc_drawSoldier(ctx, x, y, flip, phase, state, alpha, s) {
      ...
      if (flip) { ctx.translate(x, y + cy); ctx.scale(-1, 1); }
      else        ctx.translate(x, y + cy);
    ...
    function _bc_initField(fieldId, pN, eN) {
      ...
      const W=_bc_canvas.width, sp=Math.min(24,(W*.40)/pN);
      _bc_soldiers=[];
      // Soldati partono FUORI schermo e marciano verso il centro
      for(let i=0;i<pN;i++) {
        const startX = -30 - i*sp;            // fuori a sinistra
        const targetX = 8 + i*sp;             // posizione iniziale dentro campo
        _bc_soldiers.push({id:'p'+i,x:startX,tx:targetX,side:'p',alive:true,phase:Math.random()*6.28,dieA:0,alpha:1,state:'march'});
      }
      for(let i=0;i<eN;i++) {
        const startX = W + 30 + i*sp;         // fuori a destra
        const targetX = W - 28 - i*sp;        // posizione iniziale dentro campo
        _bc_soldiers.push({id:'e'+i,x:startX,tx:targetX,side:'e',alive:true,phase:Math.random()*6.28,dieA:0,alpha:1,state:'march'});
      }
      _bc_running=true;
      _bc_loop(fieldId);
    }
    ...
    function _bc_loop(fieldId) {
      ...
      // Avanzamento fluido
      _bc_soldiers.forEach(s=>{ if(s.alive){ const dx=s.tx-s.x; s.x+=dx*0.055; }});

      // Stato soldati
      _bc_soldiers.forEach(s=>{
        if (!s.alive) return;
        if (Math.abs(s.tx-s.x)>4) s.state='march';
      ...
      // Soldati vivi
      _bc_soldiers.filter(s=>s.alive).forEach(s=>_bc_drawSoldier(_bc_ctx,s.x,gnd-4,s.side==='e',s.phase,s.state,s.alpha,s));
    ...
    function _bc_advance(progress) {
      if (!_bc_canvas) return;
      const W=_bc_canvas.width, center=W/2;
      const pS=_bc_soldiers.filter(s=>s.side==='p'&&s.alive);
      const eS=_bc_soldiers.filter(s=>s.side==='e'&&s.alive);
      ...
      const sp=14; // spaziatura tra soldati
      // Giocatore: indice 0 = prima linea (più vicino al centro, quindi x più alta)
      // indice 0 → center-gap, indice 1 → center-gap-sp, ecc. (si allontana dal centro)
      pS.forEach((s,i)=>{ s.tx = center - gap - i*sp; });
      // Nemico: indice 0 = prima linea (x più bassa)
      // indice 0 → center+gap, indice 1 → center+gap+sp, ecc.
      eS.forEach((s,i)=>{ s.tx = center + gap + i*sp; });
    }

    function _bc_kill(side, count) {
      if (!_bc_canvas) return;
      const H=_bc_canvas.height, gnd=H-30;
      // Uccidi i soldati in PRIMA LINEA (indice 0 = più vicino al centro)
      const alive=_bc_soldiers.filter(s=>s.side===side&&s.alive);
      // Ordina per prossimità al centro: il più vicino muore per primo
      const W=_bc_canvas.width, center=W/2;
      const sorted=alive.slice().sort((a,b)=>
        side==='p' ? b.x-a.x : a.x-b.x // p: muore quello più a destra (più avanzato); e: più a sinistra
      );
      sorted.slice(0,Math.min(count,sorted.length)).forEach((s,idx)=>{
        setTimeout(()=>{
          s.alive=false; s.dieA=0;
          // Stagger ai soldati vicini nella stessa formazione
          _bc_soldiers.filter(o=>o.alive&&o.side===s.side&&Math.abs(o.x-s.x)<32).forEach(o=>{o._stagger=(o._stagger||0)+0.6;});
          // Schizzi sangue abbondanti
          for(let i=0;i<16;i++) _bc_blood.push(_bc_createBlood(s.x, gnd-18));
          // Scintille impatto
          for(let i=0;i<6;i++) _bc_sparks.push({
          ...
          // Dopo morte → i superstiti avanzano automaticamente
          setTimeout(()=>{
            const pAliveN=_bc_soldiers.filter(s=>s.side==='p'&&s.alive).length;
            const eAliveN=_bc_soldiers.filter(s=>s.side==='e'&&s.alive).length;
            const totalDead=(10-pAliveN)+(10-eAliveN);
            const prog=Math.min(1.0, 0.35+totalDead/20*0.65);
            _bc_advance(prog);
          }, 250);
        }, idx*200);
      });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => _bc_advance(0.0), 50);
      setTimeout(() => _bc_advance(0.55), 2200);
    }, 80);
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      setTimeout(() => _bc_advance(progress), 350);
  ...
  function showBattleAnimation(playerForce, kingForce, onComplete, allowRetreat, bgConfig) {
    ...
    function killTroops(side, _unused, _unused2) {
      ...
      const progress = Math.min(0.9, 0.25 + totalDead / (TROOP_COUNT * 2) * 0.65);
      setTimeout(() => _bc_advance(progress), 350);
