...
const MultiplayerEngine = (() => {
  ...
  // ── Helper per ottenere CHARACTERS dall'API pubblica di Game ──
  function _getChars()  { return (typeof Game !== 'undefined' && Game._getCharacters) ? Game._getCharacters() : []; }
  function _getHouses() { return (typeof Game !== 'undefined' && Game._getHousesDef)  ? Game._getHousesDef()  : []; }
  ...
  let _sessionCode = null;   // peer ID dell'host (usato come codice)
  ...
  // ── Config PeerJS con STUN Google (funziona anche ethernet↔wifi) ──
  const PEER_CONFIG = {
    debug: 0,
    config: {
      iceServers: [
  ...
  function _injectStyle() {
    ...
    s.textContent = `
      ...
      .mp-screen {
        position:fixed;inset:0;z-index:600;background:#0a0a0f;
        display:flex;flex-direction:column;align-items:center;
        overflow-y:auto;padding:1.5rem 1rem 2rem;
      ...
      .mp-input {
        ...
        background:#0a0a0f;border:1px solid rgba(201,168,76,0.3);
        border-radius:3px;font-family:'Cinzel',serif;
        font-size:0.78rem;color:#e8dcc8;
        box-sizing:border-box;margin-bottom:0.4rem;
      }
      .mp-btn-gold {
        width:100%;padding:0.75rem;
        ...
        text-transform:uppercase;cursor:pointer;color:#0a0a0f;margin-top:0.5rem;
      }
      .mp-btn-green {
        width:100%;padding:0.75rem;
        background:linear-gradient(135deg,#14532d,#16a34a,#14532d);
        border:none;border-radius:3px;font-family:'Cinzel',serif;
        font-size:0.8rem;font-weight:700;letter-spacing:0.12em;
        text-transform:uppercase;cursor:pointer;color:#fff;margin-top:0.5rem;
      }
      .mp-btn-blue {
        width:100%;padding:0.75rem;
      ...
      .mp-btn-red {
        ...
        text-transform:uppercase;cursor:pointer;color:#fff;margin-top:0.4rem;
      }
      .mp-btn-ghost {
      ...
      .mp-slot {
        ...
        padding:0.5rem 0.65rem;background:#12121a;
        border:1px solid rgba(201,168,76,0.18);
        border-radius:5px;margin-bottom:0.35rem;
        font-family:'Cinzel',serif;
      }
      .mp-slot.ready { border-color:rgba(74,222,128,0.5); }
      .mp-slot.me    { border-color:rgba(201,168,76,0.7); }
      .mp-code-box {
        font-family:'Cinzel Decorative',serif;font-size:0.75rem;
        color:#c9a84c;letter-spacing:0.06em;text-align:center;
        padding:0.75rem;background:rgba(201,168,76,0.06);
        border:1px solid rgba(201,168,76,0.25);border-radius:6px;
        margin:0.5rem 0;cursor:pointer;word-break:break-all;
        transition:background 0.2s;user-select:all;
      }
      .mp-code-box:hover { background:rgba(201,168,76,0.12); }
      .mp-hud {
      ...
      .mp-enemy-card {
        ...
        padding:0.3rem 0.4rem;background:#12121a;border-radius:4px;
        border:1px solid rgba(201,168,76,0.15);min-width:52px;
        font-size:0.55rem;cursor:pointer;transition:border-color 0.2s;
      }
      .mp-enemy-card:hover  { border-color:rgba(201,168,76,0.5); }
      .mp-enemy-card.is-king { border-color:rgba(192,132,252,0.6); }
      .mp-char-btn {
        display:flex;flex-direction:column;align-items:center;
        padding:0.4rem 0.55rem;background:#12121a;
        border:1px solid rgba(201,168,76,0.2);border-radius:5px;
        cursor:pointer;min-width:64px;transition:border-color 0.15s;
        font-family:'Cinzel',serif;
      }
      .mp-char-btn:hover:not(:disabled) { border-color:rgba(201,168,76,0.65); }
      .mp-char-btn:disabled { opacity:0.3;cursor:not-allowed;background:rgba(30,30,30,0.4); }
    `;
    document.head.appendChild(s);
  }
  ...
  function showLobbyScreen() {
    ...
    wrap.id = 'mp-lobby';
    wrap.className = 'mp-screen';
    wrap.innerHTML = `
      <div style="width:100%;max-width:420px">
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.1rem;color:#c9a84c;text-align:center;margin-bottom:0.25rem">⚔ Multiplayer</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.88rem;color:#6b5e4a;text-align:center;margin-bottom:1.2rem;font-style:italic">Fino a 4 giocatori · stesso WiFi o Internet</div>

        <div class="mp-card">
          <div style="font-size:0.68rem;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.6rem">👑 Crea Partita (Host)</div>
          <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#9a8a6a;margin-bottom:0.6rem">Crea la stanza e condividi il codice con gli altri.</div>
          <input id="mp-host-name" class="mp-input" placeholder="Il tuo nome (es. Jon Snow)" maxlength="20">
          <button class="mp-btn-gold" onclick="MultiplayerEngine.createSession()">Crea Partita</button>
        </div>

        <div class="mp-card">
          <div style="font-size:0.68rem;color:#60a5fa;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.6rem">🗡 Unisciti a una Partita</div>
          <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#9a8a6a;margin-bottom:0.6rem">Inserisci il codice di 5 caratteri mostrato dall'host (es: <strong style="color:#c9a84c;letter-spacing:0.1em">IA3X7</strong>).</div>
          <input id="mp-guest-name" class="mp-input" placeholder="Il tuo nome" maxlength="20">
          <input id="mp-join-code" class="mp-input" placeholder="Codice (es: IA3X7)" maxlength="8"
            style="letter-spacing:0.25em;text-transform:uppercase;font-size:1rem;text-align:center"
            oninput="this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'')">
          <button class="mp-btn-blue" onclick="MultiplayerEngine.joinSession()">Unisciti</button>
        </div>

        <button class="mp-btn-ghost" onclick="MultiplayerEngine.closeLobby()" style="max-width:420px">← Torna al Menu</button>
      </div>`;
  ...
  // ── Genera codice corto (5 lettere+cifre leggibili) ──
  function _genShortCode() {
    const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // no I,O,0,1 per evitare confusione
    let code = 'IA'; // prefisso fisso "Iron & Alliances"
    for (let i = 0; i < 3; i++) code += chars[Math.floor(Math.random() * chars.length)];
    return code; // es: IA3X7, IAK9M
  }

  // ══════════════════════════════
  // CREA SESSIONE (HOST) — usa ID corto custom
  // ══════════════════════════════
  function createSession() {
    const name = (_el('mp-host-name')?.value || '').trim() || 'Anonimo';
    _isHost = true;
    _loadPeerJS(() => {
      const shortCode = _genShortCode();
      // Passiamo il codice corto come ID del peer → il guest si connette direttamente con quel codice
      _peer = new Peer('ia-' + shortCode.toLowerCase(), PEER_CONFIG);
      _peer.on('open', (id) => {
        _myPeerId    = id;
        _sessionCode = shortCode; // mostriamo solo il codice leggibile
        _players[id] = _makePlayer(id, name, true);
        _el('mp-lobby')?.remove();
        _showRoomScreen();
      });
      _peer.on('connection', _onGuestConnect);
      _peer.on('error', (e) => {
        // Se il codice è già in uso riprova con uno nuovo
        if (e.type === 'unavailable-id') {
          _mpToast('Codice già in uso, genero un nuovo codice…');
          if (_peer) { _peer.destroy(); _peer = null; }
          setTimeout(createSession, 300);
        } else {
          _mpToast('❌ Errore connessione: ' + e.type + '. Riprova.', 'warn');
        }
      });
    });
  }
  ...
  // ══════════════════════════════
  // UNISCITI (GUEST) — inserisce solo codice corto
  // ══════════════════════════════
  function joinSession() {
    const name = (_el('mp-guest-name')?.value || '').trim() || 'Anonimo';
    let   code = (_el('mp-join-code')?.value  || '').trim().toUpperCase().replace(/\s/g,'');
    if (!code) { _mpToast('Inserisci il codice dell\'host', 'warn'); return; }
    // Supporta sia "IA3X7" che "ia-ia3x7" (copia completa)
    if (code.startsWith('IA-')) code = code.slice(3);
    const hostPeerId = 'ia-' + code.toLowerCase(); // ricostruisce il Peer ID completo
    ...
    _loadPeerJS(() => {
      ...
      _peer.on('open', (myId) => {
        _myPeerId = myId;
        _players[myId] = _makePlayer(myId, name, true);
        const conn = _peer.connect(hostPeerId, { reliable: true });
        let connected = false;
        conn.on('open', () => {
          connected = true;
          _connections[hostPeerId] = conn;
          _setupConn(conn);
          _send(conn, { type:'JOIN_REQUEST', name, peerId: myId });
          _el('mp-lobby')?.remove();
          _showRoomScreen();
        });
        conn.on('error', (e) => _mpToast('❌ Connessione fallita. Verifica il codice.', 'warn'));
        setTimeout(() => {
          if (!connected) _mpToast('❌ Timeout: host non raggiungibile. Verifica il codice.', 'warn');
        }, 12000);
      });
  ...
  function _setupConn(conn) {
    ...
    conn.on('close', ()    => _onDisconnect(conn.peer));
  ...
  // ROUTER MESSAGGI
  // ══════════════════════════════
  function _onMsg(msg, from) {
    switch (msg.type) {
      case 'JOIN_REQUEST':   _hJoinRequest(msg, from);   break;
      case 'PLAYER_LIST':    _hPlayerList(msg);           break;
      case 'CHAR_SELECTED':  _hCharSelected(msg, from);   break;
      case 'PLAYER_READY':   _hPlayerReady(from);         break;
      case 'GAME_START':     _hGameStart(msg);            break;
      case 'STATE_UPDATE':   _hStateUpdate(msg);          break;
      case 'DIPLO_REQUEST':  _hDiploRequest(msg, from);   break;
      case 'DIPLO_RESPONSE': _hDiploResponse(msg, from);  break;
      case 'WAR_DECL':       _hWarDecl(msg, from);        break;
      case 'WAR_UPDATE':     _hWarUpdate(msg);            break;
      case 'WAR_RESULT':     _hWarResult(msg);            break;
      case 'THRONE_ON':      _hThroneOn(msg, from);       break;
      case 'THRONE_OFF':     _hThroneOff(from);           break;
      case 'PLAYER_WIN':     _hPlayerWin(msg);            break;
      case 'CHAT':           _hChat(msg);                 break;
      case 'RELAY':          _hRelay(msg, from);          break;
      case 'ROOM_FULL':      _mpToast('❌ Stanza piena (max 4)', 'warn'); break;
    }
  }
  ...
  // JOIN REQUEST
  // ══════════════════════════════
  function _hJoinRequest(msg, from) {
    if (!_isHost) return;
    _players[from] = _makePlayer(from, msg.name, false);
    _broadcastList();
    _refreshRoom();
  }

  function _broadcastList() {
    ...
    _broadcast({ type:'PLAYER_LIST', players: list, takenHouses: _takenHouses });
  }

  function _hPlayerList(msg) {
  ...
  function _showRoomScreen() {
    ...
    // Codice da condividere (solo host)
    const codeSection = _isHost ? `
      <div class="mp-card">
        <div style="font-size:0.68rem;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem">📋 Codice Sessione</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#9a8a6a;margin-bottom:0.5rem">Di' ad alta voce questo codice agli altri giocatori — o toccalo per copiarlo.</div>
        <div id="mp-code-box" onclick="MultiplayerEngine._copyCode()" title="Tocca per copiare"
          style="font-family:'Cinzel Decorative',serif;font-size:3rem;color:#c9a84c;letter-spacing:0.35em;
          text-align:center;padding:0.6rem 0.5rem;background:rgba(201,168,76,0.08);
          border:2px solid rgba(201,168,76,0.5);border-radius:8px;margin:0.25rem 0;
          cursor:pointer;transition:all 0.2s;user-select:all;word-break:break-all">
          …
        </div>
        <div style="font-size:0.62rem;color:#6b5e4a;text-align:center;font-family:'EB Garamond',serif;font-style:italic;margin-top:0.25rem">
          👆 Tocca per copiare · Codice di 5 caratteri facile da leggere
        </div>
      </div>` : `
      <div class="mp-card" style="border-color:rgba(74,222,128,0.3)">
        <div style="font-size:0.68rem;color:#4ade80;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.3rem">✅ Connesso alla stanza</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#9a8a6a">Sei connesso come ospite. Scegli il tuo personaggio e premi Pronto.</div>
      </div>`;

    wrap.innerHTML = `
      <div style="width:100%;max-width:420px">
        <div style="font-family:'Cinzel Decorative',serif;font-size:1rem;color:#c9a84c;text-align:center;margin-bottom:0.75rem">
          ⚔ Sala del Trono <span style="font-size:0.6rem;color:${_isHost?'#c9a84c':'#60a5fa'}">(${_isHost?'tu sei l\'host':'ospite'})</span>
        </div>

        ${codeSection}

        <div class="mp-card">
          <div style="font-size:0.68rem;color:#9a8a6a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.5rem">Giocatori Connessi</div>
        ...
        <div class="mp-card">
          ...
          <div id="mp-char-section">
            <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#6b5e4a;margin-bottom:0.5rem">Scegli un personaggio da una casata non ancora presa dagli altri.</div>
            <button class="mp-btn-gold" onclick="MultiplayerEngine.openCharPicker()">Scegli Personaggio</button>
          </div>
        </div>

        <div class="mp-card">
          <div style="font-size:0.68rem;color:#9a8a6a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.4rem">💬 Chat</div>
          <div id="mp-chat-log" style="max-height:90px;overflow-y:auto;margin-bottom:0.5rem;font-family:'EB Garamond',serif;font-size:0.82rem;color:#9a8a6a;line-height:1.6"></div>
          <div style="display:flex;gap:0.4rem">
            <input id="mp-chat-input" class="mp-input" style="flex:1;margin:0" placeholder="Messaggio…" maxlength="80"
        ...
        <div id="mp-room-actions"></div>
        <button class="mp-btn-ghost" onclick="MultiplayerEngine._leaveRoom()">← Lascia la stanza</button>
      </div>`;

    document.body.appendChild(wrap);

    // Aggiorna codice quando disponibile
    if (_isHost && _sessionCode) {
      setTimeout(() => {
        const box = _el('mp-code-box');
        if (box) box.textContent = _sessionCode;
      }, 100);
    }
  ...
  function _refreshRoom() {
    ...
    const codeEl    = _el('mp-code-box');
    if (!listEl) return;

    if (codeEl && _sessionCode) codeEl.textContent = _sessionCode;

    const me = _players[_myPeerId];
    const CHARS = _getChars();

    // Lista giocatori
    listEl.innerHTML = Object.values(_players).map(p => {
      const isMe   = p.peerId === _myPeerId;
      const charDef = p.charId ? CHARS.find(c => c.id === p.charId) : null;
      const icon   = charDef ? charDef.icon : '❓';
      const cname  = charDef ? charDef.name : 'Non scelto';
      const rdMark = p.ready
        ? '<span style="color:#4ade80;font-size:0.65rem">✓ Pronto</span>'
        : '<span style="color:#6b5e4a;font-size:0.65rem">In attesa…</span>';
      return `<div class="mp-slot ${p.ready?'ready':''} ${isMe?'me':''}">
        <span style="font-size:1.35rem">${icon}</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:0.75rem;color:#e8dcc8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
            ${p.name}${isMe?' <span style="color:#c9a84c;font-size:0.6rem">(tu)</span>':''}
          </div>
          <div style="font-size:0.6rem;color:#9a8a6a">${cname}</div>
        </div>
        ${rdMark}
      </div>`;
    }).join('');
    ...
    if (charEl && me) {
      if (me.charId) {
        const charDef = CHARS.find(c => c.id === me.charId);
        charEl.innerHTML = `
          ...
          <button class="mp-btn-ghost" onclick="MultiplayerEngine.openCharPicker()" style="margin-top:0">Cambia personaggio</button>
          ${!me.ready
            ? '<button class="mp-btn-green" onclick="MultiplayerEngine.setReady()">✓ Sono Pronto</button>'
            : '<div style="text-align:center;color:#4ade80;font-size:0.82rem;margin-top:0.5rem;font-family:\'Cinzel\',serif">✓ Pronto!</div>'}`;
      }
    }
    ...
    if (actEl && _isHost) {
      ...
      actEl.innerHTML = allRdy
        ? '<button class="mp-btn-green" onclick="MultiplayerEngine.hostStartGame()">⚔ Avvia la Partita!</button>'
        : `<div style="font-family:'EB Garamond',serif;font-size:0.82rem;color:#6b5e4a;text-align:center;padding:0.5rem">
             ...
             <br><span style="font-size:0.72rem">${list.filter(p=>p.ready).length}/${list.length} pronti</span>
           </div>`;
    }
  }
  ...
  function openCharPicker() {
    ...
    if (!CHARS || CHARS.length === 0) {
      _mpToast('Personaggi non ancora disponibili, riprova tra un momento.', 'warn');
      return;
    }
    ...
    Object.entries(houses).forEach(([houseName, chars]) => {
      const isTaken   = takenByOthers.includes(houseName);
      ...
      inner += `
        <div style="margin-bottom:0.85rem">
          <div style="font-size:0.63rem;letter-spacing:0.1em;text-transform:uppercase;
            padding:0.22rem 0.5rem;border-radius:3px;margin-bottom:0.35rem;
            ${isTaken
      ...
      chars.forEach(c => {
        inner += `<button
          class="mp-char-btn" ${isTaken ? 'disabled' : ''}
          onclick="${isTaken ? '' : `MultiplayerEngine.selectChar('${c.id}','${houseName.replace(/'/g,"\\'")}');document.getElementById('mp-char-picker')?.remove()`}">
          <span style="font-size:1.25rem">${c.icon}</span>
          <span style="font-size:0.5rem;color:#e8dcc8;max-width:62px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:0.1rem">${c.name.split(' ')[0]}</span>
        </button>`;
      });
      inner += `</div></div>`;
    });
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:700;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;padding:1rem;animation:mp-fadein 0.2s ease';
    overlay.innerHTML = `
      <div style="background:#0e0e16;border:1px solid rgba(201,168,76,0.4);border-radius:8px;width:100%;max-width:420px;max-height:88vh;overflow-y:auto;padding:1.2rem;font-family:'Cinzel',serif;">
        ...
        <div style="font-family:'EB Garamond',serif;font-size:0.8rem;color:#6b5e4a;margin-bottom:0.75rem">
          Le casate in rosso sono già prese. Scegli qualsiasi personaggio di una casata libera.
        </div>
        ${inner}
      </div>`;
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
  ...
  function selectChar(charId, house) {
    ...
    if (_isHost) { _broadcastList(); }
    else         { _sendToHost(msg); }
    _refreshRoom();
  }

  function _hCharSelected(msg, from) {
  ...
  function setReady() {
    ...
    me.ready = true;
    if (_isHost) _broadcastList();
    else         _sendToHost({ type:'PLAYER_READY', peerId: _myPeerId });
    _refreshRoom();
  }

  function _hPlayerReady(from) {
  ...
  function hostStartGame() {
    const list = Object.values(_players);
    if (!list.every(p => p.ready && p.charId)) { _mpToast('Non tutti sono pronti!', 'warn'); return; }
    _broadcast({ type:'GAME_START' });
    _startLocalGame();
  }

  function _hGameStart() { _startLocalGame(); }

  function _startLocalGame() {
    _el('mp-room')?.remove();
    _el('mp-lobby')?.remove();
    const charId = _players[_myPeerId]?.charId;
    if (!charId) { _mpToast('Personaggio non trovato!', 'warn'); return; }
    const CHARS = _getChars();
    const char  = CHARS.find(c => c.id === charId);
    if (!char)  { _mpToast('Personaggio non trovato!', 'warn'); return; }

    // ── Avvio diretto: bypassa le schermate di selezione ──
    // Chiama l'API interna di Game per inizializzare lo stato e avviare
    if (typeof Game !== 'undefined') {
      // Simula la selezione del personaggio e avvio partita
      Game._mpMode    = true;
      Game._mpEngine  = MultiplayerEngine;
      // Naviga a char select, poi seleziona automaticamente
      Game.showCharacterSelect();
      // Aspetta che la griglia sia renderizzata e clicca il personaggio
      _autoSelectChar(char, 0);
    }
  ...
  function _autoSelectChar(char, attempt) {
    if (attempt > 20) { _mpToast('Avvio manuale: scegli ' + char.name, 'warn'); return; }
    setTimeout(() => {
      // Cerca la card del personaggio nella griglia (data-char-id o cerca per contenuto)
      const cards = document.querySelectorAll('.char-card');
      let found = false;
      cards.forEach(card => {
        const nameEl = card.querySelector('.char-card-name, .char-name');
        if (nameEl && nameEl.textContent.trim() === char.name) {
          card.click();
          found = true;
          // Aspetta overlay dettaglio e clicca Avvia
          setTimeout(() => _clickStartBtn(char, 0), 500);
        }
        // Anche via data-char-id se presente
        if (!found && card.dataset.charId === char.id) {
          card.click();
          found = true;
          setTimeout(() => _clickStartBtn(char, 0), 500);
        }
      ...
      if (!found) {
        // Potrebbe essere nascosta dietro la selezione casa → naviga prima alla casa
        const houseCards = document.querySelectorAll('.house-card, [data-house]');
        houseCards.forEach(hc => {
          const houseText = hc.textContent || '';
          if (houseText.includes(char.house.replace('Casa ',''))) hc.click();
        });
        _autoSelectChar(char, attempt + 1);
      }
    }, attempt === 0 ? 400 : 300);
  }

  function _clickStartBtn(char, attempt) {
    if (attempt > 10) return;
    const overlay = _el('char-detail-overlay');
    if (!overlay) { setTimeout(() => _clickStartBtn(char, attempt+1), 300); return; }
    // Cerca il pulsante "Avvia" / "Inizia"
    const btns = overlay.querySelectorAll('button');
    let clicked = false;
    btns.forEach(b => {
      const t = b.textContent.toLowerCase();
      if ((t.includes('avvia') || t.includes('inizia') || t.includes('gioca') || t.includes('start')) && !clicked) {
        b.click(); clicked = true;
      }
    });
    if (!clicked) setTimeout(() => _clickStartBtn(char, attempt+1), 300);
  }
  ...
  function _startStateSync() {
    ...
    window._mpSync = setInterval(() => {
      ...
      if (_players[_myPeerId]) {
        Object.assign(_players[_myPeerId], {
          resources: snap.resources, turn: snap.turn,
          isKing: snap.isKing, state: snap.gameOver ? 'dead' : 'playing',
        });
      }
      // Broadcast
      const m = { type:'STATE_UPDATE', snap };
      if (_isHost) _broadcast(m);
      else         _sendToHost({ type:'RELAY', payload: m });

      _updateMpHud();
      _checkThroneTimer(snap);

      // Notifica trono
      if (snap.isKing && _throneHolderPeerId !== _myPeerId) {
        const tm = { type:'THRONE_ON', peerId:_myPeerId, name:_players[_myPeerId]?.name };
        if (_isHost) _broadcast(tm); else _sendToHost({ type:'RELAY', payload:tm });
      } else if (!snap.isKing && _throneHolderPeerId === _myPeerId) {
        const tm = { type:'THRONE_OFF', peerId:_myPeerId };
        if (_isHost) _broadcast(tm); else _sendToHost({ type:'RELAY', payload:tm });
      }
    }, 3000);
  }

  function _hStateUpdate(msg) {
    const p = _players[msg.snap.peerId];
    if (p) Object.assign(p, {
      resources: msg.snap.resources, turn: msg.snap.turn,
      isKing: msg.snap.isKing, state: msg.snap.gameOver ? 'dead' : 'playing',
    });
    _updateMpHud();
  }
  ...
  function _hRelay(msg, from) {
    ...
    if (dest) {
      ...
      Object.entries(_connections).forEach(([pid, c]) => {
        if (pid !== from) _send(c, msg.payload);
      });
    }
    _onMsg(msg.payload, from);
  }

  // ══════════════════════════════
  // TRONO & TIMER 5 MIN
  // ══════════════════════════════
  function _checkThroneTimer(snap) {
    if (snap.isKing) {
      if (_throneHolderPeerId !== _myPeerId) {
        _throneHolderPeerId = _myPeerId;
        _throneStart = Date.now();
        _mpToast('👑 Hai il Trono! Mantienilo 5 minuti per vincere.', 'good');
  ...
  function _startThroneCountdown() {
    ...
    _throneTimer = setInterval(() => {
      ...
      if (elapsed >= THRONE_WIN_MS) {
        ...
        const inWar = s?.inActiveBattle || s?.pendingWarTarget;
        if (inWar) {
          _throneWarBlocked = true;
          _mpToast('👑 5 min scaduti! L\'esito della guerra deciderà il vincitore.', 'good');
        } else {
          _declareVictory();
        }
      }
  ...
  function _declareVictory() {
    _mpToast('🏆 HAI VINTO! 5 minuti sul Trono!', 'good');
    const m = { type:'PLAYER_WIN', peerId:_myPeerId, name:_players[_myPeerId]?.name };
    if (_isHost) _broadcast(m); else _sendToHost({ type:'RELAY', payload:m });
    _showVictoryOverlay(_players[_myPeerId]?.name || 'Tu', true);
  }

  function _hThroneOn(msg, from) {
    if (from === _myPeerId) return;
    _throneHolderPeerId = from;
    _mpToast(`👑 ${_players[from]?.name||'?'} ha conquistato il Trono! Scalzalo entro 5 min.`, 'warn');
    _updateThroneBar();
  }
  function _hThroneOff(from) {
    if (_throneHolderPeerId === from) { _throneHolderPeerId = null; _updateThroneBar(); }
  }
  function _hPlayerWin(msg) {
    const isMe = msg.peerId === _myPeerId;
    _showVictoryOverlay(msg.name || '?', isMe);
  }
  ...
  function sendDiploRequest(targetPeerId, action) {
    const me = _players[_myPeerId];
    const m  = { type:'DIPLO_REQUEST', from:_myPeerId, fromName:me?.name, action, charId:me?.charId };
    _sendToPlayer(targetPeerId, m);
    _mpToast(action==='alliance' ? '🤝 Proposta di alleanza inviata…' : '⚔ Dichiarazione di guerra inviata…');
  }

  function _hDiploRequest(msg, from) {
    const fromName = msg.fromName || _players[from]?.name || '?';
    const CHARS    = _getChars();
    const fromChar = CHARS.find(c => c.id === msg.charId);
    const isWar    = msg.action === 'war';
    const color    = isWar ? '#f87171' : '#4ade80';
    const label    = isWar ? '⚔ DICHIARAZIONE DI GUERRA' : '🤝 PROPOSTA DI ALLEANZA';

    _el('mp-diplo-popup')?.remove();
    const popup = document.createElement('div');
    popup.id = 'mp-diplo-popup';
    popup.style.cssText = 'position:fixed;inset:0;z-index:850;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.82);backdrop-filter:blur(4px);animation:mp-fadein 0.25s ease';
    popup.innerHTML = `
      <div style="background:linear-gradient(160deg,#1a1508,#12100a);border:2px solid ${color}44;border-radius:8px;width:88%;max-width:360px;padding:1.5rem;font-family:'Cinzel',serif;text-align:center">
        <div style="font-size:2rem;margin-bottom:0.4rem">${fromChar?.icon||'⚔'}</div>
        <div style="font-family:'Cinzel Decorative',serif;color:${color};font-size:0.88rem;margin-bottom:0.4rem">${label}</div>
        <div style="font-family:'EB Garamond',serif;font-size:0.9rem;color:#9a8a6a;margin-bottom:1.2rem;line-height:1.6;font-style:italic">
          «<strong style="color:#e8dcc8">${fromName}</strong> ${isWar ? 'ti dichiara guerra. Sei pronto a combattere?' : 'ti propone un\'alleanza. Accetti?'}»
        </div>
        <div style="display:flex;gap:0.75rem">
          <button onclick="MultiplayerEngine._respondDiplo('${from}','${msg.action}','accept')"
            style="flex:1;padding:0.7rem;background:${isWar?'linear-gradient(135deg,#7f1d1d,#dc2626)':'linear-gradient(135deg,#14532d,#16a34a)'};border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;cursor:pointer;color:#fff">
            ${isWar ? '⚔ Combatti!' : '✓ Accetta'}
          </button>
          <button onclick="MultiplayerEngine._respondDiplo('${from}','${msg.action}','refuse')"
            style="flex:1;padding:0.7rem;background:transparent;border:1px solid rgba(201,168,76,0.4);border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;letter-spacing:0.1em;cursor:pointer;color:#c9a84c">
            ${isWar ? '🏃 Fuggi (−10 ⚔)' : '✗ Rifiuta'}
          </button>
        </div>
      </div>`;
    document.body.appendChild(popup);
  ...
  function _respondDiplo(targetPeerId, action, response) {
    ...
    } else if (action === 'alliance' && response === 'accept') {
      _mpToast(`🤝 Alleanza stretta con ${_players[targetPeerId]?.name||'?'}!`, 'good');
    }
  }

  function _hDiploResponse(msg, from) {
    const fromName = msg.fromName || _players[from]?.name || '?';
    if (msg.action === 'alliance') {
      _mpToast(msg.response === 'accept'
        ? `🤝 ${fromName} ha accettato l'alleanza!`
        : `✗ ${fromName} ha rifiutato l'alleanza.`, msg.response==='accept'?'good':'warn');
      ...
      if (msg.response === 'accept') {
        ...
        _mpToast(`🏃 ${fromName} è fuggito! +5 ⚔`, 'good');
      }
    }
  }
  ...
  // GUERRA PvP
  // ══════════════════════════════
  function _startPvpWar(oppPeerId) {
    const me  = _players[_myPeerId];
    const opp = _players[oppPeerId];
    _broadcast({
      type:'WAR_DECL',
      p1:{ peerId:_myPeerId,  name:me?.name,  charId:me?.charId,  army:me?.resources?.army||50 },
      p2:{ peerId:oppPeerId,  name:opp?.name, charId:opp?.charId, army:opp?.resources?.army||50 },
    });
    _showBattleOverlay(oppPeerId);
  }

  function _hWarDecl(msg, from) {
    const involv = [msg.p1?.peerId, msg.p2?.peerId];
    if (involv.includes(_myPeerId)) return; // gestito da _startPvpWar
    // Sono spettatore
    _showSpectatorPrompt(msg);
  }

  function _showBattleOverlay(oppPeerId) {
    _el('mp-battle')?.remove();
    const me  = _players[_myPeerId];
    const opp = _players[oppPeerId];
    const CHARS   = _getChars();
    const myChar  = CHARS.find(c => c.id === me?.charId);
    ...
    const ov = document.createElement('div');
    ov.id = 'mp-battle';
    ov.style.cssText = 'position:fixed;inset:0;z-index:800;background:#0a0a0f;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:1.5rem;font-family:"Cinzel",serif;animation:mp-fadein 0.3s ease';
    ov.innerHTML = `
      <div style="font-family:'Cinzel Decorative',serif;font-size:1rem;color:#f87171;margin-bottom:1rem;text-shadow:0 0 20px rgba(239,68,68,0.5)">⚔ BATTAGLIA ⚔</div>
      <div style="display:flex;align-items:center;gap:1.5rem;margin-bottom:1.2rem;width:100%;max-width:360px;justify-content:center">
        <div style="text-align:center;flex:1">
          <div style="font-size:2.5rem">${myChar?.icon||'⚔'}</div>
          <div style="font-size:0.75rem;color:#e8dcc8;margin-top:0.2rem">${me?.name}</div>
          <div style="font-size:0.62rem;color:#4ade80;margin-top:0.2rem">⚔ ${myArmy}</div>
        </div>
        ...
        <div style="text-align:center;flex:1">
          ...
          <div style="font-size:0.75rem;color:#e8dcc8;margin-top:0.2rem">${opp?.name}</div>
          <div style="font-size:0.62rem;color:#f87171;margin-top:0.2rem">⚔ ${oppArmy}</div>
        </div>
      </div>
      <div id="mp-battle-bars" style="width:100%;max-width:360px;margin-bottom:0.75rem">
        <div style="display:flex;justify-content:space-between;font-size:0.62rem;color:#9a8a6a;margin-bottom:0.2rem">
          <span>${me?.name}</span><span>${opp?.name}</span>
        </div>
        <div style="display:flex;height:9px;border-radius:5px;overflow:hidden;background:rgba(255,255,255,0.05)">
          <div id="mp-bar-me"  style="background:#4ade80;transition:width 0.6s;width:50%"></div>
          <div id="mp-bar-opp" style="background:#f87171;transition:width 0.6s;width:50%"></div>
        </div>
      </div>
      <div id="mp-battle-log" style="width:100%;max-width:360px;height:160px;overflow-y:auto;background:#12121a;border:1px solid rgba(239,68,68,0.2);border-radius:6px;padding:0.65rem;font-family:'EB Garamond',serif;font-size:0.82rem;color:#e8dcc8;line-height:1.7;margin-bottom:0.75rem"></div>
      <div id="mp-battle-result"></div>`;
    document.body.appendChild(ov);
    _runBattle(oppPeerId, myArmy, oppArmy);
  }

  function _runBattle(oppPeerId, myArmy, oppArmy) {
    const log  = _el('mp-battle-log');
    const barMe  = _el('mp-bar-me');
    const barOpp = _el('mp-bar-opp');
    const resEl  = _el('mp-battle-result');
    const me  = _players[_myPeerId];
    const opp = _players[oppPeerId];
    let myHp  = myArmy;
    let oppHp = oppArmy;
    const myBonus  = 0.6 + Math.random() * 0.5;
    const oppBonus = 0.6 + Math.random() * 0.5;
    let round = 0;
    const total = myArmy + oppArmy;
    ...
    const iv = setInterval(() => {
      round++;
      const myAtk  = Math.round((4 + Math.random() * 9) * myBonus);
      const oppAtk = Math.round((4 + Math.random() * 9) * oppBonus);
      oppHp = Math.max(0, oppHp - myAtk);
      myHp  = Math.max(0, myHp  - oppAtk);
      const pMe  = Math.round((myHp  / total) * 100);
      const pOpp = Math.round((oppHp / total) * 100);
      if (barMe)  barMe.style.width  = pMe  + '%';
      if (barOpp) barOpp.style.width = pOpp + '%';
      logLine(`T${round}: <span style="color:#4ade80">${me?.name}</span> −${oppAtk} al nemico · <span style="color:#f87171">${opp?.name}</span> −${myAtk}`);

      // Invia aggiornamento spettatori
      const upd = { type:'WAR_UPDATE', round, p1Hp:myHp, p2Hp:oppHp, p1Name:me?.name, p2Name:opp?.name };
      if (_isHost) _broadcast(upd); else _sendToHost({ type:'RELAY', payload:upd });

      if (myHp <= 0 || oppHp <= 0 || round >= 12) {
        clearInterval(iv);
        const iWon = myHp > oppHp || (myHp === oppHp && Math.random() > 0.5);
        const s = typeof Game !== 'undefined' ? Game._getState() : null;
        if (s) {
          s.resources.army = Math.max(1, iWon ? Math.round(myHp + 5) : Math.round(myHp * 0.4));
          s.resources.power = Math.min(100, Math.max(1, (s.resources.power||50) + (iWon ? 10 : -8)));
        }
        const wr = { type:'WAR_RESULT', winnerId: iWon ? _myPeerId : oppPeerId };
        if (_isHost) _broadcast(wr); else _sendToHost({ type:'RELAY', payload:wr });

        if (_throneWarBlocked && iWon && _players[_myPeerId]?.isKing) {
          _throneWarBlocked = false; _declareVictory();
        }
        logLine(iWon ? '<span style="color:#c9a84c">👑 HAI VINTO!</span>' : '<span style="color:#f87171">💀 HAI PERSO.</span>');
        if (resEl) resEl.innerHTML = `
          <div style="text-align:center;margin-top:0.5rem">
            <div style="font-family:'Cinzel Decorative',serif;font-size:0.95rem;color:${iWon?'#c9a84c':'#f87171'};margin-bottom:0.6rem">${iWon?'🏆 Vittoria!':'💀 Sconfitta'}</div>
            <button onclick="document.getElementById('mp-battle')?.remove()" style="padding:0.6rem 1.5rem;background:linear-gradient(135deg,#78350f,#c9a84c,#78350f);border:none;border-radius:3px;font-family:'Cinzel',serif;font-size:0.78rem;font-weight:700;letter-spacing:0.1em;cursor:pointer;color:#0a0a0f">Torna al Gioco</button>
          </div>`;
      }
    }, 850);
  }

  function _hWarUpdate(msg) {
    const log = _el('mp-spec-log');
    if (!log) return;
    const d = document.createElement('div');
    d.style.cssText = 'font-family:"EB Garamond",serif;font-size:0.82rem;color:#e8dcc8;line-height:1.6';
    d.innerHTML = `T${msg.round}: <span style="color:#4ade80">${msg.p1Name}</span> ${msg.p1Hp}HP · <span style="color:#f87171">${msg.p2Name}</span> ${msg.p2Hp}HP`;
    log.appendChild(d); log.scrollTop = log.scrollHeight;
  }

  function _hWarResult(msg) {
    const winner = _players[msg.winnerId];
    if (winner && msg.winnerId !== _myPeerId)
      _mpToast(`🏆 ${winner.name} ha vinto la battaglia!`, 'good');
  }

  // ══════════════════════════════
  // SPETTATORI
  // ══════════════════════════════
  function _showSpectatorPrompt(msg) {
    _el('mp-spec-prompt')?.remove();
    const p = document.createElement('div');
    p.id = 'mp-spec-prompt';
    p.style.cssText = 'position:fixed;bottom:5rem;left:50%;transform:translateX(-50%);z-index:799;background:#12121a;border:1px solid rgba(239,68,68,0.4);border-radius:8px;padding:0.85rem 1rem;font-family:"Cinzel",serif;width:88%;max-width:340px;animation:mp-fadein 0.3s ease';
    p.innerHTML = `
      ...
      <div style="font-family:'EB Garamond',serif;font-size:0.85rem;color:#e8dcc8;margin-bottom:0.6rem">
        <strong>${msg.p1?.name||'?'}</strong> vs <strong>${msg.p2?.name||'?'}</strong>
      </div>
      <div style="display:flex;gap:0.5rem">
        <button onclick="MultiplayerEngine._openSpec()" style="flex:1;padding:0.5rem;background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.4);border-radius:3px;font-family:'Cinzel',serif;font-size:0.65rem;cursor:pointer;color:#fca5a5">👁 Guarda</button>
  ...
  function _openSpec() {
    _el('mp-spec-prompt')?.remove();
    _el('mp-spec-overlay')?.remove();
    const ov = document.createElement('div');
    ov.id = 'mp-spec-overlay';
    ov.style.cssText = 'position:fixed;inset:0;z-index:790;background:rgba(10,10,15,0.97);display:flex;flex-direction:column;align-items:center;padding:1.5rem 1rem;font-family:"Cinzel",serif;';
    ov.innerHTML = `
      ...
      <div id="mp-spec-log" style="width:100%;max-width:380px;flex:1;overflow-y:auto;background:#12121a;border:1px solid rgba(239,68,68,0.2);border-radius:6px;padding:0.75rem;font-family:'EB Garamond',serif;font-size:0.85rem;color:#e8dcc8;line-height:1.7;margin-bottom:1rem;min-height:200px"></div>
      <button onclick="document.getElementById('mp-spec-overlay')?.remove()" style="padding:0.6rem 1.5rem;background:transparent;border:1px solid rgba(201,168,76,0.35);border-radius:3px;font-family:'Cinzel',serif;font-size:0.72rem;cursor:pointer;color:#c9a84c">Torna al gioco</button>`;
    document.body.appendChild(ov);
  }
  ...
  function _injectMpHud() {
    ...
    hud.innerHTML = `
      <div style="font-size:0.58rem;color:#c9a84c;letter-spacing:0.06em;flex-shrink:0">⚔ AVVERSARI</div>
      <div id="mp-enemy-cards" style="display:flex;gap:0.3rem;flex-wrap:wrap;flex:1"></div>
      <button onclick="MultiplayerEngine._showActions()" style="padding:0.3rem 0.6rem;background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.3);border-radius:3px;font-family:'Cinzel',serif;font-size:0.58rem;cursor:pointer;color:#fca5a5;flex-shrink:0">⚔ Azioni</button>`;
    document.body.appendChild(hud);
  }

  function _updateMpHud() {
    const el = _el('mp-enemy-cards');
    if (!el) return;
    const CHARS  = _getChars();
    ...
    el.innerHTML = others.map(p => {
      ...
      return `<div class="mp-enemy-card ${p.isKing?'is-king':''}" onclick="MultiplayerEngine._showActions()" title="${p.name}">
        ...
        <span style="color:${p.isKing?'#c084fc':p.state==='dead'?'#6b5e4a':'#9a8a6a'}">${p.isKing?'👑':''}${p.state==='dead'?'💀':''} ⚔${army}</span>
      </div>`;
    }).join('');
  }
  ...
  function _injectThroneBar() {
    ...
    b.id = 'mp-throne-bar';
    b.className = 'mp-throne-bar';
    b.innerHTML = `<span id="mp-throne-label" style="flex:1">👑 Nessuno sul Trono</span><span id="mp-throne-timer"></span>`;
    document.body.appendChild(b);
  }

  function _updateThroneBar(elapsed) {
    const lbl = _el('mp-throne-label');
    const tmr = _el('mp-throne-timer');
    if (!lbl) return;
    if (!_throneHolderPeerId) {
      lbl.textContent = '👑 Nessuno sul Trono';
    ...
    if (tmr && elapsed !== undefined) {
      ...
      tmr.textContent = `⏱ ${mins}:${secs.toString().padStart(2,'0')}`;
    }
  }
  ...
  function _showActions() {
    ...
    if (!others.length) { _mpToast('Nessun avversario disponibile', 'warn'); return; }

    const ov = document.createElement('div');
    ov.id = 'mp-actions';
    ov.style.cssText = 'position:fixed;inset:0;z-index:820;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,0.7);animation:mp-fadein 0.2s ease';
    ov.innerHTML = `
      <div style="background:#0e0e16;border:1px solid rgba(201,168,76,0.35);border-radius:10px 10px 0 0;width:100%;max-width:480px;padding:1rem;font-family:'Cinzel',serif;max-height:60vh;overflow-y:auto">
        <div style="font-family:'Cinzel Decorative',serif;font-size:0.88rem;color:#c9a84c;margin-bottom:0.75rem;text-align:center">⚔ Azioni verso Giocatori</div>
        ${others.map(p => {
          const ch = CHARS.find(c => c.id === p.charId);
          return `<div style="background:#12121a;border:1px solid rgba(201,168,76,0.15);border-radius:5px;padding:0.6rem 0.75rem;margin-bottom:0.4rem">
            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.4rem">
              <span style="font-size:1.15rem">${ch?.icon||'⚔'}</span>
              <div><div style="font-size:0.75rem;color:#e8dcc8">${p.name}</div>
              <div style="font-size:0.58rem;color:#9a8a6a">⚔${p.resources?.army?Math.round(p.resources.army):'?'} · ${ch?.house||''}</div></div>
              ${p.isKing?'<span style="margin-left:auto;font-size:0.7rem;color:#c084fc">👑 Re</span>':''}
            </div>
            <div style="display:flex;gap:0.35rem">
              <button onclick="MultiplayerEngine.sendDiploRequest('${p.peerId}','alliance');document.getElementById('mp-actions')?.remove()"
              ...
              <button onclick="MultiplayerEngine.sendDiploRequest('${p.peerId}','war');document.getElementById('mp-actions')?.remove()"
        ...
        <button onclick="document.getElementById('mp-actions')?.remove()" style="width:100%;padding:0.55rem;background:transparent;border:1px solid rgba(201,168,76,0.25);border-radius:3px;font-family:'Cinzel',serif;font-size:0.68rem;cursor:pointer;color:#9a8a6a;margin-top:0.25rem">← Chiudi</button>
      </div>`;
    ov.addEventListener('click', e => { if (e.target === ov) ov.remove(); });
    document.body.appendChild(ov);
  }
  ...
  function _showVictoryOverlay(winnerName, isMe) {
    _el('mp-victory')?.remove();
    const ov = document.createElement('div');
    ov.id = 'mp-victory';
    ov.style.cssText = 'position:fixed;inset:0;z-index:900;background:rgba(10,10,15,0.97);display:flex;flex-direction:column;align-items:center;justify-content:center;animation:mp-fadein 0.5s ease';
    ov.innerHTML = `
      <div style="text-align:center;padding:2rem;font-family:'Cinzel',serif">
        <div style="font-size:4rem;margin-bottom:0.5rem">${isMe?'🏆':'👑'}</div>
        <div style="font-family:'Cinzel Decorative',serif;font-size:1.3rem;color:#c9a84c;margin-bottom:0.5rem">${isMe?'HAI VINTO!':winnerName+' ha vinto'}</div>
        <div style="font-family:'EB Garamond',serif;font-size:1rem;color:#9a8a6a;margin-bottom:1.5rem;font-style:italic;max-width:300px;line-height:1.6">
          ${isMe
            ? '«Avete mantenuto il Trono per 5 minuti. I Sette Regni vi appartengono.»'
            : `«${winnerName} ha tenuto il Trono per 5 minuti senza essere scalzato.»`}
        </div>
  ...
  function _hChat(msg) {
    _appendChat(msg);
    if (!_el('mp-room')) _mpToast(`💬 ${msg.name}: ${msg.text}`);
  }
  ...
  function _broadcast(msg)    { Object.values(_connections).forEach(c => _send(c, msg)); }
  function _sendToHost(msg)   { const c = Object.values(_connections)[0]; if (c) _send(c, msg); }
  function _sendToPlayer(pid, msg) {
    if (_isHost) { const c = _connections[pid]; if (c) _send(c, msg); }
    else         _sendToHost({ type:'RELAY', payload:{ ...msg, _to:pid } });
  }
  ...
  function _copyCode() {
    ...
    // Copia solo il codice breve (5 caratteri), non il peer ID interno
    const txt = _sessionCode;
    if (navigator.clipboard?.writeText) {
      navigator.clipboard.writeText(txt)
        .then(() => _mpToast('📋 Codice copiato! Dillo o invialo agli altri.', 'good'))
        .catch(() => prompt('Copia questo codice (5 caratteri):', txt));
    } else {
      prompt('Copia questo codice (5 caratteri):', txt);
    }
    const box = _el('mp-code-box');
    if (box) {
      box.style.background = 'rgba(201,168,76,0.3)';
      box.style.borderColor = 'rgba(201,168,76,0.9)';
      setTimeout(() => {
        box.style.background = 'rgba(201,168,76,0.08)';
        box.style.borderColor = 'rgba(201,168,76,0.5)';
      }, 500);
    }
  }
  ...
  // DISCONNECT
  // ══════════════════════════════
  function _onDisconnect(pid) {
    const p = _players[pid];
    if (p) { _mpToast(`⚠ ${p.name} si è disconnesso.`, 'warn'); p.state = 'dead'; }
  ...
  function _loadPeerJS(cb) {
    ...
    s.onload  = cb;
    s.onerror = () => _mpToast('❌ Impossibile caricare PeerJS. Verifica la connessione internet.', 'warn');
    document.head.appendChild(s);
  }
  ...
  // CLEANUP
  // ══════════════════════════════
  function _leaveRoom()   { _endSession(); showLobbyScreen(); }
  function _endSession() {
    clearInterval(_throneTimer); _throneTimer = null;
    clearInterval(window._mpSync); window._mpSync = null;
    ...
    ['mp-lobby','mp-room','mp-hud','mp-throne-bar','mp-battle','mp-victory',
     'mp-actions','mp-diplo-popup','mp-spec-overlay','mp-spec-prompt'].forEach(id => _el(id)?.remove());
  }

  // ══════════════════════════════
  // INIT
  // ══════════════════════════════
  if (document.readyState === 'complete') setTimeout(injectLobbyButton, 400);
  else window.addEventListener('load', () => setTimeout(injectLobbyButton, 400));

  return {
    createSession, joinSession, closeLobby,
    openCharPicker, selectChar, setReady,
    hostStartGame, sendChat, sendDiploRequest,
    _respondDiplo, _showActions, _openSpec,
    _copyCode, _leaveRoom, _endSession,
  };
